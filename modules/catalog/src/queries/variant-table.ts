...
import type { VariantRow, VariantTableParams, VariantTableResponse, FilterValue, } from '../responses.ts';
...
function variantMatches(row: VariantRow, filters: Record<string, FilterValue>): boolean {
    for (const [attr, fv] of Object.entries(filters)) {
        ...
        if (fv.kind === 'equals') {
            ...
            if (fv.gte !== undefined && !(n >= fv.gte))
                return false;
            if (fv.lte !== undefined && !(n <= fv.lte))
                return false;
        }
    }
    return true;
}
function parseSortSpec(s: string): {
    attr: string;
    dir: 'asc' | 'desc';
} {
    const idx = s.lastIndexOf('.');
    if (idx === -1)
        return { attr: s, dir: 'asc' };
...
export async function queryVariantTable(tenantId: string, familyKey: string, params: VariantTableParams, projections: ProjectionStore): Promise<VariantTableResponse | null> {
    const stored = readProjection<VariantTableResponse>(await projections.get(projectionKey(familyKey, tenantId)));
    if (!stored)
        return null;
    const allRows = stored.rows ?? [];
    const filters = params.filters ?? {};
    let filtered = allRows.filter(function (r) {
        return variantMatches(r, filters);
    });
    if (params.sort) {
