...
export async function handleFamilyPublish(cmd: FamilyPublishCommand, catalogState: CatalogStateStore, eventStore: EventStore): Promise<FamilyPublishResult> {
    ...
    const family = seed.families.find(function (f) {
        return f.key === cmd.familyKey;
    });
    if (!family) {
        throw new CatalogError('FAMILY_NOT_FOUND', `family not found: ${cmd.familyKey}`);
    }
