...
export async function handleSeedPackageApply(cmd: SeedPackageApplyCommand, catalogState: CatalogStateStore, eventStore: EventStore): Promise<SeedPackageApplyResult> {
    const taxonomyNodeCount = cmd.payload.taxonomyTrees.reduce(function (acc, t) {
        return acc + t.nodes.length;
    }, 0);
    const variantCount = cmd.payload.families.reduce(function (acc, f) {
        return acc + f.variants.length;
    }, 0);
    ...
    const familyIds = cmd.payload.families.map(function (f) {
        return deterministicUuid('family', cmd.tenantId, f.key);
    });
    const taxonomyTreeKeys = cmd.payload.taxonomyTrees.map(function (t) {
        return t.key;
    });
    ...
    for (const tk of taxonomyTreeKeys)
        tags.push(`TaxonomyTree:${tk}`);
    for (const fid of familyIds)
        tags.push(`Family:${fid}`);
