...
export async function rebuildTaxonomyNavigation(tenantId: string, catalogState: CatalogStateStore, projections: ProjectionStore): Promise<Array<{
    treeKey: string;
    payload: unknown;
}>> {
    ...
    const familiesByNodeKey = new Map<string, Array<{
        key: string;
        name: string;
    ...
    for (const tree of seed.taxonomyTrees) {
        ...
        const nodes = sortedNodes.map(function (n) {
