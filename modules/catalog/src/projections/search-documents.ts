...
export async function rebuildSearchDocuments(tenantId: string, catalogState: CatalogStateStore, search: SearchEngine): Promise<number> {
    ...
    for (const tree of seed.taxonomyTrees) {
        const sortedNodes = [...tree.nodes].sort(function (a, b) {
            return a.path.localeCompare(b.path);
        });
        for (const fam of seed.families) {
            const node = sortedNodes.find(function (n) {
                return n.key === fam.defaultTaxonomyNode;
            });
    ...
    for (const fam of seed.families) {
        ...
        const sortedVariants = [...fam.variants].sort(function (a, b) {
        ...
        if (taxonomyPath !== null)
            familyFields['taxonomy_path'] = taxonomyPath;
        ...
        for (let idx = 0; idx < sortedVariants.length; idx++) {
            ...
            if (taxonomyPath !== null)
                variantFields['taxonomy_path'] = taxonomyPath;
