...
export async function rebuildVariantMatrix(tenantId: string, catalogState: CatalogStateStore, projections: ProjectionStore): Promise<Array<{
    familyKey: string;
    payload: unknown;
}>> {
    ...
    for (const fam of seed.families) {
        ...
        const rows = sortedVariants.map(function (v) {
        ...
        const facetsObj: Record<string, Array<{
            value: string;
            count: number;
        }>> = {};
        ...
        for (const attr of sortedAttrKeys) {
            ...
            facetsObj[attr] = sortedValues.map(function (v) {
