...
import { getPageEntity, listPageEntities, } from './entities/page.ts';
import { getRenderTreeEntity, toRenderTree, } from './entities/page-render-tree.ts';
...
export async function listPages(deps: ContentPagesQueryDeps): Promise<PageSummary[]> {
    ...
    const sorted = [...docs].sort(function (a, b) {
...
export async function getPage(deps: ContentPagesQueryDeps, pageId: string): Promise<PageDocument | null> {
 ...
 */
export async function getRenderTree(deps: ContentPagesQueryDeps, pageId: string): Promise<RenderTree | null> {
    const renderTreeId = await findRenderTreeIdFor(deps.relations, deps.tenantId, pageId);
    if (!renderTreeId)
        return null;
    const attrs = await getRenderTreeEntity(deps.entities, deps.tenantId, pageId);
