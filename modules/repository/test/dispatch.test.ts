...
describe('repositoryDispatcher (I12)', function () {
    it('replays [Created, Uploaded, Uploaded] producing the same canonical state as the in-line path', async function () {
        const tenantId = 'acme';
        ...
        const [repo] = repos;
        if (!repo)
            throw new Error('expected one repo after dispatch');
        ...
        const revIds = revs.map(function (r) {
            return r.revisionId;
        }).sort();
        expect(revIds).toEqual(['rev-1', 'rev-2']);
        const rev1 = revs.find(function (r) {
            return r.revisionId === 'rev-1';
        });
        const rev2 = revs.find(function (r) {
            return r.revisionId === 'rev-2';
        });
        if (!rev1 || !rev2)
            throw new Error('expected rev-1 and rev-2 after dispatch');
        expect(rev1.byteCount).toBe(100);
        expect(rev2.byteCount).toBe(250);
    });
    it('is idempotent on re-dispatch (a second pass does not double-write)', async function () {
        const tenantId = 'acme';
    ...
    it('ignores events outside the repository event-type set', async function () {
        const repos = new InMemoryRepositoryStore();
        const revs = new InMemoryRepositoryRevisionStore();
        const dispatch = repositoryDispatcher({
            repositories: repos,
