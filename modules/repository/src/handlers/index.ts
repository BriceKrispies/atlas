 ...
 */
import type { Crypto, HandlerRegistry, IntentHandler, IntentHandlerContext, HandlerResult, RepositoryStore, RepositoryRevisionStore, } from '@atlas/ports';
...
import { REPOSITORY_CREATED_EVENT_TYPE, REPOSITORY_CREATED_SCHEMA_ID, REPOSITORY_CREATED_SCHEMA_VERSION, } from '../events.ts';
...
function ctxAsRepoCtx(ctx: IntentHandlerContext): RepositoryHandlerContext {
    ...
    if (!c.repositories || !c.revisions || !c.crypto) {
        throw new Error('repository handlers require `repositories` + `revisions` + `crypto` on IntentHandlerContext (wired by apps/server)');
    }
...
const repositoryUploadHandler: IntentHandler = {
    async handle(ctx: IntentHandlerContext, envelope: IntentEnvelope): Promise<HandlerResult> {
        ...
        }, c.repositories, c.revisions, ctx.eventStore, c.crypto);
        return { primary: result.envelope, follow: [] };
    },
};
export function repositoryHandlerEntries(): ReadonlyArray<readonly [
    string,
    IntentHandler
]> {
    return [
