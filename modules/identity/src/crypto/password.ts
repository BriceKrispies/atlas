...
export function validatePasswordComplexity(password: string): void {
    if (password.length < MIN_LENGTH) {
        throw new IdentityError(codes.PASSWORD_COMPLEXITY, `password must be at least ${MIN_LENGTH} characters`, 400);
    }
    if (password.length > MAX_LENGTH) {
        throw new IdentityError(codes.PASSWORD_COMPLEXITY, `password must be at most ${MAX_LENGTH} characters`, 400);
    }
    ...
    if (classes < 2) {
        throw new IdentityError(codes.PASSWORD_COMPLEXITY, 'password must mix at least two of: lowercase, uppercase, digit/symbol', 400);
    }
}
...
function decode(encoded: string): DecodeResult {
    ...
    for (const kv of params.split(',')) {
        const [k, v] = kv.split('=');
        if (!k || !v)
            return { ok: false, reason: 'malformed_param_kv' };
        const n = Number(v);
    ...
    if (salt.length === 0)
        return { ok: false, reason: 'empty_salt' };
    if (hash.length === 0)
...
export async function verifyPasswordDetailed(password: string, storedHash: string): Promise<VerifyPasswordResult> {
    const decoded = decode(storedHash);
    if (!decoded.ok) {
        const out: VerifyPasswordResult = { ok: false, reason: decoded.reason };
        if (decoded.cause !== undefined)
            (out as {
                cause?: unknown;
            }).cause = decoded.cause;
        return out;
