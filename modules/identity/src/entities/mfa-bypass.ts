...
export async function getMfaBypassEntity(
  store: EntityStore,
  tenantId: string,
  bypassId: string,
): Promise<MfaBypassDocument | null> {
  const row = await store.get<MfaBypassDocument>(
    tenantId,
...
export async function putMfaBypassEntity(
  store: EntityStore,
  doc: MfaBypassDocument,
): Promise<void> {
...
export async function findMfaBypassesByLookup(
  store: EntityStore,
  tenantId: string,
  userId: string,
  secretLookup: string,
): Promise<MfaBypassDocument[]> {
  const rows = await store.query<MfaBypassDocument>(
    tenantId,
    MFA_BYPASS_ENTITY_TYPE,
    { attrsEqual: { userId, secretLookup } },
  );
