...
export async function getUserEntity(store: EntityStore, tenantId: string, userId: string): Promise<UserDocument | null> {
    const row = await store.get<UserDocument>(tenantId, USER_ENTITY_TYPE, userId);
    if (!row || row.status !== 'active')
        return null;
    return row.attrs;
}
export async function putUserEntity(store: EntityStore, doc: UserDocument, tenantId: string): Promise<void> {
 ...
 * index.
 */
export async function findUserByIdpSubject(store: EntityStore, tenantId: string, primaryIdpSubject: string): Promise<UserDocument | null> {
 ...
 */
export async function findUserByEmail(store: EntityStore, tenantId: string, email: string): Promise<UserDocument | null> {
    const rows = await store.query<UserDocument>(tenantId, USER_ENTITY_TYPE, {
...
export async function listUsers(store: EntityStore, tenantId: string): Promise<UserDocument[]> {
