...
export async function getInviteTokenEntity(
  store: EntityStore,
  tenantId: string,
  tokenId: string,
): Promise<InviteTokenDocument | null> {
  const row = await store.get<InviteTokenDocument>(
    tenantId,
...
export async function putInviteTokenEntity(
  store: EntityStore,
  doc: InviteTokenDocument,
): Promise<void> {
 ...
 * tolerated.
 */
export async function findInviteTokensByLookup(
  store: EntityStore,
  tenantId: string,
  tokenLookup: string,
): Promise<InviteTokenDocument[]> {
  const rows = await store.query<InviteTokenDocument>(
    tenantId,
    INVITE_TOKEN_ENTITY_TYPE,
    { attrsEqual: { tokenLookup, status: 'pending' } },
  );
