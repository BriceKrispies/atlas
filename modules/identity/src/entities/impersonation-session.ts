...
export async function getImpersonationSessionEntity(store: EntityStore, tenantId: string, impersonationId: string): Promise<ImpersonationSessionDocument | null> {
    const row = await store.get<ImpersonationSessionDocument>(tenantId, IMPERSONATION_SESSION_ENTITY_TYPE, impersonationId);
    if (!row || row.status === 'deleted')
        return null;
    return row.attrs;
}
export async function putImpersonationSessionEntity(store: EntityStore, doc: ImpersonationSessionDocument): Promise<void> {
...
export async function findImpersonationByLookup(store: EntityStore, tenantId: string, tokenLookup: string): Promise<ImpersonationSessionDocument[]> {
    const rows = await store.query<ImpersonationSessionDocument>(tenantId, IMPERSONATION_SESSION_ENTITY_TYPE, { attrsEqual: { tokenLookup } });
    return rows.map(function (r) {
        return r.attrs;
    });
}
export async function listActiveImpersonationsForTenant(store: EntityStore, tenantId: string): Promise<ImpersonationSessionDocument[]> {
    const rows = await store.query<ImpersonationSessionDocument>(tenantId, IMPERSONATION_SESSION_ENTITY_TYPE, { attrsEqual: { status: 'active' } });
    return rows.map(function (r) {
        return r.attrs;
    });
}
