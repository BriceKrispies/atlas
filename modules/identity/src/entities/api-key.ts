...
export async function getApiKeyEntity(store: EntityStore, tenantId: string, keyId: string): Promise<ApiKeyDocument | null> {
    const row = await store.get<ApiKeyDocument>(tenantId, API_KEY_ENTITY_TYPE, keyId);
    if (!row || row.status === 'deleted')
        return null;
    return row.attrs;
}
export async function putApiKeyEntity(store: EntityStore, doc: ApiKeyDocument): Promise<void> {
...
export async function listApiKeysForOwner(store: EntityStore, tenantId: string, ownerKey: 'userId' | 'servicePrincipalId', ownerId: string): Promise<ApiKeyDocument[]> {
    const rows = await store.query<ApiKeyDocument>(tenantId, API_KEY_ENTITY_TYPE, { attrsEqual: { [ownerKey]: ownerId } });
 ...
 */
export function parseApiKeyBearer(bearer: string): {
    keyId: string;
    secret: string;
} | null {
    if (!bearer.startsWith('atlas_'))
        return null;
