...
import type { IdentityProviderDocument, IdentityProviderStatus, } from '../types.ts';
export const IDENTITY_PROVIDER_ENTITY_TYPE = 'IdentityProvider';
export const IDENTITY_PROVIDER_LATEST_VERSION = 1;
export async function getIdentityProviderEntity(store: EntityStore, tenantId: string, idpId: string): Promise<IdentityProviderDocument | null> {
    const row = await store.get<IdentityProviderDocument>(tenantId, IDENTITY_PROVIDER_ENTITY_TYPE, idpId);
    if (!row || row.status === 'deleted')
        return null;
    return row.attrs;
}
export async function putIdentityProviderEntity(store: EntityStore, doc: IdentityProviderDocument): Promise<void> {
 ...
 */
export async function findActiveProviderByIssuer(store: EntityStore, tenantId: string, issuer: string): Promise<IdentityProviderDocument | null> {
    const rows = await store.query<IdentityProviderDocument>(tenantId, IDENTITY_PROVIDER_ENTITY_TYPE, {
        attrsEqual: {
            issuer,
 ...
 * surface.
 */
export async function listIdentityProviders(store: EntityStore, tenantId: string): Promise<IdentityProviderDocument[]> {
    const rows = await store.list<IdentityProviderDocument>(tenantId, IDENTITY_PROVIDER_ENTITY_TYPE);
