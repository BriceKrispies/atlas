...
export async function getScimTokenEntity(
  store: EntityStore,
  tenantId: string,
  tokenId: string,
): Promise<ScimTokenDocument | null> {
  const row = await store.get<ScimTokenDocument>(
    tenantId,
...
export async function putScimTokenEntity(
  store: EntityStore,
  doc: ScimTokenDocument,
): Promise<void> {
...
export async function findScimTokensByLookup(
  store: EntityStore,
  tenantId: string,
  secretLookup: string,
): Promise<ScimTokenDocument[]> {
  const rows = await store.query<ScimTokenDocument>(
    tenantId,
    SCIM_TOKEN_ENTITY_TYPE,
...
export async function listScimTokens(
  ...
  const rows = await store.list<ScimTokenDocument>(
    tenantId,
    SCIM_TOKEN_ENTITY_TYPE,
  );
  return rows.map((r) => r.attrs);
