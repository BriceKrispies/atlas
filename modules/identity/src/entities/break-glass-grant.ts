...
export async function getBreakGlassGrantEntity(
  store: EntityStore,
  tenantId: string,
  grantId: string,
): Promise<BreakGlassGrantDocument | null> {
  const row = await store.get<BreakGlassGrantDocument>(
    tenantId,
...
export async function putBreakGlassGrantEntity(
  store: EntityStore,
  doc: BreakGlassGrantDocument,
): Promise<void> {
...
export async function listActiveGrantsForPrincipal(
  store: EntityStore,
  tenantId: string,
  grantedTo: string,
): Promise<BreakGlassGrantDocument[]> {
  const rows = await store.query<BreakGlassGrantDocument>(
    tenantId,
    BREAK_GLASS_GRANT_ENTITY_TYPE,
...
export async function listGrantsForTenant(
  store: EntityStore,
  tenantId: string,
): Promise<BreakGlassGrantDocument[]> {
  const rows = await store.query<BreakGlassGrantDocument>(
