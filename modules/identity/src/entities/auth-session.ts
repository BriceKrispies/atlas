...
export async function getSessionEntity(
  store: EntityStore,
  tenantId: string,
  sessionId: string,
): Promise<AuthSessionDocument | null> {
  const row = await store.get<AuthSessionDocument>(
    tenantId,
...
export async function putSessionEntity(
  store: EntityStore,
...
export async function listActiveSessionsForUser(
  ...
  const rows = await store.query<AuthSessionDocument>(
    tenantId,
    AUTH_SESSION_ENTITY_TYPE,
    { attrsEqual: { userId, status: 'active' satisfies AuthSessionStatus } },
  );
  const docs = rows.map((r) => r.attrs);
  docs.sort((a, b) => (a.issuedAt < b.issuedAt ? -1 : a.issuedAt > b.issuedAt ? 1 : 0));
  return docs;
}
 ...
 */
export async function findSessionsByRefreshLookup(
  store: EntityStore,
  tenantId: string,
