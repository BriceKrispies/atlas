...
export async function getSamlSpKeyEntity(
  store: EntityStore,
  tenantId: string,
  keyId: string,
): Promise<SamlSpKeyDocument | null> {
  const row = await store.get<SamlSpKeyDocument>(
    tenantId,
...
export async function putSamlSpKeyEntity(
  store: EntityStore,
  doc: SamlSpKeyDocument,
): Promise<void> {
 ...
 */
export async function findActiveSamlSpKey(
  store: EntityStore,
  tenantId: string,
): Promise<SamlSpKeyDocument | null> {
  const rows = await store.query<SamlSpKeyDocument>(
    tenantId,
    SAML_SP_KEY_ENTITY_TYPE,
    { attrsEqual: { status: 'active' satisfies SamlSpKeyStatus } },
  );
 ...
 */
export async function listMetadataSamlSpKeys(
  store: EntityStore,
  tenantId: string,
): Promise<SamlSpKeyDocument[]> {
  const rows = await store.list<SamlSpKeyDocument>(
    tenantId,
    SAML_SP_KEY_ENTITY_TYPE,
  );
  const now = Date.now();
