...
export async function handleWebAuthnAssertBegin(
  cmd: WebAuthnAssertBeginCommand,
  entities: EntityStore,
): Promise<WebAuthnAssertBeginResult> {
  ...
  let allow: { id: string }[] = [];
  if (cmd.userId) {
    const factors = await listActiveFactorsForUserByKind(
      entities,
      cmd.tenantId,
      cmd.userId,
      cmd.factorKind,
...
export async function handleWebAuthnAssertFinish(
  cmd: WebAuthnAssertFinishCommand,
  eventStore: EventStore,
  entities: EntityStore,
): Promise<WebAuthnAssertFinishResult> {
  ...
  if (challenge.kind !== 'authenticate') {
    throw new IdentityError(
      codes.WEBAUTHN_VERIFICATION_FAILED,
      `challenge is for ${challenge.kind}, not authenticate`,
      400,
    );
  }
  ...
  const factor = await findFactorByCredentialId(
    entities,
    cmd.tenantId,
    credentialId,
  );
  if (!factor) {
    throw new IdentityError(
      codes.PASSKEY_NOT_REGISTERED,
      'no factor matches the presented credential',
      401,
    );
  }
  if (factor.kind !== cmd.factorKind) {
    throw new IdentityError(
      codes.WEBAUTHN_VERIFICATION_FAILED,
      `factor kind mismatch: expected ${cmd.factorKind}, got ${factor.kind}`,
      400,
    );
  }
  if (factor.status !== 'active') {
    throw new IdentityError(
      ...
      `factor ${factor.factorId} is in status ${factor.status}`,
      401,
    );
  }
  ...
  if (
    challenge.userId !== '_passkey_discovery' &&
  ...
  if (newSignCount !== 0 && newSignCount <= wAttrs.signCount) {
    ...
    if (wAttrs.signCount > 0) {
      throw new IdentityError(
        codes.WEBAUTHN_SIGN_COUNT_REGRESSION,
        `signCount regression: stored ${wAttrs.signCount}, presented ${newSignCount}`,
        401,
      );
    }
  }
