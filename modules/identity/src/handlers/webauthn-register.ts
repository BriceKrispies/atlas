...
import type { AuthFactorDocument, AuthFactorKind, IdentityPolicy, WebAuthnFactorAttrs, } from '../types.ts';
 ...
 */
export async function handleWebAuthnRegisterBegin(cmd: WebAuthnRegisterBeginCommand, entities: EntityStore): Promise<WebAuthnRegisterBeginResult> {
    // Existing factors of this kind become `excludeCredentials` so
    // the authenticator refuses to enroll the same key twice.
    const existing = await listActiveFactorsForUserByKind(entities, cmd.tenantId, cmd.userId, cmd.factorKind);
    const exclude = existing
        .map(function (f) {
        return isWebAuthnAttrs(f.attrs) && f.attrs.credentialId
...
export async function handleWebAuthnRegisterFinish(cmd: WebAuthnRegisterFinishCommand, eventStore: EventStore, entities: EntityStore): Promise<WebAuthnRegisterFinishResult> {
    ...
    if (challenge.kind !== 'register') {
        throw new IdentityError(codes.WEBAUTHN_VERIFICATION_FAILED, `challenge is for ${challenge.kind}, not register`, 400);
    }
    if (challenge.userId !== cmd.userId) {
        throw new IdentityError(codes.WEBAUTHN_VERIFICATION_FAILED, 'challenge userId mismatch', 403);
    }
    ...
    try {
        ...
        throw new IdentityError(codes.WEBAUTHN_VERIFICATION_FAILED, `attestation verification failed: ${e instanceof Error ? e.message : String(e)}`, 400);
    }
    if (!verification.verified || !verification.registrationInfo) {
        throw new IdentityError(codes.WEBAUTHN_VERIFICATION_FAILED, 'attestation not verified', 400);
    }
