 ...
 */
export async function handleOAuthIssueToken(
  cmd: OAuthIssueCommand,
  ...
  if (cmd.clientBearer) {
    ...
    if (!parsed) {
      throw new IdentityError(
        codes.OAUTH_INVALID_CLIENT,
        'malformed client bearer',
        401,
      );
    }
    keyId = parsed.keyId;
  ...
  if (apiKey.status === 'rotated') {
    if (
      ...
      throw new IdentityError(
        codes.OAUTH_INVALID_CLIENT,
        'client rotation overlap window expired',
        401,
      );
    }
  }
  ...
  const exceeded = requested.filter((s) => !apiKey.scopes.includes(s));
  if (exceeded.length > 0) {
    throw new IdentityError(
      codes.OAUTH_INVALID_SCOPE,
      `requested scopes exceed key: ${exceeded.join(', ')}`,
      400,
    );
  }
