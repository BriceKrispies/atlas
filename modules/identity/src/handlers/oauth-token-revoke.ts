 ...
 */
export async function handleOAuthRevokeToken(
  cmd: OAuthRevokeCommand,
  eventStore: EventStore,
  entities: EntityStore,
): Promise<OAuthRevokeResult> {
  const candidates = await findOAuthTokensByLookup(
    entities,
    cmd.tenantId,
    lookupOf(cmd.presentedToken),
  );
  const presentedHash = hashSecret(cmd.presentedToken);
  const token = candidates.find((t) =>
    constantTimeEqual(t.secretHash, presentedHash),
  );
  if (!token) {
  ...
  if (!revoked.tokenId) {
    throw new IdentityError(
      ...
      'unreachable: revoked token missing id',
      500,
    );
  }
