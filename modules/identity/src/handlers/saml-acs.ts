...
export async function handleSamlAcs(
  cmd: SamlAcsCommand,
  ...
  const candidates = await entities.query<IdentityProviderDocument>(
    cmd.tenantId,
    'IdentityProvider',
    { attrsEqual: { samlEntityId: claimedIssuer, status: 'active' } },
  );
  const idp =
    candidates[0]?.attrs ??
  ...
  if (!idp || idp.kind !== 'saml') {
    throw new IdentityError(
      codes.SAML_INVALID_RESPONSE,
      `no SAML IdentityProvider for issuer ${claimedIssuer}`,
      401,
    );
  }
  if (!idp.samlIdpCert) {
    throw new IdentityError(
      codes.SAML_INVALID_RESPONSE,
      `IdentityProvider ${idp.idpId} missing samlIdpCert`,
      500,
    );
  }
  ...
  const verified = await verifySamlResponse(xml, {
    ...
    recordSeenAssertion: async (assertionId, expiresAt) =>
      recordSeenAssertion(entities, cmd.tenantId, idp.idpId, assertionId, expiresAt),
  });
