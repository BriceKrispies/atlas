...
import type { AuthSessionDocument, SessionEndReason, SessionPolicy, } from '../types.ts';
...
import { getSessionEntity, listActiveSessionsForUser, } from '../entities/auth-session.ts';
 ...
 */
export async function handleSessionRefresh(cmd: SessionRefreshCommand, eventStore: EventStore, entities: EntityStore): Promise<SessionRefreshResult> {
    ...
    if (!session) {
        throw new IdentityError(codes.SESSION_NOT_FOUND, `session not found: ${cmd.sessionId}`, 401);
    }
    ...
    const matchesPrevious = session.previousRefreshTokenHash !== undefined &&
    ...
    // the ring contents.
    const matchesRevoked = !matchesCurrent &&
        !matchesPrevious &&
        (session.revokedRefreshTokenHashes ?? []).some(function (h) {
            return constantTimeEqual(h, presentedHash);
        });
    if (!matchesCurrent && !matchesPrevious && !matchesRevoked) {
        throw new IdentityError(codes.SESSION_NOT_FOUND, 'refresh token does not match', 401);
    }
    // ----- Reuse-detection ------------------------------------------
    ...
    if (matchesRevoked ||
        ...
        const allSessions = await listActiveSessionsForUser(entities, cmd.tenantId, session.userId);
        ...
        const error = new IdentityError(codes.SESSION_REUSE_DETECTED, 'refresh token reuse detected; all sessions revoked', 401);
    ...
    if (new Date(session.hardExpiresAt).getTime() <= now) {
        ...
        throw new IdentityError(codes.SESSION_HARD_TIMEOUT, 'session has reached its absolute lifetime cap', 401);
    }
    ...
    if (new Date(session.lastSeenAt).getTime() < idleCutoff) {
        ...
        expireEvent.eventId = stored.eventId;
        expireEvent.seq = stored.seq;
        throw new IdentityError(codes.SESSION_IDLE_TIMEOUT, 'session idle-timeout exceeded', 401);
    }
    // ----- Rotate ---------------------------------------------------
