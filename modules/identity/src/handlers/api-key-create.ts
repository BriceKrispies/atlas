...
export async function handleApiKeyCreate(
  cmd: ApiKeyCreateCommand,
  eventStore: EventStore,
  entities: EntityStore,
): Promise<ApiKeyCreateResult> {
  const hasUser = typeof cmd.userId === 'string' && cmd.userId.length > 0;
  const hasSp =
    typeof cmd.servicePrincipalId === 'string' && cmd.servicePrincipalId.length > 0;
  if (hasUser === hasSp) {
    throw new IdentityError(
      codes.IDENTITY_INVALID,
      'ApiKey requires exactly one of userId or servicePrincipalId',
      400,
    );
  }
  ...
  if (hasSp && cmd.servicePrincipalId !== undefined) {
    ...
    if (!sp) {
      throw new IdentityError(
        ...
        `service principal not found: ${cmd.servicePrincipalId}`,
        404,
      );
    }
    if (sp.status !== 'active') {
      throw new IdentityError(
        codes.SERVICE_PRINCIPAL_DISABLED,
        `service principal ${cmd.servicePrincipalId} is disabled`,
        409,
      );
    }
    const exceeded = cmd.scopes.filter((s) => !sp.scopes.includes(s));
    if (exceeded.length > 0) {
      throw new IdentityError(
        codes.SERVICE_PRINCIPAL_SCOPE_EXCEEDED,
        `requested scopes exceed SP ceiling: ${exceeded.join(', ')}`,
        403,
      );
    }
  }
