...
export async function handleBreakGlassIssue(
  cmd: BreakGlassIssueCommand,
  eventStore: EventStore,
  entities: EntityStore,
): Promise<BreakGlassIssueResult> {
  if (!cmd.issuedBy) {
    throw new IdentityError(
      codes.BREAK_GLASS_REQUIRES_OPERATOR,
      'BreakGlass.Issue requires an issuedBy principal',
      403,
    );
  }
  if (!cmd.justification || cmd.justification.trim().length === 0) {
    throw new IdentityError(
      codes.BREAK_GLASS_JUSTIFICATION_REQUIRED,
      'BreakGlass.Issue requires a non-empty justification',
      400,
    );
  }
  if (!cmd.incidentUrl || cmd.incidentUrl.trim().length === 0) {
    throw new IdentityError(
      codes.BREAK_GLASS_INCIDENT_REQUIRED,
      'BreakGlass.Issue requires an incidentUrl',
      400,
    );
  }
  ...
  if (duration <= 0 || duration > MAX_DURATION_HARD_CAP_MIN) {
    throw new IdentityError(
      codes.BREAK_GLASS_DURATION_INVALID,
  ...
  if (cmd.grantedRoles.length === 0) {
    throw new IdentityError(
      ...
      'grantedRoles must be non-empty',
      400,
    );
  }
...
export async function handleBreakGlassApprove(
  cmd: BreakGlassApproveCommand,
  eventStore: EventStore,
  entities: EntityStore,
): Promise<BreakGlassApproveResult> {
  const existing = await getBreakGlassGrantEntity(
    entities,
    cmd.tenantId,
    cmd.grantId,
  );
  if (!existing) {
    throw new IdentityError(
      codes.BREAK_GLASS_GRANT_NOT_FOUND,
      'break-glass grant not found',
      404,
    );
  }
  if (existing.status !== 'pending_approval') {
    throw new IdentityError(
      codes.BREAK_GLASS_NOT_PENDING,
      `grant in status ${existing.status}`,
      409,
    );
  }
  if (existing.issuedBy === cmd.approvedBy) {
    throw new IdentityError(
      codes.BREAK_GLASS_SELF_APPROVAL_FORBIDDEN,
      'issuer and approver must differ (4-eyes)',
      403,
    );
  }
  const occurredAt = new Date().toISOString();
  ...
  const expiresAt = new Date(
    Date.now() + existing.maxDurationMin * 60 * 1000,
  ).toISOString();
...
export async function handleBreakGlassDeny(
  cmd: BreakGlassDenyCommand,
  eventStore: EventStore,
  entities: EntityStore,
): Promise<BreakGlassDenyResult> {
  const existing = await getBreakGlassGrantEntity(
    entities,
    cmd.tenantId,
    cmd.grantId,
...
export async function handleBreakGlassAction(
  cmd: BreakGlassActionCommand,
  eventStore: EventStore,
...
export async function handleBreakGlassRevoke(
  cmd: BreakGlassRevokeCommand,
  eventStore: EventStore,
  entities: EntityStore,
): Promise<BreakGlassRevokeResult> {
  const existing = await getBreakGlassGrantEntity(
    entities,
    cmd.tenantId,
    cmd.grantId,
  ...
  if (existing.status !== 'active' && existing.status !== 'pending_approval') {
    throw new IdentityError(
      codes.BREAK_GLASS_NOT_ACTIVE,
      `grant in status ${existing.status}`,
      409,
    );
  }
  const occurredAt = new Date().toISOString();
  const status: BreakGlassGrantDocument['status'] =
    cmd.reason === 'auto_expired' ? 'expired' : 'revoked';
  const next: BreakGlassGrantDocument = {
    ...existing,
    status,
    endedAt: occurredAt,
 ...
 */
export async function resolveActiveGrants(
  entities: EntityStore,
  tenantId: string,
  grantedTo: string,
): Promise<BreakGlassGrantDocument[]> {
  const rows = await entities.query<BreakGlassGrantDocument>(
    tenantId,
    'BreakGlassGrant',
    { attrsEqual: { grantedTo, status: 'active' } },
  );
  const now = Date.now();
  return rows
    .map((r) => r.attrs)
