...
import type { AuthFactorDocument, IdentityPolicy, } from '../types.ts';
import { newEventId } from '../ids.ts';
import { getAuthFactorEntity, listFactorsForUser, } from '../entities/auth-factor.ts';
 ...
 */
export async function handleFactorRevoke(cmd: FactorRevokeCommand, eventStore: EventStore, entities: EntityStore): Promise<FactorRevokeResult> {
    const factor = await getAuthFactorEntity(entities, cmd.tenantId, cmd.factorId);
    if (!factor) {
        throw new IdentityError(codes.MFA_FACTOR_NOT_FOUND, `auth factor not found: ${cmd.factorId}`, 404);
    }
    if (cmd.policy?.mfaRequired && !cmd.force) {
        const allFactors = await listFactorsForUser(entities, cmd.tenantId, factor.userId);
        const otherActive = allFactors.filter(function (f) {
            return f.factorId !== factor.factorId && f.status === 'active';
        });
        if (otherActive.length === 0 && factor.status === 'active') {
            throw new IdentityError(codes.MFA_LAST_FACTOR_PROTECTED, `cannot revoke the last active factor while tenant policy requires MFA — enroll a replacement first or pass force=true`, 409);
        }
    }
