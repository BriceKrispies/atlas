 ...
 */
import { PLATFORM_ROBOT_PRINCIPAL_ID, type EventEnvelope, } from '@atlas/platform-core';
import type { EntityStore, EventStore } from '@atlas/ports';
import { IdentityError, codes } from '../errors.ts';
import type { IdentityProviderDocument, MembershipDocument, RoleMapping, UserDocument, } from '../types.ts';
import { handleUserCreate } from './user-create.ts';
import { findUserByIdpSubject } from '../entities/user.ts';
import { getMembershipEntity, putMembershipEntity, } from '../entities/membership.ts';
 ...
 */
function readGroupClaim(raw: Record<string, unknown>, path: string): string[] {
    ...
    if (!Array.isArray(cursor))
        return [];
    return cursor.filter(function (v): v is string {
 ...
 */
function resolveRolesForLogin(idp: IdentityProviderDocument, claims: JitClaims): string[] {
    ...
    const groupsLower = groups.map(function (g) {
...
export async function handleJitProvision(cmd: JitProvisionCommand, eventStore: EventStore, entities: EntityStore): Promise<JitProvisionResult> {
    const events: EventEnvelope[] = [];
    const existing = await findUserByIdpSubject(entities, cmd.tenantId, cmd.claims.sub);
    const desiredRoles = resolveRolesForLogin(cmd.idp, cmd.claims);
    if (existing) {
        // Returning user. Reconcile Membership.roles when groups changed.
        let membership = await getMembershipEntity(entities, cmd.tenantId, existing.userId);
        if (!membership) {
            ...
            const same = desiredSorted.length === currentSorted.length &&
                desiredSorted.every(function (r, i) {
                    return r === currentSorted[i];
                });
            if (!same) {
    ...
    if (cmd.idp.requireInvite) {
        throw new IdentityError(codes.JIT_PROVISIONING_DISABLED, `IdP ${cmd.idp.idpId} requires invite before login (no User found for sub=${cmd.claims.sub})`, 403);
    }
