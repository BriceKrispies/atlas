...
import type { IdentityPolicy, RecoveryCodeDocument, } from '../types.ts';
import { DEFAULT_IDENTITY_POLICY } from '../types.ts';
import { newEventId, newRecoveryBatchId, newRecoveryCodeId, } from '../ids.ts';
import { constantTimeEqual, generateSecret, hashSecret, lookupOf, } from '../crypto/secret-hash.ts';
import { findRecoveryCodesByLookup, listRecoveryCodesForUser, putRecoveryCodeEntity, } from '../entities/recovery-code.ts';
// =====================================================================
 ...
 */
export async function handleGenerateRecoveryCodes(cmd: GenerateRecoveryCodesCommand, eventStore: EventStore, entities: EntityStore): Promise<GenerateRecoveryCodesResult> {
    ...
    const hasActive = existing.some(function (c) {
        return c.status === 'active';
    });
    if (hasActive) {
        throw new IdentityError(codes.RECOVERY_CODE_INVALID, `recovery codes already exist for user ${cmd.userId}; use regenerate instead`, 409);
    }
    return mintBatch(cmd, eventStore, entities, policy.recoveryCodeCount, false);
}
export async function handleRegenerateRecoveryCodes(cmd: GenerateRecoveryCodesCommand, eventStore: EventStore, entities: EntityStore): Promise<GenerateRecoveryCodesResult> {
    const policy = cmd.policy ?? DEFAULT_IDENTITY_POLICY;
...
async function mintBatch(cmd: GenerateRecoveryCodesCommand, eventStore: EventStore, entities: EntityStore, count: number, isRegen: boolean): Promise<GenerateRecoveryCodesResult> {
    const occurredAt = new Date().toISOString();
    ...
    const envelope: EventEnvelope = {
        ...
        payload: {
            batchId,
            codeIds: documents.map(function (d) {
                return d.codeId;
            }),
            count,
        },
...
export async function handleRedeemRecoveryCode(cmd: RedeemRecoveryCodeCommand, eventStore: EventStore, entities: EntityStore): Promise<RedeemRecoveryCodeResult> {
    const candidates = await findRecoveryCodesByLookup(entities, cmd.tenantId, cmd.userId, lookupOf(cmd.presentedCode));
    ...
    if (!matched) {
        throw new IdentityError(codes.RECOVERY_CODE_INVALID, 'recovery code not found or already consumed', 401);
    }
    const occurredAt = new Date().toISOString();
    ...
    const remaining = all.filter(function (c) {
        return c.status === 'active';
    }).length;
    const envelope: EventEnvelope = {
        eventId: newEventId(),
