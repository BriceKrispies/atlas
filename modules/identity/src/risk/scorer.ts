...
export function defaultRiskScorer(opts: DefaultScorerOptions = {}): RiskScorer {
    return function (signals: RiskSignals): RiskScore {
        const contributions: Record<string, number> = {};
        // Geo: only contributes when we have an expected set to compare against.
        const signalGeo = signals.geo;
        if (opts.expectedGeo !== undefined &&
            signalGeo !== undefined &&
            signalGeo !== 'unknown') {
            const geoLc = signalGeo.toLowerCase();
            const matched = opts.expectedGeo.some(function (g) {
                return g.toLowerCase() === geoLc;
            });
        ...
        contributions['recentFailureRate'] = Math.max(0, Math.min(0.4, failureRate * 0.4));
        // Time-of-day: small constant when nightly UTC.
        if (signals.hourUtc !== undefined &&
            signals.hourUtc >= 0 &&
            signals.hourUtc < 6) {
        ...
        const total = Object.values(contributions).reduce(function (a, b) {
            return a + b;
        }, 0);
...
export function decideFromScore(score: number, thresholds: {
    stepUpMfaThreshold: number;
    hardDenyThreshold: number;
}): RiskDecision {
    if (score >= thresholds.hardDenyThreshold)
        return 'hard_deny';
    if (score >= thresholds.stepUpMfaThreshold)
        return 'step_up';
    return 'allow';
}
