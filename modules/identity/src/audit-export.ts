...
export class InMemoryUploader implements Uploader {
  pushed: Array<{ destination: AuditExportS3Destination; key: string; body: Buffer }> = [];
  async upload(
    destination: AuditExportS3Destination,
    objectKey: string,
    body: Buffer,
...
export interface AuditExportRunOptions {
   ...
   */
  objectKey?: (ctx: { tenantId: string; runId: string; isoDate: string }) => string;
}
...
export async function runAuditExport(
  config: AuditExportConfigDocument,
  uploader: Uploader,
  eventStore: EventStore,
  entities: EntityStore,
  opts: AuditExportRunOptions = {},
  ...
  const objectKey =
    opts.objectKey?.({ tenantId: config.tenantId, runId, isoDate }) ??
...
export async function exportTenantAudit(
  ...
  if (config.status !== 'active') {
    throw new Error(
      `audit export config ${configId} is in status ${config.status} (must be active)`,
    );
  }
