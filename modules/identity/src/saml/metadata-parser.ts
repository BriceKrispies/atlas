...
import type { SamlAttributeMappings, SamlNameIdFormat, } from '../types.ts';
...
export function parseIdpMetadata(xml: string): ParsedIdpMetadata {
    ...
    try {
        ...
        throw new IdentityError(codes.SAML_INVALID_METADATA, `metadata XML parse failed: ${message}`, 400);
    }
    if (!parsedRoot) {
        throw new IdentityError(codes.SAML_INVALID_METADATA, 'metadata XML did not parse to a document element', 400);
    }
    const ed = asXmlRecord(parsedRoot['EntityDescriptor']) ??
        asXmlRecord(parsedRoot['EntitiesDescriptor']);
    if (!ed) {
        throw new IdentityError(codes.SAML_INVALID_METADATA, 'EntityDescriptor element missing', 400);
    }
    const entityId = asXmlString(ed['@_entityID']) ?? asXmlString(ed['@_entityId']);
    if (!entityId) {
        throw new IdentityError(codes.SAML_INVALID_METADATA, 'EntityDescriptor@entityID missing', 400);
    }
    const idpSsoDescriptor = asXmlRecord(ed['IDPSSODescriptor']);
    if (!idpSsoDescriptor) {
        throw new IdentityError(codes.SAML_INVALID_METADATA, 'IDPSSODescriptor missing — only IdP metadata is supported here', 400);
    }
    // ----- SSO URL: prefer POST binding, fall back to Redirect -------
    const ssoServices = asXmlRecordArray(idpSsoDescriptor['SingleSignOnService']);
    const post = ssoServices.find(function (s) {
        return asXmlString(s['@_Binding']) === POST_BINDING;
    });
    const redirect = ssoServices.find(function (s) {
        return asXmlString(s['@_Binding']) === REDIRECT_BINDING;
    });
    ...
    if (!chosen || !chosenLocation) {
        throw new IdentityError(codes.SAML_INVALID_METADATA, 'no SingleSignOnService/@Location in metadata', 400);
    }
    ...
    const signingKD = keyDescriptors.find(function (k) {
        return asXmlString(k['@_use']) === 'signing';
    }) ??
        keyDescriptors.find(function (k) {
            return asXmlString(k['@_use']) === undefined;
        }) ??
        keyDescriptors[0];
    if (!signingKD) {
        throw new IdentityError(codes.SAML_INVALID_METADATA, 'no KeyDescriptor in metadata', 400);
    }
    ...
    if (!x509Cert) {
        throw new IdentityError(codes.SAML_INVALID_METADATA, 'no X509Certificate in KeyInfo', 400);
    }
    ...
    const matchedNid = nameIds
        .map(function (n) {
        return NAMEID_FORMAT_MAP[n];
    })
        .filter(function (v): v is SamlNameIdFormat {
        return v !== undefined;
    })[0] ?? 'unspecified';
    return {
        entityId,
