...
describe('handleJitProvision — provisions on first login', () => {
  it('mints User + Membership, both events carry Tenant:<tenantId> tag (I10)', async () => {
    const fx = newFixture();
    ...
    const userCreated = result.events.find(
      (e) => e.eventType === 'Identity.UserCreated',
    );
    const membershipCreated = result.events.find(
...
describe('handleJitProvision — requireInvite gate', () => {
  it('rejects unknown subject when idp.requireInvite=true; appends NO events', async () => {
    const fx = newFixture();
    await expect(
      handleJitProvision(
        {
