...
describe('handleMfaBypassIssue', () => {
  it('emits MfaBypassIssued with retention:1y and exact cache tags', async () => {
    const fx = newFixture();
  ...
  it('plaintext secret never persists on the document', async () => {
    const fx = newFixture();
    const result = await handleMfaBypassIssue(
      {
        tenantId: fx.tenantId,
  ...
  it('rejects empty principalId with IDENTITY_INVALID (audit-trail requirement)', async () => {
    const fx = newFixture();
    await expect(
      handleMfaBypassIssue(
        {
...
describe('handleMfaBypassUse', () => {
  ...
  it('emits MfaBypassUsed and flips status to used', async () => {
    const fx = newFixture();
  ...
  it('rejects unknown bypass with BYPASS_TOKEN_NOT_FOUND', async () => {
    const fx = newFixture();
    await expect(
      handleMfaBypassUse(
        {
    ...
    ).rejects.toMatchObject({ code: identityErrorCodes.BYPASS_TOKEN_NOT_FOUND });
    expect(fx.events.events).toHaveLength(0);
  });

  it('rejects already-used bypass with BYPASS_TOKEN_USED on second redeem', async () => {
    const fx = newFixture();
    const issued = await issue(fx);
    ...
    ).rejects.toMatchObject({ code: identityErrorCodes.BYPASS_TOKEN_USED });
  });

  it('rejects expired bypass (issuedAt + ttl in the past) with BYPASS_TOKEN_EXPIRED', async () => {
    const fx = newFixture();
    ...
    ).rejects.toMatchObject({ code: identityErrorCodes.BYPASS_TOKEN_EXPIRED });
  });
