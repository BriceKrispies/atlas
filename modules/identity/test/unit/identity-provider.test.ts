...
describe('handleIdpConfigure', function () {
    it('emits IdentityProviderConfigured with status=configured (NOT active)', async function () {
        const fx = newFixture();
        const result = await handleIdpConfigure({
            tenantId: fx.tenantId,
    ...
    it('rejects when neither jwksUri nor discoveryDocument.jwks_uri is provided', async function () {
        const fx = newFixture();
        await expect(handleIdpConfigure({
            ...
            issuer: 'https://idp.example.com',
            audience: 'https://atlas.example.com',
        }, fx.events)).rejects.toMatchObject({ code: identityErrorCodes.IDP_INVALID_CONFIG });
        expect(fx.events.events).toHaveLength(0);
    });
    it('accepts discoveryDocument.jwks_uri as fallback when jwksUri not provided', async function () {
        const fx = newFixture();
        const result = await handleIdpConfigure({
            tenantId: fx.tenantId,
            correlationId: 'c',
    ...
    it('rejects empty issuer or audience with IDP_INVALID_CONFIG', async function () {
        const fx = newFixture();
        await expect(handleIdpConfigure({
            tenantId: fx.tenantId,
            correlationId: 'c',
    ...
    it('defaults kind=oidc, requireInvite=false, priority=100', async function () {
        const fx = newFixture();
        const result = await handleIdpConfigure({
            tenantId: fx.tenantId,
            correlationId: 'c',
    ...
    it('persists overrides for kind, requireInvite, priority, groupClaimPath', async function () {
        const fx = newFixture();
        const result = await handleIdpConfigure({
            tenantId: fx.tenantId,
            correlationId: 'c',
...
describe('handleIdpActivate', function () {
    it('flips status from configured to active', async function () {
        const fx = newFixture();
    ...
    it('is idempotent — activating an already-active IdP returns NoOp event', async function () {
        const fx = newFixture();
        const idp = await configureIdp(fx);
        ...
        expect(second.envelope.eventType).toBe('Identity.IdentityProviderActivated.NoOp');
    });
    it('rejects unknown idpId with IDP_NOT_FOUND', async function () {
        const fx = newFixture();
        await expect(handleIdpActivate({
            tenantId: fx.tenantId,
            correlationId: 'c',
            principalId: 'admin',
            idpId: 'idp-fake',
        }, fx.events, fx.entities)).rejects.toMatchObject({ code: identityErrorCodes.IDP_NOT_FOUND });
    });
});
describe('handleIdpDisable', function () {
...
describe('handleIdpRotateJwks', function () {
    it('clears jwksFetchedAt and emits RotatedJwks with Jwks tag', async function () {
        const fx = newFixture();
        const idp = await configureIdp(fx);
    ...
    it('updates jwksUri when override provided', async function () {
        const fx = newFixture();
        const idp = await configureIdp(fx);
        const result = await handleIdpRotateJwks({
            tenantId: fx.tenantId,
    ...
    it('rejects unknown idpId with IDP_NOT_FOUND', async function () {
        const fx = newFixture();
        await expect(handleIdpRotateJwks({
            tenantId: fx.tenantId,
            correlationId: 'c',
            principalId: 'admin',
            idpId: 'idp-fake',
...
describe('handleJitProvision — smoke', function () {
    ...
    async function activeIdp(fx: ReturnType<typeof newFixture>, overrides: Partial<Parameters<typeof handleIdpConfigure>[0]> = {}) {
        ...
        await dispatchAll(fx);
        const entity = await fx.entities.get<IdentityProviderDocument>(fx.tenantId, 'IdentityProvider', idp.idpId);
        if (!entity)
            throw new Error(`IdentityProvider ${idp.idpId} missing after activation`);
        return entity.attrs;
    }
    it('creates a new User + Membership on first login', async function () {
        const fx = newFixture();
        ...
        expect(result.events.some(function (e) {
    ...
    it('reuses the existing User on second login (created=false)', async function () {
        const fx = newFixture();
        const idp = await activeIdp(fx, {
            defaultRolesOnFirstLogin: ['Author'],
        });
        ...
        const first = await handleJitProvision({ tenantId: fx.tenantId, correlationId: 'jit-1', idp, claims }, fx.events, fx.entities);
        await dispatchAll(fx);
        const second = await handleJitProvision({ tenantId: fx.tenantId, correlationId: 'jit-2', idp, claims }, fx.events, fx.entities);
        expect(second.created).toBe(false);
        expect(second.user.userId).toBe(first.user.userId);
    });
});
describe('IdP suite — tenant scoping', function () {
    it('IdP in tenant B is invisible to tenant A handler invocations', async function () {
...
describe('IdP suite — error type', function () {
    it('all rejection paths throw IdentityError instances', async function () {
        const fx = newFixture();
        await expect(handleIdpActivate({
            tenantId: fx.tenantId,
            correlationId: 'c',
