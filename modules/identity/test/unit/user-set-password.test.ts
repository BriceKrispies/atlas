...
describe('handlePasswordSet — happy path', function () {
    it('emits Identity.PasswordChanged with the documented envelope', async function () {
        const fx = newFixture();
    ...
    it('persists Argon2id hash on the document and clears failedLoginCount + lockedUntil', async function () {
        const fx = newFixture();
        const userId = await seedUser(fx);
    ...
    it('plaintext password does NOT appear in the persisted document or events', async function () {
        const fx = newFixture();
        const userId = await seedUser(fx);
        const result = await handlePasswordSet({
            tenantId: fx.tenantId,
    ...
    it('session-fixation reset: revokes every active session with reason=password_changed', async function () {
        const fx = newFixture();
        const userId = await seedUser(fx);
    ...
    it('returns empty follow + revokedSessionIds when user had no active sessions', async function () {
        const fx = newFixture();
        const userId = await seedUser(fx);
        const result = await handlePasswordSet({
            tenantId: fx.tenantId,
...
describe('handlePasswordSet — complexity validation', function () {
    const fx = function () {
        return newFixture();
    };
    it('rejects passwords shorter than 12 characters with PASSWORD_COMPLEXITY', async function () {
        const f = fx();
        ...
        await expect(handlePasswordSet({
            ...
            userId,
            newPassword: 'Short1!',
        }, f.events, f.entities)).rejects.toMatchObject({ code: identityErrorCodes.PASSWORD_COMPLEXITY });
    });
    it('rejects passwords longer than 256 characters with PASSWORD_COMPLEXITY', async function () {
        const f = fx();
        const userId = await seedUser(f);
        await expect(handlePasswordSet({
            tenantId: f.tenantId,
    ...
    it('rejects passwords with only one character class (all-lowercase) with PASSWORD_COMPLEXITY', async function () {
        const f = fx();
        const userId = await seedUser(f);
        await expect(handlePasswordSet({
            tenantId: f.tenantId,
    ...
    it('accepts mixed lower + upper (no digit/symbol)', async function () {
        const f = fx();
        const userId = await seedUser(f);
        const result = await handlePasswordSet({
            tenantId: f.tenantId,
    ...
    it('accepts lower + digit (no upper)', async function () {
        const f = fx();
        const userId = await seedUser(f);
        const result = await handlePasswordSet({
            tenantId: f.tenantId,
...
describe('handlePasswordSet — error paths', function () {
    it('rejects unknown userId with USER_NOT_FOUND', async function () {
        const fx = newFixture();
        await expect(handlePasswordSet({
            tenantId: fx.tenantId,
            correlationId: 'c',
