...
import { handleUserCreate, handlePasswordSet, handlePasswordLogin, IdentityError, type UserDocument, } from '../../src/index.ts';
...
function payloadRecord(env: EventEnvelope): Record<string, unknown> {
    return recordOf(env.payload, `payload on ${env.eventType} (${env.eventId})`);
}
async function seedUserWithPassword(fx: ReturnType<typeof newFixture>, email: string, password: string): Promise<UserDocument> {
...
describe('handlePasswordLogin — happy path', function () {
    it('emits LoginSucceeded + UserUpdated + SessionIssued in order', async function () {
        const fx = newFixture();
        const user = await seedUserWithPassword(fx, 'happy@example.com', 'CorrectHorseBatteryStaple1!');
    ...
    it('exact cacheInvalidationTags on LoginSucceeded: Tenant + User', async function () {
        const fx = newFixture();
        const user = await seedUserWithPassword(fx, 'tags@example.com', 'Pa55word!Pa55word!');
        const result = await handlePasswordLogin({
            tenantId: fx.tenantId,
            correlationId: 'c',
    ...
    it('clears failedLoginCount and stamps lastLoginAt on the updated user', async function () {
        const fx = newFixture();
    ...
    it('returns a sessionResult with plaintexts surfaced once and not stored', async function () {
        const fx = newFixture();
...
describe('handlePasswordLogin — configuration knobs', function () {
    it('honors issueSession=false (no session, no SessionIssued in follow)', async function () {
        const fx = newFixture();
    ...
    it('surfaces attemptIp on LoginSucceeded payload and on the session document', async function () {
        const fx = newFixture();
    ...
    it('passes attemptUserAgent through to the minted session', async function () {
        const fx = newFixture();
...
describe('handlePasswordLogin — reject paths', function () {
    it('unknown_user: emits LoginRejected with emailHash (PII reduction)', async function () {
        const fx = newFixture();
        const result = await handlePasswordLogin({
            tenantId: fx.tenantId,
            correlationId: 'c',
    ...
    it('wrong_password: emits LoginRejected with email plaintext (user exists)', async function () {
        const fx = newFixture();
    ...
    it('wrong_password: emits UserUpdated follow that bumps failedLoginCount', async function () {
        const fx = newFixture();
    ...
    it('account_locked: 5th consecutive wrong_password emits AccountLocked instead of UserUpdated', async function () {
        const fx = newFixture();
        ...
        const lockedFollow = assertDefined(fifth.follow[0], 'expected AccountLocked follow event on attempt 5');
        expect(lockedFollow.eventType).toBe('Identity.AccountLocked');
        const lockedDoc = recordOf(payloadRecord(lockedFollow)['document'], 'AccountLocked document');
        expect(lockedDoc['failedLoginCount']).toBe(5);
        expect(lockedDoc['lockedUntil']).toBeDefined();
    });
    it('account_locked rejects subsequent attempts with reason=account_locked (no UserUpdated follow)', async function () {
        const fx = newFixture();
    ...
    it('user_inactive: rejects active=false users (deactivation hardens against stolen creds)', async function () {
        const fx = newFixture();
    ...
    it('throws no IdentityError on rejection — rejection is a normal Result, not an error', async function () {
        const fx = newFixture();
        await expect(handlePasswordLogin({
            tenantId: fx.tenantId,
            correlationId: 'c',
            email: 'unknown@example.com',
            password: 'whatever',
        }, fx.events, fx.entities)).resolves.not.toBeInstanceOf(IdentityError);
    });
});
describe('handlePasswordLogin — tenant scoping', function () {
    it('user in tenant B cannot login through tenant A handler invocation', async function () {
