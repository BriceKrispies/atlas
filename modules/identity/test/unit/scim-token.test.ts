...
describe('handleScimTokenEnable', function () {
    it('emits ScimTokenEnabled with status=active and exact cache tags', async function () {
        const fx = newFixture();
        ...
        expect(result.envelope.schemaId).toBe('domain.identity.scim_token.enabled.v1');
    ...
    it('plaintext secret surfaced once and not on the document', async function () {
        const fx = newFixture();
        const result = await handleScimTokenEnable({
            tenantId: fx.tenantId,
            correlationId: 'c',
    ...
    it('persists optional expiresAt', async function () {
        const fx = newFixture();
...
describe('handleScimTokenRotate', function () {
    it('mints successor, flips predecessor to rotated, returns new plaintext', async function () {
        const fx = newFixture();
    ...
    it('rejects unknown tokenId with SCIM_TOKEN_NOT_FOUND', async function () {
        const fx = newFixture();
        await expect(handleScimTokenRotate({
            ...
            principalId: 'admin',
            tokenId: 'scim-fake',
        }, fx.events, fx.entities)).rejects.toMatchObject({ code: identityErrorCodes.SCIM_TOKEN_NOT_FOUND });
        expect(fx.events.events).toHaveLength(0);
    });
    it('rejects rotation of already-rotated token with SCIM_TOKEN_REVOKED', async function () {
        const fx = newFixture();
        const seeded = await seedActive(fx);
        ...
        await expect(handleScimTokenRotate({
            ...
            principalId: 'admin',
            tokenId: seeded.document.tokenId,
        }, fx.events, fx.entities)).rejects.toMatchObject({ code: identityErrorCodes.SCIM_TOKEN_REVOKED });
    });
});
describe('handleScimTokenRevoke', function () {
    it('flips status to revoked and emits ScimTokenRevoked', async function () {
        const fx = newFixture();
        const seeded = await seedActive(fx);
    ...
    it('rejects unknown tokenId with SCIM_TOKEN_NOT_FOUND', async function () {
        const fx = newFixture();
        await expect(handleScimTokenRevoke({
            tenantId: fx.tenantId,
            correlationId: 'c',
            principalId: 'admin',
            tokenId: 'scim-fake',
...
describe('ScimToken trio — tenant scoping', function () {
    it('token in tenant B is invisible to tenant A handlers', async function () {
