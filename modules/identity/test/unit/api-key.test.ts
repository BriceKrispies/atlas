...
import {
  ...
  identityErrorCodes,
  type ApiKeyDocument,
  type ServicePrincipalDocument,
} from '../../src/index.ts';
import { newFixture, dispatchAll } from '../lib/fixtures.ts';

async function seedSp(
...
describe('handleApiKeyCreate — happy path', () => {
  it('mints a user-owned ApiKey, emits ApiKeyCreated, returns plaintext bearer atlas_<keyId>.<secret>', async () => {
    const fx = newFixture();
    ...
    expect(result.plaintextBearer).toMatch(
      new RegExp(`^atlas_${result.document.keyId}\\.[A-Za-z0-9_-]+$`),
    );
  });

  it('plaintext secret never appears on the persisted document', async () => {
    const fx = newFixture();
    const result = await handleApiKeyCreate(
      {
        tenantId: fx.tenantId,
  ...
  it('mints an SP-owned ApiKey when scopes are within the SP ceiling', async () => {
    const fx = newFixture();
...
describe('handleApiKeyCreate — error paths', () => {
  it('rejects when neither userId nor servicePrincipalId is set', async () => {
    const fx = newFixture();
    await expect(
      handleApiKeyCreate(
        {
  ...
  it('rejects when both userId AND servicePrincipalId are set', async () => {
    const fx = newFixture();
    await expect(
      handleApiKeyCreate(
        {
  ...
  it('rejects when servicePrincipalId references a non-existent SP', async () => {
    const fx = newFixture();
    await expect(
      handleApiKeyCreate(
        {
  ...
  it('rejects when SP is disabled', async () => {
    const fx = newFixture();
  ...
  it('rejects when scopes exceed the SP ceiling', async () => {
    const fx = newFixture();
  ...
  it('throws IdentityError instances for every rejection', async () => {
    const fx = newFixture();
    await expect(
      handleApiKeyCreate(
        {
...
describe('handleApiKeyRotate — happy path', () => {
  async function seedActiveKey(
    fx: ReturnType<typeof newFixture>,
  ): Promise<{ keyId: string; document: ApiKeyDocument }> {
  ...
  it('mints successor and flips predecessor to rotated with overlapUntil set', async () => {
    const fx = newFixture();
  ...
  it('emits two events: ApiKeyRotated (predecessor) primary + ApiKeyCreated (successor) follow', async () => {
    const fx = newFixture();
    const { keyId } = await seedActiveKey(fx);
    const result = await handleApiKeyRotate(
      {
  ...
  it('returns plaintext bearer for the successor only (predecessor secret stays unknown)', async () => {
    const fx = newFixture();
    const { keyId } = await seedActiveKey(fx);
    const result = await handleApiKeyRotate(
      {
    ...
    expect(result.plaintextBearer).toMatch(
      new RegExp(`^atlas_${result.successor.keyId}\\.[A-Za-z0-9_-]+$`),
    );
...
describe('handleApiKeyRotate — error paths', () => {
  it('rejects rotating an unknown keyId with API_KEY_NOT_FOUND', async () => {
    const fx = newFixture();
    await expect(
      handleApiKeyRotate(
        {
    ...
    ).rejects.toMatchObject({ code: identityErrorCodes.API_KEY_NOT_FOUND });
    expect(fx.events.events).toHaveLength(0);
  });

  it('rejects rotating an already-rotated key with API_KEY_REVOKED (status=rotated)', async () => {
    const fx = newFixture();
    ...
    ).rejects.toMatchObject({ code: identityErrorCodes.API_KEY_REVOKED });
  });
});

describe('handleApiKeyRevoke', () => {
  it('flips status to revoked and emits ApiKeyRevoked', async () => {
    const fx = newFixture();
    const seeded = await handleApiKeyCreate(
      {
        tenantId: fx.tenantId,
  ...
  it('rejects revoking an unknown keyId with API_KEY_NOT_FOUND', async () => {
    const fx = newFixture();
    await expect(
      handleApiKeyRevoke(
        {
...
describe('ApiKey trio — tenant scoping', () => {
  it("rotation cannot reach a key in tenant B from tenant A's call", async () => {
