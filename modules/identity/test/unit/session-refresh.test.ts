...
async function issue(fx: ReturnType<typeof newFixture>, userId = 'user-1'): Promise<{
    sessionId: string;
...
describe('handleSessionRefresh — happy path envelope shape', function () {
    it('emits Identity.SessionRefreshed with the documented envelope', async function () {
        const fx = newFixture();
        ...
        expect(result.envelope.schemaId).toBe('domain.identity.session.refreshed.v1');
    ...
    it('returns new plaintexts and cookiePayload formatted as <sessionId>.<refreshSecret>', async function () {
        const fx = newFixture();
        const { sessionId, refreshSecret } = await issue(fx);
        const result = await handleSessionRefresh({
            tenantId: fx.tenantId,
        ...
        expect(result.cookiePayload).toBe(`${sessionId}.${result.plaintextRefreshToken}`);
    });
});
describe('handleSessionRefresh — error paths not covered by session.test.ts', function () {
    it('rejects refresh on a revoked session with SESSION_REVOKED', async function () {
        const fx = newFixture();
        const { sessionId, refreshSecret } = await issue(fx);
        ...
        await expect(handleSessionRefresh({
            ...
            sessionId,
            presentedRefreshSecret: refreshSecret,
        }, fx.events, fx.entities)).rejects.toMatchObject({ code: identityErrorCodes.SESSION_REVOKED });
    });
    it('rejects refresh on an evicted session with SESSION_EXPIRED', async function () {
        const fx = newFixture();
        const { sessionId, refreshSecret } = await issue(fx);
        // Manually flip the persisted entity to evicted.
        const row = await fx.entities.get<AuthSessionDocument>(fx.tenantId, 'AuthSession', sessionId);
        if (!row)
        ...
        await expect(handleSessionRefresh({
            ...
            sessionId,
            presentedRefreshSecret: refreshSecret,
        }, fx.events, fx.entities)).rejects.toMatchObject({ code: identityErrorCodes.SESSION_EXPIRED });
    });
    it('reuse-detection via revokedRefreshTokenHashes ring trips after two rotations + replay', async function () {
        const fx = newFixture();
        ...
        await dispatchAll(fx);
        const r1 = assertDefined(r1Result.plaintextRefreshToken, 'rotation result always carries the new plaintext refresh token');
        ...
        await expect(handleSessionRefresh({
            ...
            sessionId,
            presentedRefreshSecret: r0,
        }, fx.events, fx.entities)).rejects.toMatchObject({ code: identityErrorCodes.SESSION_REUSE_DETECTED });
        ...
        const anomaly = fx.events.events.find(function (e) {
...
describe('handleSessionRefresh — defensive tenant cross-check', function () {
    it('session in tenant B is invisible to refresh under tenant A (defensive)', async function () {
...
describe('handleSessionRefresh — ring cap', function () {
    it('revokedRefreshTokenHashes is capped at 16 entries', async function () {
        const fx = newFixture();
        ...
        for (let i = 0; i < 20; i += 1) {
            ...
            await dispatchAll(fx);
            current = assertDefined(r.plaintextRefreshToken, 'rotation result always carries the new plaintext refresh token');
        }
        const finalRow = await fx.entities.get<AuthSessionDocument>(fx.tenantId, 'AuthSession', sessionId);
