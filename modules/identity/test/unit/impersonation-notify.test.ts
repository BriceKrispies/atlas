...
describe('emitNotificationsForA7Event — cache-tag invariant (I10)', () => {
  it('every Notifications.* follow-up carries Tenant:<tenantId>', async () => {
    const fx = newFixture();
  ...
  it('idempotency key is deterministic (notif.<sourceEventId>.<channel>) so dispatcher replays dedupe', async () => {
    const fx = newFixture();
    const source = impersonationStartedEvent(fx.tenantId);
    const first = await emitNotificationsForA7Event(source, fx.events);
    expect(first).toHaveLength(2);
    const keys = first.map((e) => e.idempotencyKey).sort();
    expect(keys).toEqual(
      [
    ...
    const secondKeys = second.map((e) => e.idempotencyKey).sort();
