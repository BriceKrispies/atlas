...
async function seedIdp(
  fx: ReturnType<typeof newFixture>,
  overrides: Partial<IdentityProviderDocument> = {},
...
describe('handleSamlAcs — pre-verify branches', () => {
  it('rejects when the SAML Response has no <Issuer>', async () => {
    const fx = newFixture();
    await expect(
      handleSamlAcs(
        {
    ...
    ).rejects.toMatchObject({ code: identityErrorCodes.SAML_INVALID_RESPONSE });
    expect(fx.events.events).toHaveLength(0);
  });

  it('rejects when the issuer matches no configured IdP', async () => {
    const fx = newFixture();
    await expect(
      handleSamlAcs(
        {
  ...
  it("rejects when the matching IdP exists but kind !== 'saml'", async () => {
    const fx = newFixture();
    await seedIdp(fx, {
      kind: 'oidc',
  ...
  it("rejects when the IdP exists but the IdP isn't active", async () => {
    const fx = newFixture();
  ...
  it('rejects when the matching IdP is missing its samlIdpCert', async () => {
    const fx = newFixture();
    await seedIdp(fx, {
      kind: 'saml',
  ...
  it('throws IdentityError instances for every pre-verify rejection', async () => {
    const fx = newFixture();
    await expect(
      handleSamlAcs(
        {
...
describe('handleSamlAcs — tenant scoping', () => {
  it("an IdP in tenant B is invisible to a SAML ACS call in tenant A", async () => {
...
describe.skip('handleSamlAcs — crypto-bearing branches (covered by Layer 3 e2e)', () => {
