...
describe('handleBreakGlassIssue — happy path', () => {
  it('emits Authorization.BreakGlassIssued with retention:10y and pending_approval status by default', async () => {
    const fx = newFixture();
  ...
  it('exact cacheInvalidationTags: Tenant + Principal + BreakGlassGrant', async () => {
    const fx = newFixture();
    const result = await handleBreakGlassIssue(
      { tenantId: fx.tenantId, correlationId: 'c', ...VALID },
      fx.events,
  ...
  it('default duration is 60 minutes', async () => {
    const fx = newFixture();
  ...
  it('requireApproval=false makes the grant active immediately and stamps issuer as approvedBy', async () => {
    const fx = newFixture();
    const result = await handleBreakGlassIssue(
      {
  ...
  it('passes resourceTypeAllowList through', async () => {
    const fx = newFixture();
    const result = await handleBreakGlassIssue(
      {
        tenantId: fx.tenantId,
...
describe('handleBreakGlassIssue — error paths', () => {
  const fx = () => newFixture();

  it('rejects empty issuedBy with BREAK_GLASS_REQUIRES_OPERATOR', async () => {
    const f = fx();
    await expect(
      handleBreakGlassIssue(
        { tenantId: f.tenantId, correlationId: 'c', ...VALID, issuedBy: '' },
        f.events,
        f.entities,
      ),
  ...
  it('rejects empty / whitespace justification with BREAK_GLASS_JUSTIFICATION_REQUIRED', async () => {
    const f = fx();
    await expect(
      handleBreakGlassIssue(
        { tenantId: f.tenantId, correlationId: 'c', ...VALID, justification: '   ' },
        f.events,
        f.entities,
      ),
    ).rejects.toMatchObject({
  ...
  it('rejects empty incidentUrl with BREAK_GLASS_INCIDENT_REQUIRED', async () => {
    const f = fx();
    await expect(
      handleBreakGlassIssue(
        { tenantId: f.tenantId, correlationId: 'c', ...VALID, incidentUrl: '' },
        f.events,
        f.entities,
      ),
    ).rejects.toMatchObject({
  ...
  it('rejects maxDurationMin <= 0 or > 12h with BREAK_GLASS_DURATION_INVALID', async () => {
    const f = fx();
    await expect(
      handleBreakGlassIssue(
        { tenantId: f.tenantId, correlationId: 'c', ...VALID, maxDurationMin: 0 },
        f.events,
        f.entities,
      ),
    ).rejects.toMatchObject({
    ...
    await expect(
      handleBreakGlassIssue(
        { tenantId: f.tenantId, correlationId: 'c', ...VALID, maxDurationMin: 12 * 60 + 1 },
        f.events,
        f.entities,
      ),
    ).rejects.toMatchObject({
  ...
  it('rejects empty grantedRoles with IDENTITY_INVALID', async () => {
    const f = fx();
    await expect(
      handleBreakGlassIssue(
        { tenantId: f.tenantId, correlationId: 'c', ...VALID, grantedRoles: [] },
        f.events,
        f.entities,
      ),
    ).rejects.toMatchObject({ code: identityErrorCodes.IDENTITY_INVALID });
  });
...
describe('handleBreakGlassApprove — 4-eyes activation', () => {
  ...
  it('flips status to active, stamps approvedBy + approvedAt, resets expiresAt from approval time', async () => {
    const fx = newFixture();
    ...
    const expiresMs = new Date(result.document.expiresAt).getTime();
    expect(expiresMs - before).toBeGreaterThanOrEqual(
      issued.document.maxDurationMin * 60 * 1000 - 5000,
    );
  });

  it('rejects unknown grantId with BREAK_GLASS_GRANT_NOT_FOUND', async () => {
    const fx = newFixture();
    await expect(
      handleBreakGlassApprove(
        {
  ...
  it('rejects approval of non-pending grant with BREAK_GLASS_NOT_PENDING', async () => {
    const fx = newFixture();
    const issued = await issuePending(fx);
  ...
  it('rejects self-approval (issuer = approver) with BREAK_GLASS_SELF_APPROVAL_FORBIDDEN — the 4-eyes contract', async () => {
    const fx = newFixture();
    const issued = await issuePending(fx);
...
describe('handleBreakGlassDeny', () => {
  ...
  it('emits BreakGlassDenied, flips status to denied, optionally records reason', async () => {
    const fx = newFixture();
    const issued = await issuePending(fx);
  ...
  it('rejects self-denial with the same 4-eyes guard', async () => {
    const fx = newFixture();
    const issued = await issuePending(fx);
    await expect(
      handleBreakGlassDeny(
...
describe('handleBreakGlassAction', () => {
  it('emits BreakGlassAction with retention:10y and audit-shape payload', async () => {
    const fx = newFixture();
  ...
  it('omits resourceType / resourceId when not provided', async () => {
    const fx = newFixture();
    const result = await handleBreakGlassAction(
      {
        tenantId: fx.tenantId,
...
describe('handleBreakGlassRevoke', () => {
  ...
  it('flips status to revoked on operator_revoke, stamps revokedBy, eventType=BreakGlassRevoked', async () => {
    const fx = newFixture();
  ...
  it('flips status to expired on auto_expired, eventType=BreakGlassExpired, no revokedBy', async () => {
    const fx = newFixture();
    const issued = await issueActive(fx);
    const result = await handleBreakGlassRevoke(
      {
  ...
  it('rejects unknown grantId', async () => {
    const fx = newFixture();
    await expect(
      handleBreakGlassRevoke(
        {
  ...
  it('rejects revoke of already-revoked grant with BREAK_GLASS_NOT_ACTIVE', async () => {
    const fx = newFixture();
    const issued = await issueActive(fx);
...
describe('resolveActiveGrants', () => {
  it('returns active grants for a principal, omits expired-by-clock', async () => {
    const fx = newFixture();
    // Active grant.
    ...
    expect(grants.map((g) => g.grantId)).not.toContain('bgg-stale');
  });

  it('returns empty when principal has no active grants', async () => {
    const fx = newFixture();
...
describe('break-glass — tenant scoping', () => {
  it('grant in tenant B is invisible to tenant A operations', async () => {
