...
describe('handleImpersonationStart — happy path', function () {
    it('emits Authorization.ImpersonationStarted with platform-tier retention', async function () {
        const fx = newFixture();
        ...
        expect(result.envelope.schemaId).toBe('domain.authorization.impersonation_started.v1');
    ...
    it('exact cacheInvalidationTags: Tenant + User (target) + Impersonation', async function () {
        const fx = newFixture();
        const result = await handleImpersonationStart({
            tenantId: fx.tenantId,
            correlationId: 'c',
    ...
    it('default duration is 30 minutes', async function () {
        const fx = newFixture();
    ...
    it('plaintext token surfaced once and bearer formatted as <impersonationId>.<secret>; document stores hash only', async function () {
        const fx = newFixture();
        const result = await handleImpersonationStart({
            tenantId: fx.tenantId,
            correlationId: 'c',
        ...
        expect(result.bearerToken).toBe(`${result.document.impersonationId}.${result.plaintextToken}`);
        const docJson = JSON.stringify(result.document);
        expect(docJson).not.toContain(result.plaintextToken);
    });
    it('records reason, ticketUrl, expiresAt on the audit event payload', async function () {
        const fx = newFixture();
        const result = await handleImpersonationStart({
            tenantId: fx.tenantId,
            correlationId: 'c',
    ...
    it('honors custom maxDurationMin within the 8-hour cap', async function () {
        const fx = newFixture();
        const result = await handleImpersonationStart({
            tenantId: fx.tenantId,
            correlationId: 'c',
    ...
    it('passes readonlyResourceTypes through to the persisted session', async function () {
        const fx = newFixture();
        const result = await handleImpersonationStart({
            tenantId: fx.tenantId,
            correlationId: 'c',
...
describe('handleImpersonationStart — error paths', function () {
    it('rejects empty operatorId with IMPERSONATION_REQUIRES_OPERATOR', async function () {
        const fx = newFixture();
    ...
    it('rejects empty reason with IMPERSONATION_REASON_REQUIRED', async function () {
        const fx = newFixture();
        await expect(handleImpersonationStart({
            tenantId: fx.tenantId,
            correlationId: 'c',
    ...
    it('rejects whitespace-only reason with IMPERSONATION_REASON_REQUIRED', async function () {
        const fx = newFixture();
        await expect(handleImpersonationStart({
            tenantId: fx.tenantId,
            correlationId: 'c',
    ...
    it('rejects empty ticketUrl with IMPERSONATION_TICKET_REQUIRED', async function () {
        const fx = newFixture();
        await expect(handleImpersonationStart({
            tenantId: fx.tenantId,
            correlationId: 'c',
    ...
    it('rejects maxDurationMin <= 0 with IMPERSONATION_DURATION_INVALID', async function () {
        const fx = newFixture();
        await expect(handleImpersonationStart({
            tenantId: fx.tenantId,
            correlationId: 'c',
    ...
    it('rejects maxDurationMin > 8h hard cap with IMPERSONATION_DURATION_INVALID', async function () {
        const fx = newFixture();
        await expect(handleImpersonationStart({
            tenantId: fx.tenantId,
            correlationId: 'c',
    ...
    it('throws IdentityError for every rejection', async function () {
        const fx = newFixture();
        await expect(handleImpersonationStart({
            tenantId: fx.tenantId,
            correlationId: 'c',
...
describe('handleImpersonationAction', function () {
    it('emits ImpersonationAction with retention:7y and minimal cache tags', async function () {
        const fx = newFixture();
    ...
    it('includes optional resourceType and resourceId when provided', async function () {
        const fx = newFixture();
        const result = await handleImpersonationAction({
            tenantId: fx.tenantId,
            correlationId: 'c',
    ...
    it('omits resourceType / resourceId from payload when not provided', async function () {
        const fx = newFixture();
        const result = await handleImpersonationAction({
            tenantId: fx.tenantId,
            correlationId: 'c',
...
describe('handleImpersonationEnd', function () {
    async function startSession(fx: ReturnType<typeof newFixture>) {
        const start = await handleImpersonationStart({ tenantId: fx.tenantId, correlationId: 'seed', ...VALID }, fx.events, fx.entities);
        return start;
    }
    it('emits ImpersonationEnded and flips status to ended on operator_ended reason', async function () {
        const fx = newFixture();
    ...
    it('flips status to revoked on tenant_revoked and stamps revokedBy', async function () {
        const fx = newFixture();
        const start = await startSession(fx);
        const result = await handleImpersonationEnd({
            tenantId: fx.tenantId,
    ...
    it('flips status to revoked on platform_revoked', async function () {
        const fx = newFixture();
        const start = await startSession(fx);
        const result = await handleImpersonationEnd({
            tenantId: fx.tenantId,
    ...
    it('flips status to expired on auto_expired (no revokedBy)', async function () {
        const fx = newFixture();
        const start = await startSession(fx);
        const result = await handleImpersonationEnd({
            tenantId: fx.tenantId,
    ...
    it('rejects unknown impersonationId with IMPERSONATION_NOT_FOUND', async function () {
        const fx = newFixture();
    ...
    it('rejects already-ended session with IMPERSONATION_ENDED', async function () {
        const fx = newFixture();
        const start = await startSession(fx);
        ...
        await expect(handleImpersonationEnd({
            ...
            principalId: VALID.operatorId,
            reason: 'operator_ended',
        }, fx.events, fx.entities)).rejects.toMatchObject({ code: identityErrorCodes.IMPERSONATION_ENDED });
    });
});
describe('resolveImpersonationToken', function () {
    it('returns ok=true for a valid bearer on an active session', async function () {
        const fx = newFixture();
        const start = await handleImpersonationStart({ tenantId: fx.tenantId, correlationId: 'c', ...VALID }, fx.events, fx.entities);
        const result = await resolveImpersonationToken(fx.entities, fx.tenantId, start.bearerToken);
        expect(result.ok).toBe(true);
        if (result.ok) {
            expect(result.document.impersonationId).toBe(start.document.impersonationId);
        }
    });
    it('rejects malformed bearer (no dot) with reason=malformed', async function () {
        const fx = newFixture();
        const result = await resolveImpersonationToken(fx.entities, fx.tenantId, 'no-dot');
        expect(result).toEqual({ ok: false, reason: 'malformed' });
    });
    it('rejects bearer with empty id or secret as malformed', async function () {
        const fx = newFixture();
        expect(await resolveImpersonationToken(fx.entities, fx.tenantId, '.secret')).toEqual({ ok: false, reason: 'malformed' });
        expect(await resolveImpersonationToken(fx.entities, fx.tenantId, 'id.')).toEqual({ ok: false, reason: 'malformed' });
    });
    it('rejects unknown impersonationId with reason=not_found', async function () {
        const fx = newFixture();
        const result = await resolveImpersonationToken(fx.entities, fx.tenantId, 'imp-fake.some-secret');
        expect(result).toEqual({ ok: false, reason: 'not_found' });
    });
    it('rejects wrong secret with reason=hash_mismatch', async function () {
        const fx = newFixture();
        const start = await handleImpersonationStart({ tenantId: fx.tenantId, correlationId: 'c', ...VALID }, fx.events, fx.entities);
        const result = await resolveImpersonationToken(fx.entities, fx.tenantId, `${start.document.impersonationId}.wrong-secret`);
        expect(result).toEqual({ ok: false, reason: 'hash_mismatch' });
    });
    it('rejects revoked session with reason=revoked', async function () {
        const fx = newFixture();
        const start = await handleImpersonationStart({ tenantId: fx.tenantId, correlationId: 'c', ...VALID }, fx.events, fx.entities);
        await handleImpersonationEnd({
            tenantId: fx.tenantId,
...
describe('Impersonation — tenant scoping', function () {
    it("session in tenant B is invisible to End/Resolve in tenant A", async function () {
        ...
        const resolve = await resolveImpersonationToken(fx.entities, 'tenant-a', start.bearerToken);
