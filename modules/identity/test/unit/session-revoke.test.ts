...
describe('handleSessionRevoke — happy path', () => {
  it('emits Identity.SessionEnded with status=revoked and the reason persisted', async () => {
    const fx = newFixture();
    ...
    expect(result.document.endReason).toBe('manual_revoke');
  });

  it('exact cacheInvalidationTags: Tenant + User + Session', async () => {
    const fx = newFixture();
  ...
  it('payload carries the ended document and reason', async () => {
    const fx = newFixture();
    const sessionId = await issue(fx);
    const result = await handleSessionRevoke(
      {
  ...
  it('is idempotent at the handler level — second revoke also succeeds', async () => {
    const fx = newFixture();
    const sessionId = await issue(fx);
...
describe('handleSessionRevoke — error paths', () => {
  ...
  it("rejects cross-tenant revoke with SESSION_NOT_FOUND (defensive)", async () => {
...
describe('handleSessionRevokeAllForUser', () => {
  it('emits one SessionEnded per active session for the user', async () => {
    const fx = newFixture();
    ...
    expect(
      result.envelopes.every((e) => e.eventType === 'Identity.SessionEnded'),
    ).toBe(true);
  });

  it('returns empty arrays when the user has no active sessions', async () => {
    const fx = newFixture();
    const result = await handleSessionRevokeAllForUser(
      {
        tenantId: fx.tenantId,
  ...
  it('does not re-revoke sessions that are already revoked', async () => {
    const fx = newFixture();
    const a = await issue(fx, 'user-1');
    const b = await issue(fx, 'user-1');
    // Pre-revoke `a`.
  ...
  it('emits cache tags including User:<userId> and Session:<sessionId> per envelope', async () => {
    const fx = newFixture();
...
describe('Session revoke — tenant scoping', () => {
  it("revoke-all in tenant A does not touch tenant B's sessions for the same userId", async () => {
    const fx = newFixture('tenant-a');
