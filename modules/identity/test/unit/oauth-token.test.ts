...
describe('handleOAuthIssueToken — happy path', () => {
  it('issues a Bearer token from clientBearer, emits OAuthTokenIssued with full envelope', async () => {
    const fx = newFixture();
    ...
    expect(result.envelope.schemaId).toBe(
      'domain.identity.oauth.token_issued.v1',
    );
  ...
  it('also accepts split client_id + client_secret form', async () => {
    const fx = newFixture();
    const seeded = await seedSpAndKey(fx, ['read']);
  ...
  it('plaintext access_token is surfaced once and not stored on the document', async () => {
    const fx = newFixture();
  ...
  it('narrows token scopes to requestedScopes when caller asks for a subset', async () => {
    const fx = newFixture();
...
describe('handleOAuthIssueToken — error paths', () => {
  it('rejects malformed clientBearer with OAUTH_INVALID_CLIENT', async () => {
    const fx = newFixture();
    await expect(
      handleOAuthIssueToken(
        {
  ...
  it('rejects when no credentials provided at all (no clientBearer, no client_id/secret)', async () => {
    const fx = newFixture();
    await expect(
      handleOAuthIssueToken(
        {
  ...
  it('rejects unknown client (keyId not in store) with OAUTH_INVALID_CLIENT', async () => {
    const fx = newFixture();
    await expect(
      handleOAuthIssueToken(
        {
  ...
  it('rejects revoked client with OAUTH_INVALID_CLIENT', async () => {
    const fx = newFixture();
    const seeded = await seedSpAndKey(fx);
  ...
  it('rejects wrong client_secret with OAUTH_INVALID_CLIENT', async () => {
    const fx = newFixture();
    const seeded = await seedSpAndKey(fx);
    await expect(
      handleOAuthIssueToken(
  ...
  it('rejects requestedScopes that exceed the ApiKey scopes with OAUTH_INVALID_SCOPE', async () => {
    const fx = newFixture();
    const seeded = await seedSpAndKey(fx, ['read']);
    await expect(
      handleOAuthIssueToken(
    ...
    ).rejects.toMatchObject({ code: identityErrorCodes.OAUTH_INVALID_SCOPE });
  });

  it('rejects expired ApiKey with API_KEY_EXPIRED', async () => {
    const fx = newFixture();
    ...
    ).rejects.toMatchObject({ code: identityErrorCodes.API_KEY_EXPIRED });
  });
  ...
  it('throws IdentityError instances for every rejection', async () => {
    ...
    await expect(
      handleOAuthIssueToken(
        { tenantId: fx.tenantId, correlationId: 'c' },
        fx.events,
        fx.entities,
      ),
...
describe('handleOAuthRevokeToken — RFC 7009 semantics', () => {
  it('returns null envelope on unknown token (no enumeration leak)', async () => {
    const fx = newFixture();
  ...
  it('flips status to revoked and emits OAuthTokenRevoked on first revoke', async () => {
    const fx = newFixture();
    const seeded = await seedSpAndKey(fx);
  ...
  it('emits no fresh event on second revoke (idempotent)', async () => {
    const fx = newFixture();
    const seeded = await seedSpAndKey(fx);
    const issued = await handleOAuthIssueToken(
      {
  ...
  it('honors custom revoke reason (admin_revoke vs client_revoke)', async () => {
    const fx = newFixture();
    const seeded = await seedSpAndKey(fx);
    const issued = await handleOAuthIssueToken(
      {
