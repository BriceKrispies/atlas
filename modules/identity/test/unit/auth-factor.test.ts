...
describe('handleFactorRevoke', function () {
    it('emits AuthFactorRevoked, flips status, stamps endReason=user_revoke for self-revoke', async function () {
        const fx = newFixture();
    ...
    it('stamps endReason=admin_revoke when principalId !== userId', async function () {
        const fx = newFixture();
    ...
    it('rejects unknown factorId with MFA_FACTOR_NOT_FOUND', async function () {
        const fx = newFixture();
        await expect(handleFactorRevoke({
            tenantId: fx.tenantId,
            correlationId: 'c',
            principalId: 'user-1',
            factorId: 'fct-fake',
        }, fx.events, fx.entities)).rejects.toMatchObject({ code: identityErrorCodes.MFA_FACTOR_NOT_FOUND });
        expect(fx.events.events).toHaveLength(0);
    });
    it('refuses last-factor revoke when policy.mfaRequired and not forced', async function () {
        const fx = newFixture();
    ...
    it('allows last-factor revoke when force=true (admin override)', async function () {
        const fx = newFixture();
    ...
    it('allows revoke when user has multiple active factors (last-factor guard does not fire)', async function () {
        const fx = newFixture();
