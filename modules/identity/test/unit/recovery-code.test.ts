...
describe('handleGenerateRecoveryCodes', function () {
    it('mints policy.recoveryCodeCount codes and emits RecoveryCodesGenerated', async function () {
        const fx = newFixture();
        ...
        expect(result.plaintextCodes).toHaveLength(DEFAULT_IDENTITY_POLICY.recoveryCodeCount);
        expect(result.documents).toHaveLength(DEFAULT_IDENTITY_POLICY.recoveryCodeCount);
    });
    it('plaintext codes never appear in stored documents or events', async function () {
        const fx = newFixture();
        const result = await handleGenerateRecoveryCodes({
            tenantId: fx.tenantId,
            correlationId: 'c',
    ...
    it('refuses generate when active codes already exist with RECOVERY_CODE_INVALID', async function () {
        const fx = newFixture();
...
describe('handleRegenerateRecoveryCodes', function () {
    it('invalidates prior batch and mints fresh codes; emits RecoveryCodesRegenerated', async function () {
        const fx = newFixture();
        ...
        expect(second.documents.every(function (d) {
        ...
        await expect(handleRedeemRecoveryCode({
            ...
            principalId: 'user-4',
            userId: 'user-4',
            presentedCode: assertDefined(first.plaintextCodes[0], 'first generate must mint at least one plaintext code'),
        }, fx.events, fx.entities)).rejects.toMatchObject({
            code: identityErrorCodes.RECOVERY_CODE_INVALID,
        });
    });
});
describe('handleRedeemRecoveryCode', function () {
    ...
    it('redeems a valid code and emits RecoveryCodeConsumed with remaining count', async function () {
        const fx = newFixture();
        ...
        const result = await handleRedeemRecoveryCode({
            ...
            principalId: 'user-5',
            userId: 'user-5',
            presentedCode: assertDefined(generated.plaintextCodes[0], 'gen() must mint at least one plaintext code'),
        }, fx.events, fx.entities);
        expect(result.envelope.eventType).toBe('Identity.RecoveryCodeConsumed');
        expect(result.document.status).toBe('consumed');
        expect(result.remaining).toBe(DEFAULT_IDENTITY_POLICY.recoveryCodeCount - 1);
    });
    it('rejects a code on second redemption (single-use enforcement)', async function () {
        const fx = newFixture();
        const generated = await gen(fx);
        const code = assertDefined(generated.plaintextCodes[0], 'gen() must mint at least one plaintext code');
    ...
    it('rejects a bogus code with RECOVERY_CODE_INVALID', async function () {
        const fx = newFixture();
        await gen(fx);
        await expect(handleRedeemRecoveryCode({
            tenantId: fx.tenantId,
