...
describe('handleSessionIssue — happy path', () => {
  it('emits Identity.SessionIssued with full envelope shape', async () => {
    ...
    expect(result.envelope.idempotencyKey).toMatch(
      /^identity\.session\.issue\.[a-z0-9_-]+$/i,
    );
    expect(result.envelope.occurredAt).toMatch(/^\d{4}-\d{2}-\d{2}T/);
  });

  it('emits cacheInvalidationTags exactly: Tenant + User + Session', async () => {
    const fx = newFixture();
    const result = await handleSessionIssue(
      {
        tenantId: fx.tenantId,
  ...
  it('returns plaintexts and cookiePayload formatted as <sessionId>.<refreshSecret>', async () => {
    const fx = newFixture();
    const result = await handleSessionIssue(
      {
        tenantId: fx.tenantId,
  ...
  it('persists hashes (not plaintexts) on the AuthSession document', async () => {
    const fx = newFixture();
    const result = await handleSessionIssue(
      {
        tenantId: fx.tenantId,
    ...
    expect(result.document.refreshTokenHash).toBe(
      hashSecret(result.plaintextRefreshToken),
    );
    expect(result.document.refreshTokenLookup).toBe(
      lookupOf(result.plaintextRefreshToken),
    );
    expect(result.document.accessTokenHash).toBe(
      hashSecret(result.plaintextAccessToken),
    );
    expect(result.document.accessTokenLookup).toBe(
      lookupOf(result.plaintextAccessToken),
    );
  ...
  it('marks the new document active with status=active and follow=[]', async () => {
    const fx = newFixture();
    const result = await handleSessionIssue(
      {
        tenantId: fx.tenantId,
  ...
  it('appends the SessionIssued event to the EventStore', async () => {
    const fx = newFixture();
    const result = await handleSessionIssue(
      {
        tenantId: fx.tenantId,
...
describe('handleSessionIssue — configuration knobs', () => {
  it('honors custom accessTokenTtlSeconds (overrides the 1-hour default)', async () => {
    const fx = newFixture();
  ...
  it('honors custom session policy (hardTimeoutHours)', async () => {
    const fx = newFixture();
  ...
  it('passes through optional ip', async () => {
    const fx = newFixture();
    const result = await handleSessionIssue(
      {
        tenantId: fx.tenantId,
  ...
  it('passes through optional userAgent', async () => {
    const fx = newFixture();
    const result = await handleSessionIssue(
      {
        tenantId: fx.tenantId,
  ...
  it('omits ip and userAgent fields when not provided', async () => {
    const fx = newFixture();
    const result = await handleSessionIssue(
      {
        tenantId: fx.tenantId,
...
describe('handleSessionIssue — error paths', () => {
  it('throws IdentityError with code IDENTITY_INVALID when principalId !== userId', async () => {
    const fx = newFixture();
    await expect(
      handleSessionIssue(
        {
  ...
  it('rejects mismatched-principal request before any side effect (no event emitted)', async () => {
    const fx = newFixture();
    await expect(
      handleSessionIssue(
        {
  ...
  it('allows principalId=null (front-door auth flows: password-login, invite-accept)', async () => {
    const fx = newFixture();
    const result = await handleSessionIssue(
      {
        tenantId: fx.tenantId,
...
describe('handleSessionIssue — concurrent-session cap', () => {
  async function issueN(
  ...
  it('does not evict when under the cap', async () => {
    ...
    const ended = fx.events.events.filter(
      (e) => e.eventType === 'Identity.SessionEnded',
    );
    expect(ended).toHaveLength(0);
  });

  it('evicts oldest session when at the cap (evict 1, follow before primary)', async () => {
    const fx = newFixture();
    const policy: SessionPolicy = {
      ...DEFAULT_SESSION_POLICY,
  ...
  it('evicts N when over the cap by N', async () => {
  ...
  it('evicted SessionEnded events have cache tags pointing at the EVICTED sessionId', async () => {
    const fx = newFixture();
    const policy: SessionPolicy = {
      ...DEFAULT_SESSION_POLICY,
...
describe('handleSessionIssue — tenant scoping', () => {
  it('cap calculation is per-tenant — tenant B sessions do not count toward tenant A cap', async () => {
