...
async function setup(fx: ReturnType<typeof newFixture>, userId = 'user-1') {
    ...
    await fx.entities.put({
        ...
        attrs: {
            ...
            lastRefreshedAt: now,
            lastSeenAt: now,
            hardExpiresAt: new Date(Date.now() + 24 * 3600000).toISOString(),
            status: 'mfa_pending',
        },
    });
    return { sessionId, factor: enroll.document };
}
function totpCode(factor: AuthFactorDocument, fx: ReturnType<typeof newFixture>): string {
    if (factor.kind !== 'totp') {
...
describe('handleMfaChallengeSubmit — happy path (TOTP)', function () {
    it('flips mfa_pending session to active and emits SessionMfaSatisfied', async function () {
        const fx = newFixture();
...
describe('handleMfaChallengeSubmit — session-state guards', function () {
    it('rejects unknown sessionId with SESSION_NOT_FOUND', async function () {
        const fx = newFixture();
        await expect(handleMfaChallengeSubmit({
            tenantId: fx.tenantId,
            correlationId: 'c',
            principalId: 'user-1',
            sessionId: 'sess-fake',
        ...
        }, fx.events, fx.entities, fx.secrets)).rejects.toMatchObject({ code: identityErrorCodes.SESSION_NOT_FOUND });
    });
    it('returns idempotent NoOp envelope for an already-active session', async function () {
        const fx = newFixture();
    ...
    it('rejects revoked / expired sessions with SESSION_REVOKED', async function () {
        const fx = newFixture();
        const issued = await handleSessionIssue({
            tenantId: fx.tenantId,
            correlationId: 's',
        ...
        }, fx.events, fx.entities, fx.secrets)).rejects.toMatchObject({ code: identityErrorCodes.SESSION_REVOKED });
    });
});
describe('handleMfaChallengeSubmit — method / payload mismatch', function () {
    async function pendingSessionId(fx: ReturnType<typeof newFixture>): Promise<string> {
        const { sessionId } = await setup(fx);
        return sessionId;
    }
    it('rejects method=totp with missing totp params', async function () {
        const fx = newFixture();
        ...
        await expect(handleMfaChallengeSubmit({
            ...
            sessionId,
            method: 'totp',
        }, fx.events, fx.entities, fx.secrets)).rejects.toMatchObject({ code: identityErrorCodes.MFA_CHALLENGE_INVALID });
    });
    it('rejects method=webauthn with missing webauthn params', async function () {
        const fx = newFixture();
        const sessionId = await pendingSessionId(fx);
        await expect(handleMfaChallengeSubmit({
            tenantId: fx.tenantId,
    ...
    it('rejects method=recovery_code with missing recoveryCode params', async function () {
        const fx = newFixture();
        const sessionId = await pendingSessionId(fx);
        await expect(handleMfaChallengeSubmit({
            tenantId: fx.tenantId,
    ...
    it('rejects method=bypass with missing bypass params', async function () {
        const fx = newFixture();
        const sessionId = await pendingSessionId(fx);
        await expect(handleMfaChallengeSubmit({
            tenantId: fx.tenantId,
    ...
    it('throws IdentityError instances for missing-payload rejections', async function () {
        const fx = newFixture();
        const sessionId = await pendingSessionId(fx);
        await expect(handleMfaChallengeSubmit({
            tenantId: fx.tenantId,
