...
describe('handleInviteAccept — happy path (new user)', function () {
    it('emits UserCreated → MembershipCreated → InviteAccepted → SessionIssued in order', async function () {
        const fx = newFixture();
    ...
    it('exact cacheInvalidationTags on InviteAccepted: Tenant + Invite + User', async function () {
        const fx = newFixture();
        const { plaintextToken, tokenId } = await seedPendingInvite(fx, 'tags@example.com');
        const result = await handleInviteAccept({
            tenantId: fx.tenantId,
            correlationId: 'c',
            principalId: null,
    ...
    it('UserCreated and MembershipCreated cache tags include tenant + user (and membership for membership)', async function () {
        const fx = newFixture();
        ...
        const userCreated = result.follow.find(function (e) {
            return e.eventType === 'Identity.UserCreated';
        });
        const membershipCreated = result.follow.find(function (e) {
    ...
    it('mints membership with the invite roles', async function () {
        const fx = newFixture();
    ...
    it('returns a sessionResult with plaintexts surfaced once and not stored on the document', async function () {
        const fx = newFixture();
        ...
        expect(result.sessionResult?.cookiePayload).toBe(`${result.sessionResult?.document.sessionId}.${result.sessionResult?.plaintextRefreshToken}`);
    });
    it('email match is case-insensitive and trims whitespace', async function () {
        const fx = newFixture();
...
describe('handleInviteAccept — happy path (existing user)', function () {
    it('reuses the existing user and skips UserCreated in follow chain', async function () {
        const fx = newFixture();
        ...
        expect(secondAccept.follow.map(function (e) {
            return e.eventType;
        })).toEqual([
            'Identity.MembershipCreated',
            'Identity.SessionIssued',
...
describe('handleInviteAccept — configuration knobs', function () {
    it('binds primaryIdpSubject to the new user', async function () {
        const fx = newFixture();
    ...
    it('persists givenName and familyName when provided', async function () {
        const fx = newFixture();
    ...
    it('honors issueSession=false (no session minted, no SessionIssued in follow)', async function () {
        const fx = newFixture();
        ...
        expect(result.follow.some(function (e) {
            return e.eventType === 'Identity.SessionIssued';
        })).toBe(false);
    });
    it('passes ip and userAgent through to the minted session', async function () {
        const fx = newFixture();
    ...
    it('honors custom sessionPolicy on the minted session', async function () {
        const fx = newFixture();
        ...
        if (!sessionResult)
            throw new Error('invite-accept did not produce a sessionResult');
...
describe('handleInviteAccept — error paths', function () {
    it('rejects unknown plaintext token with INVITE_NOT_FOUND', async function () {
        const fx = newFixture();
    ...
    it('rejects expired token with INVITE_EXPIRED', async function () {
        const fx = newFixture();
        const { plaintextToken } = await seedPendingInvite(fx, 'expired@example.com', ['Author'], -1);
        await expect(handleInviteAccept({
            tenantId: fx.tenantId,
            correlationId: 'c',
            principalId: null,
    ...
    it('rejects an already-accepted token with INVITE_NOT_FOUND (lookup is pending-scoped)', async function () {
    ...
    it('rejects mismatched email with opaque INVITE_NOT_FOUND (avoid email-probing leak)', async function () {
        const fx = newFixture();
    ...
    it('throws IdentityError instances (not bare Error) for every rejection', async function () {
        const fx = newFixture();
    ...
    it('emits no events when token is unknown', async function () {
        const fx = newFixture();
    ...
    it('emits no events when email mismatches', async function () {
        const fx = newFixture();
        const { plaintextToken } = await seedPendingInvite(fx, 'mismatch@example.com');
        const eventsBefore = fx.events.events.length;
        await expect(handleInviteAccept({
            tenantId: fx.tenantId,
            correlationId: 'c',
...
describe('handleInviteAccept — tenant scoping', function () {
    it("invite issued in tenant B cannot be redeemed under tenant A's accept call", async function () {
