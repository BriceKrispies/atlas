...
describe('handleMembershipCreate — happy path', () => {
  it('emits Identity.MembershipCreated with the documented envelope fields', async () => {
    const fx = newFixture();
    ...
    expect(result.envelope.schemaId).toBe(
      'domain.identity.membership.created.v1',
    );
    expect(result.envelope.idempotencyKey).toBe(
      `identity.membership.create.${fx.tenantId}.${userId}`,
    );
  });

  it('exact cacheInvalidationTags: Tenant + User + Membership', async () => {
    const fx = newFixture();
  ...
  it('persists roles as a copy (no aliasing) and defaults status to active', async () => {
    const fx = newFixture();
    const userId = await seedUser(fx);
...
describe('handleMembershipCreate — error paths', () => {
  it('rejects when user does not exist with USER_NOT_FOUND', async () => {
    const fx = newFixture();
    await expect(
      handleMembershipCreate(
        {
  ...
  it('rejects duplicate membership with MEMBERSHIP_REQUIRED', async () => {
    const fx = newFixture();
    const userId = await seedUser(fx);
    ...
    ).rejects.toMatchObject({ code: identityErrorCodes.MEMBERSHIP_REQUIRED });
  });

  it('emits no events on rejection', async () => {
    const fx = newFixture();
...
describe('handleMembershipCreate — tenant scoping', () => {
  it('a user with a membership in tenant A can have a separate membership in tenant B', async () => {
