...
describe('handleServicePrincipalCreate', () => {
  it('emits ServicePrincipalCreated with status=active and exact cache tags', async () => {
    const fx = newFixture();
    ...
    expect(result.envelope.schemaId).toBe(
      'domain.identity.service_principal.created.v1',
    );
  ...
  it('persists scopes as a copy (no aliasing)', async () => {
    const fx = newFixture();
...
describe('handleServicePrincipalSetScopes', () => {
  it('updates scopes and emits ServicePrincipalScopesChanged', async () => {
    const fx = newFixture();
  ...
  it('rejects unknown spId with SERVICE_PRINCIPAL_NOT_FOUND', async () => {
    const fx = newFixture();
    await expect(
      handleServicePrincipalSetScopes(
        {
...
describe('handleServicePrincipalDisable', () => {
  ...
  it('omits disabledBy when principalId is null', async () => {
    const fx = newFixture();
    const sp = await seedSp(fx);
    const result = await handleServicePrincipalDisable(
      {
...
describe('ServicePrincipal trio — tenant scoping', () => {
  it('SP in tenant B is invisible to tenant A operations', async () => {
