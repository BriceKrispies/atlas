...
describe('handleUserCreate — happy path', () => {
  it('emits Identity.UserCreated with the documented envelope fields', async () => {
    const fx = newFixture();
    ...
    expect(result.envelope.idempotencyKey).toBe(
      `identity.user.create.${fx.tenantId}.${result.document.userId}`,
    );
  });

  it('exact cacheInvalidationTags: Tenant + User', async () => {
    const fx = newFixture();
    const result = await handleUserCreate(
      {
        tenantId: fx.tenantId,
  ...
  it('honors caller-supplied userId (atlasctl bootstrap path)', async () => {
    const fx = newFixture();
    const result = await handleUserCreate(
      {
        tenantId: fx.tenantId,
    ...
    expect(result.envelope.idempotencyKey).toBe(
      `identity.user.create.${fx.tenantId}.usr-fixed-bootstrap`,
    );
  });

  it('generates a userId when not provided', async () => {
    const fx = newFixture();
    const result = await handleUserCreate(
      {
        tenantId: fx.tenantId,
  ...
  it('defaults status to active and primaryIdpSubject to null', async () => {
    const fx = newFixture();
    const result = await handleUserCreate(
      {
        tenantId: fx.tenantId,
...
describe('handleUserCreate — configuration knobs', () => {
  it('persists primaryIdpSubject when provided', async () => {
    const fx = newFixture();
    const result = await handleUserCreate(
      {
        tenantId: fx.tenantId,
  ...
  it('persists givenName / familyName when provided, omits when not', async () => {
    const fx = newFixture();
  ...
  it('persists passwordHash when provided', async () => {
    const fx = newFixture();
    const result = await handleUserCreate(
      {
        tenantId: fx.tenantId,
    ...
    expect(result.document.passwordHash).toBe(
...
describe('handleUserCreate — tenant scoping', () => {
