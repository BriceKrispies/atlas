...
function fx(): ReturnType<typeof newFixture> {
    return newFixture('customer');
}
// =====================================================================
// impersonation.feature
// =====================================================================
describe('impersonation.feature: Operator starts an impersonation session', function () {
    it('creates an ImpersonationSession + emits ImpersonationStarted with retention:7y', async function () {
        const f = fx();
        ...
        const stored = await getImpersonationSessionEntity(f.entities, f.tenantId, result.document.impersonationId);
        expect(stored?.status).toBe('active');
    });
});
describe('impersonation.feature: Reject impersonation without reason / ticket', function () {
    it('refuses an empty reason with IMPERSONATION_REASON_REQUIRED', async function () {
        const f = fx();
        await expect(handleImpersonationStart({
            tenantId: f.tenantId,
            correlationId: 'corr-1',
            operatorId: 'ops:bob',
            targetUserId: 'usr-alice',
            reason: '   ',
            ticketUrl: 'https://support.atlas.example/tickets/SUP-1234',
        }, f.events, f.entities)).rejects.toEqual(expect.objectContaining({ code: identityErrorCodes.IMPERSONATION_REASON_REQUIRED }));
        expect(f.events.events).toHaveLength(0);
    });
    it('refuses missing ticketUrl with IMPERSONATION_TICKET_REQUIRED', async function () {
        const f = fx();
        await expect(handleImpersonationStart({
            tenantId: f.tenantId,
            correlationId: 'corr-1',
            ...
            reason: 'Investigating ticket SUP-1234',
            ticketUrl: '',
        }, f.events, f.entities)).rejects.toEqual(expect.objectContaining({ code: identityErrorCodes.IMPERSONATION_TICKET_REQUIRED }));
    });
    it('refuses missing operator with IMPERSONATION_REQUIRES_OPERATOR', async function () {
        const f = fx();
        await expect(handleImpersonationStart({
            tenantId: f.tenantId,
            correlationId: 'corr-1',
        ...
        }, f.events, f.entities)).rejects.toEqual(expect.objectContaining({ code: identityErrorCodes.IMPERSONATION_REQUIRES_OPERATOR }));
    });
});
describe('impersonation.feature: Every action emits an audit event', function () {
    it('Action handler stamps impersonatedBy + emits ImpersonationAction at retention:7y', async function () {
        const f = fx();
...
describe('impersonation.feature: Auto-expires + token resolve', function () {
    it('rejects an expired impersonation token', async function () {
        const f = fx();
        const start = await handleImpersonationStart({
            tenantId: f.tenantId,
            correlationId: 'corr-1',
        ...
        const stored = await getImpersonationSessionEntity(f.entities, f.tenantId, start.document.impersonationId);
        if (!stored)
        ...
        const resolved = await resolveImpersonationToken(f.entities, f.tenantId, start.bearerToken);
        expect(resolved.ok).toBe(false);
        if (!resolved.ok)
            expect(resolved.reason).toBe('expired');
    });
    it('rejects malformed bearer with reason=malformed', async function () {
        const f = fx();
        const r = await resolveImpersonationToken(f.entities, f.tenantId, 'no-dot-here');
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.reason).toBe('malformed');
    });
    it('accepts a fresh, well-formed token', async function () {
        const f = fx();
        const start = await handleImpersonationStart({
            tenantId: f.tenantId,
            correlationId: 'corr-1',
...
describe('impersonation.feature: Operator ends + tenant revokes', function () {
    it('operator_ended flips status to ended + emits ImpersonationEnded', async function () {
        const f = fx();
        const start = await handleImpersonationStart({
            tenantId: f.tenantId,
            correlationId: 'corr-1',
    ...
    it('tenant_revoked flips status to revoked + records revokedBy', async function () {
        const f = fx();
        const start = await handleImpersonationStart({
            tenantId: f.tenantId,
            correlationId: 'corr-1',
    ...
    it('refuses double-end with IMPERSONATION_ENDED', async function () {
        const f = fx();
        const start = await handleImpersonationStart({
            tenantId: f.tenantId,
            correlationId: 'corr-1',
        ...
        await expect(handleImpersonationEnd({
            tenantId: f.tenantId,
            correlationId: 'corr-3',
            impersonationId: start.document.impersonationId,
            principalId: 'ops:bob',
            reason: 'operator_ended',
        }, f.events, f.entities)).rejects.toBeInstanceOf(IdentityError);
    });
});
describe('impersonation.feature: readonly resource types', function () {
    it('persists readonlyResourceTypes for the middleware to consult', async function () {
        const f = fx();
        const start = await handleImpersonationStart({
            tenantId: f.tenantId,
            correlationId: 'corr-1',
...
// break-glass.feature
// =====================================================================
describe('break-glass.feature: Operator issues a grant', function () {
    it('Issue with requireApproval=true creates pending_approval + emits BreakGlassIssued', async function () {
        const f = fx();
    ...
    it('refuses missing justification with BREAK_GLASS_JUSTIFICATION_REQUIRED', async function () {
        const f = fx();
        await expect(handleBreakGlassIssue({
            tenantId: 'ledger',
            correlationId: 'corr-1',
            issuedBy: 'ops:bob',
            grantedTo: 'ops:bob',
        ...
        }, f.events, f.entities)).rejects.toEqual(expect.objectContaining({
            code: identityErrorCodes.BREAK_GLASS_JUSTIFICATION_REQUIRED,
        }));
    });
    it('refuses missing incidentUrl with BREAK_GLASS_INCIDENT_REQUIRED', async function () {
        const f = fx();
        await expect(handleBreakGlassIssue({
            tenantId: 'ledger',
            correlationId: 'corr-1',
...
describe('break-glass.feature: 4-eyes approval', function () {
    it('Second approver activates the grant and resets expiresAt', async function () {
        const f = fx();
    ...
    it('refuses self-approval with BREAK_GLASS_SELF_APPROVAL_FORBIDDEN', async function () {
        const f = fx();
        const issued = await handleBreakGlassIssue({
            tenantId: 'ledger',
            correlationId: 'corr-1',
    ...
    it('Deny flips the grant to denied + emits BreakGlassDenied', async function () {
        const f = fx();
        const issued = await handleBreakGlassIssue({
            tenantId: 'ledger',
            correlationId: 'corr-1',
...
describe('break-glass.feature: Active grants + revoke + expire', function () {
    it('resolveActiveGrants returns the active grant for the recipient', async function () {
        const f = fx();
        const issued = await handleBreakGlassIssue({
            tenantId: 'ledger',
            correlationId: 'corr-1',
    ...
    it('Revoke flips status to revoked + emits BreakGlassRevoked', async function () {
        const f = fx();
        const issued = await handleBreakGlassIssue({
            tenantId: 'ledger',
            correlationId: 'corr-1',
    ...
    it('auto_expired produces BreakGlassExpired event + status=expired', async function () {
        const f = fx();
        const issued = await handleBreakGlassIssue({
            tenantId: 'ledger',
            correlationId: 'corr-1',
    ...
    it('rejects revoke of an unknown grant with BREAK_GLASS_GRANT_NOT_FOUND', async function () {
        const f = fx();
...
describe('break-glass.feature: getBreakGlassGrantEntity round-trip', function () {
    it('persists the grant and returns it on read', async function () {
        const f = fx();
        const issued = await handleBreakGlassIssue({
            tenantId: 'ledger',
            correlationId: 'corr-1',
        ...
        const fetched = await getBreakGlassGrantEntity(f.entities, 'ledger', issued.document.grantId);
...
// Risk engine
// =====================================================================
describe('risk engine: defaultRiskScorer', function () {
    it('returns 0 when no signals are provided', function () {
        const scorer = defaultRiskScorer();
        const r = scorer({});
        expect(r.score).toBe(0);
    });
    it('scores geo mismatch when expectedGeo is configured', function () {
    ...
    it('scores cli UA higher than browser UA', function () {
        const scorer = defaultRiskScorer();
        expect(scorer({ uaClass: 'browser' }).score).toBe(0);
        expect(scorer({ uaClass: 'cli' }).score).toBe(0.2);
    });
    it('linearly scores recentFailureRate up to 0.4', function () {
        const scorer = defaultRiskScorer();
    ...
    it('combines signals up to score=1', function () {
        const scorer = defaultRiskScorer({ expectedGeo: ['US'] });
        const r = scorer({
            geo: 'CN',
            uaClass: 'cli',
    ...
    it('fixedRiskScorer ignores signals', function () {
        const r = fixedRiskScorer(0.42)({ geo: 'CN', uaClass: 'cli' });
        expect(r.score).toBe(0.42);
    });
});
describe('risk engine: decideFromScore', function () {
    it('default thresholds: 0.5 → allow, 0.7 → step_up, 0.95 → hard_deny', function () {
    ...
    it('respects custom tenant thresholds', function () {
