...
describe('saml.feature: parseIdpMetadata', () => {
  it('extracts entityID, SSO URL, signing cert, NameID format', () => {
  ...
  it('rejects metadata missing EntityDescriptor', () => {
    expect(() => parseIdpMetadata('<NotMetadata/>')).toThrow(
      expect.objectContaining({ code: identityErrorCodes.SAML_INVALID_METADATA }),
    );
  });

  it('rejects metadata missing SingleSignOnService', () => {
  ...
  it('rejects metadata missing X509Certificate', () => {
  ...
  it('falls back to Redirect when POST binding is absent', () => {
    const xml = MINIMAL_IDP_METADATA.replace(
      /<SingleSignOnService Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST"[^>]*\/>/g,
      '',
    );
...
describe('saml.feature: buildAuthnRequest', () => {
  it('produces a well-formed AuthnRequest XML with Issuer + ACS URL', async () => {
    ...
    expect(built.xml).toContain(
      '<saml:Issuer>https://atlas.example/sso/saml/acme</saml:Issuer>',
    );
    expect(built.xml).toContain(
      'AssertionConsumerServiceURL="https://atlas.example/sso/saml/acme/acs"',
    );
...
describe('saml.feature: generateSamlSpKey', () => {
  it('produces a 2048-bit RSA key + self-signed cert', () => {
    ...
    expect(new Date(key.notAfter).getTime()).toBeGreaterThan(
      new Date(key.notBefore).getTime(),
    );
  });
});

describe('saml.feature: default attribute mappings', () => {
  it('points at the standard claim URIs (Microsoft / SAML conventions)', () => {
    expect(DEFAULT_SAML_ATTRIBUTE_MAPPINGS.email).toBe(
      'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress',
    );
    expect(DEFAULT_SAML_ATTRIBUTE_MAPPINGS.givenName).toBe(
