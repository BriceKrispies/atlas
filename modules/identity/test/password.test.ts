...
describe('validatePasswordComplexity', function () {
    it('rejects too-short passwords', function () {
        expect(function () {
            return validatePasswordComplexity('Short1!');
        }).toThrow(IdentityError);
    });
    it('rejects single-class passwords', function () {
        expect(function () {
            return validatePasswordComplexity('aaaaaaaaaaaaaa');
        }).toThrow(IdentityError);
    });
    it('accepts a passphrase with mixed case', function () {
        expect(function () {
            return validatePasswordComplexity('correct horse Battery staple');
        }).not.toThrow();
    });
    it('accepts a length+digit combo', function () {
        expect(function () {
            return validatePasswordComplexity('letmein-123456789');
        }).not.toThrow();
    });
});
describe('Identity.User.SetPassword', function () {
    it('hashes, persists, and clears any existing lockout', async function () {
        const fx = newFixture();
    ...
    it('rejects weak passwords without persisting anything', async function () {
        const fx = newFixture();
        const user = await handleUserCreate({
            tenantId: fx.tenantId,
            correlationId: 'c',
        ...
        await expect(handlePasswordSet({
            ...
            userId: user.document.userId,
            newPassword: 'weak',
        }, fx.events, fx.entities)).rejects.toMatchObject({ code: identityErrorCodes.PASSWORD_COMPLEXITY });
    });
...
describe('Identity.Login.Password', function () {
    async function seedUserWithPassword(fx: Fixture, email: string, password: string): Promise<UserDocument> {
        const user = await handleUserCreate({ tenantId: fx.tenantId, correlationId: 'c', principalId: 'admin', email }, fx.events);
        await dispatchAll(fx);
    ...
    it('happy path: emits LoginSucceeded + a follow update clearing the failure counter', async function () {
        const fx = newFixture();
    ...
    it('wrong password emits LoginRejected with reason=wrong_password', async function () {
        const fx = newFixture();
        await seedUserWithPassword(fx, 'alice@example.com', 'correct-horse-Battery-staple');
        const result = await handlePasswordLogin({
            tenantId: fx.tenantId,
        ...
        expect(payloadRecord(result.envelope)['reason']).toBe('wrong_password');
        expect(result.follow.map(function (e) {
            return e.eventType;
        })).toEqual(['Identity.UserUpdated']);
    });
    it('unknown user emits LoginRejected with reason=unknown_user (no follow)', async function () {
        const fx = newFixture();
        const result = await handlePasswordLogin({
            tenantId: fx.tenantId,
            correlationId: 'c',
        ...
        expect(result.envelope.eventType).toBe('Identity.LoginRejected');
        expect(payloadRecord(result.envelope)['reason']).toBe('unknown_user');
        expect(result.follow).toEqual([]);
    });
    it('5th consecutive failure trips the lockout', async function () {
        const fx = newFixture();
        await seedUserWithPassword(fx, 'alice@example.com', 'correct-horse-Battery-staple');
        for (let i = 0; i < 4; i += 1) {
        ...
        await dispatchAll(fx);
        expect(trip.follow.map(function (e) {
            return e.eventType;
        })).toEqual(['Identity.AccountLocked']);
    ...
    it('rejects when user has no password (federated/passkey-only)', async function () {
        const fx = newFixture();
        ...
        expect(payloadRecord(result.envelope)['reason']).toBe('no_password_factor');
    });
});
