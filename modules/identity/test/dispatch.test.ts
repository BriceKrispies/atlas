...
describe('I12 — dispatcher rebuilds every entity type from event history alone', function () {
    it('AuthSession (issued via invite-accept) + revoke', async function () {
        const fx = newFixture();
        ...
        const sessionEvent = assertDefined(fx.events.events.find(function (e) {
    ...
    it('ApiKey lifecycle: create + rotate + revoke', async function () {
        const fx = newFixture();
    ...
    it('ServicePrincipal lifecycle: create + setScopes + disable', async function () {
        const fx = newFixture();
    ...
    it('OAuthAccessToken: issue + revoke', async function () {
        const fx = newFixture();
        const sp = await handleServicePrincipalCreate({
            tenantId: fx.tenantId,
            correlationId: 'c',
    ...
    it('IdentityProvider lifecycle: configure + activate + rotateJwks + disable', async function () {
        const fx = newFixture();
    ...
    it('ScimToken lifecycle: enable + rotate + revoke', async function () {
        const fx = newFixture();
    ...
    it('AuditExportConfig lifecycle: configure + activate + disable', async function () {
        const fx = newFixture();
    ...
    it('AuthFactor (TOTP) + revoke', async function () {
        const fx = newFixture();
        ...
        const enrolled = await handleTotpEnroll({
            ...
            issuer: 'Atlas',
            accountLabel: 'totp-user@example.com',
            name: 'My phone',
    ...
    it('RecoveryCode: generate batch + redeem', async function () {
        const fx = newFixture();
        const userResult = await handleUserCreate({
            tenantId: fx.tenantId,
            correlationId: 'c',
        ...
        const firstCode = assertDefined(generated.plaintextCodes[0], 'recovery code generation must yield at least one plaintext code');
        ...
        const eventTypesEmitted = fx.events.events.map(function (e) {
            return e.eventType;
        });
        ...
        const consumed = assertDefined(fx.events.events.find(function (e) {
            return e.eventType === 'Identity.RecoveryCodeConsumed';
    ...
    it('MfaBypass: issue', async function () {
        const fx = newFixture();
        const userResult = await handleUserCreate({
            tenantId: fx.tenantId,
            correlationId: 'c',
    ...
    it('SamlSpKey: generate + rotate', async function () {
        const fx = newFixture();
    ...
    it('full multi-entity flow: User + Membership + Session + ApiKey + IdP', async function () {
    ...
    it('SessionRevokeAllForUser fans out cleanly through the dispatcher', async function () {
        const fx = newFixture();
        const issued = await handleInviteIssue({
            tenantId: fx.tenantId,
            correlationId: 'c',
