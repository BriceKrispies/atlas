...
describe('cross-tenant isolation: User queries', () => {
  it('getUser scoped to tenant A never returns tenant B rows', async () => {
  ...
  it('getUserByEmail does not leak across tenants', async () => {
  ...
  it('getUserByIdpSubject does not leak across tenants', async () => {
    await seedUser(
      TENANT_B,
      userDoc(TENANT_B, 'usr-b1', { primaryIdpSubject: 'sub-shared' }),
    );
    const aResult = await getUserByIdpSubject(depsFor(TENANT_A), 'sub-shared');
    expect(aResult).toBeNull();
  });

  it('listAllUsers returns only the requested tenant\'s users', async () => {
    ...
    expect(aResult.map((u) => u.userId).sort()).toEqual(['usr-a1', 'usr-a2']);
    // Crucially: no `usr-b1` regardless of how listAllUsers filters internally.
    expect(aResult.some((u) => u.userId === 'usr-b1')).toBe(false);
  });
});
...
describe('cross-tenant isolation: Membership queries', () => {
  ...
  it('getMembership does not return another tenant\'s membership for the same userId', async () => {
    await seedMembership(membership(TENANT_B, 'usr-shared'));
    const aResult = await getMembership(depsFor(TENANT_A), 'usr-shared');
    expect(aResult).toBeNull();
  });

  it('listMemberships returns only the requested tenant\'s rows', async () => {
    ...
    expect(aResult.map((m) => m.userId).sort()).toEqual(['usr-a1', 'usr-a2']);
    expect(aResult.every((m) => m.tenantId === TENANT_A)).toBe(true);
  });
});
...
describe('cross-tenant isolation: Session queries', () => {
  function session(
    tenantId: string,
    sessionId: string,
    userId: string,
  ...
  it('listOwnSessions returns only sessions for the requested tenant + user', async () => {
    ...
    expect(aResult.map((s) => s.sessionId)).toEqual(['sess-a1']);
    expect(aResult.every((s) => s.tenantId === TENANT_A)).toBe(true);
  });

  it('findSessionsByAccessTokenLookup never crosses tenant boundary even with the same lookup prefix', async () => {
    ...
    await seedSession(
      TENANT_A,
      session(TENANT_A, 'sess-a', 'usr-a', sharedLookup),
    );
    await seedSession(
      TENANT_B,
      session(TENANT_B, 'sess-b', 'usr-b', sharedLookup),
    );

    const aResult = await findSessionsByAccessTokenLookup(
      depsFor(TENANT_A),
      sharedLookup,
    );
    expect(aResult.map((s) => s.sessionId)).toEqual(['sess-a']);
    expect(aResult.every((s) => s.tenantId === TENANT_A)).toBe(true);
  });
});
