...
describe('audit-retention: PLATFORM_RETENTION_FLOOR', () => {
  it('contains exactly the 7y and 10y tags', () => {
...
describe('audit-retention: isPlatformFloorRetention', () => {
  it('returns true for retention:7y (impersonation)', () => {
    expect(isPlatformFloorRetention('retention:7y')).toBe(true);
  });
  it('returns true for retention:10y (break-glass)', () => {
    expect(isPlatformFloorRetention('retention:10y')).toBe(true);
  });
  it('returns false for retention:1y (default tier)', () => {
...
describe('audit-retention: shouldExportEvent — floor wins over tenant filter', () => {
  it('floor (7y) is exported even when tenant filter excludes it', () => {
  ...
  it('floor (10y) is exported even when tenant filter is empty', () => {
  ...
  it('floor (10y) is exported when filter only allows 1y', () => {
    expect(shouldExportEvent('retention:10y', ['retention:1y'])).toBe(true);
  });
});

describe('audit-retention: shouldExportEvent — tenant filter semantics for non-floor tags', () => {
  it('non-floor tag in filter → included', () => {
    expect(shouldExportEvent('retention:1y', ['retention:1y'])).toBe(true);
  });
  it('non-floor tag NOT in filter → excluded', () => {
    expect(shouldExportEvent('retention:1y', ['retention:7y'])).toBe(false);
  });
  it('non-floor tag with undefined filter → default-include', () => {
    expect(shouldExportEvent('retention:1y', undefined)).toBe(true);
  });
  it('undefined tag with undefined filter → default-include', () => {
    expect(shouldExportEvent(undefined, undefined)).toBe(true);
  });
  it('undefined tag treated as retention:1y for filter matching', () => {
    expect(shouldExportEvent(undefined, ['retention:1y'])).toBe(true);
    expect(shouldExportEvent(undefined, ['retention:7y'])).toBe(false);
  });
});

describe('audit-retention: effectiveRetentionDays — floor cannot be shortened', () => {
  it('retention:7y → 2555 days', () => {
    expect(effectiveRetentionDays('retention:7y')).toBe(2555);
  });
  it('retention:10y → 3650 days', () => {
    expect(effectiveRetentionDays('retention:10y')).toBe(3650);
  });
  it('retention:7y with tenant override of 100 days → still 2555 (floor wins)', () => {
    expect(effectiveRetentionDays('retention:7y', 100)).toBe(2555);
  });
  it('retention:10y with tenant override of 100 days → still 3650 (floor wins)', () => {
    expect(effectiveRetentionDays('retention:10y', 100)).toBe(3650);
  });
});

describe('audit-retention: effectiveRetentionDays — tenant CAN extend', () => {
  it('retention:1y with override 9999 → 9999 (tenant extends default)', () => {
    expect(effectiveRetentionDays('retention:1y', 9999)).toBe(9999);
  });
  it('retention:7y with override 4000 → 4000 (tenant extends floor)', () => {
    expect(effectiveRetentionDays('retention:7y', 4000)).toBe(4000);
  });
  it('retention:10y with override 99999 → 99999 (tenant extends floor)', () => {
    expect(effectiveRetentionDays('retention:10y', 99999)).toBe(99999);
  });
});

describe('audit-retention: effectiveRetentionDays — defaults', () => {
  it('undefined tag → 365 (default platform retention)', () => {
    expect(effectiveRetentionDays(undefined)).toBe(365);
  });
  it('retention:1y (no override) → 365', () => {
    expect(effectiveRetentionDays('retention:1y')).toBe(365);
  });
  it('unrecognised tag → 365 (default platform retention)', () => {
    expect(effectiveRetentionDays('retention:30d')).toBe(365);
  });
  it('unrecognised tag with override 1000 → 1000 (tenant extends default)', () => {
