 ...
 */
import type { EventStore, StoredEvent, Entity, EntityListOptions, EntityQueryOptions, EntityStatus, EntityStore as PortEntityStore, EntityWriteInput, Relation, RelationStore, RelationWriteInput, SecretStore, } from '@atlas/ports';
...
export class InMemoryEntityStore implements PortEntityStore {
    ...
    async get<TAttrs = unknown>(tenantId: string, entityType: string, entityId: string): Promise<Entity<TAttrs> | null> {
        ...
        if (!row || row.status === 'deleted')
            return null;
        // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- boundary: in-memory EntityStore shim; row was stored as Entity<unknown> and the caller's TAttrs is contract-pinned by the entity type used at put-time. Mirrors the cast pattern in PostgresEntityStore.
 ...
 */
export function assertEventTags(envelope: EventEnvelope, expectedTags: readonly string[]): void {
    if (!Array.isArray(envelope.cacheInvalidationTags)) {
        throw new Error(`assertEventTags: envelope ${envelope.eventType} (${envelope.eventId}) ` +
            `has no cacheInvalidationTags array (I10 violation)`);
    }
    const have = new Set(envelope.cacheInvalidationTags);
    const missing = expectedTags.filter(function (t) {
        return !have.has(t);
    });
    if (missing.length > 0) {
        throw new Error(`assertEventTags: envelope ${envelope.eventType} (${envelope.eventId}) ` +
