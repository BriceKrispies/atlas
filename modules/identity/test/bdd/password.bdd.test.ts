...
const featurePath = fileURLToPath(new URL('../../../../specs/domains/identity/features/password/password.feature', import.meta.url));
runFeature({
    featurePath,
    steps: passwordSteps,
    newWorld: function () {
        return freshWorld('smb');
    },
    // Default Tier 1 tag set; override via `BDD_TAGS=...` env.
    tags: process.env['BDD_TAGS']?.split(',').map(function (s) {
        return s.trim();
    }) ?? [
        '@phase-a1',
    ],
