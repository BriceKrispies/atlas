...
export type StepFn = (world: BddWorld, ...args: string[]) => Promise<void> | void;
...
export class StepRegistry {
    ...
    resolve(text: string, kindHint: StepKind): {
        fn: StepFn;
        args: string[];
    } | null {
        const matches: {
        ...
        for (const s of this.steps) {
            ...
            if (m)
                matches.push({ fn: s.fn, args: m.slice(1) });
        }
        if (matches.length === 0)
            return null;
        if (matches.length > 1) {
            throw new Error(`[bdd] ambiguous step "${text}" matched ${matches.length} patterns`);
        }
        const sole = matches[0];
        if (!sole)
            throw new Error('[bdd] internal: matches.length===1 but matches[0] missing');
        return sole;
    }
}
/**
...
function parseFeature(featurePath: string): ParsedFeature {
    ...
    if (!feature)
        throw new Error(`[bdd] no Feature in ${featurePath}`);
    const featureTags = new Set<string>((feature.tags ?? []).map(function (t: messages.Tag) {
        return t.name;
    }));
    ...
    for (const child of feature.children ?? []) {
        if (child.background) {
            ...
            const scTags = new Set<string>((sc.tags ?? []).map(function (t: messages.Tag) {
                return t.name;
            }));
            let lastKind: StepKind = 'Given';
...
export function runFeature(opts: RunFeatureOptions): void {
    const tags = opts.tags ??
        process.env['BDD_TAGS']?.split(',').map(function (s) {
            return s.trim();
        }) ??
        ['@phase-a1'];
    const tagSet = new Set(tags);
    const feature = parseFeature(opts.featurePath);
    describe(`[bdd] ${feature.name}`, function () {
        for (const sc of feature.scenarios) {
            const intersects = [...sc.tags].some(function (t) {
                return tagSet.has(t);
            }) ||
                [...feature.tags].some(function (t) {
                    return tagSet.has(t);
                });
            if (!intersects) {
                ...
                it.skip(`${sc.name} [tags: ${tagStr}]`, function () {
                    // empty
                });
                continue;
            }
            it(sc.name, async function () {
                ...
                for (const step of [...feature.background, ...sc.steps]) {
                    ...
                    if (!resolved) {
                        throw new Error(`[bdd] no step binding for ${step.kind} "${step.text}". ` +
