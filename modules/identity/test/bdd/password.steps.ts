...
// ---------------------------------------------------------------------
passwordSteps.Given('a tenant {string} with the identity module enabled', function (world: BddWorld, tenantId: string) {
    world.tenantId = tenantId;
});
passwordSteps.Given('tenant {string} has password authentication enabled', function (_world: BddWorld, _tenantId: string) {
    // No-op in the in-memory model; password auth is always available.
});
passwordSteps.Given('the admin is authenticated as a principal with role {string}', function (_world: BddWorld, _role: string) {
...
// ---------------------------------------------------------------------
passwordSteps.Given('an InviteToken exists for {string} in tenant {string}', async function (world: BddWorld, email: string, tenantId: string) {
...
passwordSteps.When('alice opens the magic-link URL', function (_world: BddWorld) {
    // Out-of-band step in the spec; the actual cryptographic act is the
    // POST that follows. No state change here.
});
passwordSteps.When('submits {string} with token + new password {string}', async function (world: BddWorld, _route: string, newPassword: string) {
...
passwordSteps.Then('a User entity is created with passwordHash {word} on the entity attrs', async function (world: BddWorld, _hashKind: string) {
...
passwordSteps.Then('the InviteToken status flips to {string}', async function (world: BddWorld, _expectedStatus: string) {
    // Spec says "consumed", impl uses "accepted" — flag for spec/impl
    // reconciliation. Asserting against impl behavior here.
    if (!world.pendingInvite)
        throw new Error('No invite recorded');
    const updated = await world.entities.get<InviteTokenDocument>(world.tenantId, 'InviteToken', world.pendingInvite.tokenId);
...
passwordSteps.Then('an {string} event is emitted (no plaintext, never)', function (world: BddWorld, eventType: string) {
    const expected = resolveEventName(eventType);
    const matches = world.events.events.filter(function (e) {
        return e.eventType === expected;
    });
...
passwordSteps.Then('a Membership is created with the role from the InviteToken', async function (world: BddWorld) {
    ...
    const m = await world.entities.get<MembershipDocument>(world.tenantId, 'Membership', membershipId);
...
// ---------------------------------------------------------------------
passwordSteps.Given("alice's account exists", async function (world: BddWorld) {
...
passwordSteps.When('{int} wrong-password attempts land within 1 hour', async function (world: BddWorld, count: string) {
...
passwordSteps.Then("alice's User entity attrs.lockedUntil is set 15 minutes in the future", async function (world: BddWorld) {
    const user = assertDefined(world.user, 'Background step seeded alice');
    const fresh = await world.entities.get<UserDocument>(world.tenantId, 'User', user.userId);
    const lockedUntil = assertDefined(fresh?.attrs.lockedUntil, 'lockout dispatched onto user entity');
...
passwordSteps.Then('further attempts return 401 with reason {string}', async function (world: BddWorld, expectedReason: string) {
    ...
    const reject = assertDefined(newEvents.find(function (e) {
        return e.eventType === 'Identity.LoginRejected';
    ...
    const reason = payload !== null && typeof payload === 'object' && 'reason' in payload
        ? payload.reason
        : undefined;
    expect(reason).toBe(expectedReason);
});
passwordSteps.Then('an {string} event is emitted', function (world: BddWorld, eventType: string) {
    const expected = resolveEventName(eventType);
    const matches = world.events.events.filter(function (e) {
        return e.eventType === expected;
    });
...
// ---------------------------------------------------------------------
passwordSteps.When('a user submits a password {string} via either set or reset', async function (world: BddWorld, password: string) {
    ...
    (world as BddWorld & {
        eventCountBeforeReject?: number;
    }).eventCountBeforeReject =
        eventCountBefore;
});
passwordSteps.Then('the response status is {int}', function (world: BddWorld, status: string) {
    const err = assertDefined(world.lastError, 'When-step threw IdentityError');
    expect(err.status).toBe(Number(status));
});
passwordSteps.Then('the error message lists the failing rules', function (world: BddWorld) {
    const err = assertDefined(world.lastError, 'When-step threw IdentityError');
...
passwordSteps.Then('no entity is mutated', function (world: BddWorld) {
    const before = (world as BddWorld & {
        eventCountBeforeReject?: number;
    })
