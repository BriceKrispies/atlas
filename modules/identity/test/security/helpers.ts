...
export function buildSamlResponseXml(opts: BuildResponseOptions = {}): string {
  ...
  const notOnOrAfter =
    opts.notOnOrAfter ?? new Date(Date.now() + 5 * 60_000).toISOString();
  const recipient =
    opts.subjectConfirmationRecipient ?? 'https://atlas.example.com/saml/acs';
  ...
  const extraAttrs = opts.extraAttributes
    ? Object.entries(opts.extraAttributes)
        .map(
          ([n, v]) =>
            `<saml:Attribute Name="${n}"><saml:AttributeValue>${v}</saml:AttributeValue></saml:Attribute>`,
        )
        .join('')
    : '';
 ...
 */
export function signedSamlBase64(
  buildOpts: BuildResponseOptions,
...
export function newReplayChecker(): {
  check: (id: string, expiresAt: string) => Promise<{ alreadySeen: boolean }>;
  seen: Set<string>;
} {
