...
describe('evaluateRiskGate', function () {
    it('low score allows the request', function () {
    ...
    it('mid score returns step_up when not acknowledged', function () {
        const out = evaluateRiskGate({
    ...
    it('mid score with fresh acknowledgement returns allow', function () {
        const session = fakeSession({
            riskAcknowledgedUntil: new Date(Date.now() + 60000).toISOString(),
        });
        const out = evaluateRiskGate({
            scorer: fixedRiskScorer(0.8),
            policy: DEFAULT_RISK_POLICY,
    ...
    it('mid score with stale acknowledgement still requires step_up', function () {
        const session = fakeSession({
            riskAcknowledgedUntil: new Date(Date.now() - 60000).toISOString(),
        });
        const out = evaluateRiskGate({
    ...
    it('hard_deny is NOT overridden by a fresh acknowledgement', function () {
        const session = fakeSession({
            riskAcknowledgedUntil: new Date(Date.now() + 60000).toISOString(),
        });
        const out = evaluateRiskGate({
    ...
    it('respects custom tenant thresholds', function () {
        ...
        expect(evaluateRiskGate({
            ...
            policy,
            signals: {},
        }).decision).toBe('step_up');
        expect(evaluateRiskGate({
            scorer: fixedRiskScorer(0.7),
            policy,
            signals: {},
        }).decision).toBe('hard_deny');
    });
    it('uses real defaultRiskScorer signals when provided', function () {
...
describe('acknowledgeStepUp', function () {
    it('returns a session with riskAcknowledgedUntil set in the future', function () {
        const session = fakeSession();
        const updated = acknowledgeStepUp(session);
        expect(updated.riskAcknowledgedUntil).toBeDefined();
        const ack = new Date(assertDefined(updated.riskAcknowledgedUntil, 'just acked')).getTime();
        expect(ack).toBeGreaterThan(Date.now());
        // Default window 5 minutes — clamp to a generous range to avoid flake.
        expect(ack - Date.now()).toBeLessThanOrEqual(DEFAULT_RISK_ACK_WINDOW_SECONDS * 1000 + 1000);
    });
    it('returns a NEW object — does not mutate the input', function () {
        const session = fakeSession();
    ...
    it('honours a custom window', function () {
        ...
        const ack = new Date(assertDefined(updated.riskAcknowledgedUntil, 'just acked')).getTime();
        expect(ack - Date.now()).toBeLessThanOrEqual(11000);
        expect(ack - Date.now()).toBeGreaterThan(8000);
    });
    it('honours an explicit clock for deterministic tests', function () {
        const session = fakeSession();
        const fixedNow = 1700000000000;
        const updated = acknowledgeStepUp(session, 60, fixedNow);
        expect(updated.riskAcknowledgedUntil).toBe(new Date(fixedNow + 60000).toISOString());
    });
});
describe('step-up: pen-test scenarios', function () {
    it('cannot bypass step_up by setting an acknowledgement timestamp in the past', function () {
        const session = fakeSession({
    ...
    it('cannot bypass hard_deny by acknowledging step-up', function () {
        const session = fakeSession({
            riskAcknowledgedUntil: new Date(Date.now() + 3600000).toISOString(),
        });
        const out = evaluateRiskGate({
    ...
    it('cannot forge ack via empty session — no session means no ack regardless of score', function () {
        const out = evaluateRiskGate({
            scorer: fixedRiskScorer(0.8),
            policy: DEFAULT_RISK_POLICY,
            signals: {},
