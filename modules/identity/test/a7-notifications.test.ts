...
import { BREAK_GLASS_RETENTION_TAG, IMPERSONATION_RETENTION_TAG, emitNotificationsForA7Event, handleBreakGlassIssue, handleImpersonationEnd, handleImpersonationStart, identityNotificationDispatcher, } from '../src/index.ts';
import type { Entity, EntityListOptions, EntityQueryOptions, EntityStore as PortEntityStore, EntityWriteInput, } from '@atlas/ports';
// ---------------------------------------------------------------------
...
function channelOf(env: EventEnvelope): string {
    ...
    if (payload != null && typeof payload === 'object' && 'channel' in payload) {
        const ch: unknown = (payload as {
            channel: unknown;
        }).channel;
        if (typeof ch === 'string')
            return ch;
    }
    throw new Error(`notification envelope ${env.eventId} (${env.eventType}) is missing string payload.channel`);
}
// ---------------------------------------------------------------------
...
// Tests
// ---------------------------------------------------------------------
describe('a7-notifications: ImpersonationStarted', function () {
    it('emits 2 follow-ups (tenant_admin + ops_pager) at retention:7y', async function () {
        const f = fx();
...
describe('a7-notifications: ImpersonationEnded reason=tenant_revoked', function () {
    it('emits to tenant_admin AND ops_pager (out-of-band ops notification)', async function () {
        const f = fx();
        const start = await handleImpersonationStart({
            tenantId: f.tenantId,
            correlationId: 'corr-1',
    ...
    it('emits only to tenant_admin on operator_ended (no ops fan-out)', async function () {
        const f = fx();
        const start = await handleImpersonationStart({
            tenantId: f.tenantId,
            correlationId: 'corr-1',
        ...
        expect(channelOf(assertDefined(followups[0], 'length checked above'))).toBe('tenant_admin');
    });
});
describe('a7-notifications: BreakGlassIssued', function () {
    it('emits 2 follow-ups (tenant_admin + security_pager) at retention:10y', async function () {
        const f = fx();
        ...
        const followups = await emitNotificationsForA7Event(issued.envelope, f.events);
        expect(followups).toHaveLength(2);
        const channels = followups.map(channelOf);
...
describe('a7-notifications: non-A7 events are ignored', function () {
    it('returns empty array for an unrelated event type', async function () {
        const f = fx();
    ...
    it('returns empty array for Action-shaped A7 events (too noisy)', async function () {
        const f = fx();
...
describe('a7-notifications: idempotency-key shape', function () {
    it('emits keys of the form notif.<sourceEventId>.<channel>', async function () {
        const f = fx();
        const start = await handleImpersonationStart({
            tenantId: f.tenantId,
            correlationId: 'corr-1',
        ...
        const followups = await emitNotificationsForA7Event(start.envelope, f.events);
        expect(followups).toHaveLength(2);
        const keys = followups.map(function (e) {
    ...
    it('respects channel override via opts.channels', async function () {
        const f = fx();
        const start = await handleImpersonationStart({
            tenantId: f.tenantId,
            correlationId: 'corr-1',
        ...
        const followups = await emitNotificationsForA7Event(start.envelope, f.events, { channels: ['security_pager'] });
        expect(followups).toHaveLength(1);
        expect(channelOf(assertDefined(followups[0], 'length checked above'))).toBe('security_pager');
    });
});
describe('a7-notifications: payload secrecy', function () {
    it('never leaks plaintext bearer / token / tokenHash / tokenLookup into payloads', async function () {
        const f = fx();
        const start = await handleImpersonationStart({
            tenantId: f.tenantId,
            correlationId: 'corr-1',
...
describe('a7-notifications: identityNotificationDispatcher', function () {
    it('appends follow-ups when invoked as a dispatcher closure', async function () {
        const f = fx();
        const start = await handleImpersonationStart({
            tenantId: f.tenantId,
            correlationId: 'corr-1',
        ...
        const notifEvents = f.events.events.filter(function (e) {
            return e.eventType.startsWith('Notifications.');
        });
        expect(notifEvents).toHaveLength(2);
    });
    it('is a no-op for non-A7 events', async function () {
        const f = fx();
        const dispatch = identityNotificationDispatcher(f.events);
        const fake: EventEnvelope = {
            eventId: 'evt-unrelated',
