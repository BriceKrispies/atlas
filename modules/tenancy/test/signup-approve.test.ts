...
import { TENANCY_SIGNUP_APPROVED_EVENT_TYPE, TENANCY_SIGNUP_APPROVED_SCHEMA_ID, } from '../src/types.ts';
// --- in-memory ports -------------------------------------------------
class InMemorySignupRequestStore implements SignupRequestStore {
    ...
    async list(filter?: {
        ...
        const all = Array.from(this.rows.values()).filter(function (r) {
            return filter?.status ? r.status === filter.status : true;
        });
        return all.slice(0, filter?.limit ?? 50).map(function (r) {
            return ({ ...r });
        });
    }
    async markApproved(signupId: string, tenantId: string): Promise<SignupRequest> {
        ...
        if (!row)
            throw new Error(`signup ${signupId} not found`);
...
class InMemoryCustomDomainStore implements CustomDomainStore {
    ...
    async getPrimary(tenantId: string): Promise<CustomDomain | null> {
        for (const r of this.byHostname.values()) {
            if (r.tenantId === tenantId && r.isPrimary && r.status === 'active')
                return { ...r };
        }
        return null;
    }
...
class InMemoryEventStore implements EventStore {
    ...
    async append(envelope: EventEnvelope): Promise<StoredEvent> {
        ...
        const existing = this.events.find(function (e) {
            return e.tenantId === envelope.tenantId && e.idempotencyKey === envelope.idempotencyKey;
        });
        if (existing) {
...
function buildHarness(): Harness {
    ...
    signupRequests.markApproved = async function (signupId, tenantId) {
    ...
    const deps: SignupApproveDeps = {
        ...
        customDomains,
        mailer,
        appendEvent: async function (env) {
            await eventStore.append(env);
        },
        apexDomain: 'localhost',
        ensureTenantProvisioned: async function (tenantId: string) {
            callOrder.push('ensureTenantProvisioned');
            ensureProvisionedCalls.push(tenantId);
        },
        revokeOutstandingInvites: async function (input) {
            callOrder.push('revokeOutstandingInvites');
            revokeCalls.push({ ...input });
        },
        issueInvite: async function (input) {
            callOrder.push('issueInvite');
            issueInviteCalls.push({ ...input });
            mintCounter.count += 1;
            return { plaintextToken: `tok-${mintCounter.count}` };
        },
        buildMagicLinkUrl: function ({ presentedToken, hostname, acceptedEmail }) {
            return `http://${hostname}/signup/confirm?token=${presentedToken}&email=${encodeURIComponent(acceptedEmail)}`;
        },
    };
    return {
        signupRequests,
...
// --- tests -----------------------------------------------------------
describe('handleSignupApprove', function () {
    it('happy path: provisions tenant, mints invite, sends email, marks approved, emits audit event', async function () {
        const h = buildHarness();
        seedPendingSignup(h);
        const result = await handleSignupApprove({ signupId: 'signup-abc', principalId: 'user-admin', correlationId: 'corr-1' }, h.deps);
        ...
        const sent = assertDefined(h.mailer.sends[0], 'mailer.sends asserted length === 1 immediately above');
        ...
        const evt = assertDefined(h.eventStore.events[0], 'eventStore.events asserted length === 1 immediately above');
    ...
    it('mailer rejection leaves the signup pending and propagates the error', async function () {
        const h = buildHarness();
        seedPendingSignup(h);
        h.mailer.shouldThrow = true;
        await expect(handleSignupApprove({ signupId: 'signup-abc', principalId: 'user-admin', correlationId: 'corr-2' }, h.deps)).rejects.toThrowError(/smtp-down/);
    ...
    it('idempotency on retry: revokes outstanding invites before minting a fresh one', async function () {
        ...
        h.signupRequests.markApproved = async function (signupId, tenantId) {
        ...
        await expect(handleSignupApprove({ signupId: 'signup-abc', principalId: 'user-admin', correlationId: 'corr-3' }, h.deps)).rejects.toThrowError(/db-blip/);
        expect(h.issueInviteCalls).toHaveLength(1);
        expect(h.revokeCalls).toHaveLength(1);
        expect(h.mailer.sends).toHaveLength(1);
        // Second attempt — let markApproved succeed.
        breakMark = false;
        const result = await handleSignupApprove({ signupId: 'signup-abc', principalId: 'user-admin', correlationId: 'corr-3-retry' }, h.deps);
    ...
    it('rejects with SIGNUP_NOT_PENDING when the row is already approved', async function () {
        const h = buildHarness();
        ...
        await expect(handleSignupApprove({ signupId: 'signup-abc', principalId: 'user-admin', correlationId: 'corr-x' }, h.deps)).rejects.toMatchObject({
            name: 'TenancyError',
        ...
        // catch + map.
        try {
            await handleSignupApprove({ signupId: 'signup-abc', principalId: 'user-admin', correlationId: 'corr-x' }, h.deps);
        }
        catch (e) {
