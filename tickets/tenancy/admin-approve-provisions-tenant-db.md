...
- 2026-05-24: **implementer-layer work is already DONE — do NOT dispatch an implementer.** Board-recon found `provisionAndMigrateTenant()` exported + wired in `apps/server/src/routes/admin-signups.ts` (the approve route's `ensureTenantProvisioned` callback delegates to it); the module handler `signup-approve.ts` calls it at step 3. The TDD test `apps/server/test/routes/admin-signups-provisioning.test.ts` passes **4/4** (provision-called, provision-before-getPool ordering, cache-invalidate, migratedTenants). Acceptance #1–#4 are met in committed code. REMAINING: #5–#7 — the two BDD scenarios end-to-end — which need the live stack (db + apps/server + smtp4dev). Next action is **live `pnpm bdd:server` verification**, not implementation; if green, close this and unblock `identity/tenant-admin-invites-user` + `tenancy/admin-approves-signup-bdd`.
