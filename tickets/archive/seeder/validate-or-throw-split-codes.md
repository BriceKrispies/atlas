...
- 2026-05-10 (sdet Phase 2 review): clean with caveats — adversarial pass found the body-invalid tests asymmetric vs. the new not-registered test (only the latter pinned `not.toThrow(/SEED_VALIDATION_FAILED/)`). Also no fixture-path coverage for the new code — `loadFixture` shares `validateOrThrow` but only the scenario branch was pinned, so a regression touching only the scenario path would silently lose fixture coverage. Tightened the body-invalid test with the symmetric negative assertion and added a `loadFixture`-specific `SEED_VALIDATOR_NOT_REGISTERED` regression that also asserts the message names `seed.fixture.v1`. 23/23 tests pass. No remaining concerns; ready for architect Phase 3.
- 2026-05-10 (architect Phase 3): clean. `validateOrThrow` at `adapters/seed-memory/src/in-memory-seed-corpus.ts:138-159` cleanly branches on `getSchemaValidator(...) === null` (REGISTRY) vs. `validate(body) === false` (VALIDATION); both throw before any mutation, preserving I2 ordering. `specs/crosscut/errors.md:77-78` lists both codes with parallel wording. No audit emission disturbed. Ready for merge.
