...
- 2026-05-10 (architect Phase 3): clean. Verified §9 action IDs (`Identity.User.Create`, `Identity.Membership.Create`, `Identity.Invite.Issue`, `Identity.Invite.Accept`) appear at `modules/identity/src/handlers/registry.ts:771-774` as claimed by the fix-pass. Doc-only change; no invariant surface touched. Caveat C4 (envelope `eventType` regex vs. 3-segment action-id) is a wider spec backlog item, not a gate. Ready for merge.
- 2026-05-10: done. Merged via main lineage (eda4257 → 0aa6a4b → 6270a36 → afc6182). Archived.
