...
- 2026-05-11 (architect Phase 3): clean. All 11 canonical $ids verified short-form at `specs/schemas/contracts/*.schema.json:3`; 5 duplicated copies under `packages/page-templates/src/schemas/` + `packages/widget-host/src/schemas/` match; OpenAPI embeds at `specs/openapi.tenant.json:851` + `specs/openapi.operator.json:25` use `error-envelope.v1`. I1 ingress schema gate preserved — `content_pages.page.create.v1` registered in `packages/schemas/src/loader.ts:39` resolves by its own short-form $id; the 11 normalized canonicals are consumed via `packages/page-templates/src/document.ts` (local AJV, $ref resolution exercised by the new regression test) and `adapters/node/src/migrations/seed.ts` (file-basename-keyed schema_registry rows, $id-independent). No `node:crypto` / `process.env` / `console.*` introduced; only new imports are `node:fs/promises`/`node:url`/`node:path` in the test file (acceptable, test code). Concerns flagged for follow-up tickets (not gates): (a) pre-existing canonical-vs-duplicate divergence on page_document.schema.json; (b) `sync-schemas.ts` does not regenerate the page-templates/widget-host hand-maintained duplicates; (c) widget config schemas under bundles/ + apps/authoring/ retain long-URL $ids per the explicit out-of-scope note. Ready for merge.
- 2026-05-11: done. Merged via main lineage (df8085c → bd30daf → 70087f7). Archived.
