...
- 2026-05-10: fix-pass by `port-adapter-dev` (opus) against sdet's two should-fix items.
  **Bug 1 fixed:** `loadFixture` now throws `SEED_FIXTURE_NOT_FOUND` instead of `SEED_SCENARIO_NOT_FOUND`. Added a Seeder Errors table to `specs/crosscut/errors.md` enumerating `SEED_SCENARIO_NOT_FOUND`, `SEED_FIXTURE_NOT_FOUND`, `SEED_VALIDATION_FAILED`, `SEED_FIXTURE_DEPTH_EXCEEDED`, `SEED_AXIS_RANGE_INVALID`, `SEED_AXIS_GENERATOR_UNKNOWN`, `SEED_AXIS_ID_PARSE_FAILED`.
  **Bug 2 fixed:** Consolidated `canonicalJsonStringify` per `specs/crosscut/scenario-fuzzing.md` §7. New canonical home: `packages/platform-core/src/canonical-json.ts` (with co-located `canonical-json.test.ts`). Re-exported from `@atlas/platform-core` main entry. Both `packages/seeder/src/canonical-json.ts` and the inline copy in `adapters/seed-memory/src/in-memory-seed-corpus.ts` deleted; both packages now import from `@atlas/platform-core`. Date semantics: honors `toJSON()` (Date → ISO string), mirroring `JSON.stringify`. Distinct Date values now yield distinct canonical bytes (and contentHashes), restoring the spec §4.1 determinism contract.
