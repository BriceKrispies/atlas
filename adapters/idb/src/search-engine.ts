...
const FIELD_WEIGHTS: ReadonlyArray<readonly [
    string,
    number
]> = [
    ['title', 1.0],
...
export class IdbSearchEngine implements SearchEngine {
    ...
    async search(query: string, tenantId: string, principalId: string): Promise<SearchDocument[]> {
        ...
        const terms = trimmed
            .toLowerCase()
            .split(/\s+/u)
            .filter(function (t): t is string {
            return t.length > 0;
        });
        if (terms.length === 0)
            return [];
        const rows = await this.db.getAllFromIndex('search_documents', 'by_tenant_type', IDBKeyRange.bound([tenantId, ''], [tenantId, '￿']));
        const scored: Array<{
            doc: SearchDocument;
            score: number;
        }> = [];
