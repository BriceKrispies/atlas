 ...
 * (`fake-indexeddb`).
 */
import type { Entity, EntityListOptions, EntityQueryOptions, EntityStatus, EntityStore, EntityWriteInput, } from '@atlas/ports';
...
export class IdbEntityStore implements EntityStore {
    ...
    async list<TAttrs = unknown>(tenantId: string, entityType: string, opts: EntityListOptions = {}): Promise<Entity<TAttrs>[]> {
        ...
        const filtered = all
            .filter(function (r) {
            return (status === null ? true : r.status === status);
        })
            .filter(function (r) {
            return (after === '' ? true : r.entityId > after);
        })
            .sort(function (a, b) {
    ...
    async query<TAttrs = unknown>(tenantId: string, entityType: string, opts: EntityQueryOptions): Promise<Entity<TAttrs>[]> {
        ...
        return base.filter(function (e) {
            return attrsContains(e.attrs, predicate);
        });
    }
}
