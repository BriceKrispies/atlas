...
function assertDefined<T>(v: T | null | undefined, msg: string): T {
    if (v == null)
        throw new Error(`Test invariant violation: ${msg}`);
    return v;
}
let dbCounter = 0;
...
describe('IDB multi-tab stress', function () {
    let dbName: string;
    let tabA: Tab;
    let tabB: Tab;
    ...
    test('cross-tab idempotent appends produce exactly one event with one shared eventId per key', async function () {
        ...
        for (let i = 0; i < sharedKeys.length; i++) {
            ...
            expect(aId).toBe(bId);
            const matchingStored = stored.filter(function (e) {
                return e.idempotencyKey === sharedKeys[i];
            });
            expect(matchingStored.length).toBe(1);
            expect(assertDefined(matchingStored[0], 'matchingStored[0]').eventId).toBe(aId);
        }
    });
    test('cross-tab cache writes + invalidations leave a consistent state (no torn rows)', async function () {
        // Pre-populate via tab A. 12 entries: 4 'red', 4 'blue', 4 'green'.
        const setOps: Promise<void>[] = [];
        for (let i = 0; i < 4; i++) {
            setOps.push(tabA.cache.set(`red-${i}`, i, { ttlSeconds: 0, tags: ['red'] }), tabA.cache.set(`blue-${i}`, i, { ttlSeconds: 0, tags: ['blue'] }), tabA.cache.set(`green-${i}`, i, { ttlSeconds: 0, tags: ['green'] }));
        }
    ...
    test('mixed workload: 10 cross-tab appends with shared idempotency keys + 5 cross-tab cache invalidations', async function () {
        ...
        for (let i = 0; i < 8; i++) {
            seedOps.push(tabA.cache.set(`w-${i}`, i, { ttlSeconds: 0, tags: ['workload'] }));
        }
        ...
        for (let i = 0; i < 5; i++) {
            aWork.push(tabA.events.append(makeEvent({ tab: 'A', idempotencyKey: `mw-a-${i}` })));
            bWork.push(tabB.events.append(makeEvent({ tab: 'B', idempotencyKey: `mw-b-${i}` })));
        }
        for (let i = 0; i < 5; i++) {
        ...
        expect(stored.length).toBe(15);
        for (const k of sharedIdems) {
            const matching = stored.filter(function (e) {
                return e.idempotencyKey === k;
            });
