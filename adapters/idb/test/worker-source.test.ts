 ...
 */
async function nextEvent(iter: AsyncIterator<EventEnvelope>, ms: number, label: string): Promise<EventEnvelope> {
    const got = await withTimeout(iter.next(), ms, label);
    if (got.done === true)
        throw new Error(`iterator finished before yielding: ${label}`);
    return got.value;
}
describe('IdbWorkerSource', function () {
    test('subscribe receives a manually-inserted event via polling fallback', async function () {
    ...
    test('ack persists cursor; reopened subscription past cursor yields no duplicates', async function () {
        const db = await freshDb();
    ...
    test('close resolves a pending iterator immediately', async function () {
        const db = await freshDb();
        const source = new IdbWorkerSource(db, 'test-module');
