...
policyEngineContract(async function () {
    return new CedarPolicyEngine(makeBundleLoader({}));
});
// --- Cedar-specific scenarios --------------------------------------------
describe('CedarPolicyEngine — real Cedar semantics', function () {
    test('permit on matching action when bundle permits', async function () {
    ...
    test('deny on non-matching action', async function () {
        const engine = new CedarPolicyEngine(makeBundleLoader({ [TENANT_A]: FAMILY_READ_ONLY }));
        const decision = await engine.evaluate({
            principal: { id: 'alice', tenantId: TENANT_A, attributes: {} },
    ...
    test('forbid overrides permit (Invariant I4)', async function () {
        const engine = new CedarPolicyEngine(makeBundleLoader({ [TENANT_A]: FORBID_OVERRIDES }));
    ...
    test('attribute-based: principal.department == resource.department permits', async function () {
        const engine = new CedarPolicyEngine(makeBundleLoader({ [TENANT_A]: DEPARTMENT_RBAC }));
    ...
    test('cross-tenant: tenant A bundle never evaluates against tenant B request', async function () {
    ...
    test('decision shape: matchedPolicies populated for diagnostics', async function () {
        const engine = new CedarPolicyEngine(makeBundleLoader({ [TENANT_A]: FAMILY_READ_ONLY }));
        const decision = await engine.evaluate({
            principal: { id: 'alice', tenantId: TENANT_A, attributes: {} },
            action: 'Catalog.Family.Read',
    ...
    test('cache invalidation: invalidate(tenantId) drops cached bundle', async function () {
    ...
    test('bundle without @id annotations falls back to positional ids', async function () {
    ...
    `;
        const engine = new CedarPolicyEngine(makeBundleLoader({ [TENANT_A]: noAnnotations }));
        const decision = await engine.evaluate({
            principal: { id: 'alice', tenantId: TENANT_A, attributes: {} },
            action: 'Catalog.Family.Read',
            resource: { type: 'Family', id: 'fam-1', tenantId: TENANT_A, attributes: {} },
    ...
    test('malformed bundle parses via wrapper helper', function () {
        expect(function () {
            return parseWrapper('t', 1, { format: 'rule-json', policies: '...' });
        }).toThrow(/unsupported format/);
        expect(function () {
            return parseWrapper('t', 1, { format: 'cedar-text', policies: 42 });
