...
import { CedarPolicyEngine, BundledFixtureLoader, generateCedarSchema, } from '@atlas/adapter-policy-cedar';
import type { ModuleManifest, ValidationAnswer, } from '@atlas/adapter-policy-cedar';
...
async function main(): Promise<number> {
    ...
    // bin lives at adapters/policy-cedar/bin/, so 3 levels up.
    ...
    try {
        ...
        process.stderr.write(`cedar:check: failed to read fixtures dir ${fixturesDir}: ${(e as Error).message}\n`);
        return 2;
    }
    if (files.length === 0) {
        process.stderr.write(`cedar:check: no .cedar fixtures found under ${fixturesDir}\n`);
        return 2;
    }
    ...
    for (const path of files) {
        ...
        try {
            ...
            process.stderr.write(`cedar:check: failed to read ${path}: ${(e as Error).message}\n`);
            return 2;
        }
        ...
        try {
            ...
            process.stderr.write(`cedar:check: cedar-wasm validate failed on ${path}: ${(e as Error).message}\n`);
            return 2;
        }
    ...
    for (const r of results) {
        ...
        failed += 1;
        if (r.isNegative) {
            process.stdout.write(`  FAIL ${rel(r.path, repoRoot)} — negative fixture validated unexpectedly (should fail validation)\n`);
        }
        else {
            process.stdout.write(`  FAIL ${rel(r.path, repoRoot)} — policy failed schema validation:\n`);
