...
function parseSentEvent(raw: string): SentEvent & Record<string, unknown> {
    ...
    const requireString = function (k: string): string {
        const x = v[k];
        if (typeof x !== 'string') {
    ...
    const tags = Array.isArray(rawTags)
        ? rawTags.filter(function (t): t is string {
...
describe('StdoutEventMailer', function () {
    it('persists body to email_log so the in-app mailbox panel still works', async function () {
    ...
    it('does NOT include body on the stdout JSON line (credential-leak regression guard)', async function () {
        const { sql } = fakeSql();
        const mailer = new StdoutEventMailer(sql);
        const msg = baseMsg();
    ...
    it('returns the same messageId that was inserted into email_log', async function () {
        const { sql, calls } = fakeSql();
        const mailer = new StdoutEventMailer(sql);
