...
if (HAS_DB) {
  describe('PostgresWorkerSource', () => {
    it('subscribe + receive a freshly-appended event in events() within ~500ms', async () => {
    ...
    it('ack persists across reconnect — second subscribe past cursor sees no duplicate', async () => {
      const sql = await freshSql();
      await sql.unsafe('TRUNCATE TABLE worker_cursors');
      const store = new PostgresEventStore(sql);
      const source = new PostgresWorkerSource(sql, MODULE);
      ...
      const rows = await sql<{ last_seq: string | number | bigint }[]>`
    ...
    it('close resolves any pending events() iterator immediately', async () => {
      const sql = await freshSql();
      await sql.unsafe('TRUNCATE TABLE worker_cursors');
      const source = new PostgresWorkerSource(sql, MODULE);
  ...
  describe('PostgresWorkerSource (skipped)', () => {
    it.skip('TEST_TENANT_DB_URL not set — skipping Postgres worker source tests', () => {
      // intentionally empty
    });
  });
}
