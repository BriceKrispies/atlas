 ...
 */
export async function runMigrations(sql: postgres.Sql, kind: MigrationKind): Promise<MigrationRunResult> {
    ...
    if (kind !== 'control-plane' && kind !== 'tenant') {
        throw new TypeError(`runMigrations: kind must be 'control-plane' | 'tenant', got ${String(kind)}`);
    }
    ...
    const files = all
        .filter(function (f) {
        return f.toLowerCase().endsWith('.sql');
    })
        .sort(function (a, b) {
    ...
    const existing = new Set(existingRows.map(function (r) {
        return r.filename;
    }));
