...
export class PostgresEntityStore implements EntityStore {
  ...
  constructor(
    ...
    private readonly latestVersionByType: Map<string, number> = new Map(),
  ) {}
  ...
  async put<TAttrs = unknown>(
    ...
    const version =
      input.schemaVersion ?? this.latestVersionByType.get(input.entityType) ?? 1;
    ...
    `;
    const row = rows[0];
    if (!row) throw new Error('entity put returned no row');
  ...
  async list<TAttrs = unknown>(
    ...
    const rows = await this.sql<EntityRow[]>`
      ...
      LIMIT ${limit}
    `;
    return rows.map((r) => rowToEntity<TAttrs>(r));
  }
  ...
  async query<TAttrs = unknown>(
    ...
    const containment =
      opts.attrsEqual && Object.keys(opts.attrsEqual).length > 0
        ? opts.attrsEqual
        : null;
    const rows = await this.sql<EntityRow[]>`
      SELECT tenant_id, entity_type, entity_id, schema_version,
