...
function rowToSummary(row: PolicyRow): PolicySummary {
    ...
    const description = typeof row.policy_json.description === 'string' ? row.policy_json.description : null;
    return {
...
function rowToDetail(row: PolicyRow): PolicyDetail {
    ...
    const cedarText = typeof row.policy_json.policies === 'string' ? row.policy_json.policies : '';
...
export class PostgresPolicyStore implements PolicyStore {
    ...
    async activate(input: {
        ...
        await this.sql.begin(async function (tx) {
            const rows = await tx<{
                status: string;
            }[]> `
