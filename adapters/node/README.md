...
browser-side `@atlas/adapter-idb` package implements the same ports for
...
The contract suite is shared with `@atlas/adapter-idb`. Each port has a
...
pnpm exec vitest run adapters/node/
```
