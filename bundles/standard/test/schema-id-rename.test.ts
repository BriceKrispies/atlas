...
test('regression: page-layout.v1 + page-document.v1 duplicated $ids match short-form contract', async () => {
  ...
  expect(pageDoc.$id, 'page_document duplicated copy must declare short-form $id').toBe(
    'page-document.v1',
  );
  ...
  expect(
    ref,
    'page_document.regions.<region>.items.$ref must use short-form page-layout.v1#...',
  ).toBe('page-layout.v1#/definitions/WidgetInstance');
});

test('regression: validatePageDocument resolves the page-layout.v1 $ref (rename did not break cross-schema lookup)', async () => {
  ...
  expect(
    result.ok,
    `valid doc should pass; errors: ${JSON.stringify(result.errors)}`,
  ).toBe(true);
});

test('regression: canonical specs/schemas/contracts/page-layout.v1 + page-document.v1 use short-form $ids', async () => {
  ...
  expect(pageDoc.properties?.regions?.additionalProperties?.items?.$ref).toBe(
    'page-layout.v1#/definitions/WidgetInstance',
  );
});
