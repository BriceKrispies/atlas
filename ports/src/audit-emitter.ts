 ...
 * `AUDIT_EMIT_PERMITS=true`. The pure helpers + types live here in
 * `@atlas/ports` so any wiring layer (server, worker, future ingress
 * variants) can build the envelope without depending on a Cedar adapter.
 *
 * Per ADR 0008 (Atlas-on-Atlas) leak #4: modules MUST NOT import from
 * `@atlas/adapter-*`. The previous home of these helpers
 * (`adapter-policy-cedar/src/audit-emitter.ts`) re-exports the types
 * from here for back-compat — Cedar-agnostic code is the new default.
 *
 * Event shape mirrors the spec in `specs/crosscut/events.md`.
 *
