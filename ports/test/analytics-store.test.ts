...
describe('AnalyticsStore port (TS surface)', function () {
  test('interface exposes record(event) and query(filter) with correct types', function () {
    expectTypeOf<AnalyticsStore>().toHaveProperty('record');
    expectTypeOf<AnalyticsStore>().toHaveProperty('query');
    expectTypeOf<AnalyticsStore['record']>().parameter(0).toEqualTypeOf<AnalyticsEvent>();
    expectTypeOf<AnalyticsStore['record']>().returns.toEqualTypeOf<Promise<void>>();
    expectTypeOf<AnalyticsStore['query']>().parameter(0).toEqualTypeOf<AnalyticsQuery>();
    expectTypeOf<AnalyticsStore['query']>().returns.toEqualTypeOf<Promise<AnalyticsEvent[]>>();
  });
  test('AnalyticsEvent has the documented camelCase shape', function () {
  ...
  test('AnalyticsQuery only requires tenantId; everything else optional', function () {
  ...
  test('record then query round-trips events filtered by tenant + eventType', async function () {
    ...
    expect(
      tenantBLeak.some(function (e) {
        return e.tenantId !== 'tenant-a';
      }),
    ).toBe(false);
  });
  test('query honours from/to time window and limit', async function () {
    const store: AnalyticsStore = new StubAnalyticsStore();
