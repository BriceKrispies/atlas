...
class StubAnalyticsStore implements AnalyticsStore {
    ...
    async query(filter: AnalyticsQuery): Promise<AnalyticsEvent[]> {
        return this.events.filter(function (e) {
            ...
            if (filter.from !== undefined && e.occurredAt < filter.from)
                return false;
            if (filter.to !== undefined && e.occurredAt > filter.to)
                return false;
            return true;
...
describe('AnalyticsStore port (TS surface)', function () {
    ...
    test('record then query round-trips events filtered by tenant + eventType', async function () {
        ...
        expect(tenantAOnly.every(function (e) {
        ...
        expect(tenantBLeak.some(function (e) {
            return e.tenantId !== 'tenant-a';
        })).toBe(false);
    });
