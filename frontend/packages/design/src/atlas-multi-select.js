...
import { MultiSelectCore, LIFECYCLE } from './multi-select-core.js';
 ...
 * see multi-select-core.js and its tests.
 *
 * ── Rendering strategy ───────────────────────────────────────────────
...
class AtlasMultiSelect extends AtlasElement {
  ...
  static get observedAttributes() {
    return ['options', 'value', 'label', 'placeholder', 'disabled',
            'error', 'searchable', 'allow-create', 'close-on-select', 'max'];
  }

  // ── Property API ─────────────────────────────────────────────

  get options() { return this._core.getState().options; }
  set options(next) { this._core.setOptions(next); }

  get value() { return this._core.getState().selected; }
  set value(next) {
  ...
  get selected() { return this._core.selectedOptions(); }

  /** Current lifecycle: idle / loading / ready / empty / error. */
  get status() { return this._core.getState().status; }

  /**
   * OptionsSource port. Setting this triggers loadOptions() automatically.
   * @type {{load: (q?: string) => Promise<any>} | null}
   */
  get optionsSource() { return this._core._source; }
  set optionsSource(src) {
    this._core.setOptionsSource(src);
    if (src) this._core.loadOptions();
  }

  /** Manually trigger a reload through the port (if configured). */
  reload() { return this._core.loadOptions(this._core.getState().query); }

  open() { this._core.openListbox(); }
  close() { this._core.closeListbox(); }
  toggle() {
    this._core.getState().open ? this._core.closeListbox() : this._core.openListbox();
  }
  clear() {
  ...
  attributeChangedCallback(name, oldVal, newVal) {
    ...
    switch (name) {
      case 'options':
        if (newVal) {
          try { this._core.setOptions(JSON.parse(newVal)); } catch { /* ignore */ }
        }
        break;
      case 'value':
        if (newVal !== null) {
          try { this.value = JSON.parse(newVal); }
          catch { this.value = String(newVal).split(',').map((s) => s.trim()).filter(Boolean); }
        }
        break;
      case 'max':
  ...
  _hydrateFromAttributes() {
    ...
    if (optsAttr && this.options.length === 0) {
      try { this._core.setOptions(JSON.parse(optsAttr)); } catch { /* ignore */ }
    }
    const valAttr = this.getAttribute('value');
    if (valAttr && this.value.length === 0) {
      try { this.value = JSON.parse(valAttr); }
      catch { this.value = valAttr.split(',').map((s) => s.trim()).filter(Boolean); }
    }
  }
  ...
  _buildShell() {
    ...
    if (search) {
      search.addEventListener('input', (e) => {
        this._core.setQuery(e.target.value);
        this.dispatchEvent(new CustomEvent('search', {
          bubbles: true, composed: true, detail: { query: e.target.value },
        }));
      });
      search.addEventListener('keydown', (e) => this._onListKey(e));
    }

    const optionsEl = this.shadowRoot.querySelector('.options');
    ...
    optionsEl.addEventListener('click', (e) => this._onOptionsClick(e));
    if (!search) {
      optionsEl.addEventListener('keydown', (e) => this._onListKey(e));
    }

    // Chip-remove handler also delegated onto the chips container.
    const chipsEl = this.shadowRoot.querySelector('.chips');
    chipsEl.addEventListener('click', (e) => this._onChipsClick(e));
  ...
  _updateChips() {
    const chipsEl = this.shadowRoot.querySelector('.chips');
    const placeholderEl = this.shadowRoot.querySelector('.placeholder');
    if (!chipsEl) return;
    const selected = this._core.selectedOptions();
    chipsEl.innerHTML = selected.map((o) => `
    ...
    `).join('');
    if (placeholderEl) placeholderEl.style.display = selected.length > 0 ? 'none' : '';
  }
  ...
  _updateListbox() {
    ...
    if (visible.length === 0 && !this._core.canCreate()) {
      const msg = s.status === LIFECYCLE.EMPTY ? 'No options available' : 'No matches';
      list.innerHTML = `<li class="status-row" role="presentation" data-kind="empty">${escHtml(msg)}</li>`;
      return;
    }

    let html = visible.map((o, i) => {
  ...
  _updateError() {
    const errEl = this.shadowRoot.querySelector(`#${this._errorId}`);
    if (!errEl) return;
  ...
  _onOptionsClick(e) {
    ...
    const createHint = e.target.closest('[data-action="create"]');
    if (createHint) { this._create(); return; }
    ...
    if (delta.changed) {
      ...
      this.dispatchEvent(new CustomEvent(wasSelected ? 'unselect' : 'select', {
        bubbles: true, composed: true, detail: { option },
      }));
  ...
  _onTriggerKey(e) {
    ...
    switch (e.key) {
      ...
      case ' ':
        ...
        break;
      case 'Escape':
        if (this._core.getState().open) { e.preventDefault(); this.close(); }
        break;
      case 'Backspace':
  ...
  _onListKey(e) {
    switch (e.key) {
      case 'ArrowDown':
        e.preventDefault(); this._core.moveActive(1); break;
      case 'ArrowUp':
        e.preventDefault(); this._core.moveActive(-1); break;
      case 'Home':
        e.preventDefault(); this._core.setActive(0); break;
      case 'End':
        e.preventDefault(); this._core.setActive(Number.MAX_SAFE_INTEGER); break;
      case 'Enter': {
        e.preventDefault();
        const visible = this._core.visibleOptions();
        const idx = this._core.getState().activeIndex;
        if (idx >= 0 && visible[idx]) {
          const v = visible[idx].value;
          const wasSelected = this._core.getState().selected.includes(v);
          const delta = this._core.toggle(v);
          if (delta.changed) {
            const option = this._opt(v);
      ...
      case ' ': {
        ...
        const idx = this._core.getState().activeIndex;
        if (idx >= 0 && visible[idx]) {
          const delta = this._core.toggle(visible[idx].value);
          if (delta.changed) this._emitChange(delta);
        }
        break;
      }
  ...
  _emitChange(delta, extra = {}) {
    if (this.surfaceId && this.getAttribute('name')) {
      this.emit(`${this.surfaceId}.${this.getAttribute('name')}-changed`, {
        added: delta.added, removed: delta.removed, value: this.value,
      });
    }
