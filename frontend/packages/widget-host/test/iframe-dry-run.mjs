...
async function main() {
  const boot = await import('../src/iframe-runtime/boot.js');
  ...
  const transport = await import('../src/transport/postmessage.js');
  ...
  const iframeHostPath = fileURLToPath(
    new URL('../src/hosts/iframe-host.js', import.meta.url),
  );
  ...
  for (const needle of [
    ...
    if (!iframeHostSrc.includes(needle)) {
      fail(`iframe-host.js is missing required token: ${needle}`);
    }
  }
  ...
  if (/setAttribute\(\s*['"]sandbox['"][^)]*allow-same-origin/.test(iframeHostSrc)) {
    fail('iframe-host.js must NOT grant allow-same-origin on the sandbox attribute');
  }
