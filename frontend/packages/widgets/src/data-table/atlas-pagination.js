...
class AtlasPagination extends AtlasElement {
  ...
  get pageSizeOptions() { return this._pageSizeOptions.slice(); }
  set pageSizeOptions(opts) {
  ...
  _onSizeChange(event) {
    const next = clampInt(Number(event.target.value), 1, Number.MAX_SAFE_INTEGER);
    if (next === this._pageSize) return;
    this._pageSize = next;
