...
const FORBIDDEN_IDENTIFIERS_CALL = new Set([
  'alert',
  'confirm',
  'prompt',
  'fetch',
...
function isNavigatorSendBeacon(callee) {
    ...
    return {
      CallExpression(node) {
        ...
        if (callee.type === 'Identifier' && FORBIDDEN_IDENTIFIERS_CALL.has(callee.name)) {
          ...
          if (name === 'alert' || name === 'confirm' || name === 'prompt') {
            context.report({ node: callee, messageId: 'syncPrompt', data: { name } });
          } else if (name === 'eval') {
            context.report({ node: callee, messageId: 'eval', data: { name } });
          } else {
            context.report({ node: callee, messageId: 'directIo', data: { name } });
          }
          return;
        }

        if (isNavigatorSendBeacon(callee)) {
          context.report({ node: callee, messageId: 'beacon' });
          return;
        }
      },
      ...
      NewExpression(node) {
        ...
        if (callee.name === 'Function') {
          context.report({ node: callee, messageId: 'newFn' });
        } else {
          context.report({
            node: callee,
            messageId: 'newIo',
