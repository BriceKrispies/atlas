...
function runRule(rule) {
  const behavior = rule.behavior ?? 'fixture';
  ...
  return Promise.reject(new Error(`unknown mock behavior '${behavior}'`));
}

/**
 * @param {Record<string, Array<object>>} spec
 * @returns {Record<string, (args: unknown) => Promise<unknown>>}
 */
export function buildMockCapabilities(spec) {
  const out = {};
