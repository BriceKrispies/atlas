...
async function openEditor(page: Page, pageId: string): Promise<void> {
  ...
  await page.waitForFunction((pid: string) => {
    ...
    const cp = deepQuery(`content-page[data-page-id="${pid}"]`) as (Element & { editor?: unknown }) | null;
    return !!(cp && cp.editor);
  }, pageId);
}
 ...
 */
async function shellHandle(page: Page): Promise<JSHandle<Element | null>> {
...
// Keep the helper exported-equivalent to suppress the unused-variable
// check (it's referenced for parity with the original .js module).
void shellHandle;
...
async function listEditor(
  page: Page,
  pageId: string,
): Promise<Array<{ widgetId: string; instanceId: string; region: string; config?: Record<string, unknown> }>> {
  return runEditorOp(page, pageId, 'list') as unknown as Promise<
    Array<{ widgetId: string; instanceId: string; region: string; config?: Record<string, unknown> }>
  >;
}
...
async function shellShadowQuery(
  page: Page,
  selector: string,
): Promise<{ ok: boolean; text?: string | null; html?: string }> {
  return page.evaluate((sel: string) => {
    const stack: Array<Document | ShadowRoot | Element> = [document];
    while (stack.length) {
      const root = stack.shift()!;
      if (!('querySelector' in root) || !root.querySelector) continue;
      const el = root.querySelector('sandbox-page-editor') as (Element & { shadowRoot?: ShadowRoot }) | null;
      if (el?.shadowRoot) {
...
async function clickCell(
  ...
  const handle = await page.evaluateHandle((id: string) => {
    ...
    while (stack.length) {
      const root = stack.shift()!;
      if (!('querySelector' in root) || !root.querySelector) continue;
      const shell = root.querySelector('sandbox-page-editor') as (Element & { shadowRoot?: ShadowRoot }) | null;
      if (shell?.shadowRoot) {
...
test.describe('Page Editor — property panel', () => {
  ...
  test('editing text field commits via editor.update (debounced)', async ({ page }) => {
    ...
    const inputHandle = await page.evaluateHandle(() => {
      ...
      while (stack.length) {
        ...
        const shell = ('querySelector' in root && root.querySelector)
          ...
          : null;
        if (shell?.shadowRoot) {
          const field = shell.shadowRoot.querySelector('atlas-box[name="field-content"] atlas-input') as
    ...
    await expect.poll(async () => {
      const entry = await runEditorOp(page, 'editor-starter', 'get', 'w-editor-starter-main-text') as
        { config?: { content?: string } } | undefined;
  ...
  test('clicking an enum chip commits the new enum value', async ({ page }) => {
    ...
    await expect.poll(async () => {
      const entry = await runEditorOp(page, 'editor-starter', 'get', 'w-editor-starter-main-text') as
        { config?: { variant?: string } } | undefined;
...
test.describe('Page Editor — KPI tile placement', () => {
   ...
   */
  async function readKpiTile(page: Page, instanceId: string): Promise<KpiTileReadResult> {
      ...
      while (stack.length) {
        ...
        if (shell?.shadowRoot) {
          ...
          if (cell) {
            ...
            const tile = (wrapper?.querySelector('atlas-kpi-tile') ?? null) as HTMLElement | null;
            return {
              found: true,
              wrapper: !!wrapper,
              tile: !!tile,
