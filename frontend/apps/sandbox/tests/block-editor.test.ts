...
const action = (key: string): string => `[data-testid="${SURFACE}.action.${key}"]`;
const block = (id: string): string => `[data-testid="${SURFACE}.block.${id}"]`;
...
async function openBlock(page: Page): Promise<void> {
  ...
  await expect
    .poll(async () => ((await readEditorState(page, ID)) as { surfaceId?: string } | null)?.surfaceId)
    .toBe(SURFACE);
}
...
test.describe('atlas-block-editor committed-state contract', () => {
  test('initial snapshot exposes seeded blocks', async ({ page }) => {
    ...
    const state = (await readEditorState(page, ID)) as {
  ...
  test('clicking a block commits setSelection', async ({ page }) => {
    ...
    const state = (await readEditorState(page, ID)) as { selection: unknown };
  ...
  test('bold commits applyFormatting and records formats on the block', async ({ page }) => {
    ...
    const st = (await readEditorState(page, ID)) as {
  ...
  test('save commits save intent and clears dirty', async ({ page }) => {
    await openBlock(page);
    await page.click(action('insert-text'));
    expect(((await readEditorState(page, ID)) as { dirty: boolean }).dirty).toBe(true);
    await page.click(action('save'));
    await assertCommitted(page, SURFACE, { intent: 'save' });
    expect(((await readEditorState(page, ID)) as { dirty: boolean }).dirty).toBe(false);
  });

  test('move-up at top is a no-op (no moveBlock commit)', async ({ page }) => {
    await openBlock(page);
    await page.click(block('seed-heading'));
    const before = ((await readEditorState(page, ID)) as { lastCommit: { at: number; intent: string } }).lastCommit;
    await page.click(action('move-up'));
    await page.waitForTimeout(50);
    const after = ((await readEditorState(page, ID)) as { lastCommit: { at: number; intent: string } }).lastCommit;
