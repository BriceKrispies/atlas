...
test.describe('admin-shell navigation', function () {
    // -- States --
    test.describe('states', function () {
        test('default route loads the content module', async function ({ page }) {
            await mockApi(page, { pages: samplePages });
            await page.goto('/');
            await expect(page.getByText('Welcome Page')).toBeVisible();
        });
        test('each module route shows its heading', async function ({ page }) {
        ...
        test('content route shows the pages list surface', async function ({ page }) {
            await mockApi(page, { pages: [] });
    ...
    test.describe('flows', function () {
        test('clicking a nav item changes the hash and swaps content', async function ({ page }) {
            await mockApi(page, { pages: [] });
            await page.goto('/');
        ...
        test('clicking between nav items swaps the visible module', async function ({ page }) {
            await mockApi(page, { pages: [] });
            await page.goto('/');
        ...
        test('browser back/forward works with hash routing', async function ({ page }) {
            await mockApi(page, { pages: [] });
            await page.goto('/');
            await page.getByRole('link', { name: 'Badges' }).click();
            await expect(page.getByRole('heading', { name: 'Badges', exact: true })).toBeVisible();
        ...
        test('direct hash URL navigation shows the correct module', async function ({ page }) {
            await mockApi(page, { pages: [] });
    ...
    test.describe('navigation', function () {
        test('Content nav item is active on default route', async function ({ page }) {
            await mockApi(page, { pages: [] });
            await page.goto('/');
        ...
        test('active state follows the current route', async function ({ page }) {
            await mockApi(page, { pages: [] });
        ...
        test('all module nav items are rendered', async function ({ page }) {
            await mockApi(page, { pages: [] });
            await page.goto('/');
    ...
    test.describe('accessibility', function () {
        test('nav region has an accessible label', async function ({ page }) {
            await mockApi(page, { pages: [] });
            await page.goto('/');
            const nav = page.getByRole('navigation', { name: 'Admin navigation' });
            await expect(nav).toBeVisible();
        });
        test('nav items have link role', async function ({ page }) {
            await mockApi(page, { pages: [] });
            await page.goto('/');
            for (const label of MODULES) {
        ...
        test('active nav item has aria-current="page"', async function ({ page }) {
            await mockApi(page, { pages: [] });
            await page.goto('/#/badges');
            const badgesLink = page.getByRole('link', { name: 'Badges' });
            await expect(badgesLink).toHaveAttribute('aria-current', 'page');
        });
        test('nav items are keyboard-accessible with tabindex', async function ({ page }) {
            await mockApi(page, { pages: [] });
            await page.goto('/');
            for (const label of MODULES) {
                const item = page.getByRole('link', { name: label });
