...
async function mockPolicies(page: import('@playwright/test').Page, body: unknown): Promise<void> {
    await page.route('**/api/v1/policies', async function (route) {
...
test.describe('admin.authz.policies-list surface', function () {
    ...
    test('navigates to empty state when tenant has no policies', async function ({ page }) {
    ...
    test('renders rows when policies exist', async function ({ page }) {
        await mockPolicies(page, [
            {
                tenantId: 'tenant-001',
                version: 2,
