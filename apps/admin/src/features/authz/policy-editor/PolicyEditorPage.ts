...
import { getPolicy, createPolicy, activatePolicy, type PolicyDetail, } from '@atlas/api-client';
...
class PolicyEditorPage extends AtlasSurface {
    ...
    private _applyValidationDom(errors: readonly string[]): void {
        const saveBtn = this.querySelector('atlas-button[name="save-button"]') as (HTMLElement & {
            disabled?: boolean;
        }) | null;
        const activateBtn = this.querySelector('atlas-button[name="activate-button"]') as (HTMLElement & {
            disabled?: boolean;
        }) | null;
        if (saveBtn)
            saveBtn.toggleAttribute('disabled', this._state.saving || errors.length > 0);
        if (activateBtn)
            activateBtn.toggleAttribute('disabled', this._state.activating || this._state.status !== 'draft' || errors.length > 0);
    ...
    override onMount(): void {
        ...
        void warmupSimulator().catch(function () {
      ...
      <atlas-stack gap="lg">
        ...
        ${s.validationErrors.length > 0
            ? html `
              ...
              >
                <atlas-stack gap="xs">
                  ${s.validationErrors.map(function (m) {
                return html `<atlas-text>${m}</atlas-text>`;
            })}
                </atlas-stack>
              </atlas-alert>
            `
    ...
    private _summarise(cedarText: string): {
        [k: string]: number;
    } {
    ...
    private _renderSimulatorPane(): HTMLElement {
        ...
        if (this._state.simulatorError !== null) {
            ...
            const matched = r.matchedPolicies.length > 0 ? r.matchedPolicies.join(', ') : '(none)';
    ...
    private _readSimulatorInputs(host: HTMLElement): SimulatorRequest {
        ...
        const read = function (n: string): string {
            const el = host.querySelector<HTMLElementTagNameMap['atlas-input']>(`atlas-input[name="${n}"]`);
            return el?.value ?? '';
        };
        return {
