 ...
 * params.
 */
import { openAtlasIdb, IdbEventStore, IdbCache, IdbProjectionStore, IdbSearchEngine, InMemoryControlPlaneRegistry, IdbCatalogStateStore, IdbEntityStore, IdbRelationStore, type IdbDb, } from '@atlas/adapter-idb';
...
import { catalogHandlerRegistry, catalogDispatcher, getTaxonomyNodes, getFamilyDetail, getVariantTable, searchCatalog, type CatalogQueryDeps, type SearchParams, } from '@atlas/catalog';
import { contentPagesHandlerRegistry, contentPagesDispatcher, listPages as listContentPagesQuery, getPage as getContentPageQuery, getRenderTree as getContentPageRenderTreeQuery, type ContentPagesQueryDeps, } from '@atlas/content-pages';
import { composeRegistries } from '@atlas/authz';
import { cacheTagDispatcher, composeDispatchers, type EventDispatcher, } from '@atlas/ports';
import { IngressError, type EventEnvelope, type IntentEnvelope, type IntentResponse, type SearchDocument, } from '@atlas/platform-core';
import { RoleAwareStubPolicyEngine } from './role-aware-stub.ts';
import type { AtlasSimAction, AtlasSimDebug, AtlasSnapshot, EventReadFilter, IngressFailure, SubmitRawResult, } from './types.ts';
...
async function spawnSimWorker(tenantId: string, moduleId: string): Promise<SimWorkerHandle> {
    ...
    const pendingSettles = new Map<string, (msg: WorkerSettledMsg) => void>();
    let shutdownResolve: (() => void) | null = null;
    worker.addEventListener('message', function (e: MessageEvent<WorkerInbound>) {
        const msg = e.data;
        if (msg.type === 'settled') {
    ...
    await new Promise<void>(function (resolve, reject) {
        ...
        const onReady = function (e: MessageEvent<WorkerInbound>): void {
    ...
    return {
        settle(): Promise<{
            ...
            return new Promise(function (resolve) {
                pendingSettles.set(requestId, function (msg) {
...
const noopDispatch: EventDispatcher = async function (_event) {
    // intentionally empty — Web Worker owns the chain in BDD mode
};
async function buildContext(tenantId: string, principalId: string): Promise<BootedContext> {
    ...
    const handlers = composeRegistries(catalogHandlerRegistry(), contentPagesHandlerRegistry(entities));
    ...
    const dispatch = IS_BDD
        ? noopDispatch
        : composeDispatchers(catalogDispatcher({ catalogState, projections, search, cache }), contentPagesDispatcher({
            entities,
            relations,
            cache,
            wasmHost,
...
function failureFromIngressError(e: IngressError): IngressFailure {
    ...
    if (e.correlationId)
        failure.correlationId = e.correlationId;
    return failure;
}
...
function rowToEnvelope(row: {
    seq: number;
} & Omit<EventEnvelope, 'seq'>): EventEnvelope {
...
function matchesFilter(env: {
    ...
    tenantId?: string;
}, filter: EventReadFilter): boolean {
    if (filter.type !== undefined && env.eventType !== filter.type)
        return false;
    if (filter.correlationId !== undefined && env.correlationId !== filter.correlationId)
        return false;
    if (filter.idempotencyKey !== undefined && env.idempotencyKey !== filter.idempotencyKey)
        return false;
    if (filter.tenantId !== undefined && env.tenantId !== filter.tenantId)
        return false;
    return true;
}
function buildDebugSurface(ctx: BootedContext, reboot: () => Promise<void>): AtlasSimDebug {
    ...
    return {
        async snapshot(): Promise<AtlasSnapshot> {
            const [events, projectionsAll, cacheAll, searchAll, catalogStateAll, entitiesAll, relationsAll,] = await Promise.all([
                db.getAll('events'),
...
async function boot(): Promise<void> {
    ...
    const principalId = urlPrincipal ?? `user:test-user:${tenantId}`;
    ...
    if (currentCtx) {
        if (currentCtx.worker) {
            await currentCtx.worker.shutdown().catch(function () {
                return undefined;
            });
        }
        currentCtx.db.close();
    ...
    if (IS_BDD) {
        window.__atlas_debug = buildDebugSurface(ctx, async function () {
            ...
            if (ctx.worker) {
                await ctx.worker.shutdown().catch(function () {
                    return undefined;
                });
                ctx.worker = null;
            }
