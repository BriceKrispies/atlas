 ...
 */
import { openAtlasIdb, IdbCache, IdbCatalogStateStore, IdbEntityStore, IdbProjectionStore, IdbRelationStore, IdbSearchEngine, IdbWorkerSource, type IdbDb, } from '@atlas/adapter-idb';
...
interface WorkerGlobalSurface {
    ...
    addEventListener(type: 'message', listener: (e: MessageEvent<InboundMessage>) => void): void;
    close(): void;
}
...
ctx.addEventListener('message', function (e: MessageEvent<InboundMessage>) {
    const msg = e.data;
    switch (msg.type) {
        case 'init':
...
async function handleInit(msg: InitMessage): Promise<void> {
    if (runtime) {
        ...
        throw new Error(`[atlas-sim-worker] received second init for tenant=${msg.tenantId}`);
    }
    ...
    // doc-comment.
    const dispatch = composeDispatchers(catalogDispatcher({ catalogState, projections, search, cache }), contentPagesDispatcher({
        entities,
        relations,
        cache,
...
async function readCursor(db: IdbDb, tenantId: string, moduleId: string): Promise<bigint> {
...
async function readHeadSeq(db: IdbDb, tenantId: string): Promise<bigint> {
    const range = IDBKeyRange.bound([tenantId, -Infinity], [tenantId, +Infinity]);
    const idx = db
