...
function toLayoutDocuments(raw: ReadonlyArray<unknown>): LayoutDocument[] {
  ...
  for (const entry of raw) {
    ...
    if (!result.ok) {
      throw new Error(
        `authoring stores: invalid bundled layout — ${result.errors.map((e) => `${e.path}: ${e.message}`).join('; ')}`,
      );
    }
...
function toPageDocuments(raw: ReadonlyArray<unknown>): PageDocument[] {
  ...
  for (const entry of raw) {
    ...
    if (!result.ok) {
      throw new Error(
        `authoring stores: invalid bundled page document — ${result.errors.map((e) => `${e.path}: ${e.message}`).join('; ')}`,
      );
    }
    // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- boundary: validator just passed; bundle-imported JSON
...
export const authoringLayoutStore = new ValidatingLayoutStore(
  new InMemoryLayoutStore(
    Object.fromEntries(toLayoutDocuments(presetLayouts).map((l) => [l.layoutId, l])),
  ),
);
...
export const galleryPickerEntries: ReadonlyArray<GalleryPagePickerEntry> =
  toPageDocuments(gallerySeedPages).map((doc) => {
    const entry: GalleryPagePickerEntry = { pageId: doc.pageId };
    if (typeof doc.templateId === 'string') entry.templateId = doc.templateId;
...
export function mountContentPage(
  ...
  page.edit = edit;
  page.onMediatorTrace = (evt: object) => onLog('mediator', evt);
  page.onCapabilityTrace = (evt: object) => onLog('capability', evt);
