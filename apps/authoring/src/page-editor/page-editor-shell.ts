 ...
 */
function resolveTemplateRegistry(injected: TemplateRegistry | null): TemplateRegistry {
    if (injected)
        return injected;
    return {
        has: function (id) {
            return moduleDefaultTemplateRegistry.has(id);
        },
        get: function (id) {
            return moduleDefaultTemplateRegistry.get(id);
        },
        list: function () {
            return moduleDefaultTemplateRegistry.list().map(function (row) {
                return ({
...
import { loadPanelSizes, savePanelSize, type PageEditorLeftPanelElement, type PageEditorRightPanelElement, type PageEditorBottomPanelElement, type PanelResizeEventDetail, type PanelTabEventDetail, type PanelToggleEventDetail, } from './panels/index.ts';
...
import type { EditorAPI, PageDocument, PageStore, WidgetInstance, } from '@atlas/page-templates';
import type { WrappedPageStore } from './history.ts';
import type { PageEditorPropertyPanel } from './property-panel.ts';
import { PageEditorController, PANEL_SIZE_BOUNDS, type EditorMode, type PageEditorStateSnapshot, type PanelId, type PanelsState, } from './state.ts';
...
export class AuthoringPageEditorShellElement extends AtlasSurface {
    ...
    private async _init(): Promise<void> {
        ...
        this._disposeTestState = registerTestState(controller.surfaceId, function () {
    ...
    `;
        const segmented = root.querySelector('atlas-segmented-control[name="mode"]') as (HTMLElement & {
            options: unknown;
            value: string | null;
        }) | null;
        if (segmented) {
            segmented.options = MODE_OPTIONS.map(function (m) {
                return ({ value: m.testValue, label: m.label });
            });
            segmented.value = 'mode-content';
            segmented.addEventListener('change', (ev) => {
                const value = asCustomEvent<{
                    value: string;
                }>(ev)?.detail?.value;
                const mode = MODE_OPTIONS.find(function (m) {
                    return m.testValue === value;
                })?.value;
                if (mode)
    ...
    private _buildTabSlots(): void {
        this._tabSlots.clear();
        const wireSlots = (panel: HTMLElement | null, panelId: PanelId, tabs: ReadonlyArray<string>): void => {
            if (!panel)
                return;
    ...
    private async _mountCanvas(): Promise<void> {
        if (!this._canvasStage || !this._controller)
            return;
    ...
    private _mountPreview(): void {
        ...
        if (this.templateRegistry)
            preview.templateRegistry = this.templateRegistry;
        if (this.layoutRegistry)
            preview.layoutRegistry = this.layoutRegistry;
    ...
    private _onSnapshot(snap: PageEditorStateSnapshot): void {
        ...
        if (!prev || prev.mode !== snap.mode) {
            ...
            const seg = root.querySelector('atlas-segmented-control[name="mode"]') as (HTMLElement & {
                value: string | null;
            }) | null;
            if (seg)
                seg.value = `mode-${snap.mode}`;
            ...
            if (exit)
                exit.style.display = snap.mode === 'preview' ? '' : 'none';
            if (previewBtn)
                previewBtn.style.display = snap.mode === 'preview' ? 'none' : '';
        ...
        if (!prev ||
            ...
            if (undoBtn)
                toggleDisabled(undoBtn, !snap.history.canUndo);
            if (redoBtn)
                toggleDisabled(redoBtn, !snap.history.canRedo);
        }
        if (!prev ||
    ...
    private _reflectPanelEdgeButtons(snap: PageEditorStateSnapshot): void {
        ...
        if (!root)
            return;
        const setVisible = function (edge: 'left' | 'right' | 'bottom', visible: boolean): void {
            const box = root.querySelector(`atlas-box[data-role="canvas-edge"][data-edge="${edge}"]`) as HTMLElement | null;
            if (!box)
                return;
    ...
    private async _reflectModeOnCanvas(mode: EditorMode): Promise<void> {
        ...
        if (this._canvasPage.edit === wantsEdit)
            return;
    ...
    private _reflectMultiSelect(ids: ReadonlyArray<string>): void {
        ...
        if (ids.length > 1) {
            for (const id of ids) {
                const cell = this._canvasHost.querySelector(`[data-widget-cell][data-instance-id="${CSS.escape(id)}"]`);
     ...
     */
    private _renderTabContent(snap: PageEditorStateSnapshot, prev: PageEditorStateSnapshot | null): void {
    ...
    private _buildTemplatesContent(snap: PageEditorStateSnapshot): HTMLElement {
        ...
        select.options = list.map(function (t) {
            return ({ value: t.templateId, label: t.displayName ?? t.templateId });
        });
        select.value = snap.layoutTemplateId;
        select.addEventListener('change', (ev) => {
            const next = asCustomEvent<{
                value: string;
            }>(ev)?.detail?.value ?? select.value;
    ...
    private async _switchTemplate(nextTemplateId: string): Promise<void> {
        if (!this._controller || !this.pageStore)
            return;
        const currentDoc = this._controller.getSnapshot().pageDocument;
        if (!currentDoc || currentDoc.templateId === nextTemplateId)
            return;
        const registry = resolveTemplateRegistry(this.templateRegistry);
        ...
        const nextRegionNames = new Set(nextManifest.regions.map(function (r) {
            return r.name;
        }));
        const dropped: Array<{
            instanceId: string;
        ...
        for (const [regionName, entries] of Object.entries(currentDoc.regions ?? {})) {
            if (nextRegionNames.has(regionName))
                nextRegions[regionName] = entries as WidgetInstance[];
        }
    ...
    private _handleKeyDown(e: KeyboardEvent): void {
        ...
        if (!path.includes(this))
            return;
        const inField = path.some(function (el) {
    ...
    return new Promise(function (resolve) {
        ...
        dialog.addEventListener('close', function (ev) {
            if (settled)
                return;
            settled = true;
            const returnValue = asCustomEvent<{
                returnValue: string;
            }>(ev)?.detail?.returnValue;
            dialog.remove();
...
function panelsChanged(a: PanelsState, b: PanelsState): boolean {
    return (a.left.open !== b.left.open ||
...
function leftPanelTabsForMode(mode: EditorMode): Array<{
    id: string;
    label: string;
}> {
    if (mode === 'structure')
        return [{ id: 'templates', label: 'Templates' }];
    return [
        { id: 'palette', label: 'Palette' },
        { id: 'outline', label: 'Outline' },
    ];
