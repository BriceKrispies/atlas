...
export function wrapStoreWithHistory(inner: PageStore, history: HistoryStack): WrappedPageStore {
    ...
    return {
        get: function (pageId: string) {
            return inner.get(pageId);
        },
        save: async function (pageId: string, doc: PageDocument): Promise<void> {
        ...
        delete: function (pageId: string) {
            return inner.delete(pageId);
        },
        subscribe: function (pageId: string, cb: PageSubscriber): (() => void) {
