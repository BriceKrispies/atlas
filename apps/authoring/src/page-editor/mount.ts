...
export type MountPageEditorFn = (demoEl: HTMLElement, ctx: {
    config: {
        pageId?: string;
...
export function createMountPageEditor(ctx: MountPageEditorCtx): MountPageEditorFn {
    const { pageStore, layoutRegistry, templateRegistry, tenantId, capabilities, principal, } = ctx;
    return function mountPageEditor(demoEl, { config, onLog }): () => void {
        const { pageId } = (config ?? {}) as {
            pageId?: string;
        };
        const shellEl = document.createElement('authoring-page-editor-shell');
