...
function buildGroups(manifests: ReadonlyArray<ManifestLite>, filterFn: (id: string) => boolean): GroupDescriptor[] {
    const buckets = new Map<string, string[]>();
    for (const m of manifests) {
        if (!filterFn(m.widgetId))
            continue;
    ...
    return [...buckets.entries()]
        .map(function ([id, ids]) {
        return ({ id, label: id, widgetIds: ids.slice().sort() });
    })
        .sort(function (a, b) {
...
export class PageEditorPaletteElement extends AtlasSurface {
    ...
    private _reconcileSelectedRegion(): void {
        ...
        const names = regions.map(function (r) {
            return r.name;
        });
        if (names.length === 0) {
    ...
    private _observeAddCommit(snap: PageEditorStateSnapshot): void {
        ...
        if (!c)
            return;
        if (c.at <= this._seenAddCommitAt)
            return;
        this._seenAddCommitAt = c.at;
        if (c.intent !== 'addWidget')
            return;
        ...
        if (patch === null || typeof patch !== 'object')
            return;
        const widgetIdRaw: unknown = (patch as {
            widgetId?: unknown;
        }).widgetId;
        if (typeof widgetIdRaw !== 'string' || widgetIdRaw.length === 0)
            return;
        const next = [widgetIdRaw, ...this._recents.filter(function (w) {
                return w !== widgetIdRaw;
            })];
    ...
    private _filterFn(): (id: string) => boolean {
        ...
        return function (id: string): boolean {
            return id.toLowerCase().includes(s);
        };
    }
    ...
    private _render(): void {
        ...
        const searchInput = document.createElement('atlas-input') as HTMLElement & {
            value: string;
        };
        ...
        if (regions.length > 0) {
            ...
            regionSelect.options = regions.map(function (r) {
    ...
    private _renderRecents(filter: (id: string) => boolean): HTMLElement | null {
        const visible = this._recents.filter(function (id) {
            return filter(id);
        });
        if (visible.length === 0)
            return null;
