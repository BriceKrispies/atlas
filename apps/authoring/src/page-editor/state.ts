...
export const PANEL_SIZE_BOUNDS: Record<PanelId, {
    min: number;
    max: number;
    default: number;
}> = {
 ...
 */
export type PageEditorIntent = 'setMode' | 'selectWidget' | 'openPalette' | 'openSettings' | 'closeDrawer' | 'addWidget' | 'removeWidget' | 'removeSelected' | 'moveWidget' | 'updateWidgetConfig' | 'setLayoutTemplate' | 'undo' | 'redo' | 'save' | 'panelToggle' | 'panelResize' | 'panelTab' | 'deviceChange';
...
function deriveRegions(doc: PageDocument | null): Region[] {
    ...
    for (const [name, entries] of Object.entries(doc.regions)) {
        out.push({
            name,
            widgetIds: (entries as WidgetInstance[]).map(function (e) {
                return e.instanceId;
            }),
        });
    }
...
function deriveInstances(doc: PageDocument | null): WidgetInstance[] {
    ...
    for (const entries of Object.values(doc.regions)) {
        for (const entry of entries as WidgetInstance[])
            out.push(entry);
    }
    return out;
}
 ...
 */
function derivePanelsForMode(mode: EditorMode, prev: PanelsState | null, selection: ReadonlySet<string>): PanelsState {
    const sizes = {
...
function clampSize(panel: PanelId, size: number): number {
    ...
    if (!Number.isFinite(size))
        return PANEL_SIZE_BOUNDS[panel].default;
    return Math.max(min, Math.min(max, Math.round(size)));
}
const VALID_TABS: {
    left: ReadonlySet<string>;
    right: ReadonlySet<string>;
    bottom: ReadonlySet<string>;
} = {
...
export class PageEditorController {
    ...
    selectWidget(instanceId: string | null, opts?: {
        additive?: boolean;
    }): void {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (args.index !== undefined)
                    patch['index'] = args.index;
                if (res.instanceId !== undefined)
                    patch['instanceId'] = res.instanceId;
    ...
    async removeWidget(instanceId: string): Promise<{
        ok: boolean;
        reason?: string;
    }> {
        if (!this._editor)
    ...
    async removeSelected(): Promise<{
        removed: number;
        attempted: number;
    }> {
        const ids = [...this._selection];
        if (ids.length === 0 || !this._editor)
    ...
    async updateWidgetConfig(instanceId: string, config: Record<string, unknown>): Promise<{
        ok: boolean;
        reason?: string;
    }> {
        if (!this._editor)
     ...
     */
    async setLayoutTemplate(nextDoc: PageDocument): Promise<{
        ok: boolean;
        reason?: string;
    }> {
    ...
    async redo(): Promise<boolean> {
        const frame = await this._history.redo((doc) => this._wrapped.save(this.pageId, doc));
        if (frame) {
    ...
    private _emit(): void {
        ...
        for (const fn of this._listeners) {
            try {
                ...
                const codeRaw: unknown = err !== null && typeof err === 'object'
                    ? (err as {
                        code?: unknown;
                    }).code
                    : undefined;
