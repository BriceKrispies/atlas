...
export class AuthoringPageEditorRoute extends AtlasSurface {
    ...
    private _render(): void {
        const options = editorSeedPages.map(function (doc) {
            return ({
                value: doc.pageId,
        ...
        if (options.length > 0 && !options.some((o) => o.value === this._activePageId)) {
            ...
            if (first)
                this._activePageId = first.value;
        }
    ...
    `;
        const select = this._root.querySelector('atlas-select[name="page-select"]') as (HTMLElement & {
            options: unknown;
            value: string;
        }) | null;
        if (select) {
