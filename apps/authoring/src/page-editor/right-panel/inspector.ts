...
import type { WidgetConfigSchema, WidgetConfigSchemaPreset, } from '../widget-config.ts';
/**
...
export class PageEditorInspector extends AtlasSurface {
    ...
    private _onSnapshot(snap: PageEditorStateSnapshot): void {
        ...
        if (ids.length === 1) {
            ...
            const inst = snap.widgetInstances.find(function (w) {
                return w.instanceId === id;
            });
            if (!inst) {
                this._renderEmpty();
                return;
    ...
    private _renderSingle(instanceId: string, widgetId: string): void {
        ...
        const panel = document.createElement('page-editor-property-panel') as PageEditorPropertyPanel & HTMLElement;
        ...
        queueMicrotask(() => {
            if (!this._propertyPanel || this._propertyPanel !== panel)
                return;
    ...
    private _renderMulti(instanceIds: string[], snap: PageEditorStateSnapshot): void {
        ...
        const widgetIds = instanceIds
            .map(function (id) {
            return snap.widgetInstances.find(function (w) {
                return w.instanceId === id;
            })?.widgetId;
        })
            .filter(function (wid): wid is string {
            return typeof wid === 'string';
        });
        ...
        panel.onChange = (cfg) => {
            ...
            this._recordCommit('multiSelectEdit', {
                ...
                fieldsChanged: sharedKeys.filter(function (k) {
                    return k in cfg;
                }),
            });
        };
    ...
    // ---- intents ----
    private _applyPreset(preset: PresetDescriptor, widgetId: string, instanceId: string): void {
    ...
    private _pasteConfig(widgetId: string, instanceId: string): void {
        ...
        if (!clip || clip.widgetId !== widgetId)
            return;
    ...
    private _installTestStateReader(): void {
        ...
        this._disposeReader = registerTestState(this._surfaceKey, () => {
            const snap = this._lastSnapshot ?? this._controller?.getSnapshot() ?? null;
            const ids = snap?.selectedWidgetInstanceIds ?? [];
            const mode: InspectorMode = ids.length === 0 ? 'empty' : ids.length === 1 ? 'single' : 'multi';
            const instanceId = ids.length === 1 ? (ids[0] ?? null) : null;
            const widgetId = instanceId && snap
                ? snap.widgetInstances.find(function (w) {
                    return w.instanceId === instanceId;
                })?.widgetId ?? null
                : null;
...
function computeSharedKeys(widgetIds: string[]): string[] {
    ...
    if (schemas.length === 1)
        return firstKeys;
    const others = schemas.slice(1).map(function (s) {
        return new Set(Object.keys(s.properties ?? {}));
    });
    return firstKeys.filter(function (k) {
        return others.every(function (set) {
            return set.has(k);
        });
    });
}
