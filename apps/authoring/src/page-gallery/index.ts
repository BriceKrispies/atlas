...
export class AuthoringPageGalleryRoute extends AtlasSurface {
    ...
    private _render(): void {
        const options = galleryPickerEntries.map(function (entry) {
            return ({
                value: entry.pageId,
                label: entry.title?.replace(/^Gallery\s*—\s*/i, '') ??
                    entry.templateId ??
                    entry.pageId,
            });
        });
 ...
 */
function isOptionsHost(el: HTMLElement | null): el is HTMLElement & {
    options: unknown;
    value: string;
} {
    if (!el)
