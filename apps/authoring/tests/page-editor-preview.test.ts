...
async function mountStandalonePreview(
  ...
  await page.evaluate(async (pid: string) => {
    ...
    const el: PreviewElement = Object.assign(
      document.createElement('page-editor-preview'),
      { pageId: '', controller: undefined as unknown, templateRegistry: undefined as unknown },
    );
    el.pageId = pid;
  ...
  await page.waitForFunction(
    (pid: string) => {
      const api = (window as { __atlasTest?: { keys(): string[] } }).__atlasTest;
      if (!api) return false;
...
async function clickPreviewShadow(page: Page, selector: string): Promise<void> {
  const handle = await page.evaluateHandle((sel: string) => {
    function getPreviewShadow(): ShadowRoot | null {
      ...
      if (!host) return null;
      return (host as Element & { shadowRoot: ShadowRoot | null }).shadowRoot;
    }
...
test.describe('authoring.page-editor.preview', () => {
  ...
  test.skip('the inner content-page mounts with edit falsy', async ({ page }) => {
    ...
    const editFlag = await page.evaluate(() => {
      ...
      const editVal = (cp as Element & { edit?: boolean }).edit;
