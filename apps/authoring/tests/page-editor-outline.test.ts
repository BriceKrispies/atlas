...
async function mountOutline(page: Page): Promise<void> {
    await page.evaluate(async function () {
        ...
        document.querySelectorAll('page-editor-outline[data-test-mount]').forEach(function (n) {
            return n.remove();
        });
...
async function clickOutlineRow(page: Page, instanceId: string, modifier?: 'Shift'): Promise<void> {
    const row = page.locator(`page-editor-outline[data-test-mount] atlas-box[data-row="widget"][data-instance-id="${instanceId}"]`);
    await row.first().click({ modifiers: modifier ? [modifier] : [] });
}
test.describe('authoring.page-editor.shell.left-panel.outline — states', function () {
    test('renders an empty hint on a blank page', async function ({ page }) {
        await openEditor(page, 'editor-blank');
        await mountOutline(page);
        const empty = page.locator('page-editor-outline[data-test-mount] [data-testid="authoring.page-editor.shell.left-panel.outline.outline-empty"]');
        await expect(empty).toBeVisible();
        const widgets = page.locator('page-editor-outline[data-test-mount] atlas-box[data-row="widget"]');
        await expect(widgets).toHaveCount(0);
    });
    test('renders region rows + widget rows for a populated page', async function ({ page }) {
        await openEditor(page, 'editor-starter');
        await mountOutline(page);
        const regionRows = page.locator('page-editor-outline[data-test-mount] atlas-box[data-row="region"]');
        const widgetRows = page.locator('page-editor-outline[data-test-mount] atlas-box[data-row="widget"]');
        await expect(regionRows.first()).toBeVisible();
        await expect.poll(async function () {
            return widgetRows.count();
        }).toBeGreaterThan(0);
        // Auto-test-id for the region toggle
        const toggle = page.locator('page-editor-outline[data-test-mount] atlas-button[name="outline-region-toggle"]').first();
        await expect(toggle).toHaveAttribute('data-testid', 'authoring.page-editor.shell.left-panel.outline.outline-region-toggle');
    });
});
test.describe('authoring.page-editor.shell.left-panel.outline — selection', function () {
    test('clicking a widget row commits selectWidget on the shell', async function ({ page }) {
        await openEditor(page, 'editor-starter');
        await mountOutline(page);
    ...
    test('shift-click adds to selection (additive: true)', async function ({ page }) {
        await openEditor(page, 'editor-starter');
        await mountOutline(page);
        await clickOutlineRow(page, 'w-editor-starter-main-heading');
...
test.describe('authoring.page-editor.shell.left-panel.outline — local state', function () {
    test('region toggle commits toggleRegion on the outline surface', async function ({ page }) {
        await openEditor(page, 'editor-starter');
        await mountOutline(page);
        const toggle = page.locator('page-editor-outline[data-test-mount] atlas-button[name="outline-region-toggle"][data-region="main"]');
        await toggle.first().click();
        await assertCommitted(page, 'editor:editor-starter:outline', { intent: 'toggleRegion', patch: { region: 'main', expanded: false } });
        const snap = await readOutlineState(page, 'editor-starter');
        expect(snap?.collapsedRegions).toContain('main');
    });
});
test.describe('authoring.page-editor.shell.left-panel.outline — drag-reorder', function () {
    ...
    test('commitMove records a dragEnd commit with the dropped flag', async function ({ page }) {
        await openEditor(page, 'editor-starter');
        await mountOutline(page);
        const before = await readShellSnapshot(page);
        const mainBefore = before?.regions.find(function (r) {
            return r.name === 'main';
        })?.widgetIds ?? [];
        expect(mainBefore.length).toBeGreaterThanOrEqual(2);
        const [firstId] = mainBefore;
        if (!firstId)
            throw new Error('no widget in main region');
        await page.evaluate(async function (args: {
            instanceId: string;
        }) {
            const outline = document.querySelector('page-editor-outline[data-test-mount]') as (HTMLElement & {
                commitMove: (a: {
                    instanceId: string;
                    toRegion: string;
                    toIndex: number;
