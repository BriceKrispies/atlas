...
test.describe('authoring.shell — states', function () {
    test('shell renders with auto-generated surface test id', async function ({ page }) {
        await page.goto('/');
        await expect(page.locator(SHELL)).toBeVisible();
    });
    test('default route mounts the page-editor route element', async function ({ page }) {
        await page.goto('/');
        await expect(page.locator('[data-testid="authoring.page-editor"]')).toBeVisible();
    });
});
test.describe('authoring.shell — navigation', function () {
    for (const route of ROUTES) {
        test(`#/${route.id} mounts ${route.surface}`, async function ({ page }) {
            await page.goto(`/#/${route.id}`);
            await expect(page.locator(`[data-testid="${route.surface}"]`)).toBeVisible();
        });
    }
    test('clicking a sidebar nav-item swaps the active route', async function ({ page }) {
        await page.goto('/');
        await expect(page.locator('[data-testid="authoring.page-editor"]')).toBeVisible();
    ...
    test('hash navigation updates the selected nav item', async function ({ page }) {
