    ...
    return page.evaluate(function () {
        ...
        while (stack.length) {
            ...
            if (!root || !('querySelector' in root) || !root.querySelector)
                continue;
            const el = root.querySelector('atlas-select[name="page-select"]') as (HTMLElement & {
                options?: Array<{
                    value: string;
                    label: string;
                }>;
...
async function waitForPage(page: Page, pageId: string): Promise<void> {
    await page.waitForFunction(function (pid: string) {
        ...
        while (stack.length) {
            ...
            if (!root || !('querySelector' in root) || !root.querySelector)
                continue;
            if (root.querySelector(`content-page[data-page-id="${pid}"]`))
                return true;
            const all = root.querySelectorAll('*');
            for (const e of all) {
                if (e.shadowRoot)
...
test.describe('authoring.page-gallery — states', function () {
    ...
    test('picker offers multiple gallery seeds', async function ({ page }) {
        await page.goto(`/${ROUTE}`);
        await page.locator(PICKER).waitFor();
        const opts = await pickerOptions(page);
        expect(opts.length).toBeGreaterThan(1);
    });
    test('content-page mounts on default seed', async function ({ page }) {
        await page.goto(`/${ROUTE}`);
        await page.locator(PICKER).waitFor();
        const opts = await pickerOptions(page);
...
test.describe('authoring.page-gallery — flows', function () {
    test('switching seeds remounts the content page', async function ({ page }) {
        await page.goto(`/${ROUTE}`);
        await page.locator(PICKER).waitFor();
        const opts = await pickerOptions(page);
        expect(opts.length).toBeGreaterThan(1);
