...
describe('wrapShadow — projection writes are not applied to live store', () => {
  it('records set without mutating live', async () => {
  ...
  it('records delete without mutating live', async () => {
    const projections = new MemProjections();
    const cache = new MemCache();
  ...
  it('reads pass through to live', async () => {
    const projections = new MemProjections();
    const cache = new MemCache();
...
describe('wrapShadow — projection report', () => {
  it('reports no divergence when shadow set matches live', async () => {
    const projections = new MemProjections();
    const cache = new MemCache();
  ...
  it('flags divergence when shadow set disagrees with live', async () => {
    const projections = new MemProjections();
    const cache = new MemCache();
    projections.store.set('k', { v: 1 });
    const w = wrapShadow({ projections, cache });
  ...
  it('flags divergence when shadow set has no live counterpart', async () => {
    const projections = new MemProjections();
    const cache = new MemCache();
    const w = wrapShadow({ projections, cache });
  ...
  it('flags divergence when shadow delete and live still has the key', async () => {
    const projections = new MemProjections();
    const cache = new MemCache();
    projections.store.set('k', { v: 1 });
    const w = wrapShadow({ projections, cache });
  ...
  it('reports no divergence when shadow delete matches absent live', async () => {
    const projections = new MemProjections();
    const cache = new MemCache();
    const w = wrapShadow({ projections, cache });
  ...
  it('clears the recorder after each report call', async () => {
    const projections = new MemProjections();
    const cache = new MemCache();
    const w = wrapShadow({ projections, cache });
...
describe('wrapShadow — cache wrapping', () => {
  it('records cache set without mutating live cache', async () => {
    const projections = new MemProjections();
    const cache = new MemCache();
    const w = wrapShadow({ projections, cache });
  ...
  it('flags cache divergence when shadow set disagrees with live', async () => {
    const projections = new MemProjections();
    const cache = new MemCache();
  ...
  it('flags cache invalidateByKey divergence when live still has key', async () => {
    const projections = new MemProjections();
    const cache = new MemCache();
  ...
  it('cache get passes through to live', async () => {
    const projections = new MemProjections();
    const cache = new MemCache();
    cache.store.set('ck', { hot: true });
    const w = wrapShadow({ projections, cache });
  ...
  it('invalidateByTags is recorded but never produces a divergence', async () => {
    const projections = new MemProjections();
    const cache = new MemCache();
    const w = wrapShadow({ projections, cache });
