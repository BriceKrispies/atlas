...
class MemEntityStore implements EntityStore {
  ...
  async delete(t: string, ty: string, id: string): Promise<void> {
    ...
    if (e) this.rows.set(k, { ...e, status: 'deleted' });
  }
  async list<TAttrs = unknown>(
    ...
    return Array.from(this.rows.values())
      ...
      .filter((r) => (desired === null ? true : r.status === desired)) as Entity<TAttrs>[];
  }
...
class MemRelationStore implements RelationStore {
  ...
  async add<TAttrs = unknown>(
    ...
    this.rows.set(
      this.k(input.tenantId, input.edgeType, input.fromId, input.toId),
      row as Relation<unknown>,
    );
    return row;
  }
...
function buildAdapters(tenantId: string, sub: FakeSubscription): PerTenantAdapters {
  return {
    ...
    revisions: noopThrowingProxy<RepositoryRevisionStore>(
      'RepositoryRevisionStore',
    ),
  };
}
...
function buildState(opts: FakeStateOptions): WorkerAppState {
  ...
  const sqlFn = ((..._args: unknown[]) => {
    ...
    return Promise.resolve(
      opts.tenants.map((t) => ({ tenant_id: t, last_seq: 0 })),
    );
  ...
  return {
    ...
    async adaptersForTenant(tenantId: string): Promise<PerTenantAdapters> {
      ...
      if (!sub) throw new Error(`no fake subscription for ${tenantId}`);
      return buildAdapters(tenantId, sub);
    },
  };
}

function newTestPipeline(): { pipeline: LogPipeline; sink: CollectorSink } {
...
describe('runTenantLoop — happy path', () => {
  ...
  it('acks each event in seq order after dispatch settles', async () => {
    ...
    await waitFor(() => sub.acks.length === 3, 1000);

    expect(sub.acks).toEqual([1n, 2n, 3n]);
  });

  it('does NOT ack until dispatch resolves (ordering invariant)', async () => {
    const sub = new FakeSubscription();
    const subs = new Map([['t1', sub]]);
    const state = buildState({ tenants: ['t1'], subs });
    const { pipeline } = newTestPipeline();
    ...
    await waitFor(() => sub.acks.length === 1, 1000);
...
describe('runTenantLoop — retry and dead-letter', () => {
  let stop: (() => Promise<void>) | undefined;
  ...
  it('dead-letters after MAX_RETRIES_BEFORE_DEAD_LETTER, then advances cursor', async () => {
    ...
    state.adaptersForTenant = async (tenantId: string) => {
    ...
    await waitForReal(() => sub.acks.length === 1, 1000);
    ...
    const failedCount = events.filter(
      (e) => e === 'ProjectionWorker.Dispatch.Failed',
    ).length;
    expect(failedCount).toBeGreaterThanOrEqual(5);
  });
});

describe('runTenantLoop — discovery resilience', () => {
  let stop: (() => Promise<void>) | undefined;
  ...
  it('initial discovery failure throws, healthy tenants stay isolated thereafter', async () => {
