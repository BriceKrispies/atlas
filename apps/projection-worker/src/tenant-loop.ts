...
import { cacheTagDispatcher, composeDispatchers, type EventDispatcher, type WorkerSubscription, } from '@atlas/ports';
 ...
 */
export async function runTenantLoop(baseCtx: AtlasExecutionContext, logPipeline: LogPipeline, state: WorkerAppState): Promise<() => Promise<void>> {
    if (!state.config) {
    ...
    await discoverAndStart(baseCtx, logPipeline, state, handles, function () {
        return stopped;
    });
    ...
    const timer = setInterval(function () {
        if (stopped)
            return;
        void discoverAndStart(baseCtx, logPipeline, state, handles, function () {
            return stopped;
        }).catch(function (err) {
    ...
    return async function stop(): Promise<void> {
        ...
        // exits its for-await.
        await Promise.allSettled(snapshot.map(async function (h) {
            try {
        ...
        await Promise.race([
            Promise.allSettled(snapshot.map(function (h) {
                return h.loop;
            })),
 ...
 */
async function discoverAndStart(baseCtx: AtlasExecutionContext, logPipeline: LogPipeline, state: WorkerAppState, handles: Map<string, TenantHandle>, isStopped: () => boolean): Promise<void> {
    if (isStopped())
        return;
    let rows: Array<{
        tenant_id: string;
    ...
    try {
        rows = await state.controlPlaneSql<Array<{
            tenant_id: string;
        }>> `
    ...
    for (const row of rows) {
        ...
        if (handles.has(tenantId))
            continue;
        try {
            const handle = await startTenantSubscription(baseCtx, logPipeline, state, tenantId, function () {
 ...
 */
async function startTenantSubscription(baseCtx: AtlasExecutionContext, logPipeline: LogPipeline, state: WorkerAppState, tenantId: string, onExit: () => void): Promise<TenantHandle> {
 ...
 */
async function readCursor(state: WorkerAppState, tenantId: string): Promise<bigint> {
 ...
 */
function buildEventContext(baseCtx: AtlasExecutionContext, logPipeline: LogPipeline, tenantId: string, envelope: EventEnvelope): AtlasExecutionContext {
 ...
 */
async function consumeTenantEvents(baseCtx: AtlasExecutionContext, logPipeline: LogPipeline, adapters: PerTenantAdapters, subscription: WorkerSubscription): Promise<void> {
 ...
 */
function buildDispatcherChain(adapters: PerTenantAdapters): EventDispatcher {
    ...
    return composeDispatchers(catalogDispatcher({
 ...
 */
async function runWithRetry(eventCtx: AtlasExecutionContext, tenantId: string, event: EventEnvelope, dispatch: EventDispatcher): Promise<boolean> {
    let attempt = 0;
