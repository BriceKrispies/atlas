...
S({
    ...
    mount: function (demoEl, { config, onLog }) {
        ...
        function renderItems(): void {
            const rows: string[] = [];
            for (let i = 0; i < 12; i++) {
                rows.push(`<atlas-card padding="sm"><atlas-stack gap="xs">` +
        ...
        ptr.addEventListener('refresh', function () {
            if (ptr.busy)
                return;
...
S({
    ...
    mount: function (demoEl, { config, onLog }) {
        ...
        for (const msg of messages) {
            ...
            row.appendChild(del);
            row.addEventListener('open', function (ev) {
                onLog('open', readEventDetail(ev));
            });
            row.addEventListener('close', function () {
                onLog('close', { from: msg.from });
            });
            row.addEventListener('action', function (ev) {
