...
S({
    ...
    mount: function (demoEl, { onLog }) {
        function makeSheet(opts: {
            ...
            if (opts.snapPoints)
                sheet.setAttribute('snap-points', opts.snapPoints);
            if (opts.dismissible)
                sheet.setAttribute('dismissible', opts.dismissible);
            sheet.innerHTML = `
        <atlas-stack gap="sm">
          <atlas-text>
      ...
      `;
            ...
            sheet.addEventListener('click', function (ev) {
                const t = ev.target;
                if (isElement(t) && t.closest('[data-sheet-cancel]'))
                    sheet.close('cancel');
                if (isElement(t) && t.closest('[data-sheet-confirm]'))
                    sheet.close('confirm');
            });
            sheet.addEventListener('open', function () {
                return onLog(`${opts.label}.open`, {});
            });
            sheet.addEventListener('close', function (ev) {
...
S({
    ...
    mount: function (demoEl, { onLog }) {
        function makeSheet(opts: {
            ...
            sheet.addEventListener('action', function (ev) {
    ...
    mount: function (demoEl, { onLog }) {
        ...
        function makeFab(attrs: Record<string, string>, iconName = 'add'): HTMLElement {
            ...
            fab.addEventListener('click', function () {
                return onLog('click', { position: attrs['position'] ?? 'bottom-right', label: attrs['label'] ?? '' });
            });
            return fab;
        }
