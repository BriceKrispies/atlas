...
function mountAnnouncementsHarness(demoEl: HTMLElement): () => void {
    ...
    harness.resolveWidgetModuleUrl = function (widgetId: string) {
        return widgetId === 'content.announcements'
...
function mountDataTable(demo: HTMLElement, ctx: {
    ...
    if (config.pageSize != null)
        table.setAttribute('page-size', String(config.pageSize));
    if (config.selection)
        table.setAttribute('selection', config.selection);
    if (config.emptyHeading)
        table.setAttribute('empty-heading', config.emptyHeading);
    ...
    const handlers: Array<[
        ...
        const h: EventListener = function (e) {
            return onLog(ev, customEventDetail(e) ?? {});
        };
    ...
    return function () {
        for (const [ev, h] of handlers)
            table.removeEventListener(ev, h);
        table.remove();
...
function mountChart(demo: HTMLElement, ctx: {
    config: Record<string, unknown>;
    onLog: (kind: string, payload: unknown) => void;
}): () => void {
    const { onLog } = ctx;
    ...
    if (config.height)
        chart.setAttribute('height', config.height);
    if (config.label)
        chart.setAttribute('label', config.label);
    if (config.showLegend)
        chart.setAttribute('show-legend', '');
    if (config.innerRadius != null)
        chart.setAttribute('inner-radius', String(config.innerRadius));
    chart.data = config.data;
    const handler: EventListener = function (e) {
        return onLog(e.type, customEventDetail(e) ?? {});
    };
...
function mountChartCard(demo: HTMLElement, ctx: {
    config: Record<string, unknown>;
    onLog: (kind: string, payload: unknown) => void;
}): () => void {
    const { onLog } = ctx;
  ...
  `;
    ...
    const logHandler: EventListener = function (e) {
        return onLog(e.type, customEventDetail(e) ?? {});
    };
...
declare global {
    interface Window {
         ...
         */
        __atlasTestDataSource?: {
            emit: (patch: unknown) => void;
        };
    }
}
