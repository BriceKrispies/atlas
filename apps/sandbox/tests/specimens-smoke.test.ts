...
test.describe('sandbox — specimens smoke', function () {
    test('home loads with sidebar populated', async function ({ page }) {
        await page.goto('/');
        const items = page.locator('atlas-nav-item.item');
        await expect(items.first()).toBeVisible();
        expect(await items.count()).toBeGreaterThan(3);
    });
    test('can open the announcements widget specimen', async function ({ page }) {
    ...
    test('can open the welcome content page and render its template', async function ({ page }) {
        ...
        await expect(page.locator('section[data-slot="sidebar"] [data-widget-cell]')).toHaveCount(0);
    });
    ...
    for (const { id, tag } of formControls) {
        test(`Forms specimen "${id}" renders ${tag}`, async function ({ page }) {
    ...
    for (const { id, tag } of mobileOverlays) {
        test(`Mobile overlays specimen "${id}" renders ${tag}`, async function ({ page }) {
            await openSpecimen(page, id);
    ...
    for (const { id, tag } of gestureSpecimens) {
        test(`Batch Gestures — specimen "${id}" renders ${tag}`, async function ({ page }) {
            await openSpecimen(page, id);
            await expect(page.locator(tag).first()).toBeVisible();
        });
    }
    // Batch Identity & Chips — specimens smoke
    const identityAndChips: Array<{
        id: string;
        tag: string;
    }> = [
    ...
    for (const { id, tag } of identityAndChips) {
        test(`Batch Identity & Chips — specimen "${id}" renders ${tag}`, async function ({ page }) {
            await openSpecimen(page, id);
            await expect(page.locator(tag).first()).toBeVisible();
        });
    }
    ...
    for (const { id, tag } of navStructure) {
        test(`Nav structure specimen "${id}" renders ${tag}`, async function ({ page }) {
            await openSpecimen(page, id);
            await expect(page.locator(tag).first()).toBeVisible();
        });
    }
    // Batch Data & Composites — specimens smoke
    const dataAndComposites: Array<{
        id: string;
        tag: string;
    }> = [
    ...
    for (const { id, tag } of dataAndComposites) {
        test(`Batch Data & Composites — specimen "${id}" renders ${tag}`, async function ({ page }) {
            await openSpecimen(page, id);
            await expect(page.locator(tag).first()).toBeVisible();
        });
    }
    // Batch Content authoring — specimens smoke
    const contentPrimitives: Array<{
        id: string;
        tag: string;
    }> = [
    ...
    for (const { id, tag } of contentPrimitives) {
        test(`Content authoring specimen "${id}" renders ${tag}`, async function ({ page }) {
            await openSpecimen(page, id);
            await expect(page.locator(tag).first()).toBeVisible();
        });
    }
    ...
    for (const { id, tag } of agentPrimitives) {
        test(`Batch Agent — specimens smoke "${id}" renders ${tag}`, async function ({ page }) {
            await openSpecimen(page, id);
            await expect(page.locator(tag).first()).toBeVisible();
        });
    }
    ...
    test.describe('Batch Mobile nav — specimens smoke', function () {
        const mobileNavSpecimens: Array<{
            id: string;
            tag: string;
        }> = [
        ...
        for (const { id, tag } of mobileNavSpecimens) {
            test(`Mobile nav specimen "${id}" renders ${tag}`, async function ({ page }) {
                await openSpecimen(page, id);
                await expect(page.locator(tag).first()).toBeVisible();
            });
        }
        test('bottom-nav exposes role="tablist" and one selected item', async function ({ page, }) {
            ...
            await expect(bar.locator('atlas-bottom-nav-item[aria-selected="true"]').first()).toBeVisible();
        });
        test('app-bar implies role="banner"', async function ({ page }) {
    ...
    for (const { id, tag } of popups) {
        test(`Popups specimen "${id}" renders ${tag}`, async function ({ page }) {
            await openSpecimen(page, id);
