...
import { openSpecimen, selectVariant, waitForEditor, widgetCell, widgetInstanceIdsInRegion, dropSlot, paletteChip, } from './helpers.ts';
test.describe('DnD subsystem — observable markers', function () {
    test('cells are NOT native-draggable (old HTML5 path is gone)', async function ({ page }) {
    ...
    test('palette chips are NOT native-draggable', async function ({ page }) {
        await openSpecimen(page, 'page.dashboard');
        await selectVariant(page, 'Edit');
        await waitForEditor(page, 'dashboard');
    ...
    test('pointer drag surfaces subsystem markers and commits into the empty slot', async function ({ page }) {
        ...
        const transformDuringDrag = await overlay.evaluate(function (el) {
    ...
    test('short pointer motion below 4px threshold does NOT drag', async function ({ page }) {
        await openSpecimen(page, 'page.dashboard');
        await selectVariant(page, 'Edit');
        await waitForEditor(page, 'dashboard');
