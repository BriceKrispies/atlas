...
import { readEditorState, readDragState, assertCommitted, } from '@atlas/test-fixtures';
import { openSpecimen, selectVariant, waitForEditor, paletteChip, dropSlot, } from './helpers.ts';
...
async function openEditor(page: Page): Promise<void> {
    ...
    await expect
        .poll(async function () {
        return readEditorSurfaceId(await readEditorState(page, PAGE));
    })
        .toBe(SURFACE);
}
...
test.describe('content-page editor — committed-state contract', function () {
    test('two-tap add commits an `add` intent on the editor', async function ({ page }) {
        ...
        expect(state.entries.some(function (e) {
            return e.region === 'sidebar';
        })).toBe(true);
        expect(state.dirty).toBe(true);
    });
    test('drag state reader is inactive when idle', async function ({ page }) {
        await openEditor(page);
        const drag = await readDragState(page);
        expect(drag).toEqual({ active: false, payload: null, hoveredSlotId: null });
    });
    test('pointer drag of a palette chip into an empty slot commits `drop`', async function ({ page }) {
        await openEditor(page);
        ...
        await page.mouse.move(chipBox.x + chipBox.width / 2 + 10, chipBox.y + chipBox.height / 2 + 10, { steps: 4 });
