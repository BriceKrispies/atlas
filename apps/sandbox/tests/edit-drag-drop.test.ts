...
import { openSpecimen, selectVariant, widgetCell, widgetInstanceIdsInRegion, dropSlot, paletteChip, runEditorOp, waitForEditor, } from './helpers.ts';
test.describe('content-page edit mode — drop into slot', function () {
    test('only empty regions expose a drop slot (filled regions expose none)', async function ({ page }) {
    ...
    test('palette renders chips named by widgetId', async function ({ page }) {
    ...
    test('palette chip + click on empty slot adds a widget', async function ({ page }) {
        await openSpecimen(page, 'page.welcome');
        await selectVariant(page, 'Edit');
        await waitForEditor(page, 'welcome');
        ...
        await expect
            .poll(async function () {
            return (await widgetInstanceIdsInRegion(page, 'sidebar')).length;
        })
            .toBe(1);
    });
    test('keyboard two-tap: Enter on chip, Enter on empty slot commits an add', async function ({ page }) {
        await openSpecimen(page, 'page.welcome');
        await selectVariant(page, 'Edit');
        await waitForEditor(page, 'welcome');
        const chip = paletteChip(page, 'content.announcements');
    ...
    test('Escape clears a pending selection without mutating', async function ({ page }) {
        await openSpecimen(page, 'page.dashboard');
        await selectVariant(page, 'Edit');
        await waitForEditor(page, 'dashboard');
    ...
    test('pointer-drag a palette chip into the empty slot commits an add', async function ({ page }) {
        await openSpecimen(page, 'page.welcome');
        await selectVariant(page, 'Edit');
        await waitForEditor(page, 'welcome');
        const chip = paletteChip(page, 'content.announcements');
    ...
    test('pointer-drag a cell from one slot into another empty slot', async function ({ page }) {
    ...
    test('filled region is not a drop target — dragging onto it is a no-op', async function ({ page }) {
    ...
    test('imperative API: agents can add / move / remove without the UI', async function ({ page }) {
        await openSpecimen(page, 'page.welcome');
        await selectVariant(page, 'Edit');
        await waitForEditor(page, 'welcome');
    ...
    test('imperative API: rejected operations return stable reason codes', async function ({ page }) {
        await openSpecimen(page, 'page.welcome');
        await selectVariant(page, 'Edit');
        await waitForEditor(page, 'welcome');
