...
import { openSpecimen, setMobileViewport } from './helpers.ts';
/**
 ...
 */
interface InteractiveSpecimen {
    id: string;
    selector: string;
}
const INTERACTIVE_SPECIMENS: InteractiveSpecimen[] = [
    { id: 'button', selector: 'atlas-button' },
    { id: 'input', selector: 'atlas-input' },
    { id: 'nav', selector: 'atlas-nav-item' },
];
test.describe('design system — mobile viewport contract', function () {
    test.beforeEach(async function ({ page }) {
        await setMobileViewport(page);
    });
    for (const { id, selector } of INTERACTIVE_SPECIMENS) {
        test(`${id}: no horizontal scroll and 44×44 touch targets at 360px`, async function ({ page }) {
            await openSpecimen(page, id);
            // (1) No horizontal document scroll.
            const overflowsX = await page.evaluate(function () {
    ...
    test('tokens expose --atlas-touch-target-min and breakpoint custom properties', async function ({ page }) {
