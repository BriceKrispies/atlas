...
import { readChartState, assertCommitted, } from '@atlas/test-fixtures';
 ...
 */
async function assertCommit(page: Page, key: string, match: {
    intent: string;
...
async function openCard(page: Page): Promise<void> {
    ...
    await expect
        .poll(async function () {
        return (await readChart(page, ID))?.chartId;
    })
        .toBe(ID);
}
test.describe('atlas-chart-card committed-state contract', function () {
    test('config change commits setConfig', async function ({ page }) {
        await openCard(page);
    ...
    test('time range preset commits setTimeRange', async function ({ page }) {
        await openCard(page);
    ...
    test('filter set + clear commits setFilter / clearFilter', async function ({ page }) {
        await openCard(page);
    ...
    test('legend click commits toggleSeries and hides the series', async function ({ page }) {
        await openCard(page);
        const before = await readChartOrThrow(page, ID);
        expect(before.hiddenSeries).toEqual([]);
        expect(before.data.series.map(function (s) {
            return s.id;
        ...
        expect(after.data.series.map(function (s) {
            return s.id;
        })).toEqual(['mobile']);
    });
    test('export button commits requestExport with its format', async function ({ page }) {
        await openCard(page);
    ...
    test('clicking a bar commits pushDrilldown; breadcrumb pops it', async function ({ page }) {
        await openCard(page);
        ...
        expect(drilled.data.series.map(function (s) {
            return s.id;
        })).toEqual([
    ...
    test('testids on interactive children follow {surfaceId}.{name}.{key}', async function ({ page }) {
        await openCard(page);
        // Every legend button should have a data-testid ending in .series.<id>
        const testids = await page.locator('atlas-chart-legend atlas-button').evaluateAll(function (els) {
            return els.map(function (el) {
                return el.getAttribute('data-testid');
            });
        });
        expect(testids.every(function (id) {
            return id && /\.series\.(desktop|mobile)$/.test(id);
        })).toBe(true);
    });
});
