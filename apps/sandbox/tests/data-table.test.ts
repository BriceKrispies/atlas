...
test.describe('atlas-data-table specimen', function () {
    test('renders rows, columns, and pagination', async function ({ page }) {
        ...
        await expect(table.locator('atlas-pagination [data-role="page-info"]')).toContainText('Page 1 of 3');
    });
    test('clicking a sortable header cycles aria-sort', async function ({ page }) {
        await openSpecimen(page, 'widgets.data-table');
    ...
    test('sort reorders rows ascending by title', async function ({ page }) {
        await openSpecimen(page, 'widgets.data-table');
        const titleHeader = page.locator('atlas-table-cell[header][data-column-key="title"]');
    ...
    test('pagination next/prev move pages and clamp at boundaries', async function ({ page }) {
        await openSpecimen(page, 'widgets.data-table');
    ...
    test('empty variant renders custom heading', async function ({ page }) {
        await openSpecimen(page, 'widgets.data-table');
        await selectVariant(page, 'Empty');
        const table = page.locator('atlas-data-table');
        await expect(table).toHaveAttribute('data-state', 'empty');
        await expect(table.locator('atlas-stack[data-role="empty"] atlas-heading')).toHaveText('No results found');
    });
    test('multi-select variant records row-selected events', async function ({ page }) {
        await openSpecimen(page, 'widgets.data-table');
    ...
    test('filter: typing in title filter narrows rows', async function ({ page }) {
        await openSpecimen(page, 'widgets.data-table');
        const table = page.locator('atlas-data-table');
        ...
        await expect(table.locator('atlas-table-body atlas-row')).toHaveCount(1);
    });
    test('filtered-empty state offers clear-filters', async function ({ page }) {
        await openSpecimen(page, 'widgets.data-table');
        const table = page.locator('atlas-data-table');
        const filterInput = table.locator('atlas-input[data-column-key="title"]');
    ...
    test('streaming variant applies upsert and remove patches', async function ({ page }) {
        await openSpecimen(page, 'widgets.data-table');
        ...
        await page.evaluate(function () {
            ...
            if (!ds)
                throw new Error('streaming specimen did not install __atlasTestDataSource');
            ds.emit({
                type: 'upsert',
