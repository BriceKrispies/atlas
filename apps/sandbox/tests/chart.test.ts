...
test.describe('atlas-chart specimen', function () {
    test('line variant renders SVG with series paths and points', async function ({ page }) {
    ...
    test('hidden data table fallback mirrors chart data', async function ({ page }) {
        await openSpecimen(page, 'widgets.chart');
    ...
    test('legend renders when show-legend is set', async function ({ page }) {
        await openSpecimen(page, 'widgets.chart');
        const legend = page.locator('atlas-chart atlas-chart-legend');
        await expect(legend).toBeVisible();
    });
    test('bar variant renders rect shapes', async function ({ page }) {
        await openSpecimen(page, 'widgets.chart');
    ...
    test('stacked bar variant stacks slices on one x-axis position', async function ({ page }) {
        await openSpecimen(page, 'widgets.chart');
    ...
    test('pie variant renders slice paths', async function ({ page }) {
        await openSpecimen(page, 'widgets.chart');
        await selectVariant(page, 'Pie');
        const slices = page.locator('atlas-chart svg path.slice');
        expect(await slices.count()).toBe(4);
    });
    test('donut variant renders slices with inner radius', async function ({ page }) {
        await openSpecimen(page, 'widgets.chart');
        await selectVariant(page, 'Donut');
        const slices = page.locator('atlas-chart svg path.slice');
        expect(await slices.count()).toBe(4);
    });
    test('resizing the viewport rescales SVG width', async function ({ page }) {
        await openSpecimen(page, 'widgets.chart');
        const svg = page.locator('atlas-chart svg');
        const widthA = await svg.evaluate(function (el) {
        ...
        await expect
            .poll(async function () {
            return svg.evaluate(function (el) {
                return Number(el.getAttribute('width'));
            });
        })
            .not.toBe(widthA);
