...
describe('loadConfig() — TENANT_ID forbidden in strict mode (parity with Rust forbid_in_strict)', function () {
    ...
    test('throws when ATLAS_ENV is strict (TEST_AUTH_ENABLED unset) and TENANT_ID is set', function () {
        ...
        expect(function () {
            return loadConfig();
        }).toThrow(/TENANT_ID.*strict|forbid/i);
    });
    test('succeeds when ATLAS_ENV is dev (TEST_AUTH_ENABLED=true) and TENANT_ID is set', function () {
        process.env['CONTROL_PLANE_DB_URL'] =
            'postgres://atlas_platform:local_dev_password@localhost:15433/control_plane';
    ...
    test('succeeds when strict mode and TENANT_ID is unset', function () {
        process.env['CONTROL_PLANE_DB_URL'] =
            'postgres://atlas_platform:local_dev_password@localhost:15433/control_plane';
        process.env['OIDC_ISSUER_URL'] = 'https://issuer.example.com';
        process.env['OIDC_JWKS_URL'] = 'https://issuer.example.com/jwks';
