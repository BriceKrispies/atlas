...
export function serverEventDispatcher(broadcast: ServerEventBroadcast): EventDispatcher {
    return async function (envelope: EventEnvelope): Promise<void> {
        ...
        const tagsForWire = envelopeTags && envelopeTags.length > 0 ? envelopeTags : undefined;
        // 1. Page-create → projection.updated (mirrors worker.rs)
        if (envelope.eventType === 'ContentPages.PageCreateRequested') {
            const payload = envelope.payload;
            const pageId = payload && typeof payload === 'object' && !Array.isArray(payload) &&
