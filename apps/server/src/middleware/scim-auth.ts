...
import { findScimTokensByLookup, hashSecret, lookupOf, verifyPassword, type ScimTokenDocument, } from '@atlas/identity';
 ...
 */
function scimError(c: Context<{
    Variables: ServerVariables;
}>, status: number, detail: string, scimType?: string): Response {
 ...
 */
async function validateScimBearer(c: Context<{
    Variables: ServerVariables;
}>, state: AppState): Promise<{
    tenantId: string;
    token: ScimTokenDocument;
} | null> {
    ...
    const tenantId = hostTenantId ?? c.req.query('tenant_id') ?? state.config.tenantId ?? null;
    if (!tenantId)
        return null;
    ...
    const candidates = await findScimTokensByLookup(entities, tenantId, lookupOf(presented));
    ...
    for (const t of candidates) {
        const validStatus = t.status === 'active' ||
            (t.status === 'rotated' &&
