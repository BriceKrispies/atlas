 ...
 * same order, modulo the cross-cutting `policy-cache` and `server-events`
 * which the worker does not run today).
 *
...
describe('inline dispatcher chain composition (I12 worker mirror)', () => {
  it('inline chain lists every per-module dispatcher in the canonical order', () => {
  ...
  it('worker chain is a structural prefix of the inline chain (parity)', () => {
    // The worker omits `policy-cache` (Cedar bundle invalidation) and
    // `server-events` (SSE fanout) — both are server-process-local.
    // Everything else MUST match name-for-name and order-for-order.
    const inlineCore = REQUEST_DISPATCHER_CHAIN_NAMES.filter(
      (n) => n !== 'policy-cache' && n !== 'server-events',
    );
    expect(WORKER_DISPATCHER_CHAIN_NAMES).toEqual(inlineCore);
  });

  it('cache-tag runs AFTER every per-module dispatcher in both chains', () => {
  ...
  it('policy-cache (when present) runs AFTER cache-tag', () => {
  ...
  it('server-events is last in the inline chain', () => {
    ...
    expect(
      REQUEST_DISPATCHER_CHAIN_NAMES[REQUEST_DISPATCHER_CHAIN_NAMES.length - 1],
    ).toBe('server-events');
  });
});
