...
function makeRig(): Rig {
    ...
    const logPipeline = new LogPipeline([collector], new InMemoryLevelController('debug'));
...
describe('principalMiddleware — logging instrumentation', function () {
    it('emits Authn.Resolved on successful X-Debug-Principal auth', async function () {
    ...
    it('emits Authn.Failed when X-Debug-Principal is malformed', async function () {
        const { app, collector } = makeRig();
        const res = await app.request('/echo', {
    ...
    it('emits Authn.Failed when no Authorization header is present', async function () {
        const { app, collector } = makeRig();
        const res = await app.request('/echo');
    ...
    it('Authn.Resolved correlationId matches Request.Received correlationId', async function () {
        const { app, collector } = makeRig();
        const res = await app.request('/echo', {
            headers: {
        ...
        const resolvedEvent = assertDefined(resolved[0], 'Authn.Resolved log event present after length check');
        const receivedEvent = assertDefined(received[0], 'Request.Received log event present after length check');
