...
import { policyCacheDispatcher, type CedarBundleCache, } from '@atlas/adapter-policy-cedar';
...
function throwingProxy<T extends object>(name: string): T {
    ...
    const proxy = new Proxy(target, {
        get(_t, prop) {
            throw new Error(`${name}.${String(prop)} accessed during identity-event dispatch — ` +
...
const stubRevisions = throwingProxy<RepositoryRevisionStore>('RepositoryRevisionStore');
// ---------------------------------------------------------------------
...
function buildChain(): ChainHarness {
    ...
    const dispatch = composeDispatchers(catalogDispatcher({
        catalogState: stubCatalogState,
        projections: stubProjections,
        search: stubSearch,
        cache,
    }), contentPagesDispatcher({ entities, relations, cache }), identityDispatcher({ entities, relations, cache }), repositoryDispatcher({
        repositories: stubRepositories,
        revisions: stubRevisions,
        cache,
    }), cacheTagDispatcher(cache), policyCacheDispatcher(cedarCache), serverEventDispatcher(broadcast));
...
function envelope(eventType: string, tenantId: string, cacheInvalidationTags: string[], payload: unknown = {}): EventEnvelope {
    return {
...
// Tests
// ---------------------------------------------------------------------
describe('dispatcher chain — identity events flow through without error', function () {
    let h: ChainHarness;
    beforeEach(function () {
        h = buildChain();
    });
    ...
    it.each(IDENTITY_EVENTS_WITH_TAGS)('$eventType propagates through the chain without throwing', async function ({ eventType, tags }) {
        await expect(h.dispatch(envelope(eventType, 't1', tags, { document: { tenantId: 't1' } }))).resolves.toBeUndefined();
    });
});
describe('cacheTagDispatcher — purges by exact tags', function () {
    it('Identity.SessionIssued purges the per-session cache key', async function () {
        const h = buildChain();
        // Pre-seed a cache entry tagged with the session.
        await h.cache.set('render:t1:home', { greeting: 'hi' }, { ttlSeconds: 0, tags: ['Tenant:t1', 'Session:sess-1'] });
        expect(h.cache.store.has('render:t1:home')).toBe(true);
        await h.dispatch(envelope('Identity.SessionIssued', 't1', ['Tenant:t1', 'User:user-1', 'Session:sess-1'], { document: {} }));
    ...
    it('events with empty cacheInvalidationTags do not call invalidateByTags', async function () {
        const h = buildChain();
        await h.dispatch(envelope('Identity.UserCreated', 't1', [], { document: {} }));
        expect(h.cache.invalidatedTagBatches).toEqual([]);
    });
});
describe('policyCacheDispatcher — invalidates Cedar bundle cache by tenant tag', function () {
    it('invalidates the bundle cache for the tenant in `Tenant:<id>` tag', async function () {
        const h = buildChain();
        await h.dispatch(envelope('Identity.MembershipRolesChanged', 't1', ['Tenant:t1', 'User:user-1', 'Membership:t1:user-1'], { document: {} }));
    ...
    ])('invalidates Cedar cache for tenant on %s', async function (eventType) {
        const h = buildChain();
        await h.dispatch(envelope(eventType, 't1', ['Tenant:t1', 'Principal:user-1'], {
            document: { tenantId: 't1' },
        }));
        expect(h.cedarCache.invalidatedTenants).toContain('t1');
    });
    it('does NOT invalidate Cedar cache when the envelope has no Tenant: tag', async function () {
        const h = buildChain();
        await h.dispatch(envelope('Identity.UserCreated', 't1', [], { document: {} }));
        expect(h.cedarCache.invalidatedTenants).toEqual([]);
    });
});
describe('serverEventDispatcher — emits cache.invalidated SSE for tagged identity events', function () {
    it('publishes one cache.invalidated server event per tagged envelope', async function () {
        const h = buildChain();
        await h.dispatch(envelope('Identity.SessionIssued', 't1', ['Tenant:t1', 'User:user-1', 'Session:sess-1'], { document: {} }));
        await settle();
        const invalidatedEvents = h.serverEvents.filter(function (e) {
            return e.eventType === 'cache.invalidated';
        });
    ...
    it('does not publish a cache.invalidated event when cacheInvalidationTags is empty', async function () {
        const h = buildChain();
        await h.dispatch(envelope('Identity.UserCreated', 't1', [], { document: {} }));
        await settle();
        const invalidatedEvents = h.serverEvents.filter(function (e) {
...
describe('per-tenant isolation', function () {
    it("dispatching a tenant-A event does not invalidate tenant B's cache key", async function () {
        const h = buildChain();
        // Pre-seed cache entries for both tenants.
        await h.cache.set('render:tenant-a:home', 'a', { ttlSeconds: 0, tags: ['Tenant:tenant-a'] });
        await h.cache.set('render:tenant-b:home', 'b', { ttlSeconds: 0, tags: ['Tenant:tenant-b'] });
        await h.dispatch(envelope('Identity.SessionIssued', 'tenant-a', ['Tenant:tenant-a', 'User:user-1', 'Session:sess-1'], { document: {} }));
        expect(h.cache.store.has('render:tenant-a:home')).toBe(false);
        expect(h.cache.store.has('render:tenant-b:home')).toBe(true);
    });
    it("does not invalidate tenant B's Cedar bundle when a tenant-A event fires", async function () {
        const h = buildChain();
        await h.dispatch(envelope('Identity.MembershipRolesChanged', 'tenant-a', ['Tenant:tenant-a', 'User:user-1'], { document: {} }));
        expect(h.cedarCache.invalidatedTenants).toEqual(['tenant-a']);
        expect(h.cedarCache.invalidatedTenants).not.toContain('tenant-b');
    });
    it('SSE cache.invalidated carries the originating tenant only', async function () {
        const h = buildChain();
        await h.dispatch(envelope('Identity.SessionIssued', 'tenant-a', ['Tenant:tenant-a', 'User:user-1', 'Session:sess-1'], { document: {} }));
        await settle();
        const invalidated = h.serverEvents.filter(function (e) {
            return e.eventType === 'cache.invalidated';
        });
        expect(invalidated).toHaveLength(1);
        expect(invalidated[0]?.tenantId).toBe('tenant-a');
        // No tenant-b leakage in the tags.
        expect((invalidated[0]?.tags ?? []).every(function (t) {
            return !t.includes('tenant-b');
        })).toBe(true);
    });
});
describe('error semantics — composeDispatchers runs all dispatchers even on partial failure', function () {
    it('cacheTag + policy-cache + server-events all fire even when an upstream module dispatcher throws', async function () {
        ...
        const throwingIdentity: EventDispatcher = async function () {
            throw new Error('synthetic projection failure');
        };
        const dispatch = composeDispatchers(throwingIdentity, cacheTagDispatcher(cache), policyCacheDispatcher(cedarCache), serverEventDispatcher(broadcast));
        const env = envelope('Identity.UserCreated', 't1', ['Tenant:t1', 'User:user-1'], {});
        ...
        expect(serverEvents.some(function (e) {
            return e.eventType === 'cache.invalidated';
        })).toBe(true);
    });
});
