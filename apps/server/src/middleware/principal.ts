...
import { PLATFORM_TENANT_ID } from '@atlas/platform-core';
import { createRootContext } from '@atlas/logging';
import { PostgresEntityStore, } from '@atlas/adapter-node';
import { parseApiKeyBearer, getApiKeyEntity, getSessionEntity, findActiveProviderByIssuer, findOAuthTokensByLookup, hashSecret, lookupOf, constantTimeEqual, verifyPassword, checkSessionLifetime, touchSessionLastSeen, resolveImpersonationToken, type ApiKeyDocument, type AuthSessionDocument, type IdentityProviderDocument, type OAuthAccessTokenDocument, } from '@atlas/identity';
 ...
 */
function getStringClaim(claims: Record<string, unknown>, name: string): string | null {
    const v = claims[name];
...
function isValidTenantId(value: string): boolean {
    // System-reserved platform tenant id starts with an underscore by
    // convention (see PLATFORM_TENANT_ID + ADR 0008 §1). The legacy
    // first-char rule was inherited from the Rust prototype before
    // `_platform` became a real row in control_plane.tenants. Allow it
    // explicitly so the principal middleware can resolve the seeded
    // platform-admin principal without rejecting the tenant slug.
    if (value === PLATFORM_TENANT_ID) return true;
    if (value.length === 0 || value.length > 64)
        return false;
    if (!TENANT_ID_FIRST_CHAR.test(value))
        return false;
    if (!TENANT_ID_CHARSET.test(value))
        return false;
    return true;
}
 ...
 */
function upgradeContextWithPrincipal(c: Context<{
    Variables: ServerVariables;
}>, state: AppState, principal: Principal): void {
    ...
    if (prior.requestId !== undefined)
        ctxInput.requestId = prior.requestId;
 ...
 */
function logAuthnFailed(c: Context<{
    Variables: ServerVariables;
...
function parseDebugPrincipal(raw: string, defaultTenantId: string): Principal | null {
    const parts = raw.split(':');
    if (parts.length < 2 || parts.length > 4)
        return null;
    const typeStr = parts[0]?.toLowerCase() ?? '';
    if (!VALID_DEBUG_TYPES.has(typeStr))
        return null;
    const id = parts[1] ?? '';
    if (!id)
        return null;
 ...
 */
function jwtUnverifiedIssuer(token: string, ctx?: AtlasExecutionContext): string | null {
    ...
    if (payloadSegment === undefined || payloadSegment.length === 0)
        return null;
    try {
        const raw: unknown = JSON.parse(Buffer.from(payloadSegment, 'base64url').toString('utf8'));
        if (!isClaimsObject(raw))
            return null;
 ...
 */
async function tryBearerSchemes(c: Context<{
    Variables: ServerVariables;
}>, state: AppState, correlationId: string): Promise<Principal | null | 'fail-with-error'> {
    ...
    if (token.startsWith('atlas_')) {
        ...
        const apiKey: ApiKeyDocument | null = await getApiKeyEntity(entities, tenantId, parsed.keyId);
        if (!apiKey) {
            errorResponse(c, 'API_KEY_NOT_FOUND', 'unknown API key', 401, correlationId);
            return 'fail-with-error';
        }
        const now = Date.now();
        const validStatus = apiKey.status === 'active' ||
    ...
    if (token.startsWith('imp-')) {
        ...
        if (!resolved.ok) {
            errorResponse(c, 'IMPERSONATION_NOT_FOUND', 'impersonation token invalid', 401, correlationId);
            return 'fail-with-error';
        }
...
async function tryAccessToken(c: Context<{
    Variables: ServerVariables;
}>, entities: PostgresEntityStore, tenantId: string, token: string): Promise<Principal | null> {
    const presentedHash = hashSecret(token);
    const lookup = lookupOf(token);
    // Scan the lookup bucket. The index makes this O(bucket size).
    const candidates = await entities.query<AuthSessionDocument>(tenantId, 'AuthSession', { attrsEqual: { accessTokenLookup: lookup } });
    for (const row of candidates) {
        if (!constantTimeEqual(row.attrs.accessTokenHash, presentedHash))
            continue;
...
async function tryOAuthToken(entities: PostgresEntityStore, tenantId: string, token: string): Promise<Principal | null> {
    const presentedHash = hashSecret(token);
    const lookup = lookupOf(token);
    const candidates: OAuthAccessTokenDocument[] = await findOAuthTokensByLookup(entities, tenantId, lookup);
    for (const t of candidates) {
        if (!constantTimeEqual(t.secretHash, presentedHash))
            continue;
        if (t.status !== 'active')
            continue;
...
export function principalMiddleware(state: AppState) {
    return async function (c: Context<{
        ...
        //    was updated).
        const bearerPresent = !!(c.req.header('Authorization') ?? c.req.header('authorization'));
        if (!bearerPresent) {
            ...
            if (cookie) {
                ...
                if (tenantId) {
                    try {
                        ...
                        const session = await getSessionEntity(entities, tenantId, cookie.sessionId);
                        if (session) {
        ...
        if (state.config.testAuth.enabled) {
            ...
            if (debugHeader) {
                ...
                if (!debug) {
                    ...
                    return errorResponse(c, 'PRINCIPAL_INVALID', 'Invalid X-Debug-Principal header', 400, correlationId);
                }
                if (hostTenantId !== null && hostTenantId !== debug.tenantId) {
                    ...
                    return errorResponse(c, 'PRINCIPAL_INVALID', 'Tenant scope mismatch between Host and X-Debug-Principal', 403, correlationId);
                }
        ...
        if (bearerPrincipal === 'fail-with-error')
        ...
        if (!token) {
            ...
            return errorResponse(c, 'PRINCIPAL_INVALID', 'Empty bearer token', 401, correlationId);
        }
        ...
        if (claimedIssuer && hostTenantId) {
            try {
                ...
                const entities = new PostgresEntityStore(sql);
                const idp: IdentityProviderDocument | null = await findActiveProviderByIssuer(entities, hostTenantId, claimedIssuer);
                if (idp && idp.jwksUri) {
                    ...
                    try {
                        ...
                        if (errStr.includes('kid') || errStr.includes('no applicable key')) {
                            const refetched = jwksCache.resolve(hostTenantId, idp.idpId, idp.jwksUri, { forceRefetch: true });
                            try {
                                ...
                                return errorResponse(c, 'PRINCIPAL_INVALID', `JWT verification failed after JWKS refresh: ${errStr}`, 401, correlationId);
                            }
                        }
                        else {
                            return errorResponse(c, 'PRINCIPAL_INVALID', `JWT verification failed: ${errStr}`, 401, correlationId);
                        }
                    }
                }
            }
        ...
        if (!claims) {
            if (!state.jwks || !state.config.oidc.issuerUrl) {
                return errorResponse(c, 'PRINCIPAL_INVALID', claimedIssuer
                    ? `no IdentityProvider for issuer ${claimedIssuer}`
                    : 'Token missing iss claim', 401, correlationId);
            }
            try {
                ...
                return errorResponse(c, 'PRINCIPAL_INVALID', `JWT verification failed: ${errorMessage(e)}`, 401, correlationId);
            }
        }
        ...
        if (!sub) {
            return errorResponse(c, 'PRINCIPAL_INVALID', 'Token missing sub claim', 401, correlationId);
        }
        const tenantClaim = claims['tenant_id'];
        const candidateTenant = typeof tenantClaim === 'string' && tenantClaim.length > 0
            ? tenantClaim
            : state.config.tenantId;
        if (hostTenantId !== null && hostTenantId !== candidateTenant) {
            return errorResponse(c, 'PRINCIPAL_INVALID', 'Tenant scope mismatch between Host and JWT tenant_id claim', 403, correlationId);
        }
        if (!isValidTenantId(candidateTenant)) {
            ...
            return errorResponse(c, 'PRINCIPAL_INVALID', 'Invalid tenant_id claim', 401, correlationId);
        }
