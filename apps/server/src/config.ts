...
import { envBool, envOr, envRequired, forbidInStrict, } from '@atlas/platform-core';
...
export function loadConfig(): AppConfig {
    ...
    forbidInStrict('TENANT_ID', 'TENANT_ID is a dev-only override; production tenancy must come from the authenticated principal.');
    ...
    if (policyEngineRaw !== 'stub' && policyEngineRaw !== 'cedar') {
        throw new Error(`invalid POLICY_ENGINE: ${policyEngineRaw} (expected 'stub' or 'cedar')`);
    }
    ...
    if (workerModeRaw !== 'inline' && workerModeRaw !== 'async') {
        throw new Error(`invalid WORKER_MODE: ${workerModeRaw} (expected 'inline' or 'async')`);
    }
    ...
    if (mailerModeRaw !== 'stdout' &&
        mailerModeRaw !== 'noop' &&
        mailerModeRaw !== 'smtp') {
        throw new Error(`invalid MAILER_MODE: ${mailerModeRaw} (expected 'stdout', 'noop', or 'smtp')`);
    }
    ...
    if (mailerMode === 'smtp') {
        if (!smtpHost || !smtpPortRaw || !smtpFrom) {
            throw new Error(`MAILER_MODE=smtp requires SMTP_HOST, SMTP_PORT, and SMTP_FROM to be set`);
        }
    ...
    const tenantBaseUrl = function (slug: string): string {
