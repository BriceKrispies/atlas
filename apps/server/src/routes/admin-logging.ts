 ...
 */
async function readLevelBody(
  c: AppCtx,
  correlationId: string,
): Promise<LogLevel | null | Response> {
  let body: LevelBody;
  try {
  ...
  if (body.level === null) return null;
  if (!isLogLevel(body.level)) {
    return errorResponse(
      c,
      'BAD_REQUEST',
      'level must be one of debug, info, warn, error, fatal, or null to clear',
      400,
      correlationId,
    );
  }
...
export function adminLoggingRoutes(
  state: AppState,
): Hono<{ Variables: ServerVariables }> {
  const app = new Hono<{ Variables: ServerVariables }>();

  // Snapshot all overrides + the resolved-default level.
  app.get('/api/v1/admin/logging/levels', (c: AppCtx) => {
  ...
  // Global override.
  app.post('/api/v1/admin/logging/levels/global', async (c: AppCtx) => {
    const correlationId = correlationIdFor(c);
    const denied = requireAdmin(c, correlationId);
    if (denied) return denied;
    ...
    if (parsed === null) {
      ...
      return errorResponse(
        c,
        'BAD_REQUEST',
        'global level cannot be cleared; pass an explicit level',
        400,
        correlationId,
      );
    }
  ...
  app.post(
    '/api/v1/admin/logging/levels/module/:moduleId',
    async (c: AppCtx) => {
      const correlationId = correlationIdFor(c);
      const denied = requireAdmin(c, correlationId);
      if (denied) return denied;
  ...
  app.post(
    '/api/v1/admin/logging/levels/tenant/:tenantId',
    async (c: AppCtx) => {
      const correlationId = correlationIdFor(c);
      const denied = requireAdmin(c, correlationId);
      if (denied) return denied;
  ...
  app.post(
    '/api/v1/admin/logging/levels/correlation/:correlationId',
    async (c: AppCtx) => {
  ...
  // wraps this.
  app.get(
    '/api/v1/admin/logging/correlation/:correlationId/recent',
    (c: AppCtx) => {
      const requestCorrelationId = correlationIdFor(c);
      const denied = requireAdmin(c, requestCorrelationId);
      if (denied) return denied;
