...
import { handleOAuthIssueToken, handleOAuthRevokeToken, identityDispatcher, IdentityError, } from '@atlas/identity';
 ...
 */
function oauthError(c: AppCtx, error: string, description: string, status: OAuthStatus): Response {
    return c.json({ error, error_description: description }, status);
}
const ATLAS_TO_OAUTH_ERROR: Record<string, {
    code: string;
    status: OAuthStatus;
}> = {
...
export function oauthRoutes(state: AppState): Hono<{
    Variables: ServerVariables;
}> {
    const app = new Hono<{
        Variables: ServerVariables;
    ...
    app.post('/oauth/token', async function (c: AppCtx) {
        ...
        if (grantType !== 'client_credentials') {
            return oauthError(c, 'unsupported_grant_type', `grant_type must be client_credentials (got ${grantType ?? 'missing'})`, 400);
        }
    ...
    app.post('/oauth/revoke', async function (c: AppCtx) {
        const correlationId = correlationIdFor(c);
        const tenantId = tenantIdFromContext(c, state);
        if (!tenantId) {
            return oauthError(c, 'invalid_request', 'tenant unresolved', 400);
