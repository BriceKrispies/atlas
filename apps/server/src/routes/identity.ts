...
import { handleInviteAccept, handleSessionRefresh, handleSessionRevoke, identityDispatcher, getSessionEntity, listActiveSessionsForUser, IdentityError, } from '@atlas/identity';
...
import { buildClearSessionCookie, buildSessionCookie, parseSessionCookie, } from '../middleware/cookie.ts';
...
export function identityRoutes(state: AppState): Hono<{
    Variables: ServerVariables;
}> {
    const app = new Hono<{
        Variables: ServerVariables;
    }>();
    app.post('/api/v1/identity/invite/accept', async function (c: AppCtx) {
        ...
        if (!tenantId) {
            return errorResponse(c, 'SCHEMA_VALIDATION_FAILED', 'tenantId is required', 400, correlationId);
        }
        if (!acceptedEmail) {
            return errorResponse(c, 'SCHEMA_VALIDATION_FAILED', 'acceptedEmail is required', 400, correlationId);
        }
        if (!presentedToken) {
            return errorResponse(c, 'SCHEMA_VALIDATION_FAILED', 'presentedToken is required', 400, correlationId);
        }
        if (!acceptedEmail) {
    ...
    app.post('/api/v1/identity/session/refresh', async function (c: AppCtx) {
        const correlationId = correlationIdFor(c);
        const cookie = parseSessionCookie(c.req.header('cookie'));
        if (!cookie) {
            return errorResponse(c, 'SESSION_INVALID', 'session is not valid', 401, correlationId);
        }
        ...
        const tenantId = c.get('hostTenantId') ?? c.req.query('tenantId') ?? state.config.tenantId;
        if (!tenantId) {
            return errorResponse(c, 'PRINCIPAL_INVALID', 'cannot resolve tenant for refresh', 400, correlationId);
        }
        let sql: import('postgres').Sql;
        try {
            sql = await ensureTenantMigrated(state, tenantId);
        ...
        try {
            ...
            if (e instanceof IdentityError) {
                ...
                c.header('Set-Cookie', buildClearSessionCookie(!state.config.testAuth.enabled), { append: true });
                return errorResponse(c, e.code, e.message, e.status, correlationId);
            }
    ...
    app.post('/api/v1/identity/session/logout', async function (c: AppCtx) {
        const correlationId = correlationIdFor(c);
 ...
 * X-Debug-Principal.
 */
export function identityAuthedRoutes(state: AppState): Hono<{
    Variables: ServerVariables;
}> {
    const app = new Hono<{
        Variables: ServerVariables;
    }>();
    app.get('/api/v1/identity/sessions', async function (c: AppCtx) {
        const correlationId = correlationIdFor(c);
        const principal = c.get('principal');
        if (!principal?.userId) {
            return errorResponse(c, 'PRINCIPAL_INVALID', 'missing userId', 401, correlationId);
        }
        const sql = await ensureTenantMigrated(state, principal.tenantId);
        const entities = new PostgresEntityStore(sql);
        const sessions = await listActiveSessionsForUser(entities, principal.tenantId, principal.userId);
    ...
    app.delete('/api/v1/identity/sessions/:sessionId', async function (c: AppCtx) {
        const correlationId = correlationIdFor(c);
        const principal = c.get('principal');
        if (!principal?.userId) {
            return errorResponse(c, 'PRINCIPAL_INVALID', 'missing userId', 401, correlationId);
        ...
        if (!sessionId) {
            return errorResponse(c, 'SCHEMA_VALIDATION_FAILED', 'sessionId is required', 400, correlationId);
        }
        const sql = await ensureTenantMigrated(state, principal.tenantId);
        const eventStore = new PostgresEventStore(sql);
        const entities = new PostgresEntityStore(sql);
