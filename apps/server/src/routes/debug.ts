...
import { PostgresEventStore, PostgresSearchEngine, } from '@atlas/adapter-node';
...
function parseSearchDocument(body: IndexBody, forcedTenantId: string): SearchDocument | string {
    ...
    const fieldsRecord: Record<string, unknown> = Object.fromEntries(Object.entries(fields));
    ...
    if (body.permissionAttributes !== undefined && body.permissionAttributes !== null) {
        ...
        const allowedPrincipals = (pa as {
            allowedPrincipals?: unknown;
        }).allowedPrincipals;
        if (!Array.isArray(allowedPrincipals) ||
            !allowedPrincipals.every(function (x): x is string {
...
export function debugRoutes(state: AppState): Hono<{
    Variables: ServerVariables;
}> {
    const app = new Hono<{
        Variables: ServerVariables;
    }>();
    app.get('/debug/whoami', function (c: AppCtx) {
    ...
    // across tenants.
    app.get('/debug/events/:eventId', async function (c: AppCtx) {
    ...
    app.post('/debug/search/index', async function (c: AppCtx) {
        const correlationId = c.get('correlationId');
        const principal = c.get('principal');
        let body: IndexBody;
        try {
            ...
            return errorResponse(c, 'BAD_REQUEST', `Invalid JSON body: ${e instanceof Error ? e.message : String(e)}`, 400, correlationId);
        }
    ...
    app.post('/debug/search/rebuild', async function (c: AppCtx) {
        const correlationId = c.get('correlationId');
        const principal = c.get('principal');
        try {
            const sql = await ensureTenantMigrated(state, principal.tenantId);
    ...
    app.post('/debug/cache/clear', async function (c: AppCtx) {
        const correlationId = c.get('correlationId');
        const principal = c.get('principal');
        try {
            const sql = await ensureTenantMigrated(state, principal.tenantId);
