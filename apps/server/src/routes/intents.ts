...
export function intentRoutes(state: AppState): Hono<{
    Variables: ServerVariables;
}> {
    const app = new Hono<{
        Variables: ServerVariables;
    }>();
    app.post('/api/v1/intents', async function (c: AppCtx) {
        ...
        if (!isJsonObject(raw)) {
            ...
            return errorResponse(c, 'BAD_REQUEST', 'Invalid JSON body: expected an object', 400, correlationId);
        }
        ...
        const action = typeof envelope.payload === 'object' &&
