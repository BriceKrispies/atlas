...
import { listPages, getPage, getRenderTree, } from '@atlas/content-pages';
 ...
 */
async function checkPageRead(ingress: IngressState, resourceId: string): Promise<boolean> {
...
export function contentPagesRoutes(state: AppState): Hono<{
    Variables: ServerVariables;
}> {
    const app = new Hono<{
        Variables: ServerVariables;
    }>();
    app.get('/api/v1/pages', async function (c: AppCtx) {
        ...
        try {
            ...
            if (!allowed) {
                return errorResponse(c, 'UNAUTHORIZED', 'Not authorized to perform this action', 403, correlationId);
            }
    ...
    app.get('/api/v1/pages/:pageId', async function (c: AppCtx) {
        const correlationId = c.get('correlationId');
        const principal = c.get('principal');
        ...
        try {
            ...
            if (!doc) {
                return errorResponse(c, 'NOT_FOUND', `page not found: ${pageId}`, 404, correlationId);
            }
            return c.json(doc);
        }
        catch (e) {
    ...
    app.get('/api/v1/pages/:pageId/render-tree', async function (c: AppCtx) {
        const correlationId = c.get('correlationId');
        const principal = c.get('principal');
        const pageId = c.req.param('pageId') ?? '';
        try {
            ...
            if (!tree) {
                return errorResponse(c, 'NOT_FOUND', `render tree for page '${pageId}' not found`, 404, correlationId);
            }
            return c.json(tree);
        }
        catch (e) {
    ...
    app.get('/api/v1/pages/:pageId/render', async function (c: AppCtx) {
        const correlationId = c.get('correlationId');
        const principal = c.get('principal');
        const pageId = c.req.param('pageId') ?? '';
        try {
