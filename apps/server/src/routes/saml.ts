...
import { buildAuthnRequest, getIdentityProviderEntity, handleSamlAcs, identityDispatcher, IdentityError, listMetadataSamlSpKeys, } from '@atlas/identity';
...
export function samlRoutes(state: AppState): Hono<{
    Variables: ServerVariables;
}> {
    const app = new Hono<{
        Variables: ServerVariables;
    ...
    // ----- SP metadata -----------------------------------------------
    app.get('/sso/saml/:tenantId/metadata.xml', async function (c: AppCtx) {
        ...
        if (keys.length === 0) {
            return errorResponse(c, 'SAML_SP_KEY_NOT_FOUND', `tenant ${tenantId} has no SP signing key — generate one via Identity.SamlSpKey.Generate`, 404, correlationId);
        }
        const spEntityId = spEntityIdFor(host, tenantId);
        const acsUrl = acsUrlFor(host, tenantId);
        const keyDescriptors = keys
            .map(function (k) {
            return `<KeyDescriptor use="signing"><KeyInfo xmlns="http://www.w3.org/2000/09/xmldsig#"><X509Data><X509Certificate>${pemToBareBase64(k.publicCertPem)}</X509Certificate></X509Data></KeyInfo></KeyDescriptor>`;
        })
            .join('');
    ...
    // ----- SP-initiated flow ----------------------------------------
    app.get('/sso/saml/:tenantId/initiate', async function (c: AppCtx) {
        const correlationId = correlationIdFor(c);
        const tenantId = c.req.param('tenantId') ?? '';
    ...
    // ----- ACS callback ---------------------------------------------
    app.post('/sso/saml/:tenantId/acs', async function (c: AppCtx) {
        const correlationId = correlationIdFor(c);
        const tenantId = c.req.param('tenantId') ?? '';
        if (!tenantId) {
            return errorResponse(c, 'SCHEMA_VALIDATION_FAILED', 'tenantId required', 400, correlationId);
        ...
        const samlResponseB64 = typeof body['SAMLResponse'] === 'string' ? (body['SAMLResponse'] as string) : null;
        if (!samlResponseB64) {
            return errorResponse(c, 'SCHEMA_VALIDATION_FAILED', 'SAMLResponse required', 400, correlationId);
        }
        const relayState = typeof body['RelayState'] === 'string' ? (body['RelayState'] as string) : '';
