 ...
 */
async function readFrames(res: Response, expected: number, timeoutMs = 1000): Promise<Array<{
    event: string;
    data: string;
    id: string;
}>> {
    const reader = assertDefined(res.body, 'SSE response must have a body to stream').getReader();
...
describe('GET /api/v1/events', function () {
    ...
    test('streams a published event in SSE wire format', async function () {
    ...
    test('filters events by the principal tenantId', async function () {
        const res = await app.request('/api/v1/events', {
            headers: { 'X-Debug-Principal': 'user:alice:t1' },
        });
        expect(res.status).toBe(200);
    ...
    test('multiple concurrent subscribers each receive every event', async function () {
...
describe('serverEventDispatcher', function () {
    test('maps ContentPages.PageCreateRequested to projection.updated', async function () {
    ...
    test('emits cache.invalidated when cacheInvalidationTags is non-empty', async function () {
        const broadcast = new ServerEventBroadcast(8);
        const dispatch = serverEventDispatcher(broadcast);
        const { events, unsubscribe } = broadcast.subscribe();
        const envelope: EventEnvelope = {
