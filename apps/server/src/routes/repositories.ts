...
export function repositoryRoutes(
  ...
  app.get('/api/v1/repositories', async (c: AppCtx) => {
  ...
  app.get('/api/v1/repositories/:repoId', async (c: AppCtx) => {
    const correlationId = c.get('correlationId');
  ...
  app.get('/api/v1/repositories/:repoId/revisions', async (c: AppCtx) => {
    const correlationId = c.get('correlationId');
    const repoId = c.req.param('repoId') ?? '';
    try {
  ...
  app.get(
    '/api/v1/repositories/:repoId/revisions/:revisionId/bytes',
    async (c: AppCtx) => {
      const correlationId = c.get('correlationId');
      const repoId = c.req.param('repoId') ?? '';
      const revisionId = c.req.param('revisionId') ?? '';
      try {
        const { tenantId, repositories, revisions } = await buildStores(
          state,
          c,
        );
        ...
        if (!bytes) {
          ...
          return errorResponse(
            c,
            'NOT_FOUND',
            `revision bytes not found: ${revisionId}`,
            404,
            correlationId,
          );
        }
