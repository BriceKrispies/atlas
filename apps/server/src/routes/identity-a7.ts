...
function readRoleArray(v: unknown): string[] | null {
    ...
    for (const r of v) {
        ...
        if (trimmed.length === 0 || trimmed.length > ROLE_NAME_MAX_LEN)
            return null;
...
function readTenantId(v: unknown): string | null {
    if (typeof v !== 'string')
        return null;
    if (v.length === 0 || v.length > TENANT_ID_MAX_LEN)
        return null;
...
// ----------------------------------------------------------------------
function handleIdentityError(c: AppCtx, e: unknown, correlationId: string, verb: string): Response {
...
// ----------------------------------------------------------------------
export function identityA7Routes(state: AppState): Hono<{
    Variables: ServerVariables;
}> {
    const app = new Hono<{
        Variables: ServerVariables;
     ...
     */
    app.post('/api/v1/identity/impersonation/start', async function (c: AppCtx) {
     ...
     * operators).
     */
    app.post('/api/v1/identity/impersonation/end', async function (c: AppCtx) {
        const correlationId = correlationIdFor(c);
        const principal = c.get('principal');
        if (!principal) {
            return errorResponse(c, 'PRINCIPAL_INVALID', 'auth required', 401, correlationId);
     ...
     */
    app.post('/api/v1/identity/impersonation/revoke', async function (c: AppCtx) {
        const correlationId = correlationIdFor(c);
        const principal = c.get('principal');
        if (!principal) {
            return errorResponse(c, 'PRINCIPAL_INVALID', 'auth required', 401, correlationId);
     ...
     * pre-image).
     */
    app.get('/api/v1/identity/impersonation', async function (c: AppCtx) {
        const correlationId = correlationIdFor(c);
        const principal = c.get('principal');
        if (!principal) {
            return errorResponse(c, 'PRINCIPAL_INVALID', 'auth required', 401, correlationId);
     ...
     */
    app.post('/api/v1/identity/break-glass/issue', async function (c: AppCtx) {
        const correlationId = correlationIdFor(c);
        const principal = c.get('principal');
        if (!principal) {
            return errorResponse(c, 'PRINCIPAL_INVALID', 'auth required', 401, correlationId);
     ...
     * (handler enforces).
     */
    app.post('/api/v1/identity/break-glass/approve', async function (c: AppCtx) {
        const correlationId = correlationIdFor(c);
        const principal = c.get('principal');
        if (!principal) {
            return errorResponse(c, 'PRINCIPAL_INVALID', 'auth required', 401, correlationId);
     ...
     */
    app.post('/api/v1/identity/break-glass/deny', async function (c: AppCtx) {
        const correlationId = correlationIdFor(c);
        const principal = c.get('principal');
        if (!principal) {
            return errorResponse(c, 'PRINCIPAL_INVALID', 'auth required', 401, correlationId);
     ...
     */
    app.post('/api/v1/identity/break-glass/revoke', async function (c: AppCtx) {
        const correlationId = correlationIdFor(c);
        const principal = c.get('principal');
        if (!principal) {
            return errorResponse(c, 'PRINCIPAL_INVALID', 'auth required', 401, correlationId);
     ...
     */
    app.get('/api/v1/identity/break-glass', async function (c: AppCtx) {
        const correlationId = correlationIdFor(c);
        const principal = c.get('principal');
        if (!principal) {
            return errorResponse(c, 'PRINCIPAL_INVALID', 'auth required', 401, correlationId);
