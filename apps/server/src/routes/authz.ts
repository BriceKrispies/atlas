...
import { PostgresPolicyStore } from '@atlas/adapter-node';
...
export function authzRoutes(state: AppState): Hono<{
    Variables: ServerVariables;
}> {
    const app = new Hono<{
        Variables: ServerVariables;
    ...
    app.get('/api/v1/policies', async function (c: AppCtx) {
    ...
    app.get('/api/v1/policies/:version', async function (c: AppCtx) {
        const correlationId = c.get('correlationId');
        const principal = c.get('principal');
        const versionStr = c.req.param('version') ?? '';
        const version = Number.parseInt(versionStr, 10);
        if (!Number.isFinite(version) || version < 1) {
            return errorResponse(c, 'BAD_REQUEST', `Invalid version: ${versionStr}`, 400, correlationId);
        }
        try {
            const bundle = await buildRequestBundle(state, principal, correlationId);
            const decision = await evaluateRead({
            ...
            if (!detail) {
                return errorResponse(c, 'NOT_FOUND', `policy version not found: ${version}`, 404, correlationId);
            }
