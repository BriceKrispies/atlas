...
import { handleMembershipCreate, handleSessionRevokeAllForUser, handleUserCreate, identityDispatcher, findUserByEmail, getMembershipEntity, getUserEntity, listUsers, listMembershipsForTenant, putMembershipEntity, putUserEntity, IdentityError, newMembershipId, } from '@atlas/identity';
import type { MembershipDocument, UserDocument, } from '@atlas/identity';
...
function userToScim(user: UserDocument, membership?: MembershipDocument): Record<string, unknown> {
    return {
        ...
        ...(membership ? { groups: membership.roles.map(function (r) {
...
export function scimRoutes(state: AppState): Hono<{
    Variables: ServerVariables;
}> {
    const app = new Hono<{
        Variables: ServerVariables;
    ...
    // ----- DISCOVERY (A4.6) ------------------------------------------
    app.get('/scim/v2/ServiceProviderConfig', async function (c: AppCtx) {
        return c.json({
        ...
        }, 200, SCIM_RESPONSE_HEADERS);
    });
    app.get('/scim/v2/ResourceTypes', async function (c: AppCtx) {
    ...
    app.get('/scim/v2/Schemas', async function (c: AppCtx) {
        return c.json(scimListResponse([
    ...
    // ----- USERS (A4.4) ---------------------------------------------
    app.get('/scim/v2/Users', async function (c: AppCtx) {
        ...
        const membByUser = new Map(memberships.map(function (m) {
            return [m.userId, m];
        }));
        return c.json(scimListResponse(filtered.map(function (u) {
            return userToScim(u, membByUser.get(u.userId));
        })), 200, SCIM_RESPONSE_HEADERS);
    });
    app.get('/scim/v2/Users/:userId', async function (c: AppCtx) {
        const principal = c.get('principal');
        ...
        if (!user)
            return scimError(c, 404, `user not found: ${userId}`);
    ...
    app.post('/scim/v2/Users', async function (c: AppCtx) {
        const principal = c.get('principal');
        ...
        try {
            ...
            return c.json(userToScim(userResult.document, membershipResult.document), 201, SCIM_RESPONSE_HEADERS);
        }
        catch (e) {
    ...
    app.patch('/scim/v2/Users/:userId', async function (c: AppCtx) {
        const principal = c.get('principal');
        const userId = c.req.param('userId') ?? '';
        const body = await readBodyObject(c);
        ...
        for (const op of ops) {
            ...
            if (operation !== 'replace' && operation !== 'add') {
                return scimError(c, 400, `unsupported op '${operation}' (only add/replace)`, 'invalidSyntax');
            }
            if (path === 'active') {
    ...
    app.delete('/scim/v2/Users/:userId', async function (c: AppCtx) {
        const principal = c.get('principal');
        const userId = c.req.param('userId') ?? '';
        const sql = await ensureSql(state, principal.tenantId);
        const entities = new PostgresEntityStore(sql);
        ...
        await putUserEntity(entities, { ...user, status: 'deprovisioned', updatedAt: occurredAt }, principal.tenantId);
        const membership = await getMembershipEntity(entities, principal.tenantId, userId);
        if (membership) {
            await putMembershipEntity(entities, {
                ...membership,
    ...
    // Memberships.
    app.get('/scim/v2/Groups', async function (c: AppCtx) {
        const principal = c.get('principal');
        const sql = await ensureSql(state, principal.tenantId);
        const entities = new PostgresEntityStore(sql);
        const memberships = await listMembershipsForTenant(entities, principal.tenantId);
        ...
        for (const m of memberships)
            for (const r of m.roles)
                roleSet.add(r);
        const groups = Array.from(roleSet).map(function (role) {
            return ({
                ...
                members: memberships
                    ...
                    .map(function (m) {
                    return ({ value: m.userId });
                }),
            });
        });
        return c.json(scimListResponse(groups), 200, SCIM_RESPONSE_HEADERS);
    });
    app.patch('/scim/v2/Groups/:groupId', async function (c: AppCtx) {
        const principal = c.get('principal');
        ...
        for (const op of ops) {
            ...
            for (const memberRef of members) {
                ...
                if (operation === 'add' && !has) {
                    ...
                    nextRoles = membership.roles.filter(function (r) {
                        return r !== groupId;
                    });
                }
