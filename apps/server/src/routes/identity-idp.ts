...
import { handleIdpActivate, handleIdpConfigure, handleIdpDisable, handleIdpRotateJwks, identityDispatcher, listIdentityProviders, IdentityError, type IdpConfigureCommand, } from '@atlas/identity';
 ...
 */
function readRoleMapping(v: unknown): NonNullable<IdpConfigureCommand['roleMappings']>[number] | null {
    if (!isJsonObject(v))
        return null;
    ...
    if (!Array.isArray(rolesRaw))
        return null;
    const roles = rolesRaw.filter(function (r): r is string {
        return typeof r === 'string';
    });
    return { group, roles };
}
function readRoleMappings(v: unknown): NonNullable<IdpConfigureCommand['roleMappings']> | null {
    if (!Array.isArray(v))
        return null;
...
export function identityIdpRoutes(state: AppState): Hono<{
    Variables: ServerVariables;
}> {
    const app = new Hono<{
        Variables: ServerVariables;
    }>();
    app.get('/api/v1/identity/idps', async function (c: AppCtx) {
    ...
    app.post('/api/v1/identity/idps', async function (c: AppCtx) {
        const correlationId = correlationIdFor(c);
        const principal = c.get('principal');
        if (!principal) {
            return errorResponse(c, 'PRINCIPAL_INVALID', 'auth required', 401, correlationId);
        ...
        if (!displayName || !issuer || !audience) {
            return errorResponse(c, 'SCHEMA_VALIDATION_FAILED', 'displayName, issuer, audience required', 400, correlationId);
        }
        ...
        const requireInvite = typeof requireInviteRaw === 'boolean' ? requireInviteRaw : null;
        const defaultRolesRaw = body['defaultRolesOnFirstLogin'];
        const defaultRoles = Array.isArray(defaultRolesRaw)
            ? defaultRolesRaw.filter(function (v): v is string {
    ...
    app.post('/api/v1/identity/idps/:idpId/activate', async function (c: AppCtx) {
        const correlationId = correlationIdFor(c);
        const principal = c.get('principal');
        if (!principal) {
            return errorResponse(c, 'PRINCIPAL_INVALID', 'auth required', 401, correlationId);
    ...
    app.post('/api/v1/identity/idps/:idpId/disable', async function (c: AppCtx) {
        const correlationId = correlationIdFor(c);
        const principal = c.get('principal');
        if (!principal) {
            return errorResponse(c, 'PRINCIPAL_INVALID', 'auth required', 401, correlationId);
    ...
    app.post('/api/v1/identity/idps/:idpId/rotate-jwks', async function (c: AppCtx) {
        const correlationId = correlationIdFor(c);
        const principal = c.get('principal');
        if (!principal) {
            return errorResponse(c, 'PRINCIPAL_INVALID', 'auth required', 401, correlationId);
