...
function readNumber(v: unknown, label: string): number {
    if (typeof v !== 'number')
        throw new Error(`Test invariant: ${label} not number`);
    return v;
}
function readString(v: unknown, label: string): string {
    if (typeof v !== 'string')
        throw new Error(`Test invariant: ${label} not string`);
    return v;
}
...
describe('admin-logging route — gate', function () {
    it('rejects anonymous (no principal) with 401', async function () {
        const { app } = makeRig({ principal: 'anonymous' });
        const r = await app.request('/api/v1/admin/logging/levels');
        expect(r.status).toBe(401);
    });
    it('rejects non-admin authenticated with 403', async function () {
        const { app } = makeRig({ principal: 'plain' });
        const r = await app.request('/api/v1/admin/logging/levels');
        expect(r.status).toBe(403);
    });
    it('admin gets through with 200', async function () {
        const { app } = makeRig({ principal: 'admin' });
        const r = await app.request('/api/v1/admin/logging/levels');
        expect(r.status).toBe(200);
    });
});
describe('admin-logging route — levels snapshot', function () {
    it('GET /levels returns the snapshot shape', async function () {
        const { app } = makeRig({ principal: 'admin' });
        const r = await app.request('/api/v1/admin/logging/levels');
        expect(r.status).toBe(200);
...
describe('admin-logging route — set / clear', function () {
    it('POST /levels/global sets the global level', async function () {
    ...
    it('POST /levels/global rejects null with 400', async function () {
        const { app } = makeRig({ principal: 'admin' });
        const r = await app.request('/api/v1/admin/logging/levels/global', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    it('POST /levels/global rejects invalid level with 400', async function () {
        const { app } = makeRig({ principal: 'admin' });
        const r = await app.request('/api/v1/admin/logging/levels/global', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    it('POST /levels/module/:id sets, then null clears', async function () {
        const { app, levelController } = makeRig({ principal: 'admin' });
    ...
    it('POST /levels/tenant/:id sets, null clears', async function () {
        const { app, levelController } = makeRig({ principal: 'admin' });
    ...
    it('POST /levels/correlation/:id sets, null clears', async function () {
        const { app, levelController } = makeRig({ principal: 'admin' });
    ...
    it('rejects malformed JSON body with 400', async function () {
        const { app } = makeRig({ principal: 'admin' });
        const r = await app.request('/api/v1/admin/logging/levels/global', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
describe('admin-logging route — inspect', function () {
    ...
    it('returns events filtered to the correlationId', async function () {
        ...
        expect(readEvents(body['events']).map(function (e) {
            return e.message;
        })).toEqual(['two', 'one']);
    });
    it('honors ?limit', async function () {
        const { app, inspectionSink } = makeRig({ principal: 'admin' });
        for (let i = 0; i < 10; i++)
            inspectionSink.write(makeEvent('corr-A', `m${i}`));
    ...
    it('returns empty array when correlationId not seen', async function () {
        const { app } = makeRig({ principal: 'admin' });
