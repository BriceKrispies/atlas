...
import { getTaxonomyNodes, getFamilyDetail, getVariantTable, searchCatalog, parseFilterQuery, type SearchParams, type VariantTableParams, } from '@atlas/catalog';
...
export function catalogRoutes(state: AppState): Hono<{
    ...
    app.get('/api/v1/catalog/taxonomies/:treeKey/nodes', async function (c: AppCtx) {
        ...
        try {
            ...
            if (!result) {
                return errorResponse(c, 'NOT_FOUND', `taxonomy tree '${treeKey}' not found`, 404, correlationId);
            }
    ...
    app.get('/api/v1/catalog/families/:familyKey', async function (c: AppCtx) {
        const correlationId = c.get('correlationId');
        const principal = c.get('principal');
        ...
        try {
            ...
            if (!result) {
                return errorResponse(c, 'NOT_FOUND', `family '${familyKey}' not found`, 404, correlationId);
            }
            return c.json(result);
        }
        catch (e) {
    ...
    app.get('/api/v1/catalog/families/:familyKey/variants', async function (c: AppCtx) {
        const correlationId = c.get('correlationId');
        const principal = c.get('principal');
        const familyKey = c.req.param('familyKey') ?? '';
    ...
    app.get('/api/v1/catalog/search', async function (c: AppCtx) {
        const correlationId = c.get('correlationId');
        const principal = c.get('principal');
        ...
        if (q.length === 0) {
            return errorResponse(c, 'INVALID_QUERY', 'Query parameter "q" must be non-empty', 400, correlationId);
        }
