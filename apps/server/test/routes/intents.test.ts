...
// ======================================================================
describe('POST /api/v1/intents — authn precondition', function () {
    test('returns 401 PRINCIPAL_REQUIRED when no principal was set by upstream middleware', async function () {
        const { state, collector } = buildFakeAppState();
        const app = buildApp(state, {});
        installBundle(buildFakeBundle({ state, correlationId: 'corr-test', principalId: PRINCIPAL }));
        ...
        const rejected = collector.events.find(function (e) {
            return e.eventName === 'Intent.Rejected';
        });
        expect(rejected).toBeDefined();
        expect((rejected!.properties as {
            code: string;
        }).code).toBe('PRINCIPAL_REQUIRED');
    });
});
// ======================================================================
// Body parse / required headers.
// ======================================================================
describe('POST /api/v1/intents — body validation', function () {
    test('invalid JSON body → 400 BAD_REQUEST', async function () {
        const { state, collector } = buildFakeAppState();
    ...
    test('missing idempotencyKey → 400 INVALID_IDEMPOTENCY_KEY', async function () {
        const { state, collector } = buildFakeAppState();
        installBundle(buildFakeBundle({ state, correlationId: 'corr-1', principalId: PRINCIPAL }));
        const app = buildApp(state, {
            principal: { principalId: PRINCIPAL, tenantId: TENANT },
...
// ======================================================================
describe('POST /api/v1/intents — schema validation', function () {
    test('unknown action / schema → 400', async function () {
    ...
    test('schema validator returns false → 400 SCHEMA_VALIDATION_FAILED with truncated reason', async function () {
        const { state, collector } = buildFakeAppState();
        ...
        expect(rejected).toBeDefined();
        const reason = (rejected!.properties as {
            reason: string;
        }).reason;
...
// ======================================================================
describe('POST /api/v1/intents — authz (Invariant I2)', function () {
    test('deny → 403, NO event appended, NO dispatch invoked', async function () {
        const { state, collector } = buildFakeAppState();
        ...
        // Intent.Rejected log line was emitted.
        expect(collector.events.some(function (e) {
            return e.eventName === 'Intent.Rejected';
        })).toBe(true);
    });
    test('permit → 202 with eventId + correlationId', async function () {
        const { state, collector } = buildFakeAppState();
        ...
        const accepted = collector.events.find(function (e) {
            return e.eventName === 'Intent.Accepted';
        });
        expect(accepted).toBeDefined();
        expect((accepted!.properties as {
            actionId: string;
        }).actionId).toBe(ACTION_ID);
    });
});
// ======================================================================
// Idempotency replay — Invariant I3.
// ======================================================================
describe('POST /api/v1/intents — idempotency (Invariant I3)', function () {
    test('replay of the same idempotencyKey returns the prior eventId, no new events', async function () {
        const { state } = buildFakeAppState();
...
// ======================================================================
describe('POST /api/v1/intents — correlation-id propagation (Invariant I5)', function () {
    test('inbound X-Correlation-Id header is honored on the response logger', async function () {
        const { state, collector } = buildFakeAppState();
        const bundle = buildFakeBundle({
            state,
        ...
        const submitted = collector.events.find(function (e) {
            return e.eventName === 'Intent.Submitted';
        });
        const accepted = collector.events.find(function (e) {
            return e.eventName === 'Intent.Accepted';
        });
...
// ======================================================================
describe('POST /api/v1/intents — bundle build failure', function () {
    test('buildRequestBundle throws → 500 with BUNDLE_BUILD_FAILED log line', async function () {
        const { state, collector } = buildFakeAppState();
