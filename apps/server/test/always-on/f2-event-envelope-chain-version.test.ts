...
describe('F2 — event envelope dispatcherChainVersion (always-on §4.2 / I5)', () => {
  test('JSON schema MUST declare a dispatcherChainVersion property', () => {
    ...
    expect(
      ...
      'event_envelope.schema.json must allow dispatcherChainVersion',
    ).toContain('dispatcherChainVersion');

    expect(
      schema.required,
      'dispatcherChainVersion must be required so an append cannot forget it',
    ).toContain('dispatcherChainVersion');
  });

  test('TypeScript EventEnvelope type MUST carry dispatcherChainVersion', () => {
    ...
    expect(
      Object.prototype.hasOwnProperty.call(envelope, 'dispatcherChainVersion'),
      'EventEnvelope construction must stamp dispatcherChainVersion at append time',
    ).toBe(true);
