...
export function makeFakeCache(overrides: Partial<Cache> = {}): Cache {
  const base: Cache = {
    ...
    set: vi.fn(async () => {}),
    invalidateByKey: vi.fn(async () => false),
    invalidateByTags: vi.fn(async () => 0),
  };
...
export function makeFakeProjections(
  overrides: Partial<ProjectionStore> = {},
): ProjectionStore {
...
export function makeFakeSearch(
  overrides: Partial<SearchEngine> = {},
): SearchEngine {
...
export function makeFakeCatalogState(
  overrides: Partial<CatalogStateStore> = {},
): CatalogStateStore {
...
export function makeFakeEntityStore(
  overrides: Partial<EntityStore> = {},
): EntityStore {
  const base: EntityStore = {
    ...
    async put<TAttrs = unknown>(
      input: import('@atlas/ports').EntityWriteInput<TAttrs>,
...
export function makeFakeRelationStore(
  overrides: Partial<RelationStore> = {},
): RelationStore {
  const base: RelationStore = {
    async add<TAttrs = unknown>(
      input: import('@atlas/ports').RelationWriteInput<TAttrs>,
...
export function makeFakeEventStore(
...
export function buildFakeAppState(opts: FakeAppStateOptions = {}): FakeAppState {
  ...
  const notWired = <T>(field: string): T => {
    const handler: ProxyHandler<object> = {
      get(_target, prop) {
        throw new Error(
          `AppState.${field}.${String(prop)} accessed in test — buildFakeAppState does not wire ${field}; pass a real value if the test needs it`,
        );
      },
    };
