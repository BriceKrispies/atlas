...
export async function runPush(
  ...
  if (tarBytes.byteLength > MAX_BYTES) {
    return fail(
      client,
      flags,
      `tarball is ${tarBytes.byteLength} bytes; exceeds the ${MAX_BYTES}-byte (10 MB) Phase 1 cap`,
      'UPLOAD_TOO_LARGE',
    );
  }
  ...
  if (!isValidSlug(repoSlug)) {
    return fail(
      client,
      flags,
      `repoSlug "${repoSlug}" is not kebab-case (lowercase letters/digits/hyphens, 1-63 chars)`,
      'BAD_INPUT',
    );
  }
  ...
  if (tenantId === null || tenantId === '') {
    return fail(
      client,
      flags,
      'tenantId not resolvable: pass --tenant <id>, set ATLAS_TENANT_ID, or include the tenant in --debug-principal (user:id:tenantId)',
      'BAD_INPUT',
    );
  }
  ...
  if (!createOk && !createConflict) {
    return fail(
      client,
      flags,
      `Repository.Create failed (HTTP ${createRes.status}): ${describeError(createRes.body)}`,
      extractErrorCode(createRes.body) ?? 'CREATE_FAILED',
      createRes.correlationId,
    );
  }
  ...
  if (repoId === null) {
    return fail(
      client,
      flags,
      `Repository.Create returned ${createOk ? '202' : 'conflict'} but slug "${repoSlug}" was not found via GET /api/v1/repositories — server projection lag or routing not wired yet`,
      'REPO_NOT_FOUND',
      createRes.correlationId,
    );
  }
  ...
  if (uploadRes.status < 200 || uploadRes.status >= 300) {
    return fail(
      client,
      flags,
      `Repository.Upload failed (HTTP ${uploadRes.status}): ${describeError(uploadRes.body)}`,
      extractErrorCode(uploadRes.body) ?? 'UPLOAD_FAILED',
      uploadRes.correlationId,
    );
  }
  ...
  if (detail !== null) {
    ...
    if (latest !== null) revisionId = latest;
  }
  ...
  if (flags.json) {
    ...
    process.stdout.write(
      `pushed: ${repoSlug} revision ${revisionId} (${tarBytes.byteLength} bytes)\n`,
    );
  }
  return 0;
}
...
async function resolveRepoId(
  ...
  for (const r of list) {
    if (r.repoSlug === repoSlug) return r.repoId;
  }
  return null;
}
    ...
    await tarCreate(
      {
        ...
        cwd: dirAbs,
        file: outPath,
        filter: (relPath: string): boolean => shouldInclude(relPath),
      },
      ['.'],
    );
