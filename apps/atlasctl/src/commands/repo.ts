...
export async function runRepoList(client: ClientOptions, flags: OutputFlags): Promise<number> {
...
export async function runRepoShow(client: ClientOptions, slug: string, flags: OutputFlags): Promise<number> {
    ...
    const detail = detailRes.status >= 200 && detailRes.status < 300 ? detailRes.body : repo;
    const correlationId = detailRes.status >= 200 && detailRes.status < 300
    ...
    if (str(obj['name']))
        lines.push(`name:           ${str(obj['name'])}`);
    if (str(obj['description']))
        lines.push(`description:    ${str(obj['description'])}`);
    if (str(obj['createdAt']))
        lines.push(`createdAt:      ${str(obj['createdAt'])}`);
    if (str(obj['createdBy']))
        lines.push(`createdBy:      ${str(obj['createdBy'])}`);
...
export async function runRepoDownload(client: ClientOptions, slug: string, revisionFlag: string | undefined, outFlag: string | undefined, flags: OutputFlags): Promise<number> {
    const repo = await findBySlug(client, slug);
    if (repo === null) {
        emitResult(flags, {
            correlationId: client.correlationId,
    ...
    const url = new URL(`/api/v1/repositories/${encodeURIComponent(repo.repoId)}/revisions/${encodeURIComponent(revisionId)}/bytes`, client.endpoint).toString();
...
async function findBySlug(client: ClientOptions, slug: string): Promise<RepoSummary | null> {
    const res = await request(client, { method: 'GET', path: '/api/v1/repositories' });
    if (res.status < 200 || res.status >= 300)
        return null;
    const list = readFullRepoArray(res.body);
    return list.find(function (r) {
        return r.repoSlug === slug;
    }) ?? null;
}
async function fetchLatestRevisionId(client: ClientOptions, repoId: string): Promise<string | null> {
    try {
...
function resolveOutPath(outFlag: string | undefined, slug: string, revisionId: string): string {
...
function readFullRepoArray(body: unknown): RepoSummary[] {
    ...
    if (arr === null) {
        ...
        return readRepoArray(body).map(function (r) {
            return ({
                repoId: r.repoId,
                repoSlug: r.repoSlug,
            });
    ...
    for (const e of arr) {
        ...
        if (latest !== null)
            entry.latestRevisionId = latest;
        const pushedAt = str(r['latestPushedAt']);
        if (pushedAt !== null)
            entry.latestPushedAt = pushedAt;
        if (typeof r['totalRevisions'] === 'number')
            entry.totalRevisions = r['totalRevisions'];
        if (typeof r['byteCount'] === 'number')
            entry.byteCount = r['byteCount'];
        out.push(entry);
    }
    return out;
}
...
function formatTable(rows: string[][]): string {
    ...
    const lines: string[] = [];
    for (const row of rows) {
        const padded = row.map(function (cell, i) {
            return cell.padEnd(widths[i] ?? cell.length);
        });
