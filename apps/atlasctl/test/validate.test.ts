...
function captureStdio<T>(fn: () => T): {
    ...
    process.stdout.write = (function (chunk: string | Uint8Array) {
        out.write(chunk);
        return true;
...
describe('runValidate', function () {
    it('returns 0 for a well-formed envelope', function () {
        const path = tmpJson(validEnvelope);
        const { result, out } = captureStdio(function () {
            return runValidate({ file: path }, { json: true, quiet: false });
        });
        if (result !== 0) {
    ...
    it('returns 1 when a required envelope field is missing', function () {
    ...
    it('returns 2 for malformed JSON input', function () {
        const dir = mkdtempSync(join(tmpdir(), 'atlasctl-validate-'));
