...
describe('atlasctl deps (INV-CTL-01)', () => {
  it('package.json declares no forbidden runtime dependencies', () => {
    const pkg = JSON.parse(
      readFileSync(join(PKG_ROOT, 'package.json'), 'utf-8'),
    ) as PackageJson;
    const declared = Object.keys(pkg.dependencies ?? {});
    const violations = declared.filter((d) => FORBIDDEN_DEPS.includes(d));
    expect(violations).toEqual([]);
  });

  it('source files do not import from forbidden packages or paths', () => {
    ...
    for (const file of sources) {
      ...
      while ((m = importRe.exec(text)) !== null) {
        ...
        if (spec.startsWith('.')) continue; // relative: handled by path-prefix check below
