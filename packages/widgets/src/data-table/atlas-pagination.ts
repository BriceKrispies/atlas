...
export class AtlasPagination extends AtlasElement {
    ...
    set pageSizeOptions(opts: number[]) {
        if (!Array.isArray(opts))
            return;
        this._pageSizeOptions = opts.map(function (n) {
            return clampInt(n, 1, Number.MAX_SAFE_INTEGER);
        });
        this._update();
    }
    // ── Lifecycle ────────────────────────────────────────────────
    ...
    _syncDisabled(canPrev: boolean, canNext: boolean): void {
        ...
        if (first)
            setBoolAttr(first, 'disabled', !canPrev);
        if (prev)
            setBoolAttr(prev, 'disabled', !canPrev);
        if (next)
            setBoolAttr(next, 'disabled', !canNext);
        if (last)
            setBoolAttr(last, 'disabled', !canNext);
    }
    _goto(n: number): void {
        const next = clampInt(n, 0, Math.max(0, this._pageCount - 1));
        if (next === this._page)
