...
import { DataTableCore, STATUS, type AnyColumn, type Columns, type DataTableState, } from './data-table-core.ts';
...
class AtlasDataTable<R extends Row = Row> extends AtlasElement {
    ...
    _renderToolbar(): void {
        ...
        if (!toolbar)
            return;
        toolbar.textContent = '';
        const filterable = this._columns.filter(function (c) {
            return c?.filter;
        });
    ...
    _renderFilteredEmpty(): void {
        ...
        clear.addEventListener('click', () => {
            ...
            for (const input of this.querySelectorAll('atlas-input')) {
                if (!input.hasAttribute('data-column-key'))
                    continue;
                input.value = '';
    ...
    _renderSuccess(state: DataTableState<R>): void {
        ...
        for (const col of this._columns) {
            ...
            if (col.sortable) {
                ...
                cell.setAttribute('aria-sort', sortDir === 'asc' ? 'ascending' :
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            rowEl.setAttribute('aria-rowindex', String(state.page * (state.pageSize || totalRows) + i + 1));
            for (const col of this._columns) {
                const cell = document.createElement('atlas-table-cell');
                if (col.align)
                    cell.dataset['align'] = col.align;
            ...
            if (state.selectionMode !== 'none') {
                ...
                rowEl.addEventListener('click', (event: Event) => {
                    ...
                    if (target !== rowEl && target?.closest('atlas-button,a,input,select'))
                        return;
    ...
    _onFilterInput(_event: Event): void { }
