...
export function filterRows<R extends Row>(
  ...
  columns: ReadonlyArray<FilterableColumn<R, unknown>>,
): R[] {
...
function readColumnValue<R extends Row>(
  row: R,
  column: FilterableColumn<R, unknown>,
): unknown {
...
function matchFilter<R extends Row>(
  config: FilterConfig<R, unknown> | undefined,
  filterValue: unknown,
  rowValue: unknown,
  row: R,
): boolean {
...
function matchRange(filter: unknown, value: unknown): boolean {
  ...
  if (filter && typeof filter === 'object') {
    const obj = filter as { min?: unknown; max?: unknown };
    min = obj.min;
    max = obj.max;
  }
