...
export type RowKey<R extends Row = Row> = string | ((row: R) => string | number);
export function keyOf<R extends Row>(row: R, rowKey: RowKey<R> | undefined): string | number {
    ...
    if (typeof raw === 'string' || typeof raw === 'number')
        return raw;
    throw new Error(`keyOf: row key ${typeof rowKey === 'string' ? rowKey : 'id'} is ${typeof raw}, expected string|number`);
}
/**
 ...
 */
export function applyPatch<R extends Row>(rows: R[], patch: RowPatch<R> | null | undefined, rowKey: RowKey<R> | undefined): R[] {
 ...
 */
export function diff<R extends Row>(prev: R[], next: R[], rowKey: RowKey<R> | undefined): RowPatch<R>[] {
    const prevByKey = new Map<string | number, R>();
    for (const r of prev)
        prevByKey.set(keyOf(r, rowKey), r);
    ...
    for (const [k] of prevByKey) {
        if (!nextKeys.has(k))
            patches.push({ type: 'remove', rowKey: k });
    }
    for (const r of next) {
