 ...
 */
import type { Slice } from '../data-normalize.ts';
...
export function renderPie(opts: PieRendererOptions): SVGGElement {
    ...
    const total = slices.reduce(function (n, s) {
        return n + (Number.isFinite(s.value) ? s.value : 0);
    }, 0);
