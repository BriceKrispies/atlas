...
export function renderLine(opts: LineRendererOptions): SVGGElement {
    ...
    const xOf = function (v: PointX): number {
        return xScale.scale(v) + bandwidth / 2;
    };
    ...
    series.forEach(function (s, i) {
        ...
        const d = pathString(s.values.map(function (p: Point): [
            number,
            number
        ] {
            return [xOf(p.x), yScale.scale(p.y)];
        }), mode === 'area', baselineY);
...
function pathString(points: Array<[
    number,
    number
]>, area: boolean, baselineY: number): string {
    if (points.length === 0)
        return '';
    let d = '';
