    ...
    _apply(intent: ChartIntent): void {
        switch (intent.kind) {
            ...
            case 'toggleSeries': {
                ...
                if (intent.hidden)
                    next.add(intent.seriesId);
                else
                    next.delete(intent.seriesId);
                this._hiddenSeries.set(next);
                return;
            }
            case 'setFilter': {
                const existing = this._filters.value.filter(function (f) {
                    return f.field !== intent.field;
                });
                this._filters.set([
                    ...existing,
            ...
            case 'clearFilter': {
                this._filters.set(this._filters.value.filter(function (f) {
                    return f.field !== intent.field;
                }));
                return;
            }
            case 'setTimeRange': {
                this._timeRange.set(intent.preset
    ...
    _deriveData(): ChartDataSet | null {
        ...
        if (tr || filters.length > 0) {
            series = series.map(function (s) {
                return ({
                    ...s,
                    values: s.values.filter(function (p) {
                        return inTimeRange(p.x, tr) && passesFilters(p, filters);
                    }),
                });
            });
        }
...
function inTimeRange(x: PointX, tr: ChartTimeRange | null): boolean {
    ...
    if (!Number.isFinite(t))
        return true;
    if (tr.from && t < new Date(tr.from).getTime())
        return false;
    if (tr.to && t > new Date(tr.to).getTime())
        return false;
    if (tr.preset) {
        const now = Date.now();
        const windowMs = presetWindowMs(tr.preset);
        if (windowMs != null && t < now - windowMs)
            return false;
    }
    return true;
}
...
function isRecord(v: unknown): v is {
    [k: string]: unknown;
} {
