...
function fakeBackend({ response, subscribe = true }: {
    ...
    if (subscribe) {
        backend.subscribe = function (eventType: string, cb: (e: unknown) => void) {
    ...
    return {
        backend,
        subs,
        emit: function (eventType: string, data: unknown): void {
            for (const s of subs)
...
function subscribeOrThrow<R extends Row>(ds: {
    subscribe?: (cb: (p: RowPatch<R>) => void) => () => void;
}, cb: (p: RowPatch<R>) => void): () => void {
...
describe('queryDataSource', function () {
    it('fetchAll normalizes array response', async function () {
    ...
    it('fetchAll normalizes envelope response', async function () {
    ...
    it('fetchAll on unexpected payload returns []', async function () {
    ...
    it('honours onEvent converter', function () {
        const { backend, emit } = fakeBackend({ response: [] });
        const ds = queryDataSource(backend, '/pages', {
            resourceType: 'page',
            onEvent: function (ev) {
        ...
        if (upsert.type !== 'upsert')
            throw new Error('test invariant: expected upsert patch');
        ...
        if (remove.type !== 'remove')
            throw new Error('test invariant: expected remove patch');
        expect(remove.rowKey).toBe('a');
    });
    it('subscribe unsubscribe detaches listener', function () {
        const { backend, subs, emit } = fakeBackend({ response: [] });
        const ds = queryDataSource(backend, '/pages');
        const patches: RowPatch[] = [];
        const unsub = subscribeOrThrow(ds, function (p) {
            return patches.push(p);
        });
    ...
    it('capabilities includes stream only when subscribe is available', function () {
