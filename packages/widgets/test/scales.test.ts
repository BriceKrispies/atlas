...
describe('scales', function () {
    ...
    it('bandScale produces evenly spaced bands with padding', function () {
    ...
    it('bandScale honours padding', function () {
    ...
    it('bandScale unknown key falls back to range[0]', function () {
        const s = bandScale({ domain: ['a', 'b'], range: [10, 20] });
        expect(s.scale('missing')).toBe(10);
    });
    it('timeScale normalizes Date|string|number', function () {
    ...
    it('timeScale invert returns Date', function () {
        const s = timeScale({
    ...
    it('toMs handles all accepted formats', function () {
