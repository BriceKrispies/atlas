...
describe('filterRows', function () {
    it('text filter is case-insensitive substring', function () {
    ...
    it('blank filter values are ignored', function () {
        const out = filterRows(rows, { title: '   ', status: '' }, [
            { key: 'title' }, { key: 'status' },
        ]);
        expect(out.length).toBe(3);
    });
    it('eq filter is strict string equality', function () {
    ...
    it('select filter with array is "in"', function () {
        const out = filterRows(rows, { status: ['draft', 'published'] }, [
            { key: 'status', filter: { type: 'select' } },
        ]);
        expect(out.length).toBe(3);
    });
    it('select filter with empty array is ignored', function () {
        const out = filterRows(rows, { status: [] }, [
            { key: 'status', filter: { type: 'select' } },
        ]);
        expect(out.length).toBe(3);
    });
    it('range filter inclusive', function () {
    ...
    it('range filter min-only', function () {
    ...
    it('range filter blank object is ignored', function () {
        const out = filterRows(rows, { score: { min: null, max: null } }, [
            { key: 'score', filter: { type: 'range' } },
        ]);
        expect(out.length).toBe(3);
    });
    it('custom filter uses provided matches()', function () {
        const out = filterRows(rows, { score: 15 }, [
            { key: 'score', filter: { type: 'custom', matches: function (f, v) {
    ...
    it('function column accessor is honoured', function () {
