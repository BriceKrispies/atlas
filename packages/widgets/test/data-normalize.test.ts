...
describe('data-normalize', function () {
    it('normalize array of numbers → single series with index x', function () {
        ...
        expect(series0.values.map(function (p) {
            return p.y;
        })).toEqual([1, 2, 3]);
        expect(series0.values.map(function (p) {
            return p.x;
    ...
    it('normalize array of [x, y] tuples', function () {
    ...
    it('normalize array of {x, y} objects', function () {
        const out = normalize([{ x: 'a', y: 1 }, { x: 'b', y: 2 }], 'series');
        expect(out.xKind).toBe('band');
    });
    it('normalize canonical {series:[…]}', function () {
    ...
    it('normalize detects time xKind on Date values', function () {
        const out = normalize({ series: [{ values: [{ x: new Date('2026-01-01'), y: 1 }] }] }, 'series');
        expect(out.xKind).toBe('time');
    });
    it('normalize detects time xKind on ISO date strings', function () {
        const out = normalize({ series: [{ values: [{ x: '2026-01-01', y: 1 }] }] }, 'series');
        expect(out.xKind).toBe('time');
    });
    it('normalize slices from array', function () {
    ...
    it('normalize slices from {slices: [...]}', function () {
    ...
    it('normalize slices filters out non-finite values', function () {
    ...
    it('normalize empty input returns empty series', function () {
