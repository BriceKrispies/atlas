...
describe('DataTableCore', function () {
    it('initial state is idle with empty rows', function () {
        const core = makeCore();
    ...
    it('setSort cycles asc → desc → none, resets page', function () {
        const core = makeCore();
        core.setAllRows(SAMPLE);
        core.setPage(1);
        ...
        expect(s.page).toBe(0);
        expect(core.sortedRows().map(function (r) {
            return r.title;
    ...
    it('setSort on a new column starts at asc', function () {
        const core = makeCore();
        core.setAllRows(SAMPLE);
        core.setSort('score');
    ...
    it('pagination: pageRows windows sortedRows', function () {
        const core = makeCore({ pageSize: 2 });
        core.setAllRows(SAMPLE);
        core.setSort('title');
        expect(core.pageRows().map(function (r) {
            return r.id;
        })).toEqual([1, 2]);
        core.setPage(1);
        expect(core.pageRows().map(function (r) {
    ...
    it('setPage clamps to valid range', function () {
        const core = makeCore({ pageSize: 2 });
        core.setAllRows(SAMPLE);
        core.setPage(99);
    ...
    it('setPageSize resets to page 0', function () {
        const core = makeCore({ pageSize: 2 });
        core.setAllRows(SAMPLE);
        core.setPage(1);
        core.setPageSize(10);
    ...
    it('upsertRow adds a new row; updates in place when key exists', function () {
        const core = makeCore();
        core.setAllRows(SAMPLE);
        ...
        expect(core.getState().rows.find(function (r) {
            return r.id === 1;
        })?.title).toBe('ALPHA!');
    });
    it('removeRow deletes and prunes selection', function () {
        const core = makeCore({ selectionMode: 'multi' });
        core.setAllRows(SAMPLE);
        core.selectRow(2);
    ...
    it('applyPatch(reload) is a no-op on core (caller refetches)', function () {
        const core = makeCore();
        core.setAllRows(SAMPLE);
        const delta = core.applyPatch({ type: 'reload' });
        expect(delta.changed).toBe(false);
    });
    it('selection: single mode replaces', function () {
    ...
    it('selection: toggleSelectAllOnPage scoped to visible page', function () {
    ...
    it('setSelectionMode single prunes multi selection to one', function () {
        const core = makeCore({ selectionMode: 'multi' });
        core.setAllRows(SAMPLE);
        core.selectRow(1);
        core.selectRow(2);
    ...
    it('subscribe notifies on change; unsubscribe detaches', function () {
        const core = makeCore();
        const events: string[] = [];
        const unsub = core.subscribe(function (s: DataTableState<TestRow>) {
            return events.push(s.status);
        });
        core.setAllRows(SAMPLE);
        core.setAllRows([]);
        unsub();
    ...
    it('setLoading and setError move status; setError sets error message', function () {
        const core = makeCore();
        core.setLoading();
