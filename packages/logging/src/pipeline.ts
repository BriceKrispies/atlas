...
export class LogPipeline {
    ...
    constructor(sinks: ReadonlyArray<Sink>, levelController: LevelController, options: LogPipelineOptions = {}) {
        this.sinks = sinks;
...
function installExitHandlerOnce(): void {
    ...
    const flushAll = function (): void {
        for (const p of liveExitHandlerPipelines)
            p.flushSync();
    };
