...
describe('LevelController — InMemoryLevelController precedence', () => {
  ...
  it('module override beats global', () => {
    const c = new InMemoryLevelController('info');
    c.setGlobal('error');
  ...
  it('tenant override beats module', () => {
    const c = new InMemoryLevelController('info');
    c.setModule('identity', 'debug');
  ...
  it('correlation override beats tenant', () => {
    const c = new InMemoryLevelController('info');
    c.setTenant('acme', 'warn');
...
describe('LevelController — runtime filtering through ctx.logger', () => {
  it('debug calls are filtered when global is info', () => {
    ...
    expect(rig.collector.events.map((e) => e.level)).toEqual(['info']);
  });

  it('correlation override flips a single flow to debug', () => {
    ...
    const messages = rig.collector.events.map((e) => e.message);
  ...
  it('module override flips one module noisier than the rest', () => {
    const rig = makeTestRig({ defaultLevel: 'warn' });
    ...
    expect(rig.collector.events.map((e) => e.message)).toEqual(['cat-debug — kept']);
  });

  it('fatal bypasses level filtering', () => {
    const rig = makeTestRig({ defaultLevel: 'fatal' });
    const ctx = makeTestContext({ pipeline: rig.pipeline });
    ctx.logger.error('error — dropped'); // below fatal, dropped
    ctx.logger.fatal('fatal — kept');
    expect(rig.collector.events.map((e) => e.level)).toEqual(['fatal']);
  });
});
