...
describe('ctx.logger.fatal', () => {
  it('flushes any buffered events synchronously before returning', () => {
    const stream = new CapturingStream();
    const pipeline = new LogPipeline(
      [new ConsoleJsonSink({ stream })],
  ...
  it('bypasses level filtering — emits even when level controller is at fatal', () => {
    ...
    const pipeline = new LogPipeline(
      [new ConsoleJsonSink({ stream })],
      new InMemoryLevelController('fatal'),
    );
    const ctx = createRootContext({
      pipeline,
      tenantId: 't',
    ...
    const records = stream.records();
    expect(records.map((r) => r.level)).toEqual(['fatal']);
  });

  it('emitted fatal record carries correlationId and ctx fields', () => {
    const stream = new CapturingStream();
    const pipeline = new LogPipeline(
      [new ConsoleJsonSink({ stream })],
      new InMemoryLevelController('debug'),
