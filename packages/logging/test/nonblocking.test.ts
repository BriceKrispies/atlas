...
describe('logger does not block the event loop', () => {
  it('a 10k-call burst completes in well under 100ms', () => {
    const pipeline = new LogPipeline(
      [new ConsoleJsonSink({ stream: new DevNullStream(), softCap: 50_000, hardCap: 100_000 })],
    ...
    process.stdout.write(
      `${JSON.stringify({
        ...
        ms_per_op: Number((duration / 10_000).toFixed(4)),
      })}\n`,
    );
  ...
  it('setInterval fires during/after a burst (event loop reachable)', async () => {
    const pipeline = new LogPipeline(
      [new ConsoleJsonSink({ stream: new DevNullStream(), softCap: 50_000, hardCap: 100_000 })],
      new InMemoryLevelController('debug'),
    );
  ...
  it('drainSync flushes via the pipeline immediately', () => {
    const stream: { chunks: string[] } & Writable = Object.assign(
