...
describe('ConsoleJsonSink overflow policy', () => {
  it('drops oldest debug first when soft cap is exceeded', async () => {
    ...
    for (let i = 0; i < 5; i++) sink.write(makeEvent('debug', `d${i}`));
    ...
    const debugRecords = records.filter((r) => r.level === 'debug');
    expect(debugRecords.map((r) => r.message)).toEqual(['d1', 'd2', 'd3', 'd4']);
    const overflowRecords = records.filter(
      (r) => r.eventName === 'Logging.BufferOverflow.Dropped',
    );
    expect(overflowRecords).toHaveLength(1);
  });

  it('drops oldest info next when no debugs and past hard cap', async () => {
    const stream = new CapturingStream();
    ...
    for (let i = 0; i < 7; i++) sink.write(makeEvent('info', `i${i}`));
  ...
  it('never drops warn / error / fatal — buffer can grow past hardCap with them', () => {
    const stream = new CapturingStream();
    const sink = new ConsoleJsonSink({ stream, softCap: 3, hardCap: 5 });

    for (let i = 0; i < 20; i++) sink.write(makeEvent('warn', `w${i}`));
    // Despite the cap, no warns were dropped.
    expect(sink.droppedCounts().warn).toBe(0);
    expect(sink.bufferedCount()).toBe(20);
  });

  it('emits an overflow report meta-log on next drain (warn level)', async () => {
    const stream = new CapturingStream();
  ...
  it('flushSync flushes buffered events synchronously', () => {
    const stream = new CapturingStream();
  ...
  it('fatal forces a sync flush + sync write of the fatal event', () => {
    const stream = new CapturingStream();
    const sink = new ConsoleJsonSink({ stream, softCap: 100, hardCap: 200 });
    ...
    const records = stream.records();
    expect(records.map((r) => r.message)).toEqual(['before', 'kaboom']);
  });
});
