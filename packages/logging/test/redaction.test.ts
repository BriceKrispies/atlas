...
describe('redaction', function () {
    describe('default key list', function () {
        it.each([
            'password',
            'secret',
            'token',
            ...
            'email',
            'phone',
        ])('redacts top-level key "%s"', function (key) {
    ...
    describe('sensitive() wrapper', function () {
        it('redacts a wrapped value regardless of key', function () {
        ...
        it('isSensitive identifies wrapped values', function () {
            expect(isSensitive(sensitive('x'))).toBe(true);
            expect(isSensitive('x')).toBe(false);
            expect(isSensitive({ value: 'x' })).toBe(false);
        });
    });
    describe('extra keys', function () {
        it('redacts custom extra keys when configured', function () {
            const out = asRecord(redact({ customKey: 'hide', other: 'keep' }, { extraKeys: ['customKey'] }));
            expect(out['customKey']).toBe('[REDACTED]');
            expect(out['other']).toBe('keep');
        });
    });
    describe('integration — runs before sinks see the event', function () {
        it('properties are redacted before reaching the collector', function () {
        ...
        it('error.message is NOT walked (stack-trace exemption)', function () {
        ...
        it('pipeline-level redactionExtraKeys are applied', function () {
