...
describe('AtlasExecutionContext basics', () => {
  ...
  it('preserves an inbound correlationId', () => {
    const rig = makeTestRig();
    const ctx = makeTestContext({
      pipeline: rig.pipeline,
  ...
  it('caller properties land under properties — never overwrite reserved fields', () => {
    const rig = makeTestRig();
    const ctx = makeTestContext({
      pipeline: rig.pipeline,
      tenantId: 'real',
  ...
  it('event field maps to LogEvent.eventName', () => {
    const rig = makeTestRig();
    const ctx = makeTestContext({ pipeline: rig.pipeline });
  ...
  it('optional ctx fields appear when set', () => {
    const rig = makeTestRig();
    const ctx = makeTestContext({
      pipeline: rig.pipeline,
      userId: 'u-real',
  ...
  it('all five log levels emit', () => {
    ...
    ctx.logger.error('d');
    ctx.logger.fatal('e');
    expect(rig.collector.events.map((x) => x.level)).toEqual([
      'debug',
      'info',
      'warn',
      'error',
