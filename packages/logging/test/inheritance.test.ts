...
describe('AtlasExecutionContext inheritance — immutable .with*()', () => {
  it('with() returns a new context; parent is untouched', () => {
  ...
  it('correlationId / traceId / tenantId are immutable across .with()', () => {
    const rig = makeTestRig();
  ...
  it('withModule sets moduleId, leaves everything else', () => {
    const rig = makeTestRig();
  ...
  it('inherited contexts get a fresh logger that stamps the new fields', () => {
    const rig = makeTestRig();
    const parent = makeTestContext({ pipeline: rig.pipeline });
  ...
  it('explicitly passing undefined clears an optional field', () => {
    const rig = makeTestRig();
