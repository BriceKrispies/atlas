...
describe('logger performance — hot path', function () {
    it('ctx.logger.info < 10 µs/op (hot path; CollectorSink, no JSON.stringify)', function () {
        ...
        expect(result.ns_per_op).toBeLessThan(10000);
    });
    it('full pipeline including JSON.stringify < 25 µs/op (ConsoleJsonSink → /dev/null)', function () {
        ...
        const pipeline = new LogPipeline([new ConsoleJsonSink({ stream, softCap: 200000, hardCap: 300000 })], new InMemoryLevelController('debug'));
        const ctx = createRootContext({
            pipeline,
            tenantId: 't',
            principalId: 'p',
        ...
        expect(result.ns_per_op).toBeLessThan(25000);
    });
    it('filtered call returns in under 1 µs (early-exit path)', function () {
        const collector = new CollectorSink();
        ...
        expect(result.ns_per_op).toBeLessThan(1000);
