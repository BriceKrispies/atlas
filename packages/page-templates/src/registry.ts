...
export class TemplateRegistry {
    ...
    register({ manifest, element }: TemplateRegistryEntry): void {
        ...
        if (typeof element !== 'function') {
            throw new PageTemplateError(`template element for ${manifest.templateId ?? '<unknown>'} must be a class`);
        }
        const { ok, errors } = validateTemplateManifest(manifest);
        if (!ok) {
            throw new PageTemplateError(`invalid manifest for templateId=${manifest.templateId ?? '<unknown>'}`, { errors });
        }
