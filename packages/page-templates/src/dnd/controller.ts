...
import type { CommitResult, DragPayload, DragSource, DropTarget, Point, } from './types.ts';
...
export class DndController {
    ...
    constructor(options?: DndControllerOptions) {
        ...
        this.overlay = new DragOverlay(options?.overlayContainer ? { container: options.overlayContainer } : {});
    ...
    detach(): void {
        ...
        this._active = null;
        this.sensor.detach();
        for (const unreg of this._sourceUnregs)
            unreg();
    ...
    private _onActivate({ source, pointer }: {
        source: DragSource;
        pointer: Point;
    }): void {
        if (!this._active)
            return;
        const preview = typeof source.getOverlayPreview === 'function'
            ? source.getOverlayPreview()
            : cloneSourcePreview(source.element, this._active.sourceRect ?? source.element.getBoundingClientRect());
        ...
        const candidates = [...this._targets.values()].filter(function (t) {
            return !t.accepts || t.accepts(activePayload);
        });
        this.projection.markCandidates(candidates.map(function (t) {
            return t.element;
        }));
        this._resolveTarget(pointer);
        this._emit('dragstart');
    }
    private _onMove({ pointer }: {
        pointer: Point;
    }): void {
        if (!this._active)
            return;
    ...
    private _resolveTarget(pointer: Point): void {
        ...
        for (const target of this._targets.values()) {
            if (target.accepts && !target.accepts(payload))
                continue;
        ...
        if (match === this._active.activeTarget)
            return;
