 ...
 */
import type { ApplyResult, EditorAction, EditorController, EditorReason, EntrySnapshot, Position, } from './editor-controller.ts';
...
export class EditorAPI {
    ...
    constructor({ controller, onCommit, onTelemetry, announce, isEditable, surfaceId, }: EditorAPIOptions) {
        this._controller = controller;
        this._commit = typeof onCommit === 'function' ? onCommit : async function () { };
        this._telemetry = typeof onTelemetry === 'function' ? onTelemetry : function () { };
        this._announce = typeof announce === 'function' ? announce : function () { };
        this._isEditable = typeof isEditable === 'function' ? isEditable : function () {
            return true;
        };
    ...
    can(op: ApiOp, args: AddArgs | MoveArgs | UpdateArgs | RemoveArgs): ApiResult {
        ...
        if (result.ok)
            this._controller.setDoc(preDoc); // roll back dry-run
     ...
     */
    private _dryRun(op: ApiOp, args: AddArgs | MoveArgs | UpdateArgs | RemoveArgs): ApplyResult | null {
        switch (op) {
            case 'add':
                if (!('widgetId' in args) || !('region' in args))
                    return null;
            ...
            case 'move':
                if (!('instanceId' in args) || !('region' in args))
                    return null;
            ...
            case 'update':
                if (!('instanceId' in args) || !('config' in args))
                    return null;
    ...
    private _recordCommit(result: ApplyResult & {
        ok: true;
    }): void {
        this._lastCommit = {
            surfaceId: this._surfaceId,
