 ...
 */
import { computeValidTargets, type SourcePosition, type ValidTargetsResult, type WidgetRegistryLike, } from '../drop-zones.ts';
...
export type EditorReason = 'invalid-entry' | 'unknown-widget' | 'region-invalid' | 'duplicate-instance-id' | 'index-invalid' | 'max-widgets' | 'instance-not-found' | 'source-gone' | 'required-region-empty';
...
export type EditorListener = (payload: {
    doc: PageDocument;
}) => void;
export class EditorController {
    ...
    private _emit(event: string, payload: {
        doc: PageDocument;
    }): void {
    ...
    // ---- helpers ----
    private _regionSpec(regionName: string): {
        name: string;
        maxWidgets?: number;
        required?: boolean;
    } | null {
    ...
    private _isWidgetKnown(widgetId: string): boolean {
        ...
        if (!reg)
            return false;
        try {
            if (typeof reg.has === 'function')
                return reg.has(widgetId);
            if (typeof reg.get === 'function')
                return reg.get(widgetId) != null;
        }
        catch {
            /* ignore */
        }
    ...
    private _wouldEmptyRequired(region: string, instanceId: string): boolean {
        ...
        const remaining = entries.filter(function (e) {
    ...
    applyAdd({ entry, region, index }: AddArgs): ApplyResult {
        ...
        if (!next.regions || typeof next.regions !== 'object')
            next.regions = {};
        if (!Array.isArray(next.regions[region]))
            next.regions[region] = [];
        ...
        const max = typeof spec.maxWidgets === 'number' && spec.maxWidgets >= 0
            ? spec.maxWidgets
            : null;
    ...
    validTargetsFor(widgetId: string, sourcePosition: SourcePosition | null = null): ValidTargetsResult {
        return computeValidTargets(widgetId, this._doc, this._template, this._registry, sourcePosition);
    }
}
