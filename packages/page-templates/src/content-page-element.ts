...
import { moduleDefaultTemplateRegistry, type TemplateManifest, type TemplateRegistry, } from './registry.ts';
...
import { moduleDefaultLayoutRegistry, type LayoutRegistry, } from './layout/layout-registry.ts';
 ...
 */
function resolveWidgetRegistry(injected: WidgetRegistryLike | null | undefined): WidgetRegistryLike {
    if (injected)
        return injected;
    return {
        has: function (id) {
            return moduleDefaultRegistry.has(id);
        },
        get: function (id) {
            return moduleDefaultRegistry.get(id);
        },
        list: function () {
            return moduleDefaultRegistry.list().map(function (row) {
                return ({ ...row });
            });
        },
    };
...
function compareSemver(a: string, b: string): number {
    const pa = a.split('.').map(function (n) {
        return parseInt(n, 10);
    });
    const pb = b.split('.').map(function (n) {
        return parseInt(n, 10);
    });
    for (let i = 0; i < 3; i++) {
        if ((pa[i] ?? 0) > (pb[i] ?? 0))
...
export class ContentPageElement extends AtlasSurface {
    ...
    private async _loadAndMount(): Promise<void> {
        ...
        if (typeof doc.layoutId === 'string' && doc.layoutId.length > 0) {
            ...
            if (typeof doc.layoutVersion === 'string') {
                ...
                if (cmp > 0) {
                    const msg = `Stored layoutVersion ${doc.layoutVersion} is ahead of ` +
            ...
            const slotNames = new Set(layoutDoc.slots.map(function (s) {
                return s.name;
            }));
            ...
            manifest = {
                ...
                regions: layoutDoc.slots.map(function (s) {
                    return ({ name: s.name });
                }),
            };
        }
            ...
            if (cmp > 0) {
                const msg = `Stored templateVersion ${doc.templateVersion} is ahead of registered ` +
            ...
            const manifestRegionNames = new Set(manifest.regions.map(function (r) {
                return r.name;
            }));
            for (const regionName of Object.keys(docRegions)) {
        ...
        if (this.widgetRegistry)
            hostEl.registry = this.widgetRegistry;
        ...
        if (enterEdit) {
            ...
            this._editorHandle = attachEditor({
                ...
                onTelemetry: ({ event, payload }) => telemetry(event, {
                    pageId: this.pageId,
                    correlationId: this.correlationId,
                    ...payload,
                }),
    ...
    private async _commitAndRemount(nextDoc: PageDocument, info: CommitInfo): Promise<void> {
        ...
        await must(this.pageStore, 'pageStore present (checked in _loadAndMount)').save(this.pageId, nextDoc);
