...
export class InMemoryPageStore implements PageStore {
    ...
    async list(): Promise<PageDocument[]> {
        return [...this._docs.values()].map(function (d) {
...
export class ValidatingPageStore implements PageStore {
    ...
    async get(pageId: string): Promise<PageDocument | null> {
        ...
        if (!ok) {
            throw new PageDocumentError(`page document returned by store for pageId=${pageId} is invalid`, { errors });
        }
        return doc;
    }
    async save(pageId: string, doc: PageDocument): Promise<void> {
        const { ok, errors } = validatePageDocument(doc);
        if (!ok) {
            throw new PageDocumentError(`page document for pageId=${pageId} is invalid`, { errors });
        }
    ...
    async list(): Promise<PageDocument[]> {
        ...
        for (const doc of docs) {
            const { ok, errors } = validatePageDocument(doc);
            if (!ok) {
                throw new PageDocumentError(`page document returned by store (pageId=${(doc as {
                    pageId?: string;
                })?.pageId ?? '<unknown>'}) is invalid`, { errors });
            }
        }
        return docs;
    }
