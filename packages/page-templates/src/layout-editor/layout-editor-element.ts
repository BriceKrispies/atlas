...
import { validateLayoutDocument, cloneLayoutDocument, emptyLayoutDocument, nextFreeRect, type LayoutDocument, type LayoutSlot, } from '../layout/layout-document.ts';
import { AtlasLayoutElement } from '../layout/layout-element.ts';
import { ensureLayoutStyles } from '../layout/layout-styles.ts';
import { must, expect, isHtmlElement, isHtmlInputElement, } from '../internal/assert.ts';
 ...
 */
function hasStringValue(v: unknown): v is {
    value: string;
} {
...
export class AtlasLayoutEditorElement extends AtlasElement {
    ...
    private _wireToolbar(): void {
        ...
        if (saveBtn)
            saveBtn.addEventListener('click', () => void this._save());
    }
    ...
    private _applyLayoutToCanvas(): void {
        ...
        if (!(layoutEl instanceof AtlasLayoutElement))
            return;
    ...
    private _onSectionPointerDown(ev: PointerEvent, slotName: string): void {
        ...
        if (!slot)
            return;
        const section = layoutEl.querySelector(`:scope > section[data-slot="${CSS.escape(slotName)}"]`);
    ...
    private __onPointerMove(ev: PointerEvent): void {
        ...
        drag.rafId = requestAnimationFrame(() => {
            drag.rafId = 0;
            if (this._drag === drag && drag.phase === 'active')
                this._applyDragFrame();
        });
    }
    ...
    private _applyDragFrame(): void {
        ...
        if (drag.mode === 'resize') {
            ...
            if (drag.edge === 'e' || drag.edge === 'se') {
                next.colSpan = Math.max(1, Math.min(columns - next.col + 1, pCol - next.col + 1));
            }
    ...
    private _teardownDrag(): void {
        ...
        if (drag.canvas)
            drag.canvas.removeAttribute('data-drag-mode');
        try {
    ...
    private _applySlotChange(slotName: string, nextSlot: LayoutSlot): void {
        this._mutate(function (doc) {
            const i = doc.slots.findIndex(function (s) {
    ...
    private _renameSlot(fromName: string, toName: string): void {
        if (fromName === toName)
            return;
        if (this._requireDoc().slots.some(function (s) {
            return s.name === toName;
        }))
            return;
        this._mutate(function (doc) {
            const slot = doc.slots.find(function (s) {
                return s.name === fromName;
            });
            if (!slot)
                return;
            slot.name = toName;
        });
        if (this._selectedSlotName === fromName)
            this._selectedSlotName = toName;
    }
    private _deleteSlot(slotName: string): void {
        this._mutate(function (doc) {
            doc.slots = doc.slots.filter(function (s) {
                return s.name !== slotName;
            });
        });
        if (this._selectedSlotName === slotName)
            this._select(null);
    }
    private _addSlot(): void {
        ...
        const nameBase = 'slot';
        let i = 1;
        while (doc.slots.some(function (s) {
            return s.name === `${nameBase}-${i}`;
        }))
            i++;
        const newName = `${nameBase}-${i}`;
        const rect = nextFreeRect(doc, { colSpan: 4, rowSpan: 2 });
        this._mutate(function (d) {
    ...
    private _findSlot(slotName: string): LayoutSlot | null {
        return this._requireDoc().slots.find(function (s) {
            return s.name === slotName;
        }) ?? null;
    }
    private _renderPanel(): void {
      ...
      `);
        ...
        if (delBtn && this._selectedSlotName) {
            delBtn.addEventListener('click', () => this._deleteSlot(expect(this._selectedSlotName, 'selected slot name (truthy-checked above)')));
        }
    }
