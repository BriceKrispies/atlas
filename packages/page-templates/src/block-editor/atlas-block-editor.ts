...
import { BlockEditorController, type Block, type BlockDocument, } from './block-editor-controller.ts';
let blockCounter = 0;
...
class AtlasBlockEditor extends AtlasSurface {
    ...
    override connectedCallback(): void {
        ...
        this._disposeTest = registerTestState(this.surfaceId, () => must(this.controller, 'block-editor: controller exists once connected').getSnapshot());
    }
    ...
    private _renderBlocks(): void {
        ...
        for (const block of snap.document.blocks) {
            ...
            if (snap.selection === block.blockId)
...
class AtlasBlock extends AtlasElement {
    ...
    private _render(): void {
        ...
        const formats = new Set<string>(Array.isArray(rawFormats) ? rawFormats.filter(function (s): s is string {
