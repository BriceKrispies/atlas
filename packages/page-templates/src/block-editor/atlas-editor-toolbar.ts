...
class AtlasEditorToolbar extends AtlasElement {
    ...
    private _render(): void {
        ...
        const add = function (key: string, label: string, onClick: () => void): HTMLElement {
    ...
    private _insert(type: BlockType): void {
        ...
        const selected = snap.selection
            ? snap.document.blocks.findIndex(function (b) {
                return b.blockId === snap.selection;
            })
            : -1;
    ...
    private _move(delta: number): void {
        ...
        if (!snap.selection)
            return;
        const from = snap.document.blocks.findIndex(function (b) {
            return b.blockId === snap.selection;
        });
        if (from < 0)
            return;
        const to = from + delta;
        if (to < 0 || to >= snap.document.blocks.length)
            return;
