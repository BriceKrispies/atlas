...
export class BlockEditorController {
    ...
    private _insertBlock({ blockId, type, content, config, at }: InsertBlockPatch = {}): CommitResult {
        if (!blockId)
            return { ok: false, reason: 'invalid-blockId' };
        if (!type || !SUPPORTED_TYPES.has(type))
            return { ok: false, reason: 'unsupported-type' };
        if (this._blocks.some(function (b) {
    ...
    private _updateBlock({ blockId, patch }: UpdateBlockPatch = {}): CommitResult {
        ...
        const current = must(this._blocks[idx], 'block-editor: _indexOf returned a valid index, block must exist');
        this._blocks[idx] = {
            ...current,
    ...
    private _indexOf(blockId: string | undefined): number {
        if (!blockId)
            return -1;
        return this._blocks.findIndex(function (b) {
            return b.blockId === blockId;
        });
    }
