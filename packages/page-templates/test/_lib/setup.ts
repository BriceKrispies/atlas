...
const dom = parseHTML('<!doctype html><html><head></head><body></body></html>') as unknown as LinkedomDom;
...
if (!flag.__atlasPageTemplatesDryRunShimInstalled) {
    ...
    if (dom.HTMLInputElement)
        bag.HTMLInputElement = dom.HTMLInputElement;
    ...
    if (!globalThis.structuredClone) {
        bag.structuredClone = (function (v: unknown): unknown {
    ...
    if (typeof docWithWalker.createTreeWalker !== 'function') {
        docWithWalker.createTreeWalker = function (root: Element): TreeWalkerLike {
            const elements: Element[] = [];
            const walk = function (el: Element): void {
                elements.push(el);
 ...
 */
export function setInputValueAndChange(input: HTMLInputElement | null, value: number | string): void {
    if (!input)
        return;
