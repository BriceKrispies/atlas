...
const { validateLayoutDocument, emptyLayoutDocument } = await import('../src/layout/index.ts');
...
async function testEditor_rendersToolbarCanvasPanel(): Promise<void> {
    const el = mountEditor(emptyLayoutDocument({ layoutId: 'x', displayName: 'X' }));
...
async function testEditor_addSlotCreatesSection(): Promise<void> {
    ...
    const addBtn = must(el.querySelector<HTMLElement>('[data-action="add-slot"]'), 'add-slot button just rendered');
    addBtn.click();
    const sections = el.querySelectorAll('[data-editor-canvas] atlas-layout > section[data-slot]');
...
async function testEditor_multipleAddSlotsPlaceWithoutOverlap(): Promise<void> {
    ...
    for (let i = 0; i < doc.slots.length; i++) {
        for (let j = i + 1; j < doc.slots.length; j++) {
            ...
            const overlap = a.col < b.col + b.colSpan &&
...
async function testEditor_panelEditResizesSlot(): Promise<void> {
    ...
    const slotEl = must(el.querySelector('[data-editor-canvas] atlas-layout > section[data-slot="main"]'), 'main slot just rendered');
...
async function testEditor_panelRenameSlot(): Promise<void> {
    ...
    assert(el.querySelector('[data-editor-canvas] atlas-layout > section[data-slot="header"]'), 'section tag updated to new name');
    el.remove();
}
async function testEditor_deleteSlot(): Promise<void> {
    ...
    const slotA = must(el.querySelector('[data-editor-canvas] atlas-layout > section[data-slot="a"]'), 'slot a just rendered');
    dispatchEventOn(slotA, 'click');
    const deleteBtn = must(el.querySelector<HTMLElement>('[data-action="delete-slot"]'), 'delete-slot button just rendered');
    deleteBtn.click();
    const doc = must(el.layout, 'layout still present');
...
async function testEditor_onChangeAndOnSaveFire(): Promise<void> {
    ...
    el.onSave = function (doc) {
        savedDoc = doc;
    };
    const addBtn = must(el.querySelector<HTMLElement>('[data-action="add-slot"]'), 'add-slot button just rendered');
    addBtn.click();
    assert(changes >= 1, 'onChange fired for add');
    const saveBtn = must(el.querySelector<HTMLElement>('[data-action="save"]'), 'save button just rendered');
    saveBtn.click();
...
async function testEditor_onChangeDoesNotFireOnInvalidEdit(): Promise<void> {
    ...
    const slotEl = must(el.querySelector('[data-editor-canvas] atlas-layout > section[data-slot="main"]'), 'main slot just rendered');
    dispatchEventOn(slotEl, 'click');
    setInputValueAndChange(el.querySelector<HTMLInputElement>('input[data-field="colSpan"]'), 99);
