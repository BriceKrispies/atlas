...
const { TemplateRegistry, moduleDefaultTemplateRegistry, validateTemplateManifest, validatePageDocument, InMemoryPageStore, ValidatingPageStore, PageDocumentError, PageTemplateError, } = pkg;
...
function sortedStringify(value: unknown): string {
    ...
    return ('{' +
        keys
            .map(function (k) {
            return JSON.stringify(k) + ':' + sortedStringify(obj[k]);
        })
            .join(',') +
        '}');
}
function hasErrorBox(node: Element | null): boolean {
    const walk = function (el: Element | null): boolean {
        if (!el)
            return false;
...
async function testRoundTripByteEquivalence(): Promise<void> {
    ...
    assert(beforeStr === afterStr, `round-trip byte-equivalence failed:\n  before=${beforeStr}\n  after =${afterStr}`);
    ...
    assert(fresh.tenantId === before.tenantId, 'returned doc must not share identity with stored doc');
}
async function testValidatingPageStoreRejectsInvalid(): Promise<void> {
    ...
    assert(caught instanceof PageDocumentError, `expected PageDocumentError, got ${String(caught)}`);
    if (caught instanceof PageDocumentError) {
        const details = caught.details;
        const errors = details && typeof details === 'object' && 'errors' in details
            ? (details as {
                errors?: unknown;
            }).errors
            : undefined;
        assert(Array.isArray(errors) && errors.length > 0, 'PageDocumentError must carry ajv errors in details.errors');
    }
    ...
    assert(back !== null && back.pageId === docWelcome.pageId, 'ValidatingPageStore should round-trip a valid doc');
}
async function testTemplateRegistryRoundTrip(): Promise<void> {
    ...
    assert(entry.manifest.templateId === templateOneColumn.templateId, 'registry.get returns registered manifest');
    const list = reg.list();
    const first = must(list[0], 'list has at least one entry');
    assert(list.length === 1 && first.templateId === templateOneColumn.templateId, `registry.list should yield one entry, got ${JSON.stringify(list)}`);
    ...
    assert(regThrew instanceof PageTemplateError, 'invalid manifest must be rejected by register');
    // moduleDefaultTemplateRegistry is a distinct instance.
    assert(!moduleDefaultTemplateRegistry.has(templateOneColumn.templateId), 'moduleDefaultTemplateRegistry should be empty');
}
...
async function testContentPageHappyPath(): Promise<void> {
    ...
    assert(!hasErrorBox(page), `happy-path: content-page should not render an error box, got: ${page.textContent}`);
    const template = findDescendant(page, function (el) {
        return el instanceof TwoColumnTemplate;
    });
    assert(template, 'happy-path: two-column template element must be present');
    const host = findDescendant(page, function (el) {
        return el.tagName.toLowerCase() === 'widget-host';
    }) as WidgetHostEl | null;
    const hostEl = must(host, 'happy-path: <widget-host> must be present in the DOM');
    assert(hostEl.parentNode === template, '<widget-host> should be a child of the template element');
    ...
    assert(Array.isArray(layout.slots['main']), 'forwarded layout should carry the regions as slots');
    page.remove();
...
async function testContentPageTemplateMissing(): Promise<void> {
    ...
    assert(hasErrorBox(page), 'template-missing: content-page must render an error box');
    const host = findDescendant(page, function (el) {
        return el.tagName.toLowerCase() === 'widget-host';
    });
    assert(host === null, 'template-missing: <widget-host> must NOT be mounted when template lookup fails');
    page.remove();
    await waitMicrotasks(5);
}
async function testContentPageVersionAhead(): Promise<void> {
    ...
    document.body.appendChild(page);
    await waitMicrotasks(20);
    assert(hasErrorBox(page), 'version-ahead: content-page must render a fail-closed error box');
    const host = findDescendant(page, function (el) {
        return el.tagName.toLowerCase() === 'widget-host';
    });
    assert(host === null, 'version-ahead: <widget-host> must NOT be mounted when stored version is ahead');
    page.remove();
    await waitMicrotasks(5);
}
