...
describe('Missing Atlas counter singletons', function () {
    test('projectionsBuiltTotal exposes a Counter labelled by projection_type', function () {
        ...
        expect(out).toContain('atlas_projections_built_total{projection_type="render-page"} 2');
    });
    test('wasmExecutionsTotal exposes a Counter labelled by plugin_id + result', function () {
        ...
        expect(out).toContain('atlas_wasm_executions_total{plugin_id="demo-transform",result="ok"} 1');
        expect(out).toContain('atlas_wasm_executions_total{plugin_id="demo-transform",result="error"} 1');
    });
    test('workerHeartbeatsTotal exposes a Counter labelled by worker_id', function () {
    ...
    test('repeat singleton accessors return the same instance via registry', function () {
