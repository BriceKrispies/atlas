...
describe('Registry', function () {
    test('register + get round-trips', function () {
        const r = new Registry();
        const c = new Counter({ name: 'a', help: 'A' });
        r.register(c);
        expect(r.get('a')).toBe(c);
    });
    test('rejects double-register', function () {
        const r = new Registry();
        r.register(new Counter({ name: 'dup', help: 'H' }));
        expect(function () {
            return r.register(new Counter({ name: 'dup', help: 'H2' }));
        }).toThrow(/already registered/);
    });
    test('serialize emits HELP + TYPE lines', function () {
        const r = new Registry();
    ...
    test('serialize composes counter + histogram', function () {
        const r = new Registry();
        const c = r.register(new Counter({ name: 'c', help: 'C', labelNames: ['k'] }));
        const h = r.register(new Histogram({ name: 'h', help: 'H', labelNames: ['k'], buckets: [0.1, 1] }));
        c.inc({ k: 'v' });
        h.observe(0.5, { k: 'v' });
        const out = r.serialize();
