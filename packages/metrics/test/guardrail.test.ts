...
describe('guardrailHitsTotal counter', function () {
    test('descriptor matches Rust counterpart (name + labels)', function () {
    ...
    test('repeat singleton accessor returns same instance', function () {
        expect(guardrailHitsTotal()).toBe(guardrailHitsTotal());
    });
});
describe('guardrail() helper', function () {
    test('increments counter with the supplied labels', function () {
        guardrail({
    ...
    test('writes a structured warn record via the supplied logger', function () {
    ...
    test('omits optional fields when not supplied', function () {
        const logger = makeLogger();
        guardrail({
            kind: 'tech_debt',
            id: 'minimal',
    ...
    test('falls back to the registered process logger when no arg is passed', function () {
        const logger = makeLogger();
    ...
    test('does not throw when no logger is available — counter still ticks', function () {
        expect(function () {
            return guardrail({
                kind: 'tech_debt',
                id: 'no-logger',
...
describe('techDebt() / mvpShortcut() convenience wrappers', function () {
    test('techDebt sets kind = "tech_debt"', function () {
        techDebt({
    ...
    test('mvpShortcut sets kind = "mvp_shortcut" (runtime-only, no build break)', function () {
        mvpShortcut({
    ...
    test('techDebt and mvpShortcut keep separate counter series', function () {
    ...
    test('wrappers forward the explicit logger argument', function () {
        const logger = makeLogger();
        ...
        expect(assertDefined(logger.calls[0], 'expected techDebt logger call').fields['kind']).toBe('tech_debt');
        expect(assertDefined(logger.calls[1], 'expected mvpShortcut logger call').fields['kind']).toBe('mvp_shortcut');
    });
});
describe('counter accumulation per (kind, id, component) tuple', function () {
    test('repeat calls with same labels add up', function () {
        for (let i = 0; i < 5; i++) {
            techDebt({
                id: 'repeated',
                component: 'core',
    ...
    test('different ids are tracked as distinct series', function () {
    ...
    test('different components are tracked as distinct series', function () {
    ...
    test('counter renders correctly via registry serialize()', function () {
        ...
        expect(out).toContain('guardrail_hits_total{kind="tech_debt",id="render-test",component="core"} 2');
    });
});
