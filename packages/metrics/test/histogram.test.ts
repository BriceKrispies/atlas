...
describe('Histogram', function () {
    ...
    test('rejects non-monotonic buckets', function () {
        expect(function () {
            return new Histogram({
                name: 'h',
                help: 'h',
    ...
    test('observe places values in cumulative buckets', function () {
    ...
    test('observe with no labels works', function () {
        const h = new Histogram({
            name: 'no_labels',
            help: 'h',
            buckets: [1, 2],
    ...
    test('observe rejects unknown labels', function () {
        const h = new Histogram({
            name: 'rl',
            help: 'h',
            labelNames: ['a'],
        ...
        expect(function () {
            return h.observe(0.5, { a: 'x', extra: 'nope' });
        }).toThrow(MetricsLabelError);
    });
    test('observe rejects non-finite', function () {
        const h = new Histogram({
            name: 'nf',
            help: 'h',
            buckets: [1],
        ...
        expect(function () {
            return h.observe(Number.NaN);
        }).toThrow();
        expect(function () {
            return h.observe(Number.POSITIVE_INFINITY);
        }).toThrow();
    });
    test('empty histogram renders nothing', function () {
        const h = new Histogram({
            name: 'empty',
            help: 'h',
            buckets: [1],
