...
export class Counter implements Metric {
    ...
    inc(arg1?: number | LabelValues, arg2?: LabelValues): void {
        ...
        if (!Number.isFinite(n) || n < 0) {
            throw new Error(`counter ${this.descriptor.name}: inc requires a finite non-negative number, got ${String(n)}`);
        }
    ...
    render(): string {
        ...
        const entries = [...this.values.entries()].sort(function (a, b) {
