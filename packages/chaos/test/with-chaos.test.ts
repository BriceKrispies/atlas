...
function makeFakeStore(): FakeStore {
  ...
  let appendCount = 0;
  const store: FakeStore & { __appends(): number } = {
...
describe('withChaos', () => {
  it('passes through method calls when no chaos is configured', async () => {
  ...
  it('injects errors at the configured rate (deterministic via seed)', async () => {
    const store = makeFakeStore();
    const profile: ChaosProfile = {
      ...
      defaultMethod: {
        error: {
          probability: 1,
          factory: (method): Error => new Error(`forced error in ${method}`),
        },
      },
    };
  ...
  it('drops writes (and only writes) when configured', async () => {
    const store = makeFakeStore();
    const profile: ChaosProfile = {
      seed: 1,
    ...
    expect((store as unknown as { __appends(): number }).__appends()).toBe(0);

    // Read is NOT eligible for drop, even at probability 1.
    expect(await wrapped.read('id-1')).toBeNull();
  });

  it('adds latency at the configured probability', async () => {
    const store = makeFakeStore();
    const profile: ChaosProfile = {
      seed: 7,
      defaultMethod: {
  ...
  it('per-method override beats default', async () => {
    const store = makeFakeStore();
    const profile: ChaosProfile = {
      seed: 99,
      methods: {
        append: {
          error: {
            probability: 1,
            factory: (): Error => new Error('append-only failure'),
          },
        },
      },
      defaultMethod: {
        error: {
          probability: 0,
          factory: (): Error => new Error('default'),
        },
      },
    };
    const wrapped = withChaos(store, profile);
