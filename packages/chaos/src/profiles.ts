  ...
  defaultMethod: {
    error: {
      ...
      factory: (method): Error =>
        new Error(`chaos[light]: synthetic transient error in ${method}`),
    },
...
export const AGGRESSIVE: ChaosProfile = Object.freeze({
  defaultMethod: {
    error: {
      probability: 0.05,
      factory: (method: string, args: readonly unknown[]): Error => {
        const argSummary = args
          .map((a: unknown) => (typeof a === 'string' ? a : typeof a))
          .slice(0, 2)
          .join(',');
        return new Error(
          `chaos[aggressive]: synthetic error in ${method}(${argSummary})`,
        );
      },
    },
