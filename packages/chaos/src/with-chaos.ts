...
const DEFAULT_WRITE_PATTERN =
  /^(append|set|create|update|delete|put|insert|push|add|emit|upsert|remove|revoke)/i;
...
export function withChaos<T extends object>(adapter: T, profile: ChaosProfile): T {
  ...
  return new Proxy(adapter, {
    get(target, prop, receiver) {
      ...
      if (!chaos) return fn.bind(target);
