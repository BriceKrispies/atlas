...
import { IngressError, toLogError } from '@atlas/platform-core';
import { intentsSubmittedTotal, policyEvaluationsTotal } from '@atlas/metrics';
import type { EventStore, Cache, ProjectionStore, SearchEngine, ControlPlaneRegistry, CatalogStateStore, EntityStore, HandlerRegistry, IntentHandlerContext, PolicyEngine, PolicyDecision, PolicyEvaluationRequest, EventDispatcher, } from '@atlas/ports';
...
export interface IngressState {
     ...
     * call site.
     */
    auditPolicyEvaluated?: (request: PolicyEvaluationRequest, decision: PolicyDecision, context: {
        correlationId: string;
...
export async function submitIntent(state: IngressState, envelope: IntentEnvelope): Promise<IntentResponse> {
    ...
    try {
        const result = await submitIntentInner(state, envelope, correlationId, function (action, decision) {
...
async function submitIntentInner(state: IngressState, envelope: IntentEnvelope, correlationId: string, observe: (action: string, decision: 'permit' | 'deny' | 'error') => void): Promise<IntentResponse> {
    ...
    if (!validator) {
        err('UNKNOWN_SCHEMA', `schema not found: ${envelope.schemaId} v${envelope.schemaVersion}`, 400, correlationId);
    }
    ...
    const priorEvent = await state.eventStore.findByIdempotencyKey(state.tenantId, envelope.idempotencyKey);
    if (priorEvent) {
    ...
    if (typeof envelope.payload.resourceType !== 'string' ||
        envelope.payload.resourceType.trim().length === 0) {
        err('SCHEMA_VALIDATION_FAILED', 'payload.resourceType must be a non-empty string', 400, correlationId);
    }
    const resourceType = envelope.payload.resourceType;
    const resourceId = typeof envelope.payload.resourceId === 'string' ? envelope.payload.resourceId : '';
    ...
    if (state.entities && resourceId) {
        try {
            ...
            state.logger?.warn('entity-attr lookup failed; policy will evaluate with empty attrs', {
    ...
    if (decision.effect === 'deny') {
        ...
        err('UNAUTHORIZED', 'Not authorized to perform this action', 403, correlationId);
    }
    // 7. Handler dispatch
