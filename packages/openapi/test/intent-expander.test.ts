 ...
 * undefined deref.
 */
function envelopeProps(components: Readonly<Record<string, unknown>>, key: string): Record<string, unknown> {
...
describe('actionIdToOperationId', function () {
    it.each([
...
describe('expandIntents', function () {
    it('emits one path per matching-audience action', function () {
    ...
    it('audience filter excludes operator actions from tenant doc', function () {
        const manifest = makeManifest([
            {
                actionId: 'ContentPages.Page.Create',
                resourceType: 'Page',
    ...
    it('emits a tag per module that contributes at least one operation', function () {
        ...
        expect(result.tags.map(function (t) {
            return t.name;
        })).toEqual(['content-pages']);
    });
    it('per-action envelope wraps the bundled payload schema and stamps eventType', function () {
        const manifest = makeManifest([
            {
                actionId: 'ContentPages.Page.Create',
                resourceType: 'Page',
        ...
        const props = envelopeProps(result.schemaComponents, 'Envelope_contentPagesPageCreate');
        // payload mirror
    ...
    it('falls back to generic payload when no bundled schema is provided', function () {
        const manifest = makeManifest([
            {
