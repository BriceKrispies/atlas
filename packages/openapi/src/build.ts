 ...
 * the filesystem.
 */
import { buildErrorResponseComponents, intentAcceptedResponseSchema, } from './error-responses.ts';
import { expandIntents } from './intent-expander.ts';
import { buildSecuritySchemes, defaultSecurity } from './security-schemes.ts';
import type { Audience, BuildOpenApiInput, OpenApiDocument, } from './types.ts';
...
export function buildOpenApi(input: BuildOpenApiInput): OpenApiDocument {
    ...
    for (const ann of input.routeAnnotations) {
        if (ann.audience !== audience || !ann.tags)
            continue;
...
function buildOperationFromAnnotation(ann: BuildOpenApiInput['routeAnnotations'][number]): Record<string, unknown> {
    const op: Record<string, unknown> = {
        ...
        summary: ann.summary,
        responses: Object.fromEntries(Object.entries(ann.responses).map(function ([status, r]) {
            return [
                status,
    ...
    if (ann.tags !== undefined)
        op['tags'] = ann.tags;
    if (ann.security !== undefined)
        op['security'] = ann.security;
    if (ann.parameters !== undefined)
        op['parameters'] = ann.parameters;
