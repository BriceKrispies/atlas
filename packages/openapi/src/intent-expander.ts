 ...
 */
import type { ActionAudienceOverrides, ActionDeclaration, Audience, JsonSchema, ModuleManifest, } from './types.ts';
...
export function expandIntents(input: ExpandIntentsInput): IntentExpansion {
    ...
    for (const manifest of input.manifests) {
        ...
        const tagAdded = tags.some(function (t) {
        ...
        for (const action of manifest.actions) {
            ...
            schemaComponents[envelopeSchemaName] = wrapPayloadInEnvelope(action, input.envelopeSchema, payloadSchema);
            pathOperations[path] = {
...
export function actionIdToOperationId(actionId: string): string {
    const parts = actionId.split('.').filter(function (p) {
        return p.length > 0;
    });
    if (parts.length === 0)
        return 'unknown';
    return parts
        .map(function (seg, i) {
        return (i === 0 ? lowerFirst(seg) : upperFirst(seg));
    })
        .join('');
}
...
function buildOperation(action: ActionDeclaration, operationId: string, moduleId: string, envelopeSchemaName: string): Record<string, unknown> {
    return {
        tags: [moduleId],
        operationId,
 ...
 */
function wrapPayloadInEnvelope(action: ActionDeclaration, envelopeSchema: JsonSchema, payloadSchema: JsonSchema | undefined): Record<string, unknown> {
