...
function loadActionPayloadSchemas(): Record<string, JsonSchema> {
  ...
  for (const f of files) {
    ...
    if (actionIdProp === undefined) continue;
    const constValue = actionIdProp['const'];
    if (typeof constValue !== 'string' || constValue.length === 0) continue;
...
function main(): void {
  ...
  const errorEnvelopeSchema = readJson(
    join(SCHEMAS_DIR, 'error_envelope.schema.json'),
  );
  ...
  process.stdout.write(
    `${JSON.stringify({
      ...
      actionsExpanded: Object.keys(tenantDoc.paths).filter((p) =>
        p.startsWith('/api/v1/intents#'),
      ).length,
