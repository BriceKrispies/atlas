...
export interface AtlasSimDB extends DBSchema {
  events: {
    key: string;
    value: EventEnvelope;
    indexes: {
...
export type Db = IDBPDatabase<AtlasSimDB>;

export async function openSimDb(tenantId: string): Promise<Db> {
  const name = `atlas-sim-${tenantId}`;
  return openDB<AtlasSimDB>(name, 1, {
    upgrade(db) {
