...
import { createBrowserIngress, type BrowserIngress } from '../src/index.ts';
import { loadBadgeFamilySeed, buildSeedIntent } from '../src/test-utils/fixtures.ts';
import { newEventId } from '../src/domain/catalog/ids.ts';
...
async function makeIngress(prefix: string): Promise<{
  ...
  const ingress = await createBrowserIngress({ tenantId, principalId });
...
describe('catalog_search parity', () => {
  ...
  test('test_search_permission_filter_excludes_disallowed', async () => {
    ...
    const aliceIngress = await createBrowserIngress({
      tenantId,
      principalId: 'u_alice',
    });
...
interface IngressInternals {
  __db: import('../src/ports/db.ts').Db;
  __search: import('../src/ports/search-engine.ts').SearchEnginePort;
}
...
async function readEventTags(
  ...
  const internals = ingress as unknown as IngressInternals;
  const ev = await internals.__db.get('events', eventId);
  return ev?.cacheInvalidationTags ?? null;
}

async function truncateSearch(ingress: BrowserIngress, tenantId: string): Promise<void> {
  const internals = ingress as unknown as IngressInternals;
  const tx = internals.__db.transaction('search_documents', 'readwrite');
...
async function indexDirect(
  ingress: BrowserIngress,
  doc: import('../src/types.ts').SearchDocument,
): Promise<void> {
  const internals = ingress as unknown as IngressInternals;
  await internals.__search.index(doc);
}
