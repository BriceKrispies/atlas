...
export function cryptoContract(makeCrypto: () => Promise<Crypto>): void {
  describe('Crypto contract — full', () => {
    ...
    test('aesGcm round-trips plaintext under the same key + iv', () => {
    ...
    test('aesGcm rejects tampered ciphertext (AEAD)', () => {
      const key = c.randomBytes(32);
      const iv = c.randomBytes(12);
    ...
    test('scrypt is deterministic for the same inputs', () => {
    ...
    test('timingSafeEqual returns true on equal byte arrays', () => {
      const a = new Uint8Array([1, 2, 3, 4]);
      const b = new Uint8Array([1, 2, 3, 4]);
      expect(c.timingSafeEqual(a, b)).toBe(true);
    });

    test('timingSafeEqual returns false on length mismatch', () => {
      expect(
        c.timingSafeEqual(new Uint8Array([1, 2, 3]), new Uint8Array([1, 2])),
      ).toBe(false);
    });
  });
}
