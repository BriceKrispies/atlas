...
export function catalogStateStoreContract(makeStore: () => Promise<CatalogStateStore>): void {
  describe('CatalogStateStore contract', function () {
    let store: CatalogStateStore;
    beforeEach(async function () {
      store = await makeStore();
    });
    test('get returns null for an unseen tenant', async function () {
      const got = await store.get('never-seen');
      expect(got).toBeNull();
    });
    test('put + get round-trips a record', async function () {
      const r = rec({ tenantId: 'tenant-rt' });
      await store.put(r);
      const got = await store.get('tenant-rt');
      expect(got).toEqual(r);
    });
    test('put overwrites an existing tenant record', async function () {
      ...
      const got = assertDefined(
        await store.get('tenant-ow'),
        'expected stored record for tenant-ow',
      );
      expect(got.seedPackageVersion).toBe('1.0.1');
    });
    test('tenant isolation: storing under one tenant does not leak to another', async function () {
    ...
    test('publishedRevisions map round-trips intact', async function () {
      const revisions = { 'fam-1': 3, 'fam-2': 7 };
      await store.put(rec({ tenantId: 'tenant-rev', publishedRevisions: revisions }));
      const got = assertDefined(
        await store.get('tenant-rev'),
        'expected stored record for tenant-rev',
      );
      expect(got.publishedRevisions).toEqual(revisions);
    });
    test('payload preserves nested object shape', async function () {
      const payload = {
        kind: 'badge_family',
        families: [
      ...
      const got = assertDefined(
        await store.get('tenant-payload'),
        'expected stored record for tenant-payload',
      );
      expect(got.payload).toEqual(payload);
    });
    test('[error-shape] put with an empty tenantId is accepted at the port layer (callers must enforce non-empty)', async function () {
      ...
      expect(
        assertDefined(got, 'expected stored record for empty tenantId').seedPackageVersion,
      ).toBe('v-empty');
    });
    test('[concurrency] concurrent puts under different tenants do not interfere', async function () {
      const puts: Promise<void>[] = [];
      for (let i = 0; i < 10; i++) {
        puts.push(store.put(rec({ tenantId: `tenant-cc-${i}`, seedPackageVersion: `v${i}` })));
      }
      await Promise.all(puts);
      for (let i = 0; i < 10; i++) {
        const got = await store.get(`tenant-cc-${i}`);
        expect(got).not.toBeNull();
        expect(assertDefined(got, `expected record for tenant-cc-${i}`).seedPackageVersion).toBe(
          `v${i}`,
        );
      }
    });
    test('[concurrency] concurrent overwrites of the same tenant land one of the racing values intact', async function () {
      ...
      const candidates = Array.from({ length: 10 }, function (_, i) {
        return rec({
          tenantId,
      ...
      const winner = assertDefined(
        candidates.find(function (c) {
          return c.seedPackageVersion === got.seedPackageVersion;
        }),
