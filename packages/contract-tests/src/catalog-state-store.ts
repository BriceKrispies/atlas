...
export function catalogStateStoreContract(makeStore: () => Promise<CatalogStateStore>): void {
    describe('CatalogStateStore contract', function () {
        ...
        test('[concurrency] concurrent overwrites of the same tenant land one of the racing values intact', async function () {
            ...
            const got = assertDefined(await store.get(tenantId), 'expected stored record for overwrite-race tenant');
            // The final record must match exactly one of the candidates — version,
            // payload, and publishedRevisions all from the same source.
            const winner = assertDefined(candidates.find(function (c) {
