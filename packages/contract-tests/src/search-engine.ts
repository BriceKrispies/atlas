...
function makeDoc(opts: MakeDocOptions = {}): SearchDocument {
    ...
    const doc: SearchDocument = {
        ...
        fields,
        permissionAttributes: opts.allowedPrincipals == null
            ? null
...
export function searchEngineContract(makeEngine: () => Promise<SearchEngine>): void {
    describe('SearchEngine contract', function () {
        let engine: SearchEngine;
        beforeEach(async function () {
            engine = await makeEngine();
        });
        test('index then search returns the document', async function () {
            await engine.index(makeDoc({
        ...
        test('search returns ranked results, descending by relevance', async function () {
            await engine.index(makeDoc({
            ...
            const ids = results.map(function (r) {
                return r.documentId;
            });
            expect(ids[0]).toBe('doc-title');
            expect(ids[ids.length - 1]).toBe('doc-body');
        });
        test('tenant isolation: tenant A docs are invisible to tenant B', async function () {
            await engine.index(makeDoc({
                documentId: 'a-only',
                tenantId: 'tenant-a',
            ...
            expect(a.map(function (d) {
                return d.documentId;
            })).toEqual(['a-only']);
            expect(b.map(function (d) {
                return d.documentId;
            })).toEqual(['b-only']);
        });
        test('permission filter: doc with allowedPrincipals is hidden from non-listed principals', async function () {
            await engine.index(makeDoc({
        ...
        test('permission filter: doc with allowedPrincipals IS returned to a listed principal', async function () {
            await engine.index(makeDoc({
                documentId: 'restricted',
                tenantId: 'tenant-perm',
                title: 'anniversary briefing',
            ...
            expect(alice.map(function (d) {
                return d.documentId;
            })).toEqual(['restricted']);
        });
        test('doc with no permissionAttributes is returned to any principal in the tenant', async function () {
            await engine.index(makeDoc({
                documentId: 'public',
                tenantId: 'tenant-pub',
            ...
            expect(u1.map(function (d) {
                return d.documentId;
            })).toEqual(['public']);
            expect(u2.map(function (d) {
                return d.documentId;
            })).toEqual(['public']);
        });
        test('upsert on the same (tenantId, documentType, documentId) replaces, not duplicates', async function () {
            await engine.index(makeDoc({
                documentId: 'fam-1',
                tenantId: 'tenant-up',
                title: 'first version',
        ...
        test('[error-shape] empty query returns [] (no error, no implicit "match-all")', async function () {
        ...
        test('query that does not match any field returns no results', async function () {
            await engine.index(makeDoc({
                documentId: 'doc-y',
        ...
        test('deleteByDocument removes the row from search results', async function () {
            await engine.index(makeDoc({
        ...
        test('deleteByDocument scoped by tenantId — does not delete cross-tenant rows', async function () {
            await engine.index(makeDoc({
        ...
        test('search filter matches across documentType: family + variant in the same tenant', async function () {
            await engine.index(makeDoc({
                documentId: 'fam-1',
                documentType: 'family',
                tenantId: 'tenant-mt',
            ...
            const types = r.map(function (d) {
                return d.documentType;
            }).sort();
            expect(types).toEqual(['family', 'variant']);
        });
        test('[concurrency] concurrent index calls produce a stable, deduplicated result set', async function () {
            const ops: Promise<void>[] = [];
            for (let i = 0; i < 5; i++) {
                ops.push(engine.index(makeDoc({
                    documentId: `doc-c-${i}`,
            ...
            const ids = new Set(r.map(function (d) {
                return d.documentId;
            }));
