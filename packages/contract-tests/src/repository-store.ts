...
export function runRepositoryStoreContract(opts: RepositoryStoreFactoryOptions): void {
    describe('RepositoryStore + RepositoryRevisionStore contract', function () {
        ...
        test('create + getBySlug + get + list round-trip', async function () {
            ...
            const bySlug = assertDefined(await store.getBySlug(TENANT, 'hello-world'), 'getBySlug returns the row just created');
            expect(bySlug.repoId).toBe(input.repoId);
            expect(bySlug.repoSlug).toBe('hello-world');
            expect(bySlug.name).toBe('Hello');
            const byId = assertDefined(await store.get(TENANT, input.repoId), 'get(repoId) returns the row just created');
        ...
        test('append + getMetadata + listForRepo + getBytes round-trip', async function () {
            ...
            const meta = assertDefined(await revisions.getMetadata(TENANT, rev.revisionId), 'getMetadata returns the revision just appended');
            ...
            expect(list.map(function (r: RevisionRecord) {
                return r.revisionId;
            })).toEqual([rev.revisionId]);
            const got = assertDefined(await revisions.getBytes(TENANT, rev.revisionId), 'getBytes returns the revision just appended');
        ...
        test('bytes are exactly the BYTEA inserted (no encoding drift)', async function () {
            ...
            await revisions.append(TENANT, rev);
            const got = assertDefined(await revisions.getBytes(TENANT, rev.revisionId), 'getBytes returns the binary revision just appended');
