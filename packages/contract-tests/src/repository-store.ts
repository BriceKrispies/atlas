...
export function runRepositoryStoreContract(opts: RepositoryStoreFactoryOptions): void {
  describe('RepositoryStore + RepositoryRevisionStore contract', () => {
    ...
    test('create + getBySlug + get + list round-trip', async () => {
    ...
    test('create idempotency at adapter level — duplicate (tenantId, repoSlug) throws', async () => {
    ...
    test('get returns null for cross-tenant lookup', async () => {
    ...
    test('append + getMetadata + listForRepo + getBytes round-trip', async () => {
    ...
    test('listForRepo returns most-recent first', async () => {
      const repo = makeRepoInput();
      await store.create(TENANT, repo);
      ...
      expect(list.map((r: RevisionRecord) => r.revisionId)).toEqual([...ids].reverse());
    });

    test('getBytes returns null for unknown revision id', async () => {
      const got = await revisions.getBytes(TENANT, 'no-such-revision');
      expect(got).toBeNull();
    });

    test('bytes are exactly the BYTEA inserted (no encoding drift)', async () => {
      const repo = makeRepoInput();
      await store.create(TENANT, repo);
