...
export function wasmHostContract(factory: WasmHostFactory): void {
    describe('WasmHost contract', function () {
        test('invokes a noop plugin and returns the parsed JSON object', async function () {
        ...
        test('rejects a module with host imports (zero-authority)', async function () {
            const { loader, makeHost } = factory();
            loader.set('with-imports', withImportsWasm);
            const host = makeHost();
            await expect(host.invoke({ pluginRef: 'with-imports', input: {} })).rejects.toMatchObject({ kind: 'HasImports' });
        });
        test("rejects a module missing the `memory` export", async function () {
            const { loader, makeHost } = factory();
            loader.set('no-memory', noMemoryExportWasm);
            const host = makeHost();
            await expect(host.invoke({ pluginRef: 'no-memory', input: {} })).rejects.toMatchObject({ kind: 'MissingExport' });
        });
        test('honors the timeout when the plugin runs longer than allowed', async function () {
            ...
            await expect(host.invoke({ pluginRef: 'noop-render', input: {}, timeoutMs: 0 })).rejects.toBeDefined();
        });
        test('input + output are passed by value (fresh state each invoke)', async function () {
            const { loader, makeHost } = factory();
            loader.set('noop-render', noopRenderWasm);
            const host = makeHost();
