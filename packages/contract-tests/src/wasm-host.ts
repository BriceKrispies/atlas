...
export const noMemoryExportWasm = new Uint8Array([
  ...
  0x07, 0x12, 0x02, 0x05, 0x61, 0x6c, 0x6c, 0x6f, 0x63, 0x00, 0x00, 0x06, 0x72, 0x65, 0x6e, 0x64,
  0x65, 0x72, 0x00, 0x01,
...
export function wasmHostContract(factory: WasmHostFactory): void {
  describe('WasmHost contract', function () {
    ...
    test('rejects a module with host imports (zero-authority)', async function () {
      ...
      const host = makeHost();
      await expect(host.invoke({ pluginRef: 'with-imports', input: {} })).rejects.toMatchObject({
        kind: 'HasImports',
      });
    });
    test('rejects a module missing the `memory` export', async function () {
      ...
      const host = makeHost();
      await expect(host.invoke({ pluginRef: 'no-memory', input: {} })).rejects.toMatchObject({
        kind: 'MissingExport',
      });
    });
