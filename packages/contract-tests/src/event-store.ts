...
export function eventStoreContract(makeStore: () => Promise<EventStore>): void {
    describe('EventStore contract', function () {
        ...
        test('append + getEvent round-trip preserves envelope shape', async function () {
            ...
            const fetched = assertDefined(await store.getEvent('evt-rt'), 'getEvent(evt-rt) after append');
        ...
        test('idempotency key is tenant-scoped — same key in different tenants produces two distinct events', async function () {
            ...
            expect(aEvents.map(function (e) {
                return e.eventId;
            })).toEqual(['evt-a']);
            expect(bEvents.map(function (e) {
                return e.eventId;
            })).toEqual(['evt-b']);
        });
        ...
        test('readEvents returns events strictly ascending by seq (insertion order)', async function () {
            ...
            await store.append(makeEvent({ eventId: 'evt-late', tenantId: 'tenant-ord', occurredAt: t(30) }));
            await store.append(makeEvent({ eventId: 'evt-early', tenantId: 'tenant-ord', occurredAt: t(0) }));
            await store.append(makeEvent({ eventId: 'evt-mid', tenantId: 'tenant-ord', occurredAt: t(15) }));
        ...
        test('cacheInvalidationTags survive append + getEvent round-trip', async function () {
            ...
            await store.append(env);
            const fetched = assertDefined(await store.getEvent('evt-tags'), 'getEvent(evt-tags) after append');
        ...
        test('causationId on a child event references the parent event', async function () {
            ...
            const fetched = assertDefined(await store.getEvent('evt-child'), 'getEvent(evt-child) after append');
        ...
        test('readEvents includes events with a wide variety of payload shapes', async function () {
            ...
            await store.append(makeEvent({ eventId: 'evt-shapes-2', tenantId: 'tenant-shapes', payload: 'string-body' }));
        ...
        test('[concurrency] 5 concurrent appends with the same idempotency key produce one event and one shared eventId', async function () {
            ...
            const results = await Promise.all(envelopes.map(function (e) {
            ...
            // persisted).
            const uniqueIds = new Set(results.map(function (r) {
                return r.eventId;
            }));
            expect(uniqueIds.size).toBe(1);
            const uniqueSeqs = new Set(results.map(function (r) {
                return r.seq;
            }));
        ...
        test('[concurrency] interleaved appends across tenants do not cross-contaminate', async function () {
            ...
            for (let i = 0; i < 10; i++) {
                ops.push(store.append(makeEvent({
                    eventId: `evt-x-a-${i}`,
                    tenantId: 'tenant-x-a',
            ...
            expect(aEvents.every(function (e) {
                return e.tenantId === 'tenant-x-a';
            })).toBe(true);
            expect(bEvents.every(function (e) {
