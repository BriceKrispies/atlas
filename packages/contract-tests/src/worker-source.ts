 ...
 */
async function drain(sub: WorkerSubscription, n: number, timeoutMs: number): Promise<EventEnvelope[]> {
    const out: EventEnvelope[] = [];
    const iter = sub.events()[Symbol.asyncIterator]();
    while (out.length < n) {
        const next = iter.next();
        const timer = sleep(timeoutMs).then(function () {
...
export function runWorkerSourceContract(name: string, makeFactory: () => Promise<WorkerSourceFactory>): void {
    describe(`WorkerSource contract [${name}]`, function () {
        test('subscribe + drain — yields all appended events in seq order', async function () {
            ...
            try {
                ...
                try {
                    ...
                    expect(events.map(function (e) {
                        return e.eventId;
                    })).toEqual([s1.eventId, s2.eventId, s3.eventId]);
        ...
        test('afterSeq filter — yields only events strictly after the cursor', async function () {
            const f = await makeFactory();
            try {
        ...
        test('ack does NOT filter active reads — afterSeq is the only filter', async function () {
            ...
            try {
                ...
                try {
                    ...
                    expect(replayed.map(function (e) {
                        return e.eventId;
                    })).toEqual(stored.map(function (s) {
                        return s.eventId;
                    }));
                }
                finally {
        ...
        test('ack monotonicity — acking a lower seq after a higher seq is a no-op', async function () {
        ...
        test('live wake — append after subscribe causes the iterator to yield within ~1s', async function () {
            const f = await makeFactory();
            try {
                const sub = f.source.subscribe('tenant-a', 0n);
                try {
        ...
        test('tenant isolation — subscribing to tenant A never yields tenant B events', async function () {
            const f = await makeFactory();
            try {
                const sub = f.source.subscribe('tenant-a', 0n);
                try {
                    ...
                    expect(events.map(function (e) {
                        return e.eventId;
                    })).toEqual([a1.eventId, a2.eventId]);
                    expect(events.every(function (e) {
                        return e.tenantId === 'tenant-a';
                    })).toBe(true);
        ...
        test('close terminates the for-await loop cleanly within ~1s', async function () {
            const f = await makeFactory();
            try {
                const sub = f.source.subscribe('tenant-a', 0n);
                const seen: string[] = [];
        ...
        test('out-of-tenant subscribe — stays open with 0 events while other tenants get appends', async function () {
            const f = await makeFactory();
            try {
