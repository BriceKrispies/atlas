...
export function secretStoreContract(
  ...
  describe('SecretStore contract', () => {
    ...
    test('returns the seeded value for a known name', () => {
    ...
    test('preserves the empty string as a present-but-empty value', () => {
      expect(store.get('EMPTY_VALUE')).toBe('');
    });

    test('lookups are case-sensitive', () => {
      expect(store.get('identity_encryption_key')).toBeNull();
      expect(store.get('IDENTITY_ENCRYPTION_KEY')).not.toBeNull();
    });

    test('repeated lookups return the same value (snapshot semantics)', () => {
