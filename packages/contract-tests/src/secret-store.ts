 ...
 */
export function secretStoreContract(makeStore: (seed: Readonly<Record<string, string>>) => Promise<SecretStore>): void {
    describe('SecretStore contract', function () {
        ...
        test('returns the seeded value for a known name', function () {
            expect(store.get('IDENTITY_ENCRYPTION_KEY')).toBe('AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=');
