...
export function cacheContract(makeCache: () => Promise<Cache>): void {
    describe('Cache contract', function () {
        ...
        test('set then get round-trips the value', async function () {
            await cache.set('k1', { hello: 'world' }, { ttlSeconds: 0, tags: [] });
            const v = await cache.get('k1');
            expect(v).toEqual({ hello: 'world' });
        });
        test('set with ttlSeconds=0 stores with no expiry', async function () {
            await cache.set('k-noexp', 'v', { ttlSeconds: 0, tags: [] });
            await sleep(10);
            const v = await cache.get('k-noexp');
            expect(v).toBe('v');
        });
        test('get within TTL returns the value, get after TTL returns null', async function () {
        ...
        test('set then set with the same key overwrites the value', async function () {
        ...
        test('invalidateByKey returns true when the entry existed', async function () {
        ...
        test('invalidateByKey returns false when the entry was never set', async function () {
            const removed = await cache.invalidateByKey('never-existed');
            expect(removed).toBe(false);
        });
        test('invalidateByKey is idempotent on repeated calls', async function () {
        ...
        test('invalidateByTags purges every entry with at least one matching tag', async function () {
        ...
        test('invalidateByTags with multiple tags purges entries matching ANY tag', async function () {
        ...
        test('invalidateByTags returns 0 when no entries match', async function () {
        ...
        test('invalidateByTags with an empty tag list returns 0 and changes nothing', async function () {
        ...
        test('multiple tags on a single entry — purging by ANY one of them removes it once', async function () {
        ...
        test('cache keys with the same string but stored separately stay distinct (key-namespacing is the caller’s job)', async function () {
        ...
        test('values survive a JSON-shaped round trip including nested arrays/objects', async function () {
            const value = { a: 1, b: [1, 2, 3], c: { nested: true } };
            await cache.set('k-shape', value, { ttlSeconds: 0, tags: [] });
            const v = await cache.get('k-shape');
            expect(v).toEqual(value);
        });
        test('[concurrency] interleaved sets across keys do not lose entries', async function () {
        ...
        test('[error-shape] invalidateByTags([]) is a no-op that returns 0 (does not error)', async function () {
        ...
        test('[concurrency] two invalidateByTags calls racing over overlapping rows do not double-count', async function () {
            ...
            for (let i = 0; i < 4; i++) {
                setOps.push(cache.set(`k-a-${i}`, i, { ttlSeconds: 0, tags: ['tagA'] }), cache.set(`k-b-${i}`, i, { ttlSeconds: 0, tags: ['tagB'] }), cache.set(`k-ab-${i}`, i, { ttlSeconds: 0, tags: ['tagA', 'tagB'] }));
            }
