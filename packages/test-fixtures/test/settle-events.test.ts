...
describe('settleEvents', () => {
  ...
  it('defaults afterSeq to 0n (drains everything)', async () => {
  ...
  it('sorts ascending even when readEvents returns out-of-order events', async () => {
  ...
  it('returns afterSeq + 0 processed when stream is empty past cursor', async () => {
    const events = [envelope(1n), envelope(2n)];
    const store = fakeStore(events);
    let calls = 0;
    const dispatch: EventDispatcher = async () => {
      calls += 1;
    };
  ...
  it('rethrows dispatcher errors with the eventId in the message', async () => {
