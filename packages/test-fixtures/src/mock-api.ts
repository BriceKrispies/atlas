...
export async function mockApi(page: Page, config: MockApiConfig): Promise<void> {
    if (config.pages !== undefined) {
        ...
        await page.route(`${API_BASE}/pages`, async function (route) {
            ...
            if (response.delay !== undefined) {
                await new Promise<void>(function (r) {
                    return setTimeout(r, response.delay);
                });
            }
    ...
    if (config.intents !== undefined) {
        ...
        await page.route(`${API_BASE}/intents`, async function (route) {
...
function isIntentRouteHandler(v: IntentRouteHandler | MockValue<unknown>): v is IntentRouteHandler {
