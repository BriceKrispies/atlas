...
function normalizeHandler(
  ...
  return (ev: Event): unknown => Reflect.apply(fn, null, [ev]);
}
...
export function html(
    ...
    if (i < values.length) {
      ...
      if (value instanceof DocumentFragment || value instanceof HTMLElement) {
        ...
        if (attrMatch) {
          ...
          const attrName = must(
            attrMatch[1],
            'event/prop attr capture group present',
          );
  ...
  const elements: NodeListOf<Element> | null = bindings.some(
    (b) => b.type === 'attr',
  )
  ...
  for (const binding of bindings) {
    if (binding.type !== 'attr' || !elements) continue;
    for (const node of elements) {
      for (const attr of [...node.attributes]) {
        if (attr.value !== binding.id) continue;
