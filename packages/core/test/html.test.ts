 ...
 */
function queryHtmlElement(frag: DocumentFragment, selector: string): HTMLElement {
    const el = queryElement(frag, selector);
    if (!(el instanceof HTMLElement)) {
        throw new Error(`expected ${selector} to be an HTMLElement, got ${el.constructor.name}`);
    }
    return el;
}
describe('html — text escaping', function () {
    it('escapes < > & " \' inside interpolated text', function () {
    ...
    it('renders numbers and booleans as their string form (still escaped)', function () {
        const frag = html `<span>${42}</span>`;
        expect(queryElement(frag, 'span').textContent).toBe('42');
    });
    it('coerces null/undefined to empty string rather than printing literally', function () {
        const frag = html `<span>[${null}][${undefined}]</span>`;
        expect(queryElement(frag, 'span').textContent).toBe('[][]');
    });
});
describe('html — event binding', function () {
    it('@click=${handler} attaches a click listener and removes the attribute', function () {
        const handler = vi.fn();
    ...
    it('non-event function value (no @ prefix) is rendered as escaped text', function () {
        const fn = function (): string {
            return 'hello';
        };
        const frag = html `<span>${fn}</span>`;
        const span = queryHtmlElement(frag, 'span');
        // Coerced via String(value) and escaped — the rendered text is
        // the function's source, which always contains the `function`
        // keyword regardless of arrow-vs-expression form.
        expect(span.textContent).toContain('function');
    });
});
describe('html — property binding', function () {
    it('.prop=${fn} sets the property on the element via function-value binding', function () {
...
describe('html — attribute interpolation', function () {
    it('escapes interpolated attribute values', function () {
...
describe('html — node interpolation', function () {
    it('inlines a DocumentFragment into the parent without re-encoding', function () {
        const inner = html `<em>nested</em>`;
        const frag = html `<p>${inner}</p>`;
        const em = queryElement(frag, 'em');
        expect(em.textContent).toBe('nested');
    });
    it('inlines an HTMLElement directly', function () {
...
describe('html — array interpolation', function () {
    it('renders an array of strings escaped and concatenated', function () {
        const items = ['<a>', '<b>', '<c>'];
        const frag = html `<p>${items}</p>`;
        expect(queryElement(frag, 'p').textContent).toBe('<a><b><c>');
    });
    it('renders an array mixing fragments and strings', function () {
        const fragments = [
            html `<li>one</li>`,
            html `<li>two</li>`,
        ];
...
describe('html — conditional rendering', function () {
    it('renders one branch when a ternary picks a fragment', function () {
        const show = true;
        const frag = html `<div>${show ? html `<span>yes</span>` : ''}</div>`;
        expect(queryElement(frag, 'span').textContent).toBe('yes');
    });
    it('renders empty string for a false-y branch', function () {
        const show = false;
        const frag = html `<div>${show ? html `<span>yes</span>` : ''}</div>`;
