...
describe('sha256Hex', function () {
    it('hashes the empty string to the well-known SHA-256 zero-length digest', function () {
        // RFC test vector: SHA-256("") = e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855.
        expect(sha256Hex('', nodeCrypto)).toBe('e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855');
    });
    it('hashes an ASCII string to the well-known "abc" digest', function () {
        // RFC test vector: SHA-256("abc") = ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad.
        expect(sha256Hex('abc', nodeCrypto)).toBe('ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad');
    });
    it('hashes a UTF-8 string with non-ASCII codepoints (string is encoded as UTF-8 bytes)', function () {
    ...
    it('hashes Uint8Array input identically to the same bytes passed as a string-encoded value', function () {
        const bytes = new Uint8Array([0x61, 0x62, 0x63]); // "abc"
        expect(sha256Hex(bytes, nodeCrypto)).toBe('ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad');
    });
    it('is deterministic — repeated calls with the same input return identical hex', function () {
