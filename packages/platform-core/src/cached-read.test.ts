...
describe('CachedRead', function () {
    test('cache hit: returns cached value, compute NOT called', async function () {
        const cache = new InMemoryCache();
        cache.data.set('k', { hello: 'world' });
        const cr = new CachedRead<{
            hello: string;
        }>(cache);
        const compute = vi.fn(async function () {
            return ({ hello: 'should-not-run' });
        });
    ...
    test('cache miss + concurrent callers: compute runs exactly once', async function () {
        const cache = new InMemoryCache();
        ...
        const calls = Array.from({ length: 10 }, function () {
            return cr.get('k', opts, compute);
        });
    ...
    test('compute error: propagates to all waiters, NO cache write', async function () {
        const cache = new InMemoryCache();
        const cr = new CachedRead<number>(cache);
        const gate = deferred<void>();
        ...
        const calls = Array.from({ length: 5 }, function () {
            return cr.get('k', opts, compute).then(function (v) {
    ...
    test('successful compute writes to cache with correct TTL + tags', async function () {
        const cache = new InMemoryCache();
        const cr = new CachedRead<{
            id: string;
        }>(cache);
        ...
        const value = await cr.get('cache:doc:t-7:p-42', myOpts, async function () {
            return ({
                id: 'doc-1',
            });
        });
    ...
    test('subsequent call after settle: hits cache, does NOT re-run compute', async function () {
        const cache = new InMemoryCache();
        const cr = new CachedRead<number>(cache);
        let calls = 0;
        const compute = vi.fn(async function () {
    ...
    test('different keys do not share a single-flight slot', async function () {
        const cache = new InMemoryCache();
        const cr = new CachedRead<string>(cache);
        const gateA = deferred<void>();
        const gateB = deferred<void>();
        const fnA = vi.fn(async function () {
            await gateA.promise;
            return 'A';
        });
        const fnB = vi.fn(async function () {
            await gateB.promise;
            return 'B';
        });
    ...
    test('accepts an injected SingleFlight for shared in-flight slots', async function () {
        const cache = new InMemoryCache();
        const sf = new SingleFlight<string, number>();
        const cr = new CachedRead<number>(cache, sf);
        const value = await cr.get('k', opts, async function () {
            return 1;
        });
        expect(value).toBe(1);
        expect(cache.setCalls).toHaveLength(1);
