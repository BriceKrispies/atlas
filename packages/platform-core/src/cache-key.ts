 ...
 */
export type CacheErrorKind = 'MissingRequiredKeyPart' | 'MissingPlaceholder' | 'InvalidPrivacyConfiguration' | 'InvalidTagTemplate';
...
export class CacheError extends Error {
    ...
    constructor(kind: CacheErrorKind, detail: CacheErrorDetail, invariant?: 'I9' | 'I10') {
...
function formatCacheErrorMessage(kind: CacheErrorKind, detail: CacheErrorDetail): string {
    switch (kind) {
 ...
 */
export function buildCacheKey(artifact: CacheArtifact, keyValues: Readonly<Record<string, string>>, varyValues?: Readonly<Record<string, string>>): string {
 ...
 */
function buildVaryHash(varyBy: ReadonlyArray<VaryDimension>, varyValues: Readonly<Record<string, string>>): string {
 ...
 */
export function renderTags(artifact: CacheArtifact, keyValues: Readonly<Record<string, string>>, varyValues?: Readonly<Record<string, string>>): string[] {
    const out: string[] = [];
...
function renderTagTemplate(template: string, keyValues: Readonly<Record<string, string>>, varyValues: Readonly<Record<string, string>> | undefined): string {
    let result = template;
    const placeholders = extractAllPlaceholders(template);
    for (const placeholder of placeholders) {
        const replacement = keyValues[placeholder] ??
            (varyValues !== undefined ? varyValues[placeholder] : undefined);
        if (replacement === undefined) {
            throw new CacheError('MissingPlaceholder', { placeholder, tagTemplate: template }, 'I10');
        }
...
export function validateCacheArtifact(artifact: CacheArtifact): void {
    if (artifact.privacy !== 'PUBLIC') {
        const hasTenantTag = artifact.tags.some(function (tag) {
            return tag.includes('{tenantId}') || tag.includes('{tenant_id}');
        });
        if (!hasTenantTag) {
            throw new CacheError('InvalidPrivacyConfiguration', {
                description: 'tenantId must be in tag templates unless privacy is PUBLIC',
            }, 'I9');
        }
    }
    if (artifact.privacy === 'USER') {
        const hasUser = artifact.varyBy.some(function (dim) {
            return dim === 'USER';
        });
        if (!hasUser) {
            throw new CacheError('InvalidPrivacyConfiguration', {
 ...
 */
export function validateCacheKeyInputs(artifact: CacheArtifact, keyValues: Readonly<Record<string, string>>, varyValues?: Readonly<Record<string, string>>): void {
    for (const tag of artifact.tags) {
        const placeholders = extractAllPlaceholders(tag);
        for (const placeholder of placeholders) {
            const found = Object.prototype.hasOwnProperty.call(keyValues, placeholder) ||
                (varyValues !== undefined &&
                    Object.prototype.hasOwnProperty.call(varyValues, placeholder));
            if (!found) {
                throw new CacheError('MissingRequiredKeyPart', { placeholder }, 'I9');
            }
        }
    }
}
