...
export function jsonbPathExpr(path: string): string {
    ...
    const head = parts.slice(0, -1).map(function (p) {
...
export function createIndexSql(decl: IndexDeclarationRow): string {
    ...
    const exprs = decl.field_paths.map(function (p) {
        return jsonbPathExpr(p);
    }).join(', ');
 ...
 */
export function reconcile(declared: IndexDeclarationRow[], liveIndexNames: ReadonlySet<string>): {
    create: string[];
    drop: string[];
} {
    ...
    for (const live of liveIndexNames) {
        ...
        if (!live.startsWith('entities_') || !live.endsWith('_idx'))
            continue;
        if (live === 'entities_tenant_type_status_idx')
            continue;
        if (live === 'entities_attrs_gin_idx')
            continue;
