...
describe('SingleFlight', function () {
    test('same key, concurrent: compute fn runs once, all callers get same value', async function () {
        const sf = new SingleFlight<string, number>();
    ...
    test('different keys: compute fns run in parallel', async function () {
    ...
    test('error propagation: all waiters receive the same error', async function () {
        const sf = new SingleFlight<string, number>();
        const gate = deferred<void>();
        ...
        const calls = Array.from({ length: 5 }, function () {
            return sf.do('error-key', compute).then(function (v) {
    ...
    test('eviction after settle: subsequent call re-runs compute', async function () {
        const sf = new SingleFlight<string, number>();
        let callCount = 0;
        const compute = vi.fn(async function () {
            callCount += 1;
    ...
    test('eviction after error: subsequent call re-runs compute', async function () {
        const sf = new SingleFlight<string, number>();
        let callCount = 0;
        const compute = vi.fn(async function () {
            callCount += 1;
