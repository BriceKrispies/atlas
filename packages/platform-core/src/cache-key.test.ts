...
describe('cache-key parity (mirrors crates/core/src/cache.rs tests)', () => {
  test('build_cache_key_deterministic', () => {
    ...
    expect(key1).toBe(
      'cache:RenderPageModel@v300:acme:page-123:vary(locale=en-US)',
    );
  });

  test('build_cache_key without vary values omits vary segment', () => {
    const artifact = createTestArtifact();
    const keyValues = { tenantId: 'acme', pageId: 'page-123' };

    expect(buildCacheKey(artifact, keyValues)).toBe(
      'cache:RenderPageModel@v300:acme:page-123',
    );
  });

  test('build_cache_key with empty vary map omits vary segment', () => {
    const artifact = createTestArtifact();
    const keyValues = { tenantId: 'acme', pageId: 'page-123' };
    expect(buildCacheKey(artifact, keyValues, {})).toBe(
      'cache:RenderPageModel@v300:acme:page-123',
  ...
  test('build_cache_key with vary values that miss every dimension yields none', () => {
    const artifact = createTestArtifact();
    const keyValues = { tenantId: 'acme', pageId: 'page-123' };
    // varyBy=[LOCALE] but the only vary value is for `role` (not in varyBy)
    // → sorted is empty → varyHash is the literal "none". Mirrors Rust.
    expect(buildCacheKey(artifact, keyValues, { role: 'admin' })).toBe(
      'cache:RenderPageModel@v300:acme:page-123:none',
    );
  });

  test('build_cache_key_missing_key_part', () => {
    const artifact = createTestArtifact();
    const keyValues = { tenantId: 'acme' }; // missing pageId

    expect(() => buildCacheKey(artifact, keyValues)).toThrow(CacheError);
    try {
  ...
  test('render_tags_missing_placeholder', () => {
    const artifact = createTestArtifact();
    const keyValues = { tenantId: 'acme' }; // missing pageId
  ...
  test('validate_cache_artifact_tenant_privacy_requires_tenant_tag', () => {
  ...
  test('validate_cache_artifact_user_privacy_requires_principal', () => {
    const artifact: CacheArtifact = {
      ...createTestArtifact(),
      privacy: 'USER',
    ...
    expect(() => validateCacheArtifact(artifact)).toThrow(CacheError);
  });

  test('validate_cache_artifact_public_privacy_ok_without_tenant', () => {
    const artifact: CacheArtifact = {
      ...createTestArtifact(),
      privacy: 'PUBLIC',
    ...
    expect(() => validateCacheArtifact(artifact)).not.toThrow();
  });

  test('extract_placeholder', () => {
