...
function delay(): Promise<void> {
    return new Promise(function (resolve) {
        return setTimeout(resolve, 50 + Math.random() * 150);
    });
}
/**
 ...
 */
export async function getById(collection: string, id: string, idField: string = 'pageId'): Promise<CollectionItem | null> {
    await delay();
 ...
 */
export async function create(collection: string, item: CollectionItem): Promise<CollectionItem> {
    await delay();
    const items = collections.get(collection) ?? [];
    items.push(item);
 ...
 */
export async function remove(collection: string, id: string, idField: string = 'pageId'): Promise<boolean> {
    await delay();
    const items = collections.get(collection) ?? [];
 ...
 * Subscribe to events.
 */
export function subscribe(eventType: string, callback: BackendEventCallback): Unsubscribe {
