...
function hexAlpha(input: string): number {
    const eight = HEX8_RE.exec(input);
    if (eight && eight[1])
        return parseInt(eight[1].slice(6, 8), 16) / 255;
    return 1;
}
...
function parseSwatchList(raw: string | null): string[] {
    ...
    return raw
        ...
        .filter(function (s) {
        return s.length > 0 && hexToRgb(s) !== null;
    })
        .map(function (s) {
...
export class AtlasColorPicker extends AtlasElement {
    ...
    private _sync(name: string): void {
        ...
        switch (name) {
            ...
            case 'alpha': {
                if (this._alphaTrack) {
                    if (this.alpha)
                        this._alphaTrack.removeAttribute('hidden');
                    else
                        this._alphaTrack.setAttribute('hidden', '');
                }
                if (!this.alpha)
                    this._alphaVal = 1;
                this._renderUi();
    ...
    private _wireSv(): void {
        ...
        sv.addEventListener('pointermove', function (ev) {
            if (!sv.hasPointerCapture(ev.pointerId))
                return;
            onPointer(ev);
        });
        ...
        sv.addEventListener('keydown', (ev) => {
            ...
            switch (ev.key) {
                case 'ArrowRight':
                    this._hsv.s = clamp01(this._hsv.s + step);
                    break;
                case 'ArrowLeft':
                    this._hsv.s = clamp01(this._hsv.s - step);
                    break;
                case 'ArrowUp':
                    this._hsv.v = clamp01(this._hsv.v + step);
                    break;
                case 'ArrowDown':
                    this._hsv.v = clamp01(this._hsv.v - step);
                    break;
    ...
    private _wireHue(): void {
        ...
        tr.addEventListener('pointermove', function (ev) {
            if (!tr.hasPointerCapture(ev.pointerId))
                return;
            onPointer(ev);
        });
        ...
        tr.addEventListener('keydown', (ev) => {
            ...
            switch (ev.key) {
                case 'ArrowRight':
                    this._hsv.h = (this._hsv.h + step) % 360;
                    break;
                case 'ArrowLeft':
                    this._hsv.h = (this._hsv.h - step + 360) % 360;
                    break;
                case 'Home':
                    this._hsv.h = 0;
                    break;
    ...
    private _wireAlpha(): void {
        ...
        tr.addEventListener('keydown', (ev) => {
            ...
            switch (ev.key) {
                case 'ArrowRight':
                    this._alphaVal = clamp01(this._alphaVal + step);
                    break;
                case 'ArrowLeft':
                    this._alphaVal = clamp01(this._alphaVal - step);
                    break;
    ...
    private _renderUi(): void {
        ...
        if (this._sv) {
            ...
            this._sv.setAttribute('aria-valuetext', `Saturation ${Math.round(this._hsv.s * 100)}%, value ${Math.round(this._hsv.v * 100)}%`);
        }
        if (this._svDot) {
        ...
        if (this._alphaTrack) {
            ...
            this._alphaTrack.setAttribute('aria-valuenow', String(Math.round(this._alphaVal * 100)));
        }
        if (this._alphaThumb) {
    ...
    private _commit(emit: boolean, intermediate = false): void {
        ...
        this.dispatchEvent(new CustomEvent<AtlasColorPickerChangeDetail>('input', {
            detail,
            bubbles: true,
            composed: true,
        }));
        if (intermediate)
            return;
        this.dispatchEvent(new CustomEvent<AtlasColorPickerChangeDetail>('change', {
            detail,
            bubbles: true,
            composed: true,
        }));
