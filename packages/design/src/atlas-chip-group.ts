...
export class AtlasChipGroup extends AtlasElement {
    ...
    private _enforceSelectionMode(): void {
        ...
        if (mode === 'single') {
            const selected = this._chips().filter(function (c) {
                return c.hasAttribute('selected');
            });
    ...
    private _handleChipChange(e: Event): void {
        ...
        if (!(target instanceof HTMLElement))
            return;
        if (target.tagName.toLowerCase() !== 'atlas-chip')
            return;
        ...
        if (mode === 'single' && target.hasAttribute('selected')) {
            ...
            for (const c of this._chips()) {
                if (c !== target && c.hasAttribute('selected'))
                    c.removeAttribute('selected');
            }
        }
        const detail: AtlasChipGroupChangeDetail = { value: this._currentValue() };
        this.dispatchEvent(new CustomEvent<AtlasChipGroupChangeDetail>('change', {
            detail,
            bubbles: true,
            composed: true,
        }));
    ...
    private _currentValue(): string | string[] | null {
        ...
        const selected = chips.filter(function (c) {
            return c.hasAttribute('selected');
        });
        if (mode === 'multiple') {
            return selected.map(function (c) {
                return c.getAttribute('value') ?? c.textContent?.trim() ?? '';
            });
        }
        if (mode === 'single') {
    ...
    private _handleKey(e: KeyboardEvent): void {
        if (e.defaultPrevented)
            return;
        const enabled = this._chips().filter(function (c) {
        ...
        const rawActive = root instanceof Document || root instanceof ShadowRoot ? root.activeElement : null;
        const active = rawActive instanceof HTMLElement ? rawActive : null;
        const idx = enabled.findIndex(function (c) {
            return c === active || (active != null && c.contains(active));
        });
        if (idx < 0)
            return;
        let next = -1;
        ...
        const innerBtn = target.shadowRoot?.querySelector<HTMLButtonElement>('button.chip') ?? null;
