...
export class AtlasCommandPalette extends AtlasElement {
    ...
    set items(next: readonly PaletteItem[]) {
        this._items = next;
        if (this._built)
            this._filterAndRender(this._input?.value ?? '');
    }
    get isOpen(): boolean {
    ...
    private _render(): void {
        ...
        for (const [group, items] of groups) {
            if (group) {
                parts.push(`<div data-part="group-heading">${this._escape(group)}</div>`);
            }
            for (const item of items) {
                const active = flatIdx === this._activeIndex;
                parts.push(`<button type="button" role="option" data-palette-item data-index="${flatIdx}" aria-selected="${active ? 'true' : 'false'}">` +
    ...
    private _scrollActiveIntoView(): void {
        const active = this._list?.querySelector('[data-palette-item][aria-selected="true"]');
