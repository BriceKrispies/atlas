...
function toFilterTab(v: string): FilterTab {
    if (v === 'image' || v === 'video' || v === 'doc' || v === 'all')
        return v;
    return 'all';
}
...
export class AtlasMediaPicker extends AtlasElement {
    ...
    private _sync(name: string): void {
        ...
        switch (name) {
            ...
            case 'media-type':
                ...
                if (this._tabsEl)
                    this._tabsEl.value = this._filter;
                if (this._gridEl)
                    this._renderGrid();
                this._fireRequest();
                break;
    ...
    private _mountOverlay(): void {
        ...
        const hasBottomSheet = typeof customElements !== 'undefined' &&
        ...
        if (this._query)
            search.setAttribute('value', this._query);
        ...
        grid.setAttribute('aria-multiselectable', this.multiple ? 'true' : 'false');
    ...
    private _filteredItems(): MediaItem[] {
        ...
        return this._items.filter(function (it) {
            if (reqType === 'image' && it.kind !== 'image' && tab === 'all')
                return false;
            if (reqType === 'video' && it.kind !== 'video' && tab === 'all')
                return false;
    ...
    private _toggleSelect(id: string, tile: HTMLButtonElement): void {
        if (this.multiple) {
            if (this._selection.has(id))
                ...
                this._selection.add(id);
            tile.setAttribute('aria-checked', this._selection.has(id) ? 'true' : 'false');
        }
        else {
    ...
    private _onGridKey(ev: KeyboardEvent): void {
        ...
        switch (ev.key) {
            case 'ArrowRight':
                next = Math.min(tiles.length - 1, idx + 1);
                break;
            case 'ArrowLeft':
                next = Math.max(0, idx - 1);
                break;
            case 'ArrowDown':
                next = Math.min(tiles.length - 1, idx + cols);
                break;
            case 'ArrowUp':
    ...
    private _renderPreviewStrip(): void {
        ...
        if (ids.length === 0) {
            ...
            if (txt)
                txt.textContent = 'Browse media';
            return;
        }
        const items = ids.map((id) => this._items.find(function (it) {
            return it.id === id;
        })).filter(function (x): x is MediaItem {
            return !!x;
        });
        const max = 3;
    ...
    private _fireRequest(): void {
        ...
        this.dispatchEvent(new CustomEvent<AtlasMediaPickerRequestDetail>('request-items', {
            detail,
            bubbles: true,
            composed: true,
        }));
    ...
    private _emitChange(): void {
        ...
        this.dispatchEvent(new CustomEvent<AtlasMediaPickerChangeDetail>('change', {
            detail: { value },
            bubbles: true,
            composed: true,
        }));
