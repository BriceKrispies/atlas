...
export class AtlasToggleGroup extends AtlasElement {
    ...
    private _setValue(next: string | string[] | null | undefined, fire: boolean): void {
        if (this._selectionMode() === 'multiple') {
            ...
            if (sameSet(newSet, this._selected))
                return;
    ...
    private _syncItems(): void {
        ...
        const enabled = items.filter(function (it) {
            return !it.hasAttribute('disabled');
        });
    ...
    private _emitChange(): void {
        ...
        this.dispatchEvent(new CustomEvent<AtlasToggleGroupChangeDetail>('change', {
            detail,
            bubbles: true,
            composed: true,
        }));
    ...
    private _onKey = (ev: KeyboardEvent): void => {
        ...
        if (!(target instanceof HTMLElement))
            return;
        if (target.tagName.toLowerCase() !== 'atlas-toggle-group-item')
            return;
