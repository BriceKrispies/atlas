...
export class AtlasBreadcrumbs extends AtlasElement {
    ...
    private _render(): void {
        ...
        const items = Array.from(this.children).filter((el): el is HTMLElement => this._isItem(el));
    ...
    private _buildOverflowTrigger(items: HTMLElement[]): void {
        ...
        items.slice(1, items.length - 1).forEach(function (item) {
    ...
    private _recomputeOverflow(): void {
        ...
        if (items.length <= 2) {
            ...
            if (this._overflowItem)
                this._overflowItem.hidden = true;
            return;
        }
        // Reset.
        for (const item of items)
    ...
    private _toggleMenu(): void {
        if (!this._overflowBtn || !this._menuList)
            return;
    ...
    private readonly _onDocClick = (ev: Event): void => {
        if (!this._menuList || this._menuList.hidden)
            return;
