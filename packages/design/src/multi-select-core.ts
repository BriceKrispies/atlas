...
export class MultiSelectCore {
    ...
    constructor(config: CoreConfig = {}) {
        ...
        this._selected = Array.isArray(config.selected)
            ? config.selected
                .map(String)
                .filter((v) => this._options.some(function (o) {
                return o.value === v;
            }))
            : [];
        this._status =
    ...
    canSelect(value: string): boolean {
        ...
        if (this._selected.includes(value))
            return false;
        const opt = this._options.find(function (o) {
            return o.value === value;
        });
        if (!opt || opt.disabled)
            return false;
        if (this.max != null && this._selected.length >= this.max)
            return false;
        return true;
    }
    ...
    canCreate(): boolean {
        ...
        if (!q)
            return false;
        return !this._options.some(function (o) {
    ...
    setOptions(options: readonly unknown[]): Delta {
        ...
        const valid = new Set(this._options.map(function (o) {
            return o.value;
        }));
        const before = this._selected;
        const after = before.filter(function (v) {
            return valid.has(v);
        });
        const removed = before.filter(function (v) {
            return !valid.has(v);
        });
    ...
    setActive(index: number): Delta {
        ...
        if (n === 0) {
            ...
            this._activeIndex = clamp(index, 0, n - 1);
        this._notify();
        return NO_DELTA;
    }
    ...
    async loadOptions(query?: string): Promise<Status> {
        ...
        try {
            ...
            if (token !== this._loadToken)
                return this._status; // superseded
    ...
    private _clampActive(): void {
        ...
        if (n === 0)
            this._activeIndex = -1;
        else if (this._activeIndex >= n)
            this._activeIndex = n - 1;
    }
}
// ── helpers ───────────────────────────────────────────────────────
