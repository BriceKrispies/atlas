...
function supportsPopover(el: HTMLElement): el is PopoverHostElement {
    return (typeof Reflect.get(el, 'showPopover') === 'function' &&
...
function popoverApiAvailable(): boolean {
    return (typeof Reflect.get(HTMLElement.prototype, 'showPopover') === 'function');
}
...
export class AtlasPopover extends AtlasElement {
    ...
    private _anchorEls(): HTMLElement[] {
        ...
        return this._anchorSlot
            ...
            .filter(function (el): el is HTMLElement {
        ...
        if (entering)
            ...
            requestAnimationFrame(() => {
                ...
                const active = root instanceof Document || root instanceof ShadowRoot
                    ? root.activeElement
                    : null;
                if (active instanceof Node && this._surface?.contains(active))
                    return;
                this.close();
            });
        }
    ...
    private _handleDocPointerDown(e: PointerEvent): void {
        ...
        if (surface && path.includes(surface))
            return;
        // Anchor click is a toggle, not a dismiss — let its handler decide.
        for (const a of this._anchorEls())
            if (path.includes(a))
                return;
        this.close();
    }
    ...
    private _resolvePlacement(desired: Placement, rect: DOMRect, sw: number, sh: number, vw: number, vh: number, offset: number): Exclude<Placement, 'auto'> {
        if (desired !== 'auto') {
            // Honour the requested side; flip only if the desired side has no
            // room and the opposite does.
            const fits = function (p: Exclude<Placement, 'auto'>): boolean {
                switch (p) {
            ...
            if (!fits(desired) && fits(opposite[desired]))
                return opposite[desired];
            return desired;
        }
