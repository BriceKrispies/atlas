...
function adoptMonacoStylesIntoShadow(host: HTMLElement): () => void {
    ...
    const collect = function (): CSSStyleSheet[] {
        ...
        for (const src of Array.from(document.styleSheets)) {
            ...
            for (const rule of Array.from(rules)) {
                ...
                parts.push(txt);
                if (!hasMonaco && txt.includes('.monaco-editor'))
                    hasMonaco = true;
            }
            known.add(src);
            if (!hasMonaco || parts.length === 0)
                continue;
...
export function mount(host: HTMLElement): CodeEditorController {
    ...
    return {
        ...
        applyAttribute(name: string, value: string | null): void {
            switch (name) {
                case 'value':
                    if (value !== null && value !== editor.getValue())
