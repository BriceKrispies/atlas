...
export class AtlasFileUpload extends AtlasElement {
    ...
    private _buildShell(): void {
      ...
      <div class="drop" tabindex="${disabled ? -1 : 0}" role="button" aria-label="Upload files">
        ...
        <span class="hint">${escapeText(`${multiple ? 'Multiple files allowed' : 'One file'}${accept ? ` · ${accept}` : ''}`)}</span>
        <input
    ...
    `;
        ...
        this._list?.addEventListener('click', (e) => {
            ...
            if (!Number.isFinite(idx))
                return;
            this._files = this._files.filter(function (_, i) {
                return i !== idx;
            });
            this._renderFiles();
            this._emit();
    ...
    private _sync(name: string): void {
        ...
        switch (name) {
            ...
            case 'disabled':
                ...
                if (this._drop)
                    this._drop.tabIndex = this.disabled ? -1 : 0;
                break;
            case 'required':
    ...
    private _emit(): void {
        this._commit();
        this.dispatchEvent(new CustomEvent<AtlasFileUploadChangeDetail>('change', {
