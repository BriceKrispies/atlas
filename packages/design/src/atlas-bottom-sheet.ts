...
export class AtlasBottomSheet extends AtlasElement {
    ...
    private _buildShell(): void {
        ...
        actionsSlot.addEventListener('slotchange', function () {
        ...
        d.addEventListener('close', () => {
            ...
            this.dispatchEvent(new CustomEvent<AtlasBottomSheetCloseDetail>('close', {
    ...
    private _onGripUp(ev: PointerEvent): void {
        if (!this._dragging)
            return;
        if (ev.pointerId !== this._pointerId && ev.type !== 'pointercancel')
            return;
