...
export class AtlasMenu extends AtlasElement {
    ...
    open(anchor?: HTMLElement): void {
        ...
        if (anchor)
            ...
            this._lastAnchor = this._resolveAnchor();
    ...
    private _resolveAnchor(): HTMLElement | null {
        ...
        if (sel) {
            try {
                ...
                const found = root instanceof Document || root instanceof ShadowRoot
    ...
    private _onAnchorPressMove(e: PointerEvent): void {
        if (!this._pressOrigin || !this._pressTimer)
            return;
    ...
    private _show(): void {
        ...
        queueMicrotask(() => {
            ...
            const first = items.findIndex(function (i) {
                return !i.disabled;
            });
    ...
    private _hide(): void {
        ...
        this._items().forEach(function (i) {
            return i.removeAttribute('data-active');
        });
    ...
    private _handleDocPointerDown(e: PointerEvent): void {
        ...
        if (this._surface !== null && path.includes(this._surface))
            return;
        // Clicking the anchor itself should be a toggle, NOT a stray dismiss.
        if (this._anchorEl && path.includes(this._anchorEl))
            return;
        this.close();
    }
    ...
    private _emitSelect(value: string): void {
        this.dispatchEvent(new CustomEvent<AtlasMenuSelectDetail>('select', {
            detail: { value },
            bubbles: true,
            composed: true,
        }));
