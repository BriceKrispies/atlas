...
export class AtlasChipInput extends AtlasElement {
    ...
    set values(next: unknown) {
        const arr = Array.isArray(next) ? next.map(function (v) {
            return String(v);
        }) : [];
    ...
    set max(v: number | null) {
        if (v == null)
            ...
            this.setAttribute('max', String(Math.floor(v)));
    }
    ...
    private _removeAt(index: number): void {
        if (index < 0 || index >= this._values.length)
            return;
        this._values = this._values.filter(function (_, i) {
            return i !== index;
        });
        this._externalError = '';
        this._renderChips();
    ...
    private _dedupeIfBlocking(arr: string[]): string[] {
        if (this.getAttribute('duplicates') !== 'block')
            return arr;
    ...
    private _emitChange(): void {
        ...
        this.dispatchEvent(new CustomEvent<AtlasChipInputChangeDetail>('change', {
    ...
    private _refreshValidity(): void {
        const max = this.max;
        if (this._externalError) {
            this._internals.setValidity({ customError: true }, this._externalError);
            return;
        }
        if (max != null && this._values.length > max) {
            this._internals.setValidity({ rangeOverflow: true }, `At most ${max} entries`);
            return;
        }
