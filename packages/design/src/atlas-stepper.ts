...
export class AtlasStepper extends AtlasElement {
    ...
    override attributeChangedCallback(name: string): void {
        if (name === 'clickable' || name === 'orientation')
            this._propagate();
    }
    private _propagate(): void {
        ...
        steps.forEach(function (step, i) {
            ...
            if (clickable)
                step.setAttribute('data-clickable', '');
            else
                step.removeAttribute('data-clickable');
            // Last step gets a marker so the connector line is hidden.
            if (i === steps.length - 1)
                step.setAttribute('data-last', '');
            else
                step.removeAttribute('data-last');
        });
    }
        ...
        for (const node of path) {
            if (node instanceof Element && node.tagName.toLowerCase() === 'atlas-step') {
                ...
                if (elName && this.surfaceId)
                    this.emit(`${this.surfaceId}.${elName}-clicked`, { value });
                return;
            }
        }
    };
    private readonly _onKeydown = (ev: KeyboardEvent): void => {
        ...
        if (!step || step.parentElement !== this)
            return;
