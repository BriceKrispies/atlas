...
export class AtlasPagination extends AtlasElement {
    ...
    private _syncDisabled(): void {
        ...
        const last = this._lastPage();
        if (this._prev)
            this._prev.disabled = cur <= 1;
        if (this._next)
            this._next.disabled = cur >= last;
    }
    /**
    ...
    private _renderPages(): void {
        ...
        this._list.innerHTML = items
            ...
            return `<li><button type="button" data-page="${escapeAttr(String(item))}" aria-label="${escapeAttr(aLabel)}"${aria}>${escapeText(String(item))}</button></li>`;
        })
            .join('');
    ...
    private _onKey(ev: KeyboardEvent): void {
        ...
        switch (ev.key) {
            case 'ArrowRight':
                target = Math.min(last, cur + 1);
                break;
            case 'ArrowLeft':
                target = Math.max(1, cur - 1);
                break;
            case 'Home':
                target = 1;
                break;
    ...
    private _goto(page: number): void {
        ...
        this.dispatchEvent(new CustomEvent<AtlasPaginationChangeDetail>('change', {
            detail: { page: next },
            bubbles: true,
            composed: true,
        }));
        ...
        if (elName && this.surfaceId)
            this.emit(`${this.surfaceId}.${elName}-changed`, { page: next });
    }
}
