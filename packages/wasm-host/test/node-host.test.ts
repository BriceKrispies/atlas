...
wasmHostContract(function () {
    ...
    return {
        loader,
        makeHost: function () {
            return new WorkerWasmHost({ loader });
        },
    };
});
describe('NodeWasmHost (deprecated alias) is construction-equivalent', function () {
    test('invokes a noop plugin via the alias', async function () {
...
describe('WorkerWasmHost preemption + isolation (Chunk 12)', function () {
    test('terminates a CPU-bound plugin via worker.terminate within ~timeoutMs', async function () {
        const loader = new InMemoryPluginLoader();
        ...
        if (!(err instanceof WasmHostError))
            throw new Error('expected WasmHostError');
    ...
    test('surfaces a memory-cap error when the plugin grows past the limit', async function () {
        const loader = new InMemoryPluginLoader();
    ...
    test('a wedged plugin does not block the main event loop', async function () {
        const loader = new InMemoryPluginLoader();
        loader.set('infinite-loop', infiniteLoopWasm);
        const host = new WorkerWasmHost({ loader });
