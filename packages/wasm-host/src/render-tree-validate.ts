 ...
 * One of `"V1"` … `"V17"`.
 */
export type RenderTreeInvariant =
...
const utf8ByteLength = (() => {
  ...
  return (s: string) => Buffer.byteLength(s, 'utf8');
})();
...
export function validateRenderTree(input: unknown): asserts input is RenderTree {
  ...
  if (serialized !== undefined) {
    ...
    if (size > MAX_SERIALIZED_OUTPUT) {
      throw new RenderTreeError(
        'V15',
        `serialized render tree exceeds 1 MB limit (${size} bytes)`,
      );
    }
  }
  ...
  if (version !== 1) {
    throw new RenderTreeError(
      'V1',
      `unsupported render tree version: ${version}`,
    );
  }
...
function validateNode(
  ...
  if (counter.count > MAX_NODE_COUNT) {
    throw new RenderTreeError(
      'V14',
      `node count exceeds maximum of ${MAX_NODE_COUNT}`,
    );
  }
  ...
  if (isExtension && ctx === 'Fallback') {
    throw new RenderTreeError(
      'V10',
      `extension node '${nodeType}' is not allowed in fallback (must be primitive-only)`,
    );
  }
  ...
  if (isLeaf && 'children' in obj) {
    throw new RenderTreeError(
      'V7',
      `leaf node '${nodeType}' must not have children`,
    );
  }
  ...
  if (isExtension) {
    if (!('fallback' in obj)) {
      throw new RenderTreeError(
        'V9',
        `extension node '${nodeType}' missing required fallback`,
      );
    }
    ...
    throw new RenderTreeError(
      'V9',
      `primitive node '${nodeType}' must not have fallback`,
    );
  }
}
  ...
  switch (ctx) {
    ...
    case 'Fallback':
      if (!BLOCK_TYPES.has(nodeType)) {
        throw new RenderTreeError(
          'V8',
          `node '${nodeType}' is not allowed at block level`,
        );
      }
      break;
    case 'Inline':
      if (!INLINE_TYPES.has(nodeType)) {
        throw new RenderTreeError(
          'V8',
          `node '${nodeType}' is not allowed at inline level`,
        );
      }
      break;
    case 'ListChildren':
      if (nodeType !== 'list_item') {
        throw new RenderTreeError(
          'V8',
          `list children must be list_item, got '${nodeType}'`,
        );
      }
      break;
  }
  ...
  for (const [key, val] of Object.entries(props)) {
    if (typeof val === 'string') {
      ...
      if (size > MAX_PROP_VALUE_SIZE) {
        throw new RenderTreeError(
          'V16',
          `prop '${key}' value exceeds 100 KB limit (${size} bytes)`,
        );
      }
    } else if (
      typeof val === 'number' ||
      ...
      throw new RenderTreeError(
        'V5',
        `prop '${key}' value must be a JSON primitive (string, number, boolean, null)`,
      );
    }
  }
  ...
  switch (nodeType) {
    case 'text': {
      ...
      if (content === undefined) {
        throw new RenderTreeError(
          'V6',
          'text node missing required prop: content',
        );
      }
    ...
    case 'heading': {
      ...
      if (level === undefined) {
        throw new RenderTreeError(
          'V6',
          'heading node missing required prop: level',
        );
      }
      if (
        typeof level !== 'number' ||
    ...
    case 'image': {
      ...
      if (src === undefined) {
        throw new RenderTreeError(
          'V6',
          'image node missing required prop: src',
        );
      }
      ...
      if (alt === undefined) {
        throw new RenderTreeError(
          'V6',
          'image node missing required prop: alt',
        );
      }
    ...
    case 'link': {
      ...
      if (href === undefined) {
        throw new RenderTreeError(
          'V6',
          'link node missing required prop: href',
        );
      }
    ...
    case 'list': {
      ...
      if (ordered === undefined) {
        throw new RenderTreeError(
          'V6',
          'list node missing required prop: ordered',
        );
      }
...
function validateUrlScheme(
  ...
  throw new RenderTreeError(
    invariant,
    `${field} has disallowed scheme (allowed: ${JSON.stringify(allowed)}): ${truncated}`,
  );
}
