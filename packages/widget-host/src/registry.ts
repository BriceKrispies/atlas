...
import type { WidgetManifest, WidgetElementClass, WidgetRegistration, } from './types.ts';
...
export class WidgetRegistry {
    ...
    register({ manifest, element, schema = null }: WidgetRegisterEntry): void {
        ...
        if (typeof element !== 'function') {
            throw new WidgetManifestError(`widget element for ${manifest.widgetId ?? '<unknown>'} must be a class`);
        }
        const { ok, errors } = validateManifest(manifest);
        if (!ok) {
            throw new WidgetManifestError(`invalid manifest for widgetId=${manifest.widgetId ?? '<unknown>'}`, { errors });
        }
    ...
    list(): WidgetRegistryListing[] {
        return [...this._entries.values()].map(function ({ manifest }) {
            return ({
