...
import type { HostToWidgetMessage, WidgetToHostMessage, } from '../types.ts';
...
function isWidgetMessage(data: unknown): data is WidgetToHostMessage {
    ...
    return (kind === 'widget-ready' ||
        kind === 'publish' ||
        kind === 'capability.invoke' ||
        kind === 'log');
}
export function createPostMessageTransport({ iframe, onReady, onPublish, onCapabilityInvoke, onLog, }: PostMessageTransportArgs): PostMessageTransport {
    const handler = function (event: MessageEvent): void {
        ...
        if (data.kind === 'log') {
            if (!onLog)
                return;
            const level = data.level === 'info' || data.level === 'warn' || data.level === 'error'
                ? data.level
                : 'info';
            const args = Array.isArray(data.args)
                ? data.args.map(function (a) {
                    return String(a);
                })
                : [];
            try {
    ...
    const send = function (envelope: HostToWidgetMessage): void {
