...
import { WidgetManifestError, WidgetIsolationError, } from './errors.ts';
...
import type { CapabilityHandler, CapabilityTraceEvent, HostMountFn, IsolationMode, LayoutEntry, MediatorTraceEvent, MutationInfo, PageLayout, ResolvedWidgetModule, ResolveWidgetModuleUrl, WidgetRegistration, } from './types.ts';
 ...
 */
function telemetry(event: string, payload: TelemetryPayload, level: TelemetryLevel = 'debug'): void {
    emitTelemetry({
        eventName: event,
        level,
        source: 'widget-host',
...
export class WidgetHostElement extends AtlasElement {
    ...
    private _renderWidgetError(slotEl: HTMLElement, message: string, instanceId: string): void {
        const cell = html `
      <atlas-box
        padding="sm"
        data-widget-error
    ...
    private _mountAll(): void {
        ...
        this._mediator = new WidgetMediator(this.onMediatorTrace ? { onTrace: this.onMediatorTrace } : {});
        this._bridge = new CapabilityBridge(this.onCapabilityTrace ? { onTrace: this.onCapabilityTrace } : {});
        ...
        for (const sec of this.querySelectorAll<HTMLElement>(':scope > section[data-slot]')) {
            const name = sec.getAttribute('data-slot');
            if (name)
                existing.set(name, sec);
            sec.textContent = '';
        }
    ...
    applyMutation(info: MutationInfo): boolean {
        ...
        if (!currentLayout || !this._mediator)
            return false;
        const { action, instanceId, to, nextDoc } = info ?? {};
        if (!action || !nextDoc || !nextDoc.regions)
            return false;
        switch (action) {
            case 'add': {
                ...
                if (!slotEl)
                    return false;
                const entry = (nextDoc.regions[slotName] ?? []).find(function (e) {
            ...
            case 'remove': {
                if (!this._unmountInstance(instanceId))
                    return false;
                this._layout = { ...currentLayout, slots: nextDoc.regions };
                return true;
            }
            case 'move': {
                ...
                if (!targetSlotEl)
                    return false;
                const entry = (nextDoc.regions[targetSlot] ?? []).find(function (e) {
                    return e.instanceId === instanceId;
                });
                if (!entry)
                    return false;
            ...
            case 'update': {
                ...
                if (!slotEl)
                    return false;
                const entry = (nextDoc.regions[region] ?? []).find(function (e) {
                    return e.instanceId === instanceId;
                });
                if (!entry)
                    return false;
    ...
    private _slotEl(slotName: string): HTMLElement | null {
        return this.querySelector<HTMLElement>(`:scope > section[data-slot="${slotName}"]`);
    }
    private _unmountInstance(instanceId: string): boolean {
        const mounted = this._mounted.find(function (m) {
        ...
        this._mounted = this._mounted.filter(function (m) {
            return m.instanceId !== instanceId;
        });
        const cell = this.querySelector(`:scope > section > [data-widget-cell][data-widget-instance-id="${instanceId}"]`);
    ...
    private _mountEntry({ registry, slotEl, entry, }: MountEntryArgs): void {
        ...
        try {
            ...
            const msg = err instanceof WidgetManifestError
                ? err.message
        ...
        const resolved: ResolvedWidgetModule = typeof this.resolveWidgetModuleUrl === 'function'
            ? this.resolveWidgetModuleUrl(widgetId)
            : null;
        const widgetModuleUrl: string | null = resolved && typeof resolved === 'object'
            ? resolved.url
            : (resolved ?? null);
        const supportModuleUrls: string[] = resolved &&
        ...
        const pending = Promise.resolve()
            ...
            .then((fn) => {
            unmount = typeof fn === 'function' ? fn : function () { };
            if (!mountFailed) {
