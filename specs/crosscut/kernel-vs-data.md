...
Per-tenant entity types and DDL — the `CustomSchema` instances tenants author to express their Salesforce-shaped data model. Each tenant gets a dedicated Postgres database (`atlas_t_<tenantUuid>` is a **database** name, not a schema name, per [ADR 0005](../decisions/0005-custom-schema-storage-strategy.md) — tables inside it live in `public`, with platform-owned tables marked by the `_atlas_` prefix); schema mutations route through the constrained DDL allowlist enforced by I16 (`CREATE TABLE`, `ADD COLUMN`, `CREATE INDEX`, safe `ALTER COLUMN`, `DROP COLUMN`, `DROP TABLE`; nothing else). Isolation is at the protocol layer (separate database = separate connection target, separate catalog, separate WAL). The kernel-side surface is the `EntityTypeRegistry` port ([`ports/src/entity-type-registry.ts`](../../ports/src/entity-type-registry.ts)); the runtime data is the tenant's accumulated type declarations. Tenant schemas can change between any two requests; the kernel sees the new schema on the next read because the registry is queried per request, not captured at boot.
