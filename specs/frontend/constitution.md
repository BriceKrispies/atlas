...
**C2.8** Interactive surfaces (charts, editors, drag/resize) MUST additionally expose their state through the `@atlas/test-state` registry so tests can assert committed intents rather than scraping DOM. See `interaction-contracts.md` for the commit envelope and canonical intents per surface type.
