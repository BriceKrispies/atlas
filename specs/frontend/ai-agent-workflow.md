...
```
frontend/apps/{app}/src/features/{module}/{feature}/{surface-name}.test.js
```
...
├── {surface-name}.test.js              # Already created in Step 4 (co-located)
└── index.js                           # Public exports
```

**Implementation rules:**
1. Extend `@atlas/core` `AtlasSurface` for page/widget/dialog surfaces.
2. Use **only** atlas custom elements in `render()` — no raw HTML. Use `<atlas-box>`, `<atlas-text>`, `<atlas-heading>`, `<atlas-stack>` for layout/text, `<atlas-button>`, `<atlas-input>` for interaction, `<atlas-table>`, `<atlas-row>`, `<atlas-table-head>`, `<atlas-table-body>`, `<atlas-table-cell>` for tables.
3. Use `html` tagged template literal for all rendering — never raw `innerHTML`.
4. Set `static surfaceId` on the surface class.
5. Use `@atlas/api-client` for all API calls — never raw `fetch`.
6. Use `emit()` for custom telemetry events.
7. Add `name` attribute to elements that need test IDs — `data-testid` is auto-generated by `AtlasElement`. NEVER set `data-testid` manually.
8. Implement all required states (loading, empty, success, error, unauthorized) using signals and `setState()`.
9. ARIA attributes are handled by atlas elements (e.g., `<atlas-heading level="2">` auto-sets `role="heading" aria-level="2"`, `<atlas-table-cell header>` auto-sets `role="columnheader"`).
...
| Using raw HTML (`<div>`, `<p>`, `<h1>`, `<table>`) in `render()` | Breaks "all elements are atlas web components" rule, misses auto-testid | Use atlas elements: `<atlas-box>`, `<atlas-text>`, `<atlas-heading>`, `<atlas-table>` etc. |
| Manually setting `data-testid` in templates | Bypasses the auto-generation system, creates maintenance burden | Add `name` attribute instead — `data-testid` is auto-generated |
| Using `innerHTML` instead of `html` template | Bypasses auto-escaping, enables XSS | Always use `html` tagged template |
| Not extending `AtlasSurface` for surfaces | Misses surfaceId, auto-testid context, setState | Always extend `@atlas/core` `AtlasSurface` |
| Not extending `AtlasElement` for custom elements | Misses auto-testid, lifecycle integration | Always extend `@atlas/core` `AtlasElement` |
...
| Adding `data-testid` manually instead of using `name` attr | Bypasses auto-generation, creates drift | Use `name` attr on atlas elements — testid is derived automatically |
