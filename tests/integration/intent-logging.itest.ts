...
test.describe('intent submission emits a correlated debug log trace', () => {
  ...
  test.beforeAll(async () => {
    ...
    if (!bump.ok) {
      test.skip(
        true,
        `could not bump server log level to debug: ${bump.status}`,
      );
    }
  });
  ...
  test('full pipeline produces all required boundary events with one correlationId', async () => {
    ...
    const submitted = trace.find((r) => r.eventName === 'Intent.Submitted');
    expect(submitted?.properties).toMatchObject({
      actionId: 'ContentPages.Page.Create',
      eventType: 'ContentPages.PageCreated',
    });
    const accepted = trace.find((r) => r.eventName === 'Intent.Accepted');
    expect(accepted?.properties).toHaveProperty('eventId');
    const completed = trace.find((r) => r.eventName === 'Request.Completed');
    ...
    const dispatched = trace.filter((r) => r.eventName === 'Dispatcher.Ran');
