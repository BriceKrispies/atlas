...
import type { LogEntry } from '../helpers/log-reader.ts';
const SURFACE = '[data-testid="admin.content.pages-list"]';
interface IngressLogFields {
    event_type?: string;
    action_id?: string;
    route?: string;
}
interface IngressLogEntry extends LogEntry {
    fields?: IngressLogFields;
}
test.describe('content pages — full stack', function () {
    test('create page flow: browser → ingress → DB → browser', async function ({ page, api, ingressLogs, }) {
        ...
        page.on('dialog', async function (dialog: Dialog) {
        ...
        ingressLogs.assertLogged(function (entry: LogEntry): boolean {
            const e = entry as IngressLogEntry;
            return (e.fields?.event_type?.includes('PageCreate') === true ||
                e.fields?.action_id === 'ContentPages.Page.Create' ||
                JSON.stringify(entry).includes('ContentPages.Page.Create'));
        }, 'Expected ingress to log the ContentPages.Page.Create intent');
    });
    test('delete page flow: browser → ingress → DB → browser', async function ({ page, api, ingressLogs, }) {
        // Seed a page via the real API
        await api.seedPage('Page To Delete', 'page-to-delete');
        ...
        ingressLogs.assertLogged(function (entry: LogEntry): boolean {
            return JSON.stringify(entry).includes('ContentPages.Page.Delete');
        }, 'Expected ingress to log the ContentPages.Page.Delete intent');
    });
    test('page list loads real data from the database', async function ({ page, api, ingressLogs, }) {
        ...
        ingressLogs.assertLogged(function (entry: LogEntry): boolean {
            const e = entry as IngressLogEntry;
            return (JSON.stringify(entry).includes('/api/v1/pages') ||
                e.fields?.route === '/api/v1/pages');
