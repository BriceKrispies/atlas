...
async function submitIntent(actionId: string, payload: Record<string, unknown>, principal: string, correlationId: string): Promise<IntentResponse> {
    const u = uid();
    const envelope = {
        eventId: `evt-${u}`,
        eventType: actionId,
...
test.describe('e2e — API key lifecycle', function () {
    test.beforeAll(async function () {
        if (!(await ingressUp())) {
            test.skip(true, `apps/server not reachable at ${INGRESS}`);
        }
    });
    test('create → use → rotate → predecessor in overlap → revoke → reject', async function () {
        ...
        const createSp = await submitIntent('Identity.ServicePrincipal.Create', {
            spId: SP_ID,
            ownerUserId: 'admin',
            displayName: 'itest sp',
            scopes: ['read', 'write'],
        }, adminPrincipal, correlationId);
        expect(createSp.correlationId).toBe(correlationId);
        // 2. Mint an API key bound to that SP.
        const createKey = await submitIntent('Identity.ApiKey.Create', {
            name: 'itest key',
            servicePrincipalId: SP_ID,
            scopes: ['read'],
        }, adminPrincipal, correlationId);
        const plaintextBearer = createKey.result?.['plaintextBearer'] as string | undefined;
        ...
        const keyId = (createKey.result?.['document'] as {
            keyId: string;
        }).keyId;
        const rotateResult = await submitIntent('Identity.ApiKey.Rotate', { keyId, overlapHours: 1 }, adminPrincipal, correlationId);
        const successorBearer = rotateResult.result?.['plaintextBearer'] as string | undefined;
        ...
        await submitIntent('Identity.ApiKey.Revoke', { keyId: successorKeyId }, adminPrincipal, correlationId);
