...
test.describe('e2e — OAuth client_credentials (Atlas-issued tokens)', () => {
  ...
  test('mint via /oauth/token, use Bearer against /api/v1, then revoke', async () => {
    ...
    const keyResult = await submitIntent(
  ...
  test('rejects malformed client credentials with 401 and OAUTH_INVALID_CLIENT shape', async () => {
    const tokenRes = await fetch(`${INGRESS}/oauth/token`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({
...
test.describe('e2e — Keycloak-issued OAuth tokens against Atlas', () => {
  test.beforeAll(async () => {
    if (!(await ingressUp())) {
      test.skip(true, `apps/server not reachable at ${INGRESS}`);
    }
  ...
  test('Keycloak-issued JWT for atlas-s2s validates against the ingress principal middleware', async () => {
    ...
    if (!useRes.ok) {
      ...
      console.warn(
        `Keycloak JWT was rejected by ingress (status ${useRes.status}). ` +
          `Verify OIDC_ISSUER_URL on the server matches the realm.`,
      );
      test.skip(
        true,
        'OIDC issuer mismatch — ingress is not configured for the itest realm',
      );
    }
    expect(useRes.ok).toBe(true);
  });
