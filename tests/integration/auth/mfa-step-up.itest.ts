...
test.describe('e2e — MFA step-up', () => {
  ...
  test('seed user → enroll TOTP → satisfy challenge → high-risk action allowed', async () => {
    ...
    await submitIntent(
      ...
      { userId: USER_ID, newPassword: PASSWORD },
      adminPrincipal,
    );
    ...
    const enrollResult = await submitIntent(
    ...
    const codeRes = await fetch(
      `${INGRESS}/debug/totp/code?secret=${encodeURIComponent(plaintextBase32)}`,
      {
    ...
    if (!codeRes.ok) {
      ...
      test.skip(
        true,
        '/debug/totp/code helper not available; enrollment-only path verified',
      );
      return;
    }
    ...
    const challengeResult = await submitIntent(
      'Identity.Mfa.Totp.Challenge',
      { factorId, presentedCode: code },
      userPrincipal,
    );
    expect(challengeResult.eventId).toBeDefined();
  });

  test('invalid TOTP code is rejected with TOTP_INVALID_CODE', async () => {
    const userPrincipal = `user:${USER_ID}:${TENANT}`;
    ...
    const listRes = await fetch(
      `${INGRESS}/api/v1/identity/users/${USER_ID}/factors`,
      { headers: { 'X-Debug-Principal': userPrincipal } },
    );
    if (!listRes.ok) {
      test.skip(true, 'factor-list endpoint not exposed; unit test covers');
      return;
    }
    const factors = (await listRes.json()) as Array<{ factorId: string }>;
    if (factors.length === 0) {
