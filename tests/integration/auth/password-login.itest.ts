...
async function submitIntent(
  ...
  if (!res.ok) {
    throw new Error(
      `intent ${envelope['eventType']} failed (${res.status}): ${await res.text()}`,
    );
  }
...
function envelope(
  ...
  correlationId: string,
  uid: string = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
...
test.describe('e2e — password login', () => {
  ...
  test('admin seeds user + password, user logs in, gets session, calls authenticated endpoint', async () => {
    ...
    const loginResult = await submitIntent(
      envelope(
  ...
  test('wrong password rejects with reason=wrong_password and emits no SessionIssued', async () => {
    ...
    const result = await submitIntent(
      envelope(
        'Identity.Login.Password',
        { email: EMAIL, password: 'BadGuess1!' },
        correlationId,
      ),
      anonPrincipal,
    );
