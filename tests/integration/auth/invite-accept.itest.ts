...
async function pollForRecipient(recipient: string, timeoutMs = 10000): Promise<Smtp4DevMessage> {
    const start = Date.now();
    const r = recipient.toLowerCase();
    while (Date.now() - start < timeoutMs) {
        const msgs = await smtp4devList();
        const match = msgs.find(function (m) {
...
test.describe('e2e — admin invite → user accept', function () {
    ...
    test('admin issues invite → email arrives → click magic-link → session established', async function ({ request, }) {
        const adminPrincipal = `user:admin:${TENANT}:admin`;
        // 1. Admin issues invite.
        await submitIntent('Identity.Invite.Issue', { email: EMAIL, rolesOnAccept: ['Author'] }, adminPrincipal);
        ...
        const sessionCookie = cookies.cookies.find(function (c) {
