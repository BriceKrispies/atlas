 ...
 *
 * Sibling: tests/bdd/features/tenancy/public-signup/admin-approves-signup.feature
 *   is the executable spec for the submit → admin-approve → email-dispatch
 *   portion of this loop, runnable via `pnpm bdd:server`. This itest stays
 *   as the tighter regression net (covers the magic-link-redirect +
 *   session-cookie crossing on top of the BDD assertions).
 *
 * Admin actor: `user:platform-admin:_platform:admin` resolves to the
 * platform-seeded operator identity (see
 * apps/server/src/bootstrap-platform-admin.ts). Slice 2 of the
 * admin-approves-signup-bdd ticket replaced the previous
 * `user:admin:<slug>:admin` synthesized principal so the test exercises
 * the same auth path production does.
 */
...
async function smtp4devGetPlainTextBody(messageId: string): Promise<string> {
    ...
    if (!res.ok)
        throw new Error(`smtp4dev plaintext failed: ${res.status}`);
    return res.text();
}
function messagesForRecipient(messages: readonly Smtp4DevMessage[], recipient: string): Smtp4DevMessage[] {
...
async function pollForMessage(recipient: string, timeoutMs = 10000, intervalMs = 250): Promise<Smtp4DevMessage> {
    ...
    throw new Error(`no message arrived for ${recipient} within ${timeoutMs}ms (last error: ${String(lastErr)})`);
}
function extractMagicLink(plaintextBody: string): string {
    ...
    if (!m) {
        throw new Error(`magic-link URL not found in email body. Body was:\n${plaintextBody}`);
    }
    return m[0];
}
test.describe('public signup → smtp4dev → magic link → tenant home', function () {
    ...
    test.beforeAll(async function () {
        ...
        if (!process.env['COOKIE_DOMAIN']) {
            test.skip(true, 'COOKIE_DOMAIN env var must be set (e.g. ".localhost" for dev) so the session cookie crosses the apex→subdomain redirect');
            return;
        }
        if (CP_URL) {
        ...
        await fetch(`${SMTP4DEV}/api/Messages/*`, { method: 'DELETE' }).catch(function () { });
        ...
        expect(initial.length, `smtp4dev inbox should be empty after cleanup; got ${initial.length} message(s). A parallel runner may be sharing this smtp4dev instance.`).toBe(0);
    });
    ...
    test('signup → email visible → click link → land on tenant home', async function ({ page, }: {
        page: Page;
    }) {
        ...
        await expect(page.locator('#ok')).toBeVisible({ timeout: 5000 });
        ...
        const listRes = await fetch(`${INGRESS}/api/v1/admin/signups?status=pending`, {
            headers: { 'X-Debug-Principal': `user:platform-admin:_platform:admin` },
        });
        ...
        // the wrong row.
        const signup = listBody.signups.find(function (s) {
            return s.email === EMAIL && s.tenantSlug === TENANT_SLUG;
        });
        if (!signup) {
            throw new Error(`signup not found in admin list. Got: ${JSON.stringify(listBody)}`);
        }
        const approveRes = await fetch(`${INGRESS}/api/v1/admin/signups/${signup.signupId}/approve`, {
            method: 'POST',
            headers: { 'X-Debug-Principal': `user:platform-admin:_platform:admin` },
        });
        ...
        await Promise.all([
            page.waitForURL(/^http:\/\/[^/]+\.localhost:\d+\/?/, { timeout: 10000 }),
        ...
        const sessionCookie = cookies.find(function (c) {
            return /session/i.test(c.name);
        });
        expect(sessionCookie, `expected a session cookie (matching /session/i; expected name "atlas_session"). Got: ${JSON.stringify(cookies)}`).toBeTruthy();
    ...
    test('confirm with invalid token returns error and sets no session cookie', async function () {
        ...
        const setCookies = typeof (res.headers as unknown as {
            getSetCookie?: () => string[];
        ...
        const sessionCookieHeader = setCookies.find(function (h) {
            return /session/i.test(h);
        });
        expect(sessionCookieHeader, `expected NO session cookie on failed confirm; got: ${JSON.stringify(setCookies)}`).toBeFalsy();
    });
    ...
    test('approve replay does not double-send mail', async function () {
        ...
        const signup = listBody.signups.find(function (s) {
            return s.email === EMAIL_2 && s.tenantSlug === TENANT_SLUG_2;
        });
        if (!signup) {
            throw new Error(`replay signup not found. Got: ${JSON.stringify(listBody)}`);
        }
        // First approve — should succeed and dispatch one email.
        const first = await fetch(`${INGRESS}/api/v1/admin/signups/${signup.signupId}/approve`, {
            method: 'POST',
            headers: { 'X-Debug-Principal': `user:platform-admin:_platform:admin` },
        });
        ...
        const second = await fetch(`${INGRESS}/api/v1/admin/signups/${signup.signupId}/approve`, {
            method: 'POST',
            headers: { 'X-Debug-Principal': `user:platform-admin:_platform:admin` },
        });
        ...
        expect(forRecipient.length, `replay must not double-send; got ${forRecipient.length} messages for ${EMAIL_2}`).toBe(1);
    });
});
