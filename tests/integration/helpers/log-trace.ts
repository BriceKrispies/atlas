...
export async function fetchTrace(
  ...
  const url =
    `${ingressBase}/api/v1/admin/logging/correlation/` +
...
export function assertContainsEvents(
  ...
  const seen = new Set(records.map((r) => r.eventName).filter((n): n is string => !!n));
  const missing = expected.filter((n) => !seen.has(n));
  if (missing.length > 0) {
    throw new Error(
      `missing expected log events: ${missing.join(', ')} ` +
        `(seen: ${[...seen].join(', ')})`,
    );
  }
}
 ...
 */
export function assertAllCorrelated(
  records: ReadonlyArray<LogRecord>,
  correlationId: string,
): void {
  const wrong = records.filter((r) => r.correlationId !== correlationId);
  if (wrong.length > 0) {
    throw new Error(
      `${wrong.length}/${records.length} records have wrong correlationId ` +
