...
async function readContentPage(page: Page, pageId: string): Promise<PageDocumentLike | null> {
    const raw = await page.evaluate(function (id) {
...
// ---------------- Background ----------------
Given('a tenant {string} with the content-pages module enabled', async function ({ world, tenantId }, alias: string) {
...
// ---------------- Create ----------------
When('the admin creates a page {string} with title {string} and slug {string}', async function ({ simPage, world, tenantId, principalId }, pageId: string, title: string, slug: string) {
...
Given('the admin has created a page {string} with title {string} and slug {string}', async function ({ simPage, world, tenantId, principalId }, pageId: string, title: string, slug: string) {
    const correlationId = newEventId();
    const idempotencyKey = uniqueIdem(`page-create-${pageId}`);
    const envelope = buildPageCreateEnvelope({
        tenantId,
...
// ---------------- Update ----------------
When('the admin updates page {string} to title {string}', async function ({ simPage, world, tenantId, principalId }, pageId: string, title: string) {
...
// ---------------- Delete ----------------
When('the admin deletes page {string}', async function ({ simPage, world, tenantId, principalId }, pageId: string) {
    const correlationId = newEventId();
...
// ---------------- Listing ----------------
When('the admin lists all pages', async function ({ simPage, world }) {
    world.lastQueryResponse = await readContentPageList(simPage);
});
// ---------------- Assertions ----------------
Then('the page {string} exists with title {string} and status {string}', async function ({ simPage }, pageId: string, expectedTitle: string, expectedStatus: string) {
...
Then('the render tree for page {string} is the default tree', async function ({ simPage }, pageId: string) {
...
Then('the listing does not contain a page with id {string}', async function ({ simPage }, pageId: string) {
...
Then('the page {string} has title {string}', async function ({ simPage }, pageId: string, expectedTitle: string) {
    const maybeDoc = await readContentPage(simPage, pageId);
    expect(maybeDoc).not.toBeNull();
    const doc = assertDefined(maybeDoc, `content page ${pageId} after assert non-null`);
    expect(doc.title).toBe(expectedTitle);
});
Then('the page {string} updatedAt is later than its createdAt', async function ({ simPage }, pageId: string) {
    const maybeDoc = await readContentPage(simPage, pageId);
    expect(maybeDoc).not.toBeNull();
    const doc = assertDefined(maybeDoc, `content page ${pageId} after assert non-null`);
...
Then('the page {string} is null', async function ({ simPage }, pageId: string) {
