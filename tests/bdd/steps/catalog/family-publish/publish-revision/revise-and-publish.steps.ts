...
import { submitIntent, submitIntentRaw, getFamilyDetail, readEvents, countEventsByIdempotencyKey, } from '../../../../support/idb-probe.ts';
...
function buildSeedEnvelope(tenantId: string, principalId: string, idemKey: string): IntentEnvelope {
    const seed = seedDoc();
    return {
        eventId: newEventId(),
...
function buildPublishEnvelope(tenantId: string, principalId: string, familyKey: string, revisionNumber: number, idemKey: string, correlationId: string): IntentEnvelope<PublishPayload> {
    return {
        eventId: newEventId(),
...
function expectedHandlerIdempotencyKey(tenantId: string, familyKey: string, revisionNumber: number): string {
    return `catalog.family.publish.${tenantId}.${familyKey}.${revisionNumber}`;
}
// ---------------- Background ----------------
Given('a tenant {string} with the catalog module enabled', async function ({ world, tenantId }, alias: string) {
    world.tenantsByAlias.set(alias, tenantId);
    world.primaryTenantAlias = alias;
});
Given('the badge-family seed has been applied for tenant {string}', async function ({ simPage, world, tenantId, principalId }, alias: string) {
    const realTenant = world.tenantsByAlias.get(alias) ?? tenantId;
    const env = buildSeedEnvelope(realTenant, principalId, `bdd-seed-${realTenant}`);
...
// ---------------- Cross-tenant setup ----------------
Given('a separate tenant {string} has no catalog data', async function ({ mintAdditionalTenant }, alias: string) {
...
When('the admin re-authenticates for tenant {string}', async function ({ reauthenticate, world }, alias: string) {
...
// ---------------- Publishing ----------------
When('the admin publishes {string} at revision {int}', async function ({ simPage, world, tenantId, principalId }, familyKey: string, revision: number) {
    const correlationId = newEventId();
    const idemKey = `bdd-pub-${familyKey}-${revision}-${Date.now().toString(36)}`;
    const env = buildPublishEnvelope(tenantId, principalId, familyKey, revision, idemKey, correlationId);
...
When('the admin re-submits the same publish envelope', async function ({ simPage, world }) {
...
When('the admin attempts to publish {string} at revision {int}', async function ({ simPage, world, tenantId, principalId }, familyKey: string, revision: number) {
    const correlationId = newEventId();
...
When('the admin queries the family {string}', async function ({ simPage, world }, familyKey: string) {
    world.lastQueryResponse = await getFamilyDetail(simPage, familyKey);
});
// ---------------- Assertions ----------------
Then('a {string} event is emitted with revision {int}', async function ({ simPage, world }, eventType: string, revision: number) {
    ...
    // EventEnvelope.payload is `unknown` by default. Use a type-guard
    // so we narrow without an unsafe cast.
    function isRecord(v: unknown): v is Record<string, unknown> {
        return typeof v === 'object' && v !== null && !Array.isArray(v);
    }
    if (!isRecord(ev.payload)) {
        throw new Error('event payload missing or not an object');
    }
    expect(ev.payload['revisionNumber']).toBe(revision);
});
Then('the event carries the request correlationId', async function ({ simPage, world }) {
    const correlationId = assertDefined(world.lastCorrelationId, 'lastCorrelationId set by a prior publish step');
...
Then('the event carries cache invalidation tags including {string} and {string}', async function ({ simPage, world }, tagA: string, tagB: string) {
    const correlationId = assertDefined(world.lastCorrelationId, 'lastCorrelationId set by a prior publish step');
    const matches = await readEvents(simPage, {
        type: 'StructuredCatalog.FamilyPublished',
        correlationId,
    ...
    const resolveTag = function (tag: string): string {
...
Then('exactly one {string} event exists for that idempotency key', async function ({ simPage, world, tenantId }, eventType: string) {
...
Then('the request is denied with code {string}', async function ({ world }, code: string) {
    const failure = assertDefined(world.lastSubmitFailure, 'world.lastSubmitFailure set by a prior failing submit');
    expect(failure.code).toBe(code);
});
Then('no {string} event exists in the event store', async function ({ simPage }, eventType: string) {
    const matches = await readEvents(simPage, { type: eventType });
    expect(matches).toHaveLength(0);
});
Then('the response describes the {string} family', async function ({ world }, _alias: string) {
...
Then('the response is empty', async function ({ world }) {
