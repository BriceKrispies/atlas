...
AfterStep(async ({ page, $step, $testInfo }) => {
  if (screenshotMode !== 'always') return;
...
After('@sim', async ({ simPage, mintedTenants, world, $testInfo }) => {
  if (idbSnapshotMode === 'off') return;
  if (idbSnapshotMode === 'on-failure' && $testInfo.status === 'passed') return;

  const tenants: Array<{ alias: string; page: Page }> = [];
  ...
  for (const t of tenants) {
    ...
    try {
      ...
      body = JSON.stringify(
        { error: `failed to read IDB snapshot for tenant '${t.alias}'`, detail: message },
        null,
        2,
      );
    }
