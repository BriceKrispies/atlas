...
import type { EventEnvelope, IntentEnvelope, IntentResponse, SearchDocument, } from '@atlas/platform-core';
...
export async function submitIntent(page: Page, envelope: IntentEnvelope): Promise<IntentResponse> {
    const response = await page.evaluate(function (e) {
...
export async function submitIntentRaw(page: Page, envelope: IntentEnvelope): Promise<SubmitRawResult> {
    const result = await page.evaluate(function (e) {
        const action = window.__atlas;
        if (!action)
            throw new Error('window.__atlas is not mounted — non-BDD build?');
...
export function getVariantTable(page: Page, familyKey: string, params: Record<string, unknown> = {}): Promise<unknown> {
    return page.evaluate(function ([k, p]) {
        const action = window.__atlas;
        if (!action)
            throw new Error('window.__atlas is not mounted — non-BDD build?');
        return action.getVariantTable(k as string, p as Record<string, unknown>);
...
export function searchCatalog(page: Page, params: Record<string, unknown>): Promise<unknown> {
...
export function snapshot(page: Page): Promise<AtlasSnapshot> {
    return page.evaluate(function () {
        const dbg = window.__atlas_debug;
        if (!dbg)
            throw new Error('window.__atlas_debug is not mounted — non-BDD build?');
        return dbg.snapshot();
    });
}
export function readEvents(page: Page, filter: EventReadFilter = {}): Promise<EventEnvelope[]> {
...
export function readEventById(page: Page, id: string): Promise<EventEnvelope | null> {
...
export function readEventTags(page: Page, id: string): Promise<string[] | null> {
    return page.evaluate(function (i) {
        const dbg = window.__atlas_debug;
        if (!dbg)
            throw new Error('window.__atlas_debug is not mounted — non-BDD build?');
...
export function readSearchDocs(page: Page, tenantId: string, type?: string): Promise<SearchDocument[]> {
    return page.evaluate(function ([t, ty]) {
        const dbg = window.__atlas_debug;
        if (!dbg)
            throw new Error('window.__atlas_debug is not mounted — non-BDD build?');
        return dbg.readSearchDocs(t as string, ty as string | undefined);
 ...
 */
export async function readSingleEventByType(page: Page, type: string, correlationId: string): Promise<EventEnvelope> {
    const matches = await readEvents(page, { type, correlationId });
    if (matches.length !== 1) {
        throw new Error(`expected exactly 1 ${type} event for correlationId=${correlationId}, got ${matches.length}`);
    }
    return assertDefined(matches[0], `length-checked match[0] for ${type}/${correlationId}`);
}
/**
 * Count events of a given type emitted under an idempotency key. Used by
 * I3 (idempotency) assertions.
 */
export async function countEventsByIdempotencyKey(page: Page, type: string, idempotencyKey: string): Promise<number> {
