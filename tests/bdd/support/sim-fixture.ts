...
async function waitForReady(page: Page): Promise<void> {
  await page.waitForFunction(() => window.__atlas_ready === true, undefined, {
  ...
  // real exception.
  const bootError = await page.evaluate(() => window.__atlas_boot_error);
  if (bootError) {
...
export const test = base.extend<SimFixtures>({
  ...
  principalId: async ({ tenantId }, use) => {
    await use(principalIdForRole('TenantAdmin', tenantId));
  },

  simPage: async ({ page, tenantId, principalId }, use) => {
  ...
  reauthenticate: async ({ simPage, tenantId, principalId }, use) => {
    let currentTenant = tenantId;
    let currentPrincipal = principalId;
    const fn: SimFixtures['reauthenticate'] = async (opts) => {
  ...
  mintAdditionalTenant: async ({ context, world, mintedTenants }, use) => {
    const fn: SimFixtures['mintAdditionalTenant'] = async (opts) => {
