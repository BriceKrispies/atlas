...
d('[node] tenant_isolation parity', function () {
    test('test_two_tenants_have_isolated_databases', async function () {
        ...
        await a.ingress.submitIntent(buildSeedIntent(a.tenantId, a.principalId, `iso-${a.tenantId}`, seed));
    ...
    test('idempotency_keys_do_not_collide_across_tenants', async function () {
        ...
        const r1 = await a.ingress.submitIntent(validIntent({
            tenantId: a.tenantId,
            principalId: a.principalId,
            idempotencyKey: idem,
        }));
        const r2 = await b.ingress.submitIntent(validIntent({
            tenantId: b.tenantId,
