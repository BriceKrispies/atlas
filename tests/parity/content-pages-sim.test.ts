...
function buildPageCreateIntent(opts: {
    ...
    return {
        ...
        idempotencyKey: opts.idempotencyKey ?? uniqueIdempotencyKey(`itest-page-${opts.pageId}`),
...
describe('[sim] content-pages parity', function () {
    ...
    test('test_render_tree_is_default_shape', async function () {
    ...
    test('test_page_list_contains_created_page', async function () {
    ...
    test('test_page_delete_clears_document_and_render_tree', async function () {
        ...
        await ingress.submitIntent(buildPageDeleteIntent({ tenantId, principalId, pageId: 'gone' }));
    ...
    test('test_page_tenant_isolation', async function () {
        const a = await makeSimIngress('cp-iso-a');
        const b = await makeSimIngress('cp-iso-b');
        await a.ingress.submitIntent(buildPageCreateIntent({
            tenantId: a.tenantId,
    ...
    test('test_page_create_event_has_cache_invalidation_tags', async function () {
        const { ingress, tenantId, principalId } = await makeSimIngress('cp-tag');
        const r = await ingress.submitIntent(buildPageCreateIntent({
            tenantId,
            principalId,
            pageId: 'tagged',
            title: 'Tagged',
        ...
        const tags = assertDefined(await ingress.readEventTags(r.eventId), 'readEventTags returned non-null in the asserted-non-null branch');
