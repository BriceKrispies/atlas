...
describe('[sim] closed_loop parity', function () {
    test('test_intent_builds_projection_and_query_returns_it', async function () {
        ...
        const r = await ingress.submitIntent(buildSeedIntent(tenantId, principalId, `itest-cl-${tenantId}`, seed));
    ...
    test('test_second_intent_refreshes_projection', async function () {
        const { ingress, tenantId, principalId } = await makeSimIngress('cl-cache');
        const seed = loadBadgeFamilySeed();
        await ingress.submitIntent(buildSeedIntent(tenantId, principalId, `itest-cl-c1-${tenantId}`, seed));
        ...
        await ingress.submitIntent(buildSeedIntent(tenantId, principalId, `itest-cl-c2-${tenantId}`, bumped));
        const refreshed = await ingress.getFamilyDetail('service_anniversary_badge');
        expect(refreshed).not.toBeNull();
        await ingress.close();
    });
    test('test_unknown_action_is_rejected', async function () {
    ...
    test('test_cross_tenant_query_is_isolated', async function () {
        const a = await makeSimIngress('cl-iso-a');
        const b = await makeSimIngress('cl-iso-b');
        const seed = loadBadgeFamilySeed();
        await a.ingress.submitIntent(buildSeedIntent(a.tenantId, a.principalId, `itest-cl-iso-${a.tenantId}`, seed));
        ...
        await b.ingress.submitIntent(validIntent({
            tenantId: b.tenantId,
