...
describe('[sim] catalog_badge_family parity', function () {
    ...
    test('test_taxonomy_navigation_lists_family', async function () {
        ...
        const svc = assertDefined(nodes.find(function (n) {
    ...
    test('test_variant_table_returns_normalized_rows', async function () {
        ...
        const fiveYear = assertDefined(rows.find(function (r) {
    ...
    test('test_projection_rebuild_is_deterministic', async function () {
        ...
        const beforeRows = canonicalizeVariantRows(assertDefined(before, 'variant table fetched before rebuild').rows);
        ...
        const afterRows = canonicalizeVariantRows(assertDefined(after, 'variant table fetched after rebuild').rows);
    ...
    test('test_seed_event_has_cache_invalidation_tags', async function () {
        ...
        const stored = assertDefined(await ingress.readEventTags(r1.eventId), 'event tags persisted for seed apply');
...
describe('[sim] catalog_search parity', function () {
    ...
    test('test_search_permission_filter_excludes_disallowed', async function () {
        ...
        const seed = loadBadgeFamilySeed();
        await ingress.submitIntent(buildSeedIntent(tenantId, principalId, `itest-cs-perm-${tenantId}`, seed));
        ...
        const body = await ingress.searchCatalog({ q: 'anniversary' });
        expect(body.results.some(function (r) {
            return r.documentId === 'alice_only_anniversary';
        })).toBe(false);
        ...
        expect(aliceBody.results.some(function (r) {
            return r.documentId === 'alice_only_anniversary';
        })).toBe(true);
        await aliceIngress.close();
        await ingress.close();
    });
    test('test_search_rebuild_is_deterministic', async function () {
        const { ingress, tenantId, principalId } = await makeSimIngress('cs-rb');
        const seed = loadBadgeFamilySeed();
        await ingress.submitIntent(buildSeedIntent(tenantId, principalId, `itest-cs-rb-${tenantId}`, seed));
        ...
        const bumped = { ...seed, version: `rebuild-${tenantId}` };
        await ingress.submitIntent(buildSeedIntent(tenantId, principalId, `itest-cs-rb2-${tenantId}`, bumped));
        const after = await ingress.searchCatalog({ q: 'anniversary' });
        expect(after.results.length).toBeGreaterThanOrEqual(beforeCount);
        await ingress.close();
    });
    test('test_search_index_cache_invalidation_tag_present', async function () {
        const { ingress, tenantId, principalId } = await makeSimIngress('cs-cache');
        const seed = loadBadgeFamilySeed();
        const r = await ingress.submitIntent(buildSeedIntent(tenantId, principalId, `itest-cs-cache-${tenantId}`, seed));
        const tags = assertDefined(await ingress.readEventTags(r.eventId), 'event tags persisted for cache-invalidation check');
...
// helpers ---------------------------------------------------------------
function canonicalizeVariantRows(rows: ReadonlyArray<VariantRow>): Array<{
    key: string;
