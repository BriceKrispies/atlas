...
describe('[node] catalog_badge_family parity (debug-surface)', function () {
    test('test_seed_event_has_cache_invalidation_tags', async function () {
        ...
        const stored = assertDefined(await ingress.readEventTags(r1.eventId), `event tags must be readable for ${r1.eventId}`);
...
describe('[node] catalog_search parity (debug-surface)', function () {
    test('test_search_permission_filter_excludes_disallowed', async function () {
        ...
        const aliceIngress = await createServerIngress({
            baseUrl: assertDefined(baseUrl, 'NODE_PARITY_BASE_URL must be set; test would have been skipped otherwise'),
            tenantId,
            principalId: 'u_alice',
        });
    ...
    test('test_search_index_cache_invalidation_tag_present', async function () {
        ...
        const tags = assertDefined(await ingress.readEventTags(r.eventId), `event tags must be readable for ${r.eventId}`);
