...
describe('[node] authentication parity', function () {
    ...
    test('test_invalid_token_returns_401', async function () {
        const res = await fetchWhoami({
            Authorization: 'Bearer this-is-not-a-valid-jwt-token',
        });
        expect(res.status).toBe(401);
    });
    test('test_malformed_jwt_returns_401', async function () {
        const fake = 'eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.' +
            'eyJzdWIiOiJmYWtlLXVzZXIiLCJpc3MiOiJmYWtlLWlzc3VlciJ9.' +
            'invalid-signature';
        const res = await fetchWhoami({ Authorization: `Bearer ${fake}` });
        expect(res.status).toBe(401);
    });
    test('test_expired_token_returns_401', async function () {
    ...
    test('test_debug_principal_header_works', async function () {
        const res = await fetchWhoami({
        ...
        if (res.status === 200) {
            ...
            if (!body || typeof body !== 'object')
                throw new Error('expected JSON body on whoami');
    ...
    test('test_auth_failure_returns_401_not_403', async function () {
        const res = await fetchWhoami({ Authorization: 'Bearer invalid-token' });
        expect(res.status).toBe(401);
    });
    test('malformed_debug_principal_header_returns_400', async function () {
    ...
    test('empty_bearer_returns_401', async function () {
        const res = await fetchWhoami({ Authorization: 'Bearer ' });
        expect(res.status).toBe(401);
    });
    test('non_bearer_authorization_returns_401', async function () {
