...
`;
async function seedPolicy(sql: postgres.Sql, tenantId: string, cedarText: string, version = 1): Promise<void> {
...
describe('[node] Cedar policy engine parity', function () {
    ...
    test('permits a matching action when a tenant has a permissive bundle', async function () {
    ...
    test('cross-tenant isolation: tenant A bundle does not authorize tenant B requests', async function () {
    ...
    test('forbid-overrides-permit: a forbid for the same action wins', async function () {
    ...
    test('no-bundle tenant falls through to permissive (allow-all-with-tenant-scope)', async function () {
