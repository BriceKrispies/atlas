...
describe('[node] observability parity', function () {
    ...
    test('atlas_intents_submitted_total is exposed (counter family)', async function () {
    ...
    test('atlas_policy_evaluations_total is exposed', async function () {
        const res = await fetch(`${baseUrl}/metrics`, { method: 'GET' });
        expect(res.status).toBe(200);
        const body = await res.text();
        expect(body).toContain('# TYPE atlas_policy_evaluations_total counter');
    });
    test('atlas_intent_duration_seconds histogram type registered', async function () {
        const res = await fetch(`${baseUrl}/metrics`, { method: 'GET' });
        expect(res.status).toBe(200);
        const body = await res.text();
        expect(body).toContain('# TYPE atlas_intent_duration_seconds histogram');
    });
    test('metrics include label keys when populated', async function () {
