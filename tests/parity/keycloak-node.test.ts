...
describe('[node] keycloak parity', function () {
    test('test_keycloak_is_reachable', async function () {
        ...
        const configured = new URL(assertDefined(keycloakBaseUrl, 'KEYCLOAK_BASE_URL must be set — the dKeycloak describe gate guarantees this'));
    ...
    describe('with apps/server', function () {
        ...
        test('test_valid_keycloak_token_principal_extraction', async function () {
            const token = await mintClientCredentialsToken();
            const res = await fetch(`${serverBaseUrl}/debug/whoami`, {
                method: 'GET',
                headers: { Authorization: `Bearer ${token.access_token}` },
