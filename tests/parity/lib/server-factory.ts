 ...
 */
import { IngressFailureError, UnsupportedInMode, type BrowserIngress, type FactoryOptions, type HealthResponse, type IngressFailure, } from './factory.ts';
import type { IntentEnvelope, IntentResponse, SearchDocument, } from '@atlas/platform-core';
import type { TaxonomyNavigationResponse, FamilyDetailResponse, VariantTableParams, VariantTableResponse, SearchParams, SearchResponse, } from '@atlas/catalog';
import type { PageDocument, PageSummary, RenderTree, } from '@atlas/content-pages';
...
export async function createServerIngress(opts: ServerFactoryOptions): Promise<BrowserIngress> {
    const headers = function (extra?: Record<string, string>): Record<string, string> {
        return ({
    ...
    const get = async function <T>(path: string): Promise<{
        ok: true;
        value: T;
    } | {
        ok: false;
    ...
    const ingress: BrowserIngress = {
        ...
        async getTaxonomyNodes(treeKey: string): Promise<TaxonomyNavigationResponse | null> {
            const r = await get<TaxonomyNavigationResponse>(`/api/v1/catalog/taxonomies/${encodeURIComponent(treeKey)}/nodes`);
            if (r.ok)
                return r.value;
            if (r.failure.status === 404)
                return null;
            throw new IngressFailureError(r.failure);
        },
        async getFamilyDetail(familyKey: string): Promise<FamilyDetailResponse | null> {
            const r = await get<FamilyDetailResponse>(`/api/v1/catalog/families/${encodeURIComponent(familyKey)}`);
            if (r.ok)
                return r.value;
            if (r.failure.status === 404)
                return null;
        ...
        async getVariantTable(familyKey: string, params?: VariantTableParams): Promise<VariantTableResponse | null> {
            ...
            if (params?.filters) {
                for (const [k, v] of Object.entries(params.filters)) {
                    ...
                    if (v.kind === 'equals') {
                        ...
                        if (v.gte !== undefined)
                            qs.set(`filter.${k}.gte`, String(v.gte));
                        if (v.lte !== undefined)
                            qs.set(`filter.${k}.lte`, String(v.lte));
                    }
                }
            }
            const path = `/api/v1/catalog/families/${encodeURIComponent(familyKey)}/variants${qs.toString() ? `?${qs}` : ''}`;
        ...
        async getContentPage(pageId: string): Promise<PageDocument | null> {
            const r = await get<PageDocument>(`/api/v1/pages/${encodeURIComponent(pageId)}`);
            if (r.ok)
                return r.value;
            if (r.failure.status === 404)
                return null;
        ...
        async getContentPageRenderTree(pageId: string): Promise<RenderTree | null> {
            const r = await get<RenderTree>(`/api/v1/pages/${encodeURIComponent(pageId)}/render-tree`);
            if (r.ok)
                return r.value;
            if (r.failure.status === 404)
                return null;
        ...
        async whoami(headerOverride): Promise<{
            status: number;
            body: unknown;
        }> {
...
export async function makeServerIngress(prefix: string): Promise<{
    ...
    if (!baseUrl) {
        throw new Error('NODE_PARITY_BASE_URL must be set for node-mode parity tests');
    }
