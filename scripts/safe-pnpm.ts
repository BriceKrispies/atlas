...
const timeoutMs = Number(process.env['SAFE_PNPM_TIMEOUT_MS'] ?? 300_000);
const killGraceMs = Number(process.env['SAFE_PNPM_KILL_GRACE_MS'] ?? 5_000);
...
const deadline = setTimeout(() => {
  ...
  if (child.pid !== undefined) killTree(child.pid, false);
  setTimeout(() => {
...
const forward = (sig: NodeJS.Signals): void => {
  if (child.exitCode === null && child.pid !== undefined) {
    if (isWindows) killTree(child.pid, false);
    else child.kill(sig);
  }
};
