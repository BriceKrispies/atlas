...
// Stable marker stamped into every generated oxlint ring override's message.
// Used to find-and-replace generated overrides idempotently in .oxlintrc.json
// without disturbing hand-authored overrides. (oxlint's `extends` does NOT
// apply `overrides` from extended files — verified on oxlint 1.66 — so ring
// overrides must be injected directly into the root config.)
const RING_SENTINEL = 'Ring violation (ADR 0016)';
...
function buildOxlintRingOverrides(manifest: Manifest, packages: PkgInfo[]): OxlintOverride[] {
  const waiverKeys = new Set(
    manifest.waivers.map(function (w) {
      return `${w.from} -> ${w.to}`;
    }),
        ...
        'no-restricted-imports': [
          'error',
          {
            patterns: [
              {
                group: banned.sort(),
                message: `${RING_SENTINEL}: ${pkg.name} may not import these packages — they are in an outer ring, a sibling ring, or a different stack. See architecture/rings.json.`,
              },
            ],
          },
        ],
...
function isGeneratedRingOverride(o: unknown): boolean {
  if (typeof o !== 'object' || o === null) return false;
  const rules = (o as { rules?: unknown }).rules;
  if (typeof rules !== 'object' || rules === null) return false;
  const nri = (rules as Record<string, unknown>)['no-restricted-imports'];
  return JSON.stringify(nri ?? '').includes(RING_SENTINEL);
}

/**
 * Inject the generated ring overrides directly into .oxlintrc.json. oxlint's
 * `extends` does NOT apply `overrides` from extended files (verified on 1.66),
 * so the ring bans must live in the root config. This is idempotent: any
 * previously-generated ring override (identified by RING_SENTINEL) is stripped
 * and replaced; hand-authored overrides are preserved. `pnpm arch:emit` runs
 * biome over the result so the committed file stays canonically formatted.
 */
function writeOxlintRingConfig(generated: OxlintOverride[]): void {
  const p = join(REPO_ROOT, '.oxlintrc.json');
  const cfg = JSON.parse(readFileSync(p, 'utf8')) as {
    extends?: unknown;
    overrides?: unknown[];
    [k: string]: unknown;
  };
  // Drop the defunct extends->generated mechanism if still present.
  if (Array.isArray(cfg.extends)) {
    const kept = cfg.extends.filter(function (e) {
      return typeof e !== 'string' || !e.includes('oxlint-layers');
    });
    if (kept.length > 0) cfg.extends = kept;
    else delete cfg.extends;
  }
  const existing = Array.isArray(cfg.overrides) ? cfg.overrides : [];
  const hand = existing.filter(function (o) {
    return !isGeneratedRingOverride(o);
  });
  cfg.overrides = [...hand, ...generated];
...
function main(): number {
  ...
  if (EMIT) {
    ...
    writeOxlintRingConfig(buildOxlintRingOverrides(manifest, packages));
    process.stdout.write(
      'arch:emit — wrote architecture/generated/dep-cruiser-rules.cjs + injected ring overrides into .oxlintrc.json\n',
    );
  }
