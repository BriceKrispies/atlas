...
import { fetchTrace as fetchTraceHelper, assertContainsEvents, assertAllCorrelated, type LogRecord, } from '../tests/integration/helpers/log-trace.ts';
// ────────────────────────────────────────────────────────────────────
// Configuration
// ────────────────────────────────────────────────────────────────────
const DB_URL = process.env['CONTROL_PLANE_DB_URL'] ??
...
const c = {
    ...
    red: function (s: string) {
        return (COLOR ? `\x1b[31m${s}\x1b[0m` : s);
    },
...
function spawnServer(): ServerHandle {
    ...
    const child = spawn('pnpm', ['--filter', '@atlas/server', 'start'], {
        env: {
            ...process.env,
    ...
    child.stderr?.on('data', function (chunk: string) {
        for (const line of chunk.split('\n')) {
            const t = line.trim();
            if (t)
                console.log(c.dim(`    [server stderr] ${t}`));
        }
    });
...
function printTrace(records: LogRecord[]): void {
    ...
    for (const r of records) {
        ...
        const dur = typeof r.durationMs === 'number'
...
function assertTrace(records: LogRecord[], correlationId: string): void {
    ...
    if (outOfOrder > 0) {
        console.log(c.yellow(`  ! ${outOfOrder} records timestamp-out-of-order (likely sub-ms drift)`));
    }
    else {
